function L = R5_Build_SensorConditionedSidecar_20250527(familyFile, localizationFile, registrationFile, outputFile)
%R5_BUILD_SENSORCONDITIONEDSIDECAR_20250527 Store one response state per sensor.
% Adapt the frozen static library to the common sensor-conditioned API.
cfg = Config_20250527();
...
F=load(R5_StageMatForMatlab(familyFile,'r5_family'),'F'); X=load(R5_StageMatForMatlab(localizationFile,'r5_localization'),'T','details','latentMode'); G=load(R5_StageMatForMatlab(registrationFile,'r5_registration'),'G'); muBySensor=load_static_mu_local(cfg.files.gapLibrary);
...
sidecar=struct('schema','R5_SENSOR_CONDITIONED_SIDECAR_V1','family_file',familyFile,...
    ...
    'staticModel','frozen_absolute_surface_state', ...
    'staticTiltCarrier','absolute_surface_state', ...
    'staticTiltSource','low_speed_absolute_surface_localization', ...
    'staticTiltFrozen',true,'spaceGapSlopeUsed',false, ...
    'allowDynamicTilt',false,'extrapolationAllowed',false,...
    'sensor',{{}});
...
function S=load_static_mu_local(file)
S=repmat(struct('sensorId',NaN,'muGapPerXMm',0,'tauMm',NaN),0,1);
if ~isfile(file), return; end
Q=load(R5_StageMatForMatlab(file,'r5_gap_mu'),'CorrectedGapLibrary');
if isfield(Q.CorrectedGapLibrary,'sensor'), S=Q.CorrectedGapLibrary.sensor; end
end
