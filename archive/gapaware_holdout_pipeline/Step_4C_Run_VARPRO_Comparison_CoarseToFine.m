%% Step 4C: compare baseline raw-scan with experimental coarse-to-fine gap search
clc;
...
load(fullfile(outDir, 'stage3c_coarsetofine_gap_estimation.mat'), 'staticStateCF');
...
coarseFineResult = run_varpro_for_gap(highMap, templateLib, staticStateCF.gHat, cfg, 'coarse-to-fine-gap');
oracleResult = run_varpro_for_gap(highMap, templateLibFull, cfg.g_holdout, cfg, 'oracle-holdout');

Result = struct();
Result.truth = truth;
Result.staticState = staticState;
Result.staticStateCF = staticStateCF;
Result.fixed = fixedResult;
Result.rawScan = rawGapAwareResult;
Result.coarseFine = coarseFineResult;
Result.oracle = oracleResult;
Result.config = cfg;

summaryTable = make_summary_table(Result);
save(fullfile(outDir, 'stage4c_coarsetofine_identification_results.mat'), ...
    'Result', 'fixedResult', 'rawGapAwareResult', 'coarseFineResult', ...
    'oracleResult', 'summaryTable', '-v7.3');

figure('Name', 'Step 4C - Coarse-to-Fine VARPRO Comparison', 'Color', 'w', ...
...
plot(highMap.t_v(idxPlot) * 1000, rawGapAwareResult.VFit(idxPlot), 'b-', ...
    'DisplayName', sprintf('raw-scan %.3f', rawGapAwareResult.g_used));
plot(highMap.t_v(idxPlot) * 1000, coarseFineResult.VFit(idxPlot), 'm--', ...
    'DisplayName', sprintf('coarse-to-fine %.3f', coarseFineResult.g_used));
...
fprintf('[Step 4C] Experimental coarse-to-fine comparison:\n');
disp(summaryTable);
...
labels = ["fixed_low_gap"; "raw_scan_gap"; "coarse_to_fine_gap"; "oracle_holdout"];
methods = {Result.fixed, Result.rawScan, Result.coarseFine, Result.oracle};
[fTrue, trueOrder] = sort(Result.truth.f);
ATrue = Result.truth.A(trueOrder);
n = numel(methods);
gUsed = zeros(n, 1); gapError = zeros(n, 1); fErrMean = zeros(n, 1);
AErrMean = zeros(n, 1); rmse = zeros(n, 1);
for i = 1:n
    m = methods{i};
    gUsed(i) = m.g_used;
