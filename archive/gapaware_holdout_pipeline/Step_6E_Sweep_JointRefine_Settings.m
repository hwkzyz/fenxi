%% Step 6E: sweep local joint-refinement settings on key holdouts
clc;
...
settings = [
    make_setting("raw_scan_only", false, true, 0.00, 0.00, 0.0, 0.0)
    make_setting("joint_wide_free", true, true, 0.05, 0.05, 0.0, 0.0)
    make_setting("joint_tight_free", true, true, 0.02, 0.02, 0.0, 0.0)
    make_setting("joint_tight_pen", true, true, 0.02, 0.02, 5e-4, 5e-4)
    make_setting("joint_gonly_tight", true, false, 0.02, 0.00, 5e-4, 0.0)
];

rows = struct('setting', {}, 'holdout_gap_mm', {}, ...
    'g_used_mm', {}, 'gap_error_mm', {}, ...
    'rmse', {}, 'freq_error_Hz', {}, 'amp_error_mm', {});
...
for ih = 1:numel(holdoutSweepList)
    ...
    for iset = 1:numel(settings)
        s = settings(iset);
        if ~s.useJoint
            result = rawResult;
        else
            result = run_joint_refinement_with_setting(highMap, templateLib, staticRaw, rawResult, cfgCase, s);
        end
        [freqErr, ampErr] = calc_param_errors(result, cfgCase.f_true, cfgCase.A_true);

        rows(end+1).setting = string(s.name); %#ok<SAGROW>
        rows(end).holdout_gap_mm = gHoldout;
        rows(end).g_used_mm = result.g_used;
        rows(end).gap_error_mm = abs(result.g_used - gHoldout);
        rows(end).rmse = result.rmse;
        rows(end).freq_error_Hz = freqErr;
        rows(end).amp_error_mm = ampErr;
    end
end

detailTable = struct2table(rows);

uSettings = unique(detailTable.setting, 'stable');
aggRows = struct('setting', {}, 'mean_gap_error_mm', {}, 'mean_rmse', {}, ...
    'mean_freq_error_Hz', {}, 'mean_amp_error_mm', {}, 'combined_rank_score', {});
for i = 1:numel(uSettings)
    idx = detailTable.setting == uSettings(i);
    aggRows(end+1).setting = uSettings(i); %#ok<SAGROW>
    aggRows(end).mean_gap_error_mm = mean(detailTable.gap_error_mm(idx));
    aggRows(end).mean_rmse = mean(detailTable.rmse(idx));
    aggRows(end).mean_freq_error_Hz = mean(detailTable.freq_error_Hz(idx));
    aggRows(end).mean_amp_error_mm = mean(detailTable.amp_error_mm(idx));
end
aggregateTable = struct2table(aggRows);

rankGap = tiedrank(aggregateTable.mean_gap_error_mm);
rankRmse = tiedrank(aggregateTable.mean_rmse);
rankFreq = tiedrank(aggregateTable.mean_freq_error_Hz);
rankAmp = tiedrank(aggregateTable.mean_amp_error_mm);
aggregateTable.combined_rank_score = rankGap + rankRmse + rankFreq + rankAmp;
aggregateTable = sortrows(aggregateTable, 'combined_rank_score', 'ascend');

save(fullfile(outDir, 'stage6e_jointrefine_setting_sweep.mat'), ...
    'detailTable', 'aggregateTable', 'settings', '-v7.3');

figure('Name', 'Step 6E - Joint Refinement Setting Sweep', 'Color', 'w', ...
    'Units', 'centimeters', 'Position', [2, 2, 24, 12]);
tiledlayout(2, 2, 'Padding', 'compact', 'TileSpacing', 'compact');

nexttile;
bar(categorical(string(aggregateTable.setting)), aggregateTable.mean_gap_error_mm);
ylabel('Mean gap error (mm)');
title('Average gap error across holdouts');

nexttile;
bar(categorical(string(aggregateTable.setting)), aggregateTable.mean_rmse);
ylabel('Mean RMSE');
title('Average waveform fitting error');

nexttile;
bar(categorical(string(aggregateTable.setting)), aggregateTable.mean_freq_error_Hz);
ylabel('Mean frequency error (Hz)');
title('Average frequency error');

nexttile;
bar(categorical(string(aggregateTable.setting)), aggregateTable.mean_amp_error_mm);
ylabel('Mean amplitude error (mm)');
title('Average amplitude error');

fprintf('[Step 6E] Detailed holdout-setting results:\n');
disp(detailTable);
fprintf('\n[Step 6E] Aggregate summary by setting:\n');
disp(aggregateTable);

function s = make_setting(name, useJoint, refineDx, gHalfWidth, dxHalfWidth, wG, wDx)
s = struct();
s.name = string(name);
s.useJoint = useJoint;
s.refineDx = refineDx;
s.gHalfWidth = gHalfWidth;
s.dxHalfWidth = dxHalfWidth;
s.fHalfWidth = 20;
s.ampScaleLo = 0.50;
s.ampScaleHi = 1.50;
s.weightG = wG;
...
if setting.refineDx
    ...
    [ampLb, ampUb, fLb, fUb] = local_bounds_from_p0(p0, cfg, setting);
    lb = [gLo, dxLo, ampLb(1), -pi, fLb(1), ampLb(2), -pi, fLb(2)];
    ub = [gHi, dxHi, ampUb(1),  pi, fUb(1), ampUb(2),  pi, fUb(2)];
    resFun = @(theta) residual_calc_joint(theta, t, V, x, templateLib, staticState, setting);
    ...
    result = struct('g_used', thetaOpt(1), 'dx_used', thetaOpt(2), ...
        'p', thetaOpt(3:end), 'A_id', A_raw(order), 'f_id', f_id, ...
        'phi_id', phi_raw(order), 'rmse', sqrt(mean(r(1:numel(V)).^2)));
else
    theta0 = [staticState.gHat, p0];
    gLo = max(0.05, staticState.gHat - setting.gHalfWidth);
    gHi = staticState.gHat + setting.gHalfWidth;
    [ampLb, ampUb, fLb, fUb] = local_bounds_from_p0(p0, cfg, setting);
    lb = [gLo, ampLb(1), -pi, fLb(1), ampLb(2), -pi, fLb(2)];
    ub = [gHi, ampUb(1),  pi, fUb(1), ampUb(2),  pi, fUb(2)];
    resFun = @(theta) residual_calc_gonly(theta, t, V, x, templateLib, staticState, setting);
    thetaOpt = refine_bounded_least_squares(resFun, theta0, lb, ub);
    thetaOpt(3) = wrap_to_pi_local(thetaOpt(3));
    thetaOpt(6) = wrap_to_pi_local(thetaOpt(6));
    r = resFun(thetaOpt);
    [f_id, order] = sort([thetaOpt(4), thetaOpt(7)]);
    A_raw = [abs(thetaOpt(2)), abs(thetaOpt(5))];
    phi_raw = [thetaOpt(3), thetaOpt(6)];
    result = struct('g_used', thetaOpt(1), 'dx_used', staticState.dx0, ...
        'p', thetaOpt(2:end), 'A_id', A_raw(order), 'f_id', f_id, ...
        'phi_id', phi_raw(order), 'rmse', sqrt(mean(r(1:numel(V)).^2)));
end
end

function [ampLb, ampUb, fLb, fUb] = local_bounds_from_p0(p0, cfg, setting)
ampLb = [max(0.001, setting.ampScaleLo * abs(p0(1))), ...
         max(0.001, setting.ampScaleLo * abs(p0(4)))];
ampUb = [min(0.8, max(ampLb(1) + 1e-6, setting.ampScaleHi * abs(p0(1)))), ...
         min(0.8, max(ampLb(2) + 1e-6, setting.ampScaleHi * abs(p0(4))))];
fLb = [max(min(cfg.f1Grid), p0(3) - setting.fHalfWidth), ...
       max(min(cfg.f2Grid), p0(6) - setting.fHalfWidth)];
fUb = [min(max(cfg.f1Grid), p0(3) + setting.fHalfWidth), ...
       min(max(cfg.f2Grid), p0(6) + setting.fHalfWidth)];
end

function r = residual_calc_joint(theta, t, V, x, templateLib, staticState, setting)
gFit = theta(1);
dx0 = theta(2);
p = theta(3:end);
...
r = V - VSim;
if setting.weightG > 0
    r = [r; sqrt(setting.weightG) * (gFit - staticState.gHat) / max(setting.gHalfWidth, 1e-9)];
end
if setting.weightDx > 0
    r = [r; sqrt(setting.weightDx) * (dx0 - staticState.dx0) / max(setting.dxHalfWidth, 1e-9)];
end
end

function r = residual_calc_gonly(theta, t, V, x, templateLib, staticState, setting)
gFit = theta(1);
p = theta(2:end);
u = displacement_2freq(p, t);
VSim = evaluate_template_surface(templateLib, gFit, x - staticState.dx0 - u);
r = V - VSim;
if setting.weightG > 0
    r = [r; sqrt(setting.weightG) * (gFit - staticState.gHat) / max(setting.gHalfWidth, 1e-9)];
end
