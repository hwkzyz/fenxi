%% Step 6C: compare baseline raw-scan and coarse-to-fine search on key holdouts
clc;
...
rows = struct('holdout_gap_mm', {}, 'raw_gHat_mm', {}, 'raw_gap_error_mm', {}, ...
    'coarsefine_gHat_mm', {}, 'coarsefine_gap_error_mm', {}, ...
    'raw_rmse', {}, 'coarsefine_rmse', {}, ...
    'raw_freq_error_Hz', {}, 'coarsefine_freq_error_Hz', {}, ...
    'raw_support', {}, 'coarsefine_support', {});
...
for ih = 1:numel(holdoutSweepList)
    ...
    [rawFreqErr, ~] = calc_param_errors(rawResult, cfgCase.f_true, cfgCase.A_true);
    [cfFreqErr, ~] = calc_param_errors(coarseFineResult, cfgCase.f_true, cfgCase.A_true);
    ...
    rows(end).coarsefine_gHat_mm = staticCF.gHat;
    rows(end).coarsefine_gap_error_mm = abs(staticCF.gHat - gHoldout);
    rows(end).raw_rmse = rawResult.rmse;
    rows(end).coarsefine_rmse = coarseFineResult.rmse;
    rows(end).raw_freq_error_Hz = rawFreqErr;
    rows(end).coarsefine_freq_error_Hz = cfFreqErr;
    rows(end).raw_support = string(staticRaw.supportType);
    rows(end).coarsefine_support = string(staticCF.supportType);
end

summaryTable = struct2table(rows);
save(fullfile(outDir, 'stage6c_coarsetofine_holdouts.mat'), 'summaryTable', '-v7.3');

figure('Name', 'Step 6C - Raw vs Coarse-to-Fine Holdouts', 'Color', 'w', ...
...
nexttile; hold on;
bar(labels, [summaryTable.raw_gap_error_mm, summaryTable.coarsefine_gap_error_mm], 'grouped');
ylabel('|gHat - gTruth| (mm)');
title('Gap estimation error');
legend({'raw-scan', 'coarse-to-fine'}, 'Location', 'best', 'Box', 'off');

nexttile; hold on;
bar(labels, [summaryTable.raw_rmse, summaryTable.coarsefine_rmse], 'grouped');
ylabel('RMSE');
title('Waveform fitting error');

nexttile; hold on;
bar(labels, [summaryTable.raw_freq_error_Hz, summaryTable.coarsefine_freq_error_Hz], 'grouped');
ylabel('Mean frequency error (Hz)');
title('Frequency identification error');

fprintf('[Step 6C] Raw vs coarse-to-fine summary:\n');
disp(summaryTable);
