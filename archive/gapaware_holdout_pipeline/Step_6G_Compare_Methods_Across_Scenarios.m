%% Step 6G: compare multiple methods across SNR and dual-frequency scenarios
clc;
...
methodList = ["fixed_low_gap", "raw_scan_only", "coarse_to_fine_gap", "joint_wide_free", "oracle_holdout"];
...
rows = struct('scenario', {}, 'noise_ratio', {}, 'method', {}, ...
    ...
    'mean_freq_error_Hz', {}, 'mean_amp_error_mm', {}, ...
    'rmse', {});
...
for is = 1:numel(scenarioList)
    ...
    for in = 1:numel(noiseList)
        ...
        coarseFineResult = run_varpro_for_gap(highMap, templateLib, staticCF.gHat, cfgCase, 'coarse-to-fine');
        ...
        resultMap = cell(5, 2);
        resultMap(:, 1) = cellstr(methodList);
        resultMap(:, 2) = {fixedResult, rawResult, coarseFineResult, jointWideResult, oracleResult};

        for im = 1:size(resultMap, 1)
            methodName = string(resultMap{im, 1});
            res = resultMap{im, 2};
            [freqErr, ampErr, ~, ~, fErrVec, aErrVec] = calc_param_errors(res, cfgCase.f_true, cfgCase.A_true);
...
aggRows = struct('method', {}, 'mean_gap_error_mm', {}, 'mean_f1_error_Hz', {}, ...
...
for im = 1:numel(methodList)
    m = methodList(im);
    idx = detailTable.method == m;
    aggRows(end+1).method = m; %#ok<SAGROW>
    aggRows(end).mean_gap_error_mm = mean(detailTable.gap_error_mm(idx));
    aggRows(end).mean_f1_error_Hz = mean(detailTable.f1_error_Hz(idx));
    aggRows(end).mean_f2_error_Hz = mean(detailTable.f2_error_Hz(idx));
    aggRows(end).mean_A1_error_mm = mean(detailTable.A1_error_mm(idx));
    aggRows(end).mean_A2_error_mm = mean(detailTable.A2_error_mm(idx));
    aggRows(end).mean_freq_error_Hz = mean(detailTable.mean_freq_error_Hz(idx));
    aggRows(end).mean_amp_error_mm = mean(detailTable.mean_amp_error_mm(idx));
    aggRows(end).mean_rmse = mean(detailTable.rmse(idx));
end
aggregateTable = struct2table(aggRows);

save(fullfile(outDir, 'stage6g_compare_methods_across_scenarios.mat'), ...
    'detailTable', 'aggregateTable', 'scenarioList', 'noiseList', 'methodList', '-v7.3');

figure('Name', 'Step 6G - Method Comparison Across Scenarios', 'Color', 'w', ...
    'Units', 'centimeters', 'Position', [1.5, 1.5, 26, 13]);
tiledlayout(2, 3, 'Padding', 'compact', 'TileSpacing', 'compact');

nexttile;
bar(categorical(string(aggregateTable.method)), aggregateTable.mean_gap_error_mm);
ylabel('Mean gap error (mm)');
title('Gap estimation');

nexttile;
bar(categorical(string(aggregateTable.method)), aggregateTable.mean_f1_error_Hz);
ylabel('Mean f_1 error (Hz)');
title('First frequency');

nexttile;
bar(categorical(string(aggregateTable.method)), aggregateTable.mean_f2_error_Hz);
ylabel('Mean f_2 error (Hz)');
title('Second frequency');

nexttile;
bar(categorical(string(aggregateTable.method)), aggregateTable.mean_A1_error_mm);
ylabel('Mean A_1 error (mm)');
title('First amplitude');

nexttile;
bar(categorical(string(aggregateTable.method)), aggregateTable.mean_A2_error_mm);
ylabel('Mean A_2 error (mm)');
title('Second amplitude');

nexttile;
bar(categorical(string(aggregateTable.method)), aggregateTable.mean_rmse);
ylabel('Mean RMSE');
title('Waveform fit');

fprintf('[Step 6G] Current mainline is dual-frequency identification.\n');
fprintf('[Step 6G] Detailed multi-method results:\n');
disp(detailTable);
fprintf('\n[Step 6G] Aggregate method comparison:\n');
disp(aggregateTable);
