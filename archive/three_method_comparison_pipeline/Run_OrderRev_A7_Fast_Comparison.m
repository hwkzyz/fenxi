...
%   A7: dominant-mode aligned high-template VARPRO
%   A8: physics-gated A/A7 hybrid
...
for iScenario = 1:nScenario
    for iRev = 1:nRev
        ...
        tA7 = tic;
        resultA7 = run_a7_dominant_aligned_template_varpro(cloud, cfgCase);
        elapsedA7 = toc(tA7);

        tA8 = tic;
        resultA8 = choose_a8_async_hybrid(resultA, resultA7, cfgCase);
        elapsedA8Selector = toc(tA8);
        elapsedA8 = elapsedA + elapsedA8Selector;
        if isfield(resultA8, 'hybrid_used_A7') && resultA8.hybrid_used_A7
            elapsedA8 = elapsedA8 + elapsedA7;
        end
        ...
        recId = add_method_record(records, recId, cfgCase, truth, resultA7, 'A7_dominant_aligned', ...
            targetSnr, measuredSnr, noiseRatio, iRep, seed, elapsedA7, NaN, ...
            cfg.orderScenario(iScenario).name, cfgCase.NumRevs_high);
        records = recId.records;
        recId = recId.id;

        recId = add_method_record(records, recId, cfgCase, truth, resultA8, 'A8_async_hybrid', ...
            targetSnr, measuredSnr, noiseRatio, iRep, seed, elapsedA8, NaN, ...
            cfg.orderScenario(iScenario).name, cfgCase.NumRevs_high);
        records = recId.records;
        recId = recId.id;
        ...
        fprintf('[OrderRev] %s | %2d rev | SNR %.1f dB | rep %d/%d | A %.4g Hz, A7 %.4g Hz, A8 %.4g Hz, B %.4g Hz, C %.4g Hz\n', ...
            ...
            mean(abs(sort(resultA7.f_id(:).') - sort(truth.f(:).'))), ...
            mean(abs(sort(resultA8.f_id(:).') - sort(truth.f(:).'))), ...
...
cfg.rootDir = rootDir;
cfg.outDir = outDir;
cfg.studyTag = 'order_rev_sensitivity_10dB_A8_hybrid_fast';
...
cfg.a8.minAsyncRevs = 12;
cfg.a8.fracOrderTol = 0.08;
...
function result = run_a6_frozen_high_template_varpro(cloud, cfg)
xGrid = cloud.xGrid(:);
T0 = estimate_template_from_cloud(cloud.x, cloud.V, xGrid, cfg.profile);
templateEval = make_profile_template_eval(T0, xGrid, cfg.profile);
x = cloud.x(:);
t = cloud.t(:);
V = cloud.V(:);
V0 = templateEval.F(x);
K1 = templateEval.dF(x);
DV = V - V0;
...
ub = [cfg.profile.ampUpperBound, pi, max(cfg.f1Grid), cfg.profile.ampUpperBound, pi, max(cfg.f2Grid)];
bestCost = inf;
pOpt = pCandidates(1, :);
templateRange = max(T0) - min(T0);
resFun = @(p) fixed_template_residual(p, cloud, templateRange, templateEval);
nTry = min(max(cfg.numVarproCandidates * 2, 8), size(pCandidates, 1));
for ic = 1:nTry
    p0 = min(max(pCandidates(ic, :), lb), ub);
    pTry = run_lsqnonlin_or_fminsearch(resFun, p0, lb, ub, 220);
...
pOpt(2) = wrap_to_pi_local(pOpt(2));
pOpt(5) = wrap_to_pi_local(pOpt(5));
T = transfer_template_from_anchor(cloud, T0, xGrid, pOpt, cfg.profile);
if pOpt(3) > pOpt(6)
    pOpt = [pOpt(4), pOpt(5), pOpt(6), pOpt(1), pOpt(2), pOpt(3)];
end
[rmse, residual, VFit, xi] = calc_profile_rmse(cloud, T, xGrid, pOpt);
result = struct('p', pOpt, 'A', [pOpt(1), pOpt(4)], 'f', [pOpt(3), pOpt(6)], ...
    'phi', [pOpt(2), pOpt(5)], 'T', T(:), 'xGrid', xGrid(:), 'xi', xi(:), ...
    'VFit', VFit(:), 'residual', residual(:), 'rmse', rmse, ...
    'costHist', bestCost, 'coarseCosts', coarseCosts, ...
    'label', 'A6-frozen-high-template-varpro');
result.g_used = NaN;
result.f_id = result.f;
result.A_id = result.A;
end

function result = run_a7_dominant_aligned_template_varpro(cloud, cfg)
xGrid = cloud.xGrid(:);
T0 = estimate_template_from_cloud(cloud.x, cloud.V, xGrid, cfg.profile);
p1 = estimate_dominant_mode_fixed_template(cloud, T0, xGrid, cfg);
p1As2 = [p1(1), p1(2), p1(3), 0, 0, mean(cfg.f2Grid)];
T1 = transfer_template_from_anchor(cloud, T0, xGrid, p1As2, cfg.profile);
templateEval = make_profile_template_eval(T1, xGrid, cfg.profile);
x = cloud.x(:);
t = cloud.t(:);
V = cloud.V(:);
V0 = templateEval.F(x);
...
[pCandidates, coarseCosts] = coarse_varpro_2freq_candidates(t, DV, K1, ...
    cfg.f1Grid, cfg.f2Grid, cfg.numVarproCandidates);
pCandidates = expand_order_alias_starts(pCandidates, cfg.profile, cfg.RPM_high / 60);
lb = [0.001, -pi, max(min(cfg.f1Grid), p1(3) - 15), 0.001, -pi, min(cfg.f2Grid)];
ub = [cfg.profile.ampUpperBound, pi, min(max(cfg.f1Grid), p1(3) + 15), cfg.profile.ampUpperBound, pi, max(cfg.f2Grid)];
bestCost = inf;
pOpt = pCandidates(1, :);
templateRange = max(T1) - min(T1);
resFun = @(p) fixed_template_residual(p, cloud, templateRange, templateEval);
nTry = min(max(cfg.numVarproCandidates * 2, 8), size(pCandidates, 1));
for ic = 1:nTry
    p0 = pCandidates(ic, :);
    p0(3) = min(max(p0(3), lb(3)), ub(3));
    p0 = min(max(p0, lb), ub);
    pTry = run_lsqnonlin_or_fminsearch(resFun, p0, lb, ub, 220);
    costTry = sum(resFun(pTry).^2);
    if costTry < bestCost
...
pOpt(2) = wrap_to_pi_local(pOpt(2));
pOpt(5) = wrap_to_pi_local(pOpt(5));
T = transfer_template_from_anchor(cloud, T1, xGrid, pOpt, cfg.profile);
if pOpt(3) > pOpt(6)
    pOpt = [pOpt(4), pOpt(5), pOpt(6), pOpt(1), pOpt(2), pOpt(3)];
end
[rmse, residual, VFit, xi] = calc_profile_rmse(cloud, T, xGrid, pOpt);
result = struct('p', pOpt, 'A', [pOpt(1), pOpt(4)], 'f', [pOpt(3), pOpt(6)], ...
    'phi', [pOpt(2), pOpt(5)], 'T', T(:), 'xGrid', xGrid(:), 'xi', xi(:), ...
    'VFit', VFit(:), 'residual', residual(:), 'rmse', rmse, ...
    'costHist', bestCost, 'coarseCosts', coarseCosts, 'pDominant', p1, ...
    'label', 'A7-dominant-aligned-template-varpro');
result.g_used = NaN;
result.f_id = result.f;
result.A_id = result.A;
end

function result = choose_a8_async_hybrid(resultA, resultA7, cfg)
fr = cfg.RPM_high / 60;
q = sort(resultA.f_id(:).') / fr;
fracDist = abs(q - round(q));
isLongAsync = cfg.NumRevs_high >= cfg.a8.minAsyncRevs && max(fracDist) > cfg.a8.fracOrderTol;
if isLongAsync
    result = resultA7;
    result.label = 'A8-async-hybrid-selected-A7';
else
    result = resultA;
    result.label = 'A8-async-hybrid-selected-A';
end
result.hybrid_used_A7 = isLongAsync;
end

function p1 = estimate_dominant_mode_fixed_template(cloud, T, xGrid, cfg)
templateEval = make_profile_template_eval(T, xGrid, cfg.profile);
x = cloud.x(:);
t = cloud.t(:);
V = cloud.V(:);
V0 = templateEval.F(x);
D = -templateEval.dF(x);
valid = isfinite(V0) & isfinite(D) & isfinite(V);
tFit = t(valid);
y = V(valid) - V0(valid);
D = D(valid);
best = struct('cost', inf, 'p', [0.01, 0, mean(cfg.f1Grid)]);
lambda = 1e-9 * max(nnz(valid), 1);
for f = cfg.f1Grid(:)'
    G = [D .* sin(2*pi*f*tFit), D .* cos(2*pi*f*tFit)];
    beta = (G' * G + lambda * eye(2)) \ (G' * y);
    res = y - G * beta;
    cost = sum(res.^2);
    if cost < best.cost
        best.cost = cost;
        best.p = [hypot(beta(1), beta(2)), atan2(beta(2), beta(1)), f];
    end
end
lb = [0.001, -pi, min(cfg.f1Grid)];
ub = [cfg.profile.ampUpperBound, pi, max(cfg.f1Grid)];
resFun = @(p) fixed_template_residual_1freq(p, cloud, max(range(T), eps), templateEval);
p1 = run_lsqnonlin_or_fminsearch(resFun, best.p, lb, ub, 180);
p1(2) = wrap_to_pi_local(p1(2));
end

function r = fixed_template_residual_1freq(p, cloud, templateRange, templateEval)
u = p(1) * sin(2*pi*p(3)*cloud.t(:) + p(2));
xi = cloud.x(:) - u;
