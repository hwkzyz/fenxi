%% Three-method comparison on the same simulated high-speed waveform
% Methods:
...
[noiseRatioBySnr, snrCal] = calibrate_full_waveform_snr(cfg, gapList, xCell, yCell, templateLibFull.domain);
records = repmat(make_empty_record(), numel(cfg.targetSNRdB) * cfg.numRepeat * 5, 1);
recId = 0;

for iSnr = 1:numel(cfg.targetSNRdB)
    for iRep = 1:cfg.numRepeat
        seed = cfg.baseSeed + 1000 * iSnr + iRep;
        rng(seed, 'twister');
        noiseRatio = noiseRatioBySnr(iSnr);
        [Data_High, truth] = simulate_common_highspeed_data(cfg, gapList, xCell, yCell, templateLibFull.domain, noiseRatio);
        measuredSnr = measure_full_waveform_snr(Data_High);
        highMap = map_highspeed_to_space(Data_High, cfg.alpha_k, cfg.R_tip, templateLibFull.domain, cfg.fitActiveLevel);
        cloud = make_cloud_from_highmap(highMap, templateLibFull.domain, cfg.profile.xGridN);

        tA = tic;
        resultA = run_blind_transfer_method(cloud, gapList, xCell, yCell, cfg);
        elapsedA = toc(tA);

        tA2 = tic;
        resultA2 = run_a2_constrained_varpro(cloud, gapList, xCell, yCell, cfg);
        elapsedA2 = toc(tA2);

        tA3 = tic;
        resultA3 = run_a3_physics_template_varpro(cloud, gapList, xCell, yCell, cfg);
        elapsedA3 = toc(tA3);

        tB = tic;
        staticState = estimate_highspeed_static_gap_raw(highMap, templateLib, cfg);
        rawScanResult = run_varpro_for_gap(highMap, templateLib, staticState.gHat, cfg, 'gap_library_raw');
        resultB = run_joint_refinement(highMap, templateLib, staticState, rawScanResult, cfg, cfg.jointRefine);
        elapsedB = toc(tB);

        tC = tic;
        resultC = run_varpro_for_gap(highMap, templateLibFull, cfg.g_holdout, cfg, 'same_gap_direct');
        elapsedC = toc(tC);

        recId = add_method_record(records, recId, cfg, truth, resultA, 'A_blind_transfer', ...
            cfg.targetSNRdB(iSnr), measuredSnr, noiseRatio, iRep, seed, elapsedA, NaN);
        records = recId.records;
        recId = recId.id;

        recId = add_method_record(records, recId, cfg, truth, resultA2, 'A2_constrained_varpro', ...
            cfg.targetSNRdB(iSnr), measuredSnr, noiseRatio, iRep, seed, elapsedA2, NaN);
        records = recId.records;
        recId = recId.id;

        recId = add_method_record(records, recId, cfg, truth, resultA3, 'A3_physics_template', ...
            cfg.targetSNRdB(iSnr), measuredSnr, noiseRatio, iRep, seed, elapsedA3, NaN);
        records = recId.records;
        recId = recId.id;

        recId = add_method_record(records, recId, cfg, truth, resultB, 'B_gap_library_joint', ...
            cfg.targetSNRdB(iSnr), measuredSnr, noiseRatio, iRep, seed, elapsedB, resultB.g_used);
        records = recId.records;
        recId = recId.id;

        recId = add_method_record(records, recId, cfg, truth, resultC, 'C_same_gap_direct', ...
            cfg.targetSNRdB(iSnr), measuredSnr, noiseRatio, iRep, seed, elapsedC, cfg.g_holdout);
        records = recId.records;
        recId = recId.id;

        fprintf('[Compare] SNR %.1f dB rep %d/%d | A %.4g Hz, A2 %.4g Hz, A3 %.4g Hz, B %.4g Hz, C %.4g Hz\n', ...
            cfg.targetSNRdB(iSnr), iRep, cfg.numRepeat, ...
...
resultPrefix = sprintf('three_method_%s', cfg.studyTag);
...
cfg.rootDir = rootDir;
cfg.outDir = outDir;
cfg.studyTag = 'low_snr_A3_template_verify';
...
function out = add_method_record(records, recId, cfg, truth, result, methodName, targetSnr, measuredSnr, noiseRatio, repeatId, seed, elapsed, gUsed)
recId = recId + 1;
...
groups = unique(trialTable(:, {'method', 'target_snr_db'}), 'rows', 'stable');
summary = repmat(struct('method', "", 'target_snr_db', NaN, ...
...
for i = 1:height(groups)
    idx = trialTable.method == groups.method(i) & trialTable.target_snr_db == groups.target_snr_db(i);
...
figure('Name', 'Three Method Comparison', 'Color', 'w', ...
    'Units', 'centimeters', 'Position', [2, 2, 30, 16]);
tiledlayout(2, 3, 'Padding', 'compact', 'TileSpacing', 'compact');
methods = unique(summaryTable.method, 'stable');
colors = lines(numel(methods));
nexttile; hold on;
for i = 1:numel(methods)
    idx = summaryTable.method == methods(i);
    plot(summaryTable.target_snr_db(idx), summaryTable.freq_error_mean_Hz(idx), ...
        'o-', 'LineWidth', 1.2, 'Color', colors(i, :), 'DisplayName', methods(i));
end
...
for i = 1:numel(methods)
    idx = summaryTable.method == methods(i);
    plot(summaryTable.target_snr_db(idx), summaryTable.success_rate(idx), ...
        'o-', 'LineWidth', 1.2, 'Color', colors(i, :), 'DisplayName', methods(i));
end
set(gca, 'XDir', 'reverse'); grid on;
xlabel('Full-waveform SNR (dB)');
...
for i = 1:numel(methods)
    idx = summaryTable.method == methods(i);
    plot(summaryTable.target_snr_db(idx), summaryTable.boundary_hit_rate(idx), ...
        'o-', 'LineWidth', 1.2, 'Color', colors(i, :), 'DisplayName', methods(i));
end
set(gca, 'XDir', 'reverse'); grid on;
xlabel('Full-waveform SNR (dB)');
...
ylim([-0.05, 1.05]);
title('Frequency edge locking');

nexttile; hold on;
for i = 1:numel(methods)
    idx = summaryTable.method == methods(i);
    plot(summaryTable.target_snr_db(idx), summaryTable.amp_error_mean_mm(idx), ...
        'o-', 'LineWidth', 1.2, 'Color', colors(i, :), 'DisplayName', methods(i));
end
set(gca, 'XDir', 'reverse'); grid on;
xlabel('Full-waveform SNR (dB)');
...
for i = 1:numel(methods)
    idx = summaryTable.method == methods(i);
    plot(summaryTable.target_snr_db(idx), summaryTable.rmse_mean(idx), ...
        'o-', 'LineWidth', 1.2, 'Color', colors(i, :), 'DisplayName', methods(i));
end
set(gca, 'XDir', 'reverse'); grid on;
xlabel('Full-waveform SNR (dB)');
ylabel('RMSE');
title('Waveform/profile fit');

nexttile; hold on;
for i = 1:numel(methods)
    idx = summaryTable.method == methods(i);
    plot(summaryTable.target_snr_db(idx), summaryTable.elapsed_mean_s(idx), ...
        'o-', 'LineWidth', 1.2, 'Color', colors(i, :), 'DisplayName', methods(i));
end
set(gca, 'XDir', 'reverse'); grid on;
xlabel('Full-waveform SNR (dB)');
