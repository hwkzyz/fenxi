...
%   A6: frozen high-speed median template VARPRO
...
for iScenario = 1:nScenario
    for iRev = 1:nRev
        ...
        tA6 = tic;
        resultA6 = run_a6_frozen_high_template_varpro(cloud, cfgCase);
        elapsedA6 = toc(tA6);
        ...
        recId = add_method_record(records, recId, cfgCase, truth, resultA6, 'A6_frozen_high_template', ...
            targetSnr, measuredSnr, noiseRatio, iRep, seed, elapsedA6, NaN, ...
            cfg.orderScenario(iScenario).name, cfgCase.NumRevs_high);
        records = recId.records;
        recId = recId.id;
        ...
        fprintf('[OrderRev] %s | %2d rev | SNR %.1f dB | rep %d/%d | A %.4g Hz, A4 %.4g Hz, A5 %.4g Hz, A6 %.4g Hz, A7 %.4g Hz, A2 %.4g Hz, A3 %.4g Hz, B %.4g Hz, C %.4g Hz\n', ...
            ...
            mean(abs(sort(resultA6.f_id(:).') - sort(truth.f(:).'))), ...
...
cfg.rootDir = rootDir;
cfg.outDir = outDir;
cfg.studyTag = 'A5_boundary_noninteger_20rev_10dB_failureSeeds';
...
cfg.orderScenario = struct( ...
    'name', {"noninteger_520_1270"}, ...
    'f_true', {[520, 1270]});
cfg.baseSeed = 20260430 + 100000 + 3000;
