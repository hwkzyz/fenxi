function R = run_ortho_b_gap(highMap, templateLib, cfg, staticState)
%run_ortho_b_gap  Orthogonal gap estimation with per-waveform bias removal.
...
for ig = 1:numel(gapGrid)
    J(ig) = projected_shape_cost_b(V, x - dxHat, gapGrid(ig), templateLib, gidx, ng);
end
...
[~, bHat, ~] = fit_affine_gain_bias(Fbase, V, [1, 1]);
VFit = Fbase + bHat;

R.method = "ortho_b_gap";
R.g_used = gHat;
...
R.eta_g = eta_gap_cb(templateLib, gHat, "b");
end
