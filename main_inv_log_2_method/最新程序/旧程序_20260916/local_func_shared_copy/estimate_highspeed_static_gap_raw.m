...
if get_cfg_field(cfg,'gapStaticUseAllTrustedSamples',true)
    sampleIdx=(1:numel(x)).';
else
    sampleIdx = pick_evenly_spaced_indices(numel(x), ...
        get_cfg_field(cfg, 'gapStaticTargetSamples', 4000));
end
xCoarse = x(sampleIdx);
yCoarse = y(sampleIdx);
...
for ig = 1:numel(gapGrid)
    g = gapGrid(ig);
    yPred = eval_gap_template(templateLib, g, xq);
    yPred=reshape(yPred,numel(x),numel(dxGrid));
    residual=y(:)-yPred;valid=isfinite(yPred);
    residual(~valid)=0;
    J2D(ig,:)=sum(residual.^2,1)./max(sum(valid,1),1);
end
end
