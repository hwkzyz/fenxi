...
peakAmpMae=anchorGap; oracleTransferNrmsePct=anchorGap;
pc1Explained=anchorGap; isExtrapolation=false(size(anchorGap)); row=0;

for it=1:nTilt
    trainTilt=setdiff(1:nTilt,it);
    % C(:,k) is the full fitted (B0,B1,B2) coefficient vector used for
    % subsequent gap-law interpolation.  The response coordinate itself is
    % defined from the measured response surfaces, not from this basis.
    C=zeros(nCoeff,numel(trainTilt));
    Sface=zeros(nX*nGap,numel(trainTilt));
    ...
    mu=mean(Sface,2); [U,Sv,~]=svd(Sface-mu,'econ');
    sv=diag(Sv);
    idxTarget=(it-1)*nGap+(1:nGap);
    pc1Explained(idxTarget)=sv(1)^2/max(sum(sv.^2),eps);
    Z=U(:,1)'*(Sface-mu);
    % Use the first latent coordinate only to define the ordered surface path.
    [z1,ord]=sort(Z,'ascend'); Cord=C(:,ord);
    for ia=1:nGap
        ...
        for kt=1:nT
            q=interp1(z1,Cord.',tgrid(kt),'linear','extrap').';
            B=reshape(q,nX,3);
            predAnchor(:,kt)=B*X(ia,:).';
        end
        d=sqrt(mean((predAnchor-y).^2,1)); [~,ih]=min(d);
        latentHat(row)=tgrid(ih); anchorRmse(row)=d(ih);
        isExtrapolation(row)=latentHat(row)<z1(1) || latentHat(row)>z1(end);
        qhat=interp1(z1,Cord.',latentHat(row),'linear','extrap').';
...
overall.pc1_explained_ratio=mean(T.pc1Explained);
overall.extrapolation_fraction=mean(T.isExtrapolation);
