...
% Match the experimental R5 implementation: evaluate the complete response
% surface in vector form.  build_path_template_model precomputes this
% interpolant on the trusted (gap,x) domain, so a nonlinear optimizer call
% performs two array evaluations instead of one response lookup per sample.
if isfield(pathModel,'pathCache') && ~isempty(pathModel.pathCache) && ...
        isfield(pathModel.pathCache,'interpolant') && ~isempty(pathModel.pathCache.interpolant)
    S = pathModel.pathCache.interpolant;
    rBase = S(gBase,xLib);
    rQuery = S(gQuery,xLib);
else
...
% The frozen low-speed path used by the current bridge has a scalar gap
% (mu=0).  Evaluate the whole query vector in one interpolation call rather
% than invoking eval_gap_template once per sample.  The general varying-gap
% case retains the pointwise fallback because its x/g pairs are distinct.
if isempty(g)
    y = zeros(size(g));
    return;
end
if all(g == g(1))
    y = eval_gap_template(templateLib,g(1),xq);
    return;
end
y = zeros(size(g));
for i = 1:numel(g)
