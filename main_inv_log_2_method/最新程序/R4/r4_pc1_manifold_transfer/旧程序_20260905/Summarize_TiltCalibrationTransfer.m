function Summary = Summarize_TiltCalibrationTransfer()
...
caseIds=["null_00","easy_10_26","X01_10_12"];
for ii=1:numel(caseIds)
    ic=caseIds(ii);
    for im=1:numel(methods)
        q=D.case_id==string(ic) & D.method==methods(im);T=D(q,:);
        check=[check;table(string(ic),methods(im),all(T.true_model_wins),max(abs(T.gap_error_mm)),...
            max(abs([T.A1_error_mm;T.A2_error_mm])),min(T.objective_margin),...
...
nexttile;q=D.case_id=="easy_10_26" & (D.method=="C1_g0_only" | D.method=="C2_g0_mu");plot_pair(D(q,:),'A2_error_mm','Second amplitude error (mm)');
nexttile;q=D.case_id=="X01_10_12" & (D.method=="C1_g0_only" | D.method=="C2_g0_mu");plot_pair(D(q,:),'objective_margin','X01 objective margin');
nexttile;q=D.case_id=="X01_10_12" & (D.method=="C1_g0_only" | D.method=="C2_g0_mu");plot_pair(D(q,:),'A2_error_mm','X01 second amplitude error (mm)');
...
fprintf(fid,'The static audit uses seven real COMSOL tilt geometries and five adjacent finite clearance increments per geometry. The dynamic audit is noiseless, uses g_low=0.9 mm and g_high=1.1 mm, and compares a regular EO pair with X01.\n\n');
