...
R2/R3/R4 的正式入口和目录边界见
`R2_R3_R4_主程序索引_20260916.md`。先看该索引，再运行具体路线；不要从
`local_func` 或输出目录反推主程序。

这是当前“反比-对数双基函数间隙-振动解耦方法”的 R2/R3/R4 主程序目录。

正式程序只在 `R2`、`R3`、`R4` 三个文件夹中；根目录下的 `旧程序_20260916`
保存历史 Route 30、绘图和其他辅助程序。运行时直接进入对应文件夹的入口，
不要从根目录搜索 MATLAB 文件。
...
- `R2`、`R3`、`R4`：三条独立、可运行的正式验证路线；每个文件夹均带有本路线所需的 `local_func` 副本。
