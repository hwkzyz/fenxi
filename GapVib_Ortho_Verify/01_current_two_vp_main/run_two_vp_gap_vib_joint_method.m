function R = run_two_vp_gap_vib_joint_method(highMap, templateLib, cfg, staticState)
%run_two_vp_gap_vib_joint_method
% Fast static gap init -> VP-aided gap correction -> VP vibration fit ->
% final projected joint refinement.
...
timerBranch = tic;
branchProjected = evaluate_gap_branch("vib_projected_gap", gapStateProjected, ...
    t, V, x, templateLib, cfg, gidx, ng, muList);
...
VFit = eval_gap_template(templateLib, bestJoint.g, ...
    x - bestJoint.dx - fit_u(bestJoint.p, t));

R.method = "two_vp_gap_vib_joint_method";
...
refinedList = repmat(struct('fit', [], 'sse', inf, 'candidate_index', 0), ...
    numel(candidateList), 1);
...
for ik = 1:jointKeep
    ...
    cfgJoint = cfg;
    if ~isfield(cfgJoint, 'projectedJointDxHalfWidth') || isempty(cfgJoint.projectedJointDxHalfWidth)
        cfgJoint.projectedJointDxHalfWidth = get_cfg_field(cfg, 'twoVpDxRefineHalfWidth', 0.05);
    end
    for imu = 1:numel(muList)
        jointTry = refine_projected_joint(t, V, x, templateLib, cfgJoint, gidx, ng, ...
