%% Inv_log_2 main: compact closed-loop comparison.
%
% Set runMode = "smoke", "quick", or "full" before running if needed.
close all;
clc;
    ...
    for isnr = 1:numel(snrList)
        ...
        for im = 1:height(modelDefs)
            ...
            templateLib = make_response_template_library(gapList, xCell, yCell, ...
                gTrue, cfgAna.xGridN, modelDefs(im, :), trustOptions);
...
if runMode == "smoke"
    fileStem = "inv_log_2_closed_loop";
else
    fileStem = "inv_log_2_closed_loop_" + runMode;
end
end
