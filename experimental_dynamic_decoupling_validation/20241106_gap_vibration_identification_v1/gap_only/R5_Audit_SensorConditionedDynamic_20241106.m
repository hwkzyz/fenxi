...
n=numel(R); nS=numel(R(1).delta_gap_mm); dg=reshape([R.delta_gap_mm],nS,n).';
...
for k=1:nS
    V.(sprintf('dg_S%d_mm',cfg.case.gapSensors(k)))=dg(:,k);
    q=nan(n,1); for i=1:n, s=R(i).sensor_rmse_mv; if numel(s)>=k,q(i)=s(k);end,end
    V.(sprintf('rmse_S%d_mV',cfg.case.gapSensors(k)))=q;
end
