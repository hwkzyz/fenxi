...
requiredFiles = {
    ...
    fullfile(cfg.paths.preparation, 'Prepare_AdaptiveSG_Template_20241106.m')
    };
bundledInputs = {
...
assert(strcmp(cfg.methodFreezeId, '20241106_UNIFIED_STRUCTURED_VOLTAGE_20260811'));
...
if ismember(mode, ["code","all"])
    ...
    assert(contains(gapText, 'same calibrated forward model') && ...
        ~contains(gapText, "F0 = Bsrc.template_v(:)"), ...
...
if verbose
    ...
    fprintf('Unified route: adaptive-SG/PCHIP, single-sync structured voltage Top-3, complete voltage optimization.\n');
if ismember(mode, ["results", "all"])
