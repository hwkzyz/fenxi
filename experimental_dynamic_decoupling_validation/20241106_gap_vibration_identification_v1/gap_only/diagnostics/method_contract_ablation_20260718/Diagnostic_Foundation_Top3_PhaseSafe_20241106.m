%% 07: Foundation fixed-gap identification for 20241106
clc; close all;

% Diagnostic copy only: keep the formal source untouched.
...
packageConfig = Config_20241106();
packageEnvState = Apply_Config_Env_20241106(packageConfig);
packageEnvCleanup = onCleanup(@() Restore_Config_Env_20241106(packageEnvState));
setenv('STEP05_OUTPUT_TAG', 'diag_contract_top3_phasesafe');
setenv('STEP05_FORCE_REBUILD', '1');

%STEP05_SINGLESYNC_DIRECTTEMPLATE_IDENTIFICATION_20241106
% VERSION: FOUNDATION_MAINPULSE_ADAPTIVE_ZEROETA_20260714
%
% Key logic:
...
%   5. The formal 20241106 fixed-gap route enforces the theoretical eta_s=0.
%      Static/free eta variants belong in comparison/diagnostic scripts.
...
S = struct();
S.method_name = 'foundation_mainpulse_adaptive_zero_eta';
S.version = 'FOUNDATION_MAINPULSE_ADAPTIVE_ZEROETA_PLAINRMSE_DF_COMMONLAP_20260718';
S.method_file_tag = 'FoundationMainPulseAdaptiveZeroEta';
...
if ~isempty(window_laps_env)
    ...
    if ~isfinite(parsed_window_laps) || parsed_window_laps < 1 || ...
            parsed_window_laps ~= floor(parsed_window_laps)
        error('STEP05_WINDOW_LAPS must be a positive integer.');
    end
    S.window_laps = parsed_window_laps;
end
sliding_step_env = strtrim(getenv('STEP05_SLIDING_STEP_LAPS'));
if ~isempty(sliding_step_env)
    parsed_sliding_step = str2double(sliding_step_env);
    if ~isfinite(parsed_sliding_step) || parsed_sliding_step < 1 || ...
            parsed_sliding_step ~= floor(parsed_sliding_step)
        error('STEP05_SLIDING_STEP_LAPS must be a positive integer.');
    end
    S.sliding_step_laps = parsed_sliding_step;
end

analysis_start_time_env = strtrim(getenv('STEP05_ANALYSIS_START_TIME_SEC'));
if ~isempty(analysis_start_time_env)
    parsed_start_time = str2double(analysis_start_time_env);
    if ~isfinite(parsed_start_time) || parsed_start_time <= 0
        error('STEP05_ANALYSIS_START_TIME_SEC must be a positive number.');
    end
...
if ~isempty(run_tag_env)
    ...
    S.output_dir = fullfile(cfg.step05_output_dir, run_tag_env);
    S.figure_dir = fullfile(cfg.figure_root, ...
        ['step05_single_sync_direct_template_', run_tag_env]);
elseif ~isempty(analysis_start_time_env)
    time_tag = strrep(sprintf('T%07.3f', S.analysis_start_time_s), '.', 'p');
    auto_run_tag = sprintf('%s_W%dS%d_F%d_%d', ...
        time_tag, S.window_laps, S.sliding_step_laps, ...
        round(S.freq_search_hz(1)), round(S.freq_search_hz(2)));
    S.output_dir = fullfile(cfg.step05_output_dir, auto_run_tag);
    S.figure_dir = fullfile(cfg.figure_root, ...
        ['step05_single_sync_direct_template_', auto_run_tag]);
end
...
analysis_sensors_env = strtrim(getenv('STEP05_ANALYSIS_SENSORS'));
if ~isempty(analysis_sensors_env)
    parsed_sensors = sscanf(analysis_sensors_env, '%d').';
    if isempty(parsed_sensors) || any(parsed_sensors < 1)
        error('STEP05_ANALYSIS_SENSORS must contain positive sensor IDs.');
    end
    S.analysis_sensors = unique(parsed_sensors, 'stable');
    S.reference_sensor_id = S.analysis_sensors(1);
end
target_laps_env = strtrim(getenv('STEP05_TARGET_LAPS'));
if ~isempty(target_laps_env)
    parsed_target_laps = str2double(target_laps_env);
    if ~isfinite(parsed_target_laps) || parsed_target_laps < S.window_laps || ...
            parsed_target_laps ~= floor(parsed_target_laps)
        error('STEP05_TARGET_LAPS must be an integer not smaller than the window length.');
    end
    S.target_laps = parsed_target_laps;
end
freq_search_env = strtrim(getenv('STEP05_FREQ_SEARCH_HZ'));
if ~isempty(freq_search_env)
    parsed_freq = sscanf(freq_search_env, '%f').';
    if numel(parsed_freq) ~= 2 || any(~isfinite(parsed_freq)) || ...
            parsed_freq(1) <= 0 || parsed_freq(2) <= parsed_freq(1)
        error('STEP05_FREQ_SEARCH_HZ must contain two increasing positive values.');
    end
    S.freq_search_hz = parsed_freq;
end
...
S.vp_top_k_min = 3;
S.vp_top_k_max = 3;
S.vp_gap_ratio_keep = 0;
S.include_neighbor_eo = false;
...
S.active_eta_model = 'theoretical_zero_eta';
S.eta_models = {S.active_eta_model};
S.eta_reference_sensor = S.analysis_sensors(1);
S.sensor_eta_mode = 'fixed_zero';
S.static_eta_source = 'theoretical_zero';
S.static_eta_file = '';
...
S.objective_mode = 'plain_rmse';
S.continuousFrequencyRefine = isfield(cfg, 'frequency') && ...
    isfield(cfg.frequency, 'refineEachCandidate') && cfg.frequency.refineEachCandidate;
S.frequencyRefineHalfWidthHz = get_cfg_scalar_local(cfg, 'frequencyRefineHalfWidthHz', 2.0);
if isfield(cfg, 'frequency') && isfield(cfg.frequency, 'searchHz')
    S.freqSearchHz = cfg.frequency.searchHz(:).';
else
    S.freqSearchHz = [300 1000];
end
S.method_preset = 'foundation_mainpulse_adaptive_zero_eta';
% Step07 performs full-wave identification from this bundle. Truncating it can
% change the EO ranking, so the formal result must retain all pulse points.
...
if ~isempty(method_preset_env)
    error(['STEP05_METHOD_PRESET belongs to archived 20241106 Step05 routes. ' ...
        'The official fixed-gap Step05 uses adaptive VP Top-K screening with theoretical eta=0.']);
end

if ~isempty(strtrim(getenv('STEP05_FORCED_EO')))
    error('Final 20241106 fixed-gap Step05 does not allow forced EO. Clear STEP05_FORCED_EO.');
end

static_eta_file_env = strtrim(getenv('STEP05_STATIC_ETA_FILE'));
if ~isempty(static_eta_file_env)
    error('Final 20241106 Step05 enforces eta=0; STEP05_STATIC_ETA_FILE is no longer accepted.');
end
...
fprintf('EO candidate policy: %s (adaptive VP Top-K screening).\n', ...
...
S.static_eta_prior_info = table(S.analysis_sensors(:), ...
    ...
    repmat("theoretical_zero", numel(S.analysis_sensors), 1), ...
    'VariableNames', {'sensor_id', 'eta_prior_mm', ...
    'eta_plan_iqr_mm', 'joint_static_eta_file'});
fprintf('Static eta source: theoretical zero, eta=%s mm\n', ...
...
if isfield(cfg, 'step04_sensor_ids') && ~isempty(cfg.step04_sensor_ids)
    sensor_tag = ['S', sprintf('%d', cfg.step04_sensor_ids)];
else
...
for ib = 1:numel(S.target_blades)
    ...
    temp_result_file = fullfile(out_dir, 'Result_tmp.mat');
    if isfile(temp_result_file)
        delete(temp_result_file);
    end
    save(temp_result_file, 'Result', '-v7.3');
    [move_ok, move_message] = movefile(temp_result_file, result_file, 'f');
    if ~move_ok
        error('Unable to finalize Step05 result %s: %s', result_file, move_message);
    end
...
method.vp_seed_model = 'VP excludes eta_s; the formal full fit enforces theoretical eta_s=0';
method.eo_selection = sprintf(['adaptive_full_fit: VP scans all EO, retains at least top-%d, ' ...
    'near-best gap <= %.3g, neighbor expansion=%d, capped at %d full-fit EO'], ...
...
method.eta_gauge = 'theoretical eta_s=0 for every analysis sensor';
...
candidate_template = struct( ...
    ...
    'frequency_refinement_exitflag', NaN, ...
    'objective_score', inf, ...
    'weighted_voltage_rmse', inf, ...
    'plain_voltage_rmse', inf, ...
    'point_count', bundle.point_count, ...
...
for i = 1:numel(final_eos)
    ...
    integer_p = p_best;
    integer_freq = EO * bundle.rot_freq_mean_hz;
    refine_exitflag = NaN;
    if isfield(S, 'continuousFrequencyRefine') && S.continuousFrequencyRefine && ...
            isfield(bundle, 'T') && numel(bundle.T) == numel(bundle.theta)
        half_width = max(0, S.frequencyRefineHalfWidthHz);
        z0 = [p_best(:).', integer_freq];
        lb = [p_best(:).' - inf(size(p_best(:).')), max(S.freqSearchHz(1), integer_freq-half_width)];
        ub = [p_best(:).' + inf(size(p_best(:).')), min(S.freqSearchHz(2), integer_freq+half_width)];
        % The physical parameters are already bounded by unpack/project;
        % only the frequency coordinate is bounded explicitly here.
        zobj = @(z) direct_template_objective_frequency_local(z, EO, bundle, S);
        optsFreq = optimset(opts, 'MaxIter', max(opts.MaxIter, 400), ...
            'MaxFunEvals', max(opts.MaxFunEvals, 1200));
        try
            [zbest, ~, refine_exitflag] = fminsearch(@(z) bounded_frequency_objective_local( ...
                z, zobj, lb(end), ub(end)), z0, optsFreq);
            zbest(end) = min(max(zbest(end), lb(end)), ub(end));
            p_best = zbest(1:end-1);
            integer_p = zbest(1:end-1);
            integer_freq = zbest(end);
        catch
            p_best = integer_p;
        end
    end
    ...
    [wrmse, prmse, ~, ~, objective_score] = evaluate_full_template_model_local( ...
        bundle, EO, A, phi, dx_c, eta, S, integer_freq);
    ...
    Candidate(i).freq_hz = integer_freq;
    Candidate(i).integer_freq_hz = EO * bundle.rot_freq_mean_hz;
    Candidate(i).frequency_offset_hz = integer_freq - Candidate(i).integer_freq_hz;
    Candidate(i).frequency_refinement_exitflag = refine_exitflag;
...
result.fn_id = best.freq_hz;
result.integer_freq_hz = best.integer_freq_hz;
result.frequency_offset_hz = best.frequency_offset_hz;
...
function value = direct_template_objective_frequency_local(z, EO, bundle, S)
[A, phi, dx_c, eta] = unpack_params_local(z(1:end-1), S);
[~, ~, ~, ~, objective_score] = evaluate_full_template_model_local( ...
    bundle, EO, A, phi, dx_c, eta, S, z(end));
if ~isfinite(objective_score)
    value = 1e12;
else
    value = objective_score;
...
function value = bounded_frequency_objective_local(z, fun, lo, hi)
z(end) = min(max(z(end), lo), hi);
value = fun(z);
end

function [wrmse, prmse, Vpred, coverage, objective_score] = evaluate_full_template_model_local( ...
    bundle, EO, A, phi, dx_c, eta, S, frequency_hz)

if nargin < 8 || ~isfinite(frequency_hz)
    frequency_hz = EO * bundle.rot_freq_mean_hz;
end

sensors = S.analysis_sensors(:).';

if isfield(bundle, 'T') && numel(bundle.T) == numel(bundle.theta)
    detuning_hz = frequency_hz - EO * bundle.rot_freq_mean_hz;
    phase_arg = EO .* bundle.theta(:) + 2*pi*detuning_hz .* ...
        (bundle.T(:) - mean(bundle.T(:), 'omitnan'));
else
    phase_arg = EO .* bundle.theta(:);
end
y = A .* sin(phase_arg + phi);
...
if nnz(valid) < 5
    ...
    residual_obj = nansum(res(valid).^2);
    objective_score = residual_obj / max(nnz(valid), 1);
    wrmse = sqrt(nansum(w(valid) .* res(valid).^2) / max(nansum(w(valid)), eps));
    prmse = sqrt(objective_score);
    corr_score = NaN;
end
...
if exist('objective_score', 'var')
    ...
    coverage.overshoot_penalty = 0;
    coverage.sensor_eta_reg_penalty = 0;
else
...
if isempty(Trend) || height(Trend) == 0
    ...
    Summary.dominant_eo_window_count = 0;
    Summary.dominant_eo_window_fraction = NaN;
    Summary.excluded_mixed_eo_windows = 0;
    Summary.mean_freq_hz = NaN;
    Summary.median_freq_hz = NaN;
    Summary.std_freq_hz = NaN;
    Summary.mean_amplitude_mm = NaN;
    Summary.std_amplitude_mm = NaN;
    Summary.mean_dx_mm = NaN;
    Summary.mean_rmse_mV = NaN;
    Summary.raw_all_eo_mean_freq_hz = NaN;
    Summary.raw_all_eo_mean_amplitude_mm = NaN;
    return;
end
...
Summary.window_count = Summary.valid_window_count;
...
dominant = valid & Trend.EO_id == Summary.trend_dominant_eo;
Summary.dominant_eo_window_count = nnz(dominant);
Summary.dominant_eo_window_fraction = nnz(dominant) / nnz(valid);
Summary.excluded_mixed_eo_windows = nnz(valid & ~dominant);
Summary.mean_freq_hz = mean(Trend.fn_id(dominant), 'omitnan');
Summary.median_freq_hz = median(Trend.fn_id(dominant), 'omitnan');
Summary.std_freq_hz = std(Trend.fn_id(dominant), 'omitnan');
Summary.mean_amplitude_mm = mean(Trend.A_id(dominant), 'omitnan');
Summary.std_amplitude_mm = std(Trend.A_id(dominant), 'omitnan');
Summary.mean_dx_mm = mean(Trend.dx_c_id(dominant), 'omitnan');
Summary.mean_rmse_mV = 1000 * mean(Trend.weighted_voltage_rmse(dominant), 'omitnan');
Summary.raw_all_eo_mean_freq_hz = mean(Trend.fn_id(valid), 'omitnan');
Summary.raw_all_eo_mean_amplitude_mm = mean(Trend.A_id(valid), 'omitnan');
