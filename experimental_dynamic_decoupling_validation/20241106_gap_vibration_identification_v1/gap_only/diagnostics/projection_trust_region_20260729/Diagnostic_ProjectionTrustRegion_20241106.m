...
packageSensorTag = ['S', sprintf('%d', packageConfig.case.analysisSensors)];
packageTimeTag = strrep(sprintf('T%07.3f', ...
    packageConfig.case.analysisStartTimeSec), '.', 'p');
packageFoundationTag = sprintf('%s_W%dS%d_F%d_%d', packageTimeTag, ...
    packageConfig.window.windowBladePasses, ...
    packageConfig.window.slidingStepBladePasses, ...
    round(packageConfig.frequency.searchHz(1)), ...
    round(packageConfig.frequency.searchHz(2)));
packageFoundationFile = fullfile(packageConfig.paths.foundationResults, ...
    packageFoundationTag, packageConfig.case.dynamicCase, sprintf( ...
    'Result_Step05_FoundationMainPulseAdaptiveZeroEta_B%d_%s_20241106.mat', ...
    packageConfig.case.targetBlade, packageSensorTag));
packageDynamicFile = fullfile(packageConfig.paths.gapRuntime, sprintf( ...
    'Step06_BuildGapAwareDynamicMap_20241106_B%d_%s_%s_W%dS%d.mat', ...
    packageConfig.case.targetBlade, packageSensorTag, packageTimeTag, ...
    packageConfig.window.windowBladePasses, ...
    packageConfig.window.slidingStepBladePasses));
assert(isfile(packageFoundationFile), ...
    'Run program 07 first. Missing exact Foundation result: %s', packageFoundationFile);
assert(isfile(packageDynamicFile), ...
    'Run program 08 first. Missing exact DynamicMap: %s', packageDynamicFile);
setenv('STEP07J_FOUNDATION_STEP05_RESULT_FILE', packageFoundationFile);
setenv('STEP07J_LOCAL_BUNDLE_SOURCE', 'foundation_step05_bundle');
setenv('STEP07J_DYNAMIC_MAP_FILE', packageDynamicFile);
...
resultOutDir = fullfile(thisDir, 'results');
...
cfg.maxWindows = parse_positive_integer_env_local( ...
    'DIAG_PROJECTION_MAX_WINDOWS', cfg.maxWindowsConfig);
...
cfg.maxPointsPerWindow = parse_positive_integer_env_local('DIAG_PROJECTION_MAX_POINTS', inf);
...
diagProjectionUse = strtrim(getenv('DIAG_PROJECTION_USE'));
if ~isempty(diagProjectionUse)
    cfg.useDeltaGapProjectionLimit = parse_logical_env_local('DIAG_PROJECTION_USE', true);
end
...
resultSuffix = sanitize_suffix_local(strtrim(getenv('DIAG_PROJECTION_RESULT_SUFFIX')));
...
runtimeTemplateBankFile = fullfile(packageCfg.paths.calibrationRuntime, ...
    flowCfg.calibration.runtimeTemplateBankFile);
bundledTemplateBankFile = fullfile(packageCfg.paths.calibrationGap, ...
    flowCfg.calibration.templateBankFile);
runtimeGapBankFile = fullfile(packageCfg.paths.calibrationRuntime, ...
    flowCfg.calibration.runtimeGapBankFile);
bundledGapBankFile = fullfile(packageCfg.paths.calibrationGap, ...
    flowCfg.calibration.gapBankFile);
if isfile(runtimeTemplateBankFile)
    templateBankFile = runtimeTemplateBankFile;
else
    templateBankFile = bundledTemplateBankFile;
end
if isfile(runtimeGapBankFile)
    gapBankFile = runtimeGapBankFile;
else
    gapBankFile = bundledGapBankFile;
end
...
if isfield(Sd.DynamicMap, 'SourceSettings') && isstruct(Sd.DynamicMap.SourceSettings)
    ...
    if isfield(sourceSettings, 'windowLaps') && ...
        ...
        error(['DynamicMap uses %d-lap windows, but the active main configuration requires %d laps. ' ...
            'Rerun Main08_Build_GapAware_DynamicMap_20241106.m.'], ...
            sourceSettings.windowLaps, flowCfg.identification.windowBladePasses);
    end
...
for iw = 1:numWindows
    ...
    assert(~phaseInfo.usedPreviousWindow, ...
        'Final contract forbids previous-window phase information (window %d).', iw);
    ...
    if cfgWindow.useDeltaGapProjectionLimit
        deltaGapProjection = estimate_delta_gap_projection_limits_local( ...
            bundle, seedTable, selectedEO, cfgWindow);
        cfgWindow.DeltaGapProjection = deltaGapProjection;
    else
        deltaGapProjection = empty_delta_gap_projection_local(bundle, cfgWindow);
    end
...
for i = 1:numel(eoCandidates)
    ...
    thetaOpt = bound_params_local(thetaOpt, cfgCandidate, modeName, ...
        numel(bundle.sensorIds), bundle.sensorIds);
    detail = evaluate_static_warp_waveform_local( ...
        thetaOpt, eo, bundle, cfgCandidate, modeName);
    if cfgCandidate.continuousFrequencyRefine && mode_has_gap_local(modeName)
        detail = refine_continuous_frequency_fit_local( ...
            bundle, detail, cfgCandidate, modeName);
    end
    candidate(i).EO = eo;
...
cfgOut.deltaGapLowerMm = -cfg.deltaGapPriorScaleMm * ones(1, nSensor);
cfgOut.deltaGapUpperMm = cfg.deltaGapPriorScaleMm * ones(1, nSensor);
...
if ~mode_has_gap_local(modeName), return; end
nSensor = numel(bundle.sensorIds);
...
theta0 = bound_params_local(theta0, cfg, modeName, nSensor, bundle.sensorIds);
end
...
eoCandidates = unique(round(eoCandidates(:).'));
rowTemplate = projection_row_template_local();
...
proj = struct('alpha', cfg.deltaGapProjAlpha, 'AllTable', struct2table(allRows));
end

function rows = estimate_delta_gap_projection_candidate_local(bundle, A, phi, dx, eo, cfg, rowTemplate)
nSensor = numel(bundle.sensorIds);
active = false(1, nSensor);
for is = 1:nSensor
    active(is) = bundle_sensor_has_gap_correction_local(bundle, is);
end
...
for is = find(~active)
    ...
    rows(is).deltaGapLimitMm = 0;
    rows(is).deltaGapLowerMm = 0;
    rows(is).deltaGapUpperMm = 0;
end
if isempty(activeIdx), return; end
nPoint = numel(bundle.V);
v0 = nan(nPoint, 1);
Jg = zeros(nPoint, numel(activeIdx));
Jnu = nan(nPoint, 4);
...
for is = 1:nSensor
    ...
    if active(is)
        ...
        [v0(mask), ~] = eval_static_warp_template_local( ...
            Tpl, bundle.responseSurface, corr, 0, 0, 0, xEval, bundle, is);
        [vXp, ~] = eval_static_warp_template_local( ...
            Tpl, bundle.responseSurface, corr, 0, 0, 0, xEval + hX, bundle, is);
        [vXm, ~] = eval_static_warp_template_local( ...
            Tpl, bundle.responseSurface, corr, 0, 0, 0, xEval - hX, bundle, is);
    else
        [v0(mask), ~] = eval_direct_or_base_template_with_overshoot_local( ...
            bundle, is, Tpl, xEval);
        [vXp, ~] = eval_direct_or_base_template_with_overshoot_local( ...
            bundle, is, Tpl, xEval + hX);
        [vXm, ~] = eval_direct_or_base_template_with_overshoot_local( ...
            bundle, is, Tpl, xEval - hX);
    end
    dMdx = (vXp - vXm) ./ (2 * hX);
    Jnu(mask, 1) = -dMdx;
    ...
    ia = find(activeIdx == is, 1);
    if ~isempty(ia)
        corr = get_corrected_sensor_local(bundle.CorrectedGapLibrary, sid);
        [vGp, ~] = eval_static_warp_template_local( ...
            Tpl, bundle.responseSurface, corr, hG, 0, 0, xEval, bundle, is);
        [vGm, ~] = eval_static_warp_template_local( ...
            Tpl, bundle.responseSurface, corr, -hG, 0, 0, xEval, bundle, is);
        Jg(mask, ia) = (vGp - vGm) ./ (2 * hG);
    end
end
r = bundle.V(:) - v0;
valid = isfinite(r) & isfinite(bundle.W(:)) & ...
    all(isfinite(Jg), 2) & all(isfinite(Jnu), 2);
if nnz(valid) < max(8, size(Jnu, 2) + numel(activeIdx))
    for is = activeIdx
        rows(is) = fallback_projection_row_local( ...
            rowTemplate, bundle.sensorIds(is), eo, cfg);
    end
...
for ia = 1:numel(activeIdx)
    ...
    if weak
        ...
        halfWidth = min(max(halfWidthRaw, cfg.deltaGapProjMinLimitMm), ...
            cfg.deltaGapProjMaxLimitMm);
        center = min(max(dgJoint(ia), -cfg.deltaGapProjMaxLimitMm), ...
            cfg.deltaGapProjMaxLimitMm);
        lower = max(-cfg.deltaGapProjMaxLimitMm, center - halfWidth);
        upper = min(cfg.deltaGapProjMaxLimitMm, center + halfWidth);
        rows(is) = rowTemplate;
        ...
        rows(is).deltaGapLsMm = abs(dgJoint(ia));
        rows(is).deltaGapLsSignedMm = dgJoint(ia);
        ...
        rows(is).clippedMax = halfWidthRaw > cfg.deltaGapProjMaxLimitMm;
    ...
    rows(is).projectedGapRank = gapRank;
    rows(is).nuisanceCondition = nuisanceCondition;
    rows(is).projectedGapCondition = gapCondition;
end
end

function row = projection_row_template_local()
row = struct('sensorId', NaN, 'EO', NaN, 'epsilonPerpMv', NaN, ...
...
row.sensorId = sensorId; row.EO = eo;
row.deltaGapProjMm = limit / max(cfg.deltaGapProjAlpha, eps);
row.deltaGapLocMm = limit; row.deltaGapLimitMm = limit;
row.deltaGapLsMm = 0; row.deltaGapLsSignedMm = 0;
row.deltaGapCenterMm = 0;
row.deltaGapLowerMm = -limit; row.deltaGapUpperMm = limit;
row.usedFallback = true;
end

function proj = empty_delta_gap_projection_local(bundle, cfg)
rows = repmat(projection_row_template_local(), numel(bundle.sensorIds), 1);
for is = 1:numel(rows)
    rows(is) = fallback_projection_row_local( ...
        rows(is), bundle.sensorIds(is), NaN, cfg);
end
proj = struct('alpha', cfg.deltaGapProjAlpha, 'AllTable', struct2table(rows));
end

function [U, rankA, condA] = stable_column_basis_local(A)
if isempty(A), U = zeros(size(A,1),0); rankA = 0; condA = 1; return; end
[Uall, S, ~] = svd(A, 'econ'); s = diag(S);
if isempty(s) || ~isfinite(s(1)) || s(1) <= 0
    U = zeros(size(A,1),0); rankA = 0; condA = inf; return;
end
rankA = sum(s > max(size(A)) * eps(s(1)));
U = Uall(:,1:rankA);
...
if isfield(F, 'gap_only')
    ...
    if isfield(selectionInfo, 'DeltaGapProjection') && ...
            isfield(selectionInfo.DeltaGapProjection, 'AllTable') && ...
            all(ismember(projectionColumns, ...
            selectionInfo.DeltaGapProjection.AllTable.Properties.VariableNames))
