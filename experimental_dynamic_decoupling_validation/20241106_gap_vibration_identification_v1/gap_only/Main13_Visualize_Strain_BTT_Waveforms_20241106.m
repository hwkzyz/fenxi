function Main13_Visualize_Strain_BTT_Waveforms_20241106(selectedCase)
%% Visualize strain-BTT validation (20241106)
...
addpath(script_dir, fullfile(script_dir, 'functions', 'foundation'), ...
    fullfile(script_dir, 'functions', 'preparation'), ...
    fullfile(script_dir, 'functions', 'gap_aware'), ...
    fullfile(script_dir, 'functions', 'utilities'));
packageCfg = Config_20241106();
case_catalog = visualization_case_catalog_local(packageCfg);
if nargin < 1
    selectedCase = [];
end
case_spec = select_visualization_case_local(case_catalog, selectedCase);
if isempty(case_spec)
    fprintf('Strain-BTT validation case selection cancelled.\n');
    return;
end

cfg = struct();
cfg.route_dir = script_dir;
cfg.foundation_dir = script_dir;
cfg.target_blade = packageCfg.case.targetBlade;
cfg.analysis_sensors = packageCfg.case.analysisSensors;
cfg.dynamic_case_name = packageCfg.case.dynamicCase;
cfg.analysis_start_time = case_spec.analysisStartTimeSec;
cfg.case_time_tag = case_spec.timeTag;
cfg.case_display_name = case_spec.displayName;
cfg.analysis_start_tolerance_sec = 0.1;
cfg.strain_to_btt_prior_offset_sec = packageCfg.strain.timeOffsetPriorSec;
cfg.strain_to_btt_offset_sec = cfg.strain_to_btt_prior_offset_sec;
cfg.global_alignment_file = fullfile(packageCfg.paths.strainValidation, sprintf( ...
    'Step12_GlobalTimeAlignment_20241106_B%d_S%s.mat', ...
    packageCfg.case.targetBlade, sprintf('%d', packageCfg.case.analysisSensors)));
cfg.global_alignment = load_global_alignment_config_local( ...
    cfg.global_alignment_file, cfg.strain_to_btt_prior_offset_sec);
cfg.strain_to_btt_offset_sec = ...
    cfg.global_alignment.strainToBttOffsetSec;
cfg.method_name = 'gap_only';
cfg.primary_strain_channel = 'AI1-04';
cfg.method_result_file = fullfile(packageCfg.paths.gapResults, ...
    case_spec.resultFileName);
cfg.show_figures = true;
cfg.save_figures = true;
cfg.figure_dir = fullfile(packageCfg.paths.strainValidation, ...
    sprintf('strain_btt_validation_%s_physicalB4_R1p13087_curved632_AI1_04', ...
    case_spec.timeTag));
cfg.figure_style = 'nature';
cfg.export_pdf = true;
cfg.zoom_half_width_ms = 0.35;
cfg.enable_strain_reference = true;
% Final strain-to-tip scaling for this 20241106 validation:
%   Workbench blade with the real curved root, mode frequency 632.388232542 Hz.
%   Gauge point in Workbench coordinates: Y=4.00 mm from the root,
%   Z=2.80 mm from the edge; X=0 and 1.4 mm are the two physical faces.
%   Transfer definition: R=abs(EPELY / UX_tip_m), with natural surface
%   interpolation performed separately on the two faces.
%   R_front=1.1308777609 1/m, R_back=1.1308564487 1/m;
%   final front/back mean R=1.1308671048 1/m.
...
cfg.fe_transfer_ratio_label = ...
    '632.388 Hz Workbench curved-root FE, Y4.00-Z2.80, R=1.130867 1/m';
cfg.fe_model_frequency_hz = 632.3882325418;
cfg.fe_gauge_location = 'Workbench Y=4.00 mm, Z=2.80 mm; faces X=0/1.4 mm';
cfg.fe_response_definition = ...
    'abs(EPELY / UX_tip_m), natural interpolation per face';
cfg.fe_transfer_ratio_front_1_per_m = 1.1308777609;
cfg.fe_transfer_ratio_back_1_per_m = 1.13085644867;
cfg.fe_transfer_ratio_1_per_m = mean([ ...
    cfg.fe_transfer_ratio_front_1_per_m, ...
    cfg.fe_transfer_ratio_back_1_per_m]);
cfg.fe_transfer_ratio_source_file = ['D:\博士-国科\试验台数据\大台子\叶片模型\' ...
    '580Hz传递比分析\workbench_direct_export\workbench_direct_nodes_632Hz.csv'];
cfg.strain_to_tip_mm = 1 / cfg.fe_transfer_ratio_1_per_m / 1000;
cfg.strain_alignment_file = fullfile(packageCfg.paths.strainEvidence, ...
...
% Keep the strain estimate independent enough to expose a wrong BTT EO.
cfg.strain_reference_freq_half_band_hz = 80;
...
TimeAudit = validate_time_selection_local(MethodResult, cfg);
Result = adapt_method_result_local(MethodResult, cfg);
style = build_step6_style_local(cfg);
StrainRef03 = build_step6_strain_reference_local(Result, cfg, 'AI1-03');
StrainRef04 = build_step6_strain_reference_local(Result, cfg);
StrainRef = StrainRef04;

fprintf('Selected validation case: %s\n', cfg.case_display_name);
print_method_summary_local(MethodResult, Result, cfg, result_file);
print_time_selection_summary_local(TimeAudit);
export_time_selection_audit_local(TimeAudit, cfg, result_file);
print_step6_strain_reference_summary_local(StrainRef);
print_step6_strain_reference_summary_local(StrainRef03);
if is_valid_strain_reference_local(StrainRef03) && is_valid_strain_reference_local(StrainRef04)
    export_step6_dual_strain_reference_csv_local(StrainRef03, StrainRef04, cfg);
    corr03 = corr(StrainRef03.BTTAmpMM, StrainRef03.AmpTipMM, 'Rows', 'complete');
    corr04 = corr(StrainRef04.BTTAmpMM, StrainRef04.AmpTipMM, 'Rows', 'complete');
    fprintf('  Amplitude-trend Pearson correlation | AI1-03 = %.4f | AI1-04 = %.4f\n', ...
        corr03, corr04);
end
...
if cfg.show_figures
    ...
    if is_valid_strain_reference_local(StrainRef)
        ...
        if is_valid_strain_reference_local(StrainRef03)
            fig_strain_channels = plot_strain_channel_comparison_local( ...
                Result, cfg, style, StrainRef03, StrainRef04);
...
function cases = visualization_case_catalog_local(packageCfg)
times = unique([packageCfg.strain.alignmentCasesSec(:); ...
    packageCfg.case.analysisStartTimeSec], 'stable');
...
for i = 1:numel(times)
    ...
    cases(i).resultFileName = formal_gap_result_name_local(packageCfg, times(i));
end
end
...
function selected = select_visualization_case_local(cases, requested)
selected = struct([]);
if isempty(requested)
    if ~usejava('desktop')
        error(['No validation case was specified. Call the function with 75.0 or 86.6, ' ...
            'for example Main13_Visualize_Strain_BTT_Waveforms_20241106(86.6).']);
    end
    [index, accepted] = listdlg('PromptString', ...
        'Select the strain-BTT validation result to display:', ...
        'SelectionMode', 'single', 'ListString', {cases.displayName}, ...
        'Name', 'Strain-BTT validation case', 'ListSize', [380, 120]);
    if ~accepted
        return;
    end
...
if isempty(index)
    ...
    error('Unknown validation case. Available cases: %s.', available);
end
selected = cases(index);
end
...
if ~isfile(result_file)
    error('Could not find the local decoupling result file: %s', result_file);
end
end


function audit = validate_time_selection_local(MethodResult, cfg)
if ~isfield(MethodResult, 'WindowResult') || isempty(MethodResult.WindowResult)
    error('The selected Step07J result has no WindowResult entries.');
end

window_result = MethodResult.WindowResult(:);
window_start = arrayfun(@(x) x.timeWindow(1), window_result);
window_end = arrayfun(@(x) x.timeWindow(2), window_result);
result_start = min(window_start);
result_end = max(window_end);
if any(~isfinite([result_start, result_end])) || result_end <= result_start
    error('The selected Step07J result has an invalid time span.');
end

if abs(result_start - cfg.analysis_start_time) > cfg.analysis_start_tolerance_sec
    error(['Step07J result time mismatch: configured start is %.6f s, but the ' ...
        'first saved window starts at %.6f s. Select the matching result file.'], ...
        cfg.analysis_start_time, result_start);
end

bundle_source = "unknown";
if isfield(MethodResult, 'cfg') && isfield(MethodResult.cfg, 'localBundleSource')
    bundle_source = string(MethodResult.cfg.localBundleSource);
end
if bundle_source ~= "foundation_step05_bundle"
    error(['The selected Step07J result did not use the matching Foundation ' ...
        'physical-blade waveform bundle (source: %s).'], bundle_source);
end

if ~isfield(MethodResult, 'flowConfig') || ...
        ~isfield(MethodResult.flowConfig, 'identification')
    error('The selected Step07J result has no saved identification configuration.');
end
identification = MethodResult.flowConfig.identification;
if ~isfield(identification, 'analysisStartTimeSec') || ...
        abs(identification.analysisStartTimeSec - cfg.analysis_start_time) > 1e-9
    error('Step07J saved start time does not match selected %.6f s case.', ...
        cfg.analysis_start_time);
end
if ~isfield(identification, 'targetBlade') || ...
        identification.targetBlade ~= cfg.target_blade
    error('Step07J target is not physical blade B%d.', cfg.target_blade);
end
if ~isfield(identification, 'analysisSensors') || ...
        ~isequal(identification.analysisSensors(:).', cfg.analysis_sensors(:).')
    error('Step07J sensors do not match the required physical-B%d sensor set %s.', ...
        cfg.target_blade, mat2str(cfg.analysis_sensors));
end
required_window_fields = {'targetBladePasses', 'windowBladePasses', ...
    'slidingStepBladePasses'};
if ~all(isfield(identification, required_window_fields))
    error('Step07J result lacks the saved blade-pass window configuration.');
end
planned_window_count = floor((identification.targetBladePasses - ...
    identification.windowBladePasses) / identification.slidingStepBladePasses) + 1;
if planned_window_count < 1 || numel(window_result) ~= planned_window_count
    error(['Step07J window count mismatch: saved result has %d windows, but ' ...
        'the saved %d-pass/%d-pass/step-%d configuration requires %d.'], ...
        numel(window_result), identification.targetBladePasses, ...
        identification.windowBladePasses, identification.slidingStepBladePasses, ...
        planned_window_count);
end

required_local_fields = {'dynamicFile', 'correctedLibFile'};
for i_field = 1:numel(required_local_fields)
    field_name = required_local_fields{i_field};
    if ~isfield(MethodResult, field_name) || isempty(MethodResult.(field_name)) || ...
            ~is_path_within_route_local(MethodResult.(field_name), cfg.route_dir)
        error('Step07J source %s is not folder-local: %s', ...
            field_name, string_or_empty_local(MethodResult, field_name));
    end
end
dynamic_data = load(MethodResult.dynamicFile, 'DynamicMap');
if ~isfield(dynamic_data, 'DynamicMap') || ...
        ~isfield(dynamic_data.DynamicMap, 'BladeNumbering') || ...
        ~isfield(dynamic_data.DynamicMap.BladeNumbering, 'scheme') || ...
        ~strcmpi(dynamic_data.DynamicMap.BladeNumbering.scheme, ...
        'foundation_physical_blade_id')
    error('Step07J DynamicMap does not use Foundation physical blade IDs.');
end
if ~isfield(dynamic_data.DynamicMap, 'TargetBlade') || ...
        dynamic_data.DynamicMap.TargetBlade ~= cfg.target_blade || ...
        ~isfield(dynamic_data.DynamicMap, 'SensorIDs') || ...
        ~isequal(dynamic_data.DynamicMap.SensorIDs(:).', cfg.analysis_sensors(:).')
    error('Step07J DynamicMap is not physical B%d with sensors %s.', ...
        cfg.target_blade, mat2str(cfg.analysis_sensors));
end
if ~isfield(MethodResult, 'templateFile') || isempty(MethodResult.templateFile) || ...
        ~is_path_within_route_local(MethodResult.templateFile, cfg.foundation_dir)
    error('Step07J template is not the Foundation physical-blade template: %s', ...
        string_or_empty_local(MethodResult, 'templateFile'));
end
if ~isfield(MethodResult, 'foundationStep05File') || ...
        isempty(MethodResult.foundationStep05File) || ...
        ~is_path_within_route_local(MethodResult.foundationStep05File, cfg.foundation_dir)
    error('Foundation physical-blade Step05 source is missing or unexpected: %s', ...
        string_or_empty_local(MethodResult, 'foundationStep05File'));
end
if isfield(MethodResult, 'cfg') && isfield(MethodResult.cfg, 'fixedSensorEtaInfo') && ...
        isfield(MethodResult.cfg.fixedSensorEtaInfo, 'file')
    eta_info = MethodResult.cfg.fixedSensorEtaInfo;
    eta_file = string(eta_info.file);
    uses_theoretical_zero = eta_file == "built_in_theoretical_zero";
    if uses_theoretical_zero
        has_zero_values = isfield(MethodResult.cfg, 'fixedSensorEtaMm') && ...
            all(MethodResult.cfg.fixedSensorEtaMm(:) == 0);
        has_zero_source = isfield(eta_info, 'source') && ...
            string(eta_info.source) == "theoretical_zero";
        if ~has_zero_values || ~has_zero_source
            error('Built-in eta source is not the verified theoretical-zero configuration.');
        end
    elseif ~is_path_within_route_local(eta_info.file, cfg.foundation_dir)
        error('Fixed eta runtime source is not the Foundation physical-blade route: %s', ...
            eta_info.file);
    end
end
if ~isfield(MethodResult, 'FoundationStep05TimeCheck') || ...
        string(MethodResult.FoundationStep05TimeCheck.action) ~= "foundation_step05_bundle"
    error('Foundation Step05 time-source consistency check was not passed.');
end

if ~isfile(cfg.strain_alignment_file)
    error('Missing Step04 resonance/alignment file: %s', cfg.strain_alignment_file);
end
alignment_data = load(cfg.strain_alignment_file, ...
    'regionTable', 'strainToBttOffsetSec');
if ~isfield(alignment_data, 'regionTable') || isempty(alignment_data.regionTable)
    error('Step04 file does not contain a nonempty regionTable.');
end
if ~isfield(alignment_data, 'strainToBttOffsetSec') || ...
        ~isfinite(alignment_data.strainToBttOffsetSec)
    error('Step04 file does not contain a finite strainToBttOffsetSec.');
end

step04_offset_sec = alignment_data.strainToBttOffsetSec;
if abs(step04_offset_sec - cfg.strain_to_btt_prior_offset_sec) > 1e-9
    error(['Strain/BTT prior mismatch: configured %.9f s, Step04 file %.9f s. ' ...
        'Do not compare the signals until the prior is reconciled.'], ...
        cfg.strain_to_btt_prior_offset_sec, step04_offset_sec);
end
offset_sec = cfg.strain_to_btt_offset_sec;

region_table = alignment_data.regionTable;
required_vars = {'bttStartSec', 'bttEndSec', 'peakBttTimeSec'};
if ~all(ismember(required_vars, region_table.Properties.VariableNames))
    error('Step04 regionTable is missing required BTT time columns.');
end
region_idx = find(result_start >= region_table.bttStartSec & ...
    result_start <= region_table.bttEndSec, 1, 'first');
if isempty(region_idx)
    error(['Step07J result starts at %.6f s, outside every Step04 resonance ' ...
        'region. Refuse to create a resonance-validation figure.'], result_start);
end

region_start = region_table.bttStartSec(region_idx);
region_end = region_table.bttEndSec(region_idx);
if result_end > region_end + cfg.analysis_start_tolerance_sec
    error(['Step07J result ends at %.6f s, outside the selected Step04 ' ...
        'resonance region [%.6f, %.6f] s.'], result_end, region_start, region_end);
end

% BTT time = raw strain time + strainToBttOffsetSec.
strain_start = result_start - offset_sec;
strain_end = result_end - offset_sec;

audit = struct();
audit.ConfiguredStartSec = cfg.analysis_start_time;
audit.ResultStartSec = result_start;
audit.ResultEndSec = result_end;
audit.ResonanceStartSec = region_start;
audit.ResonanceEndSec = region_end;
audit.ResonancePeakSec = region_table.peakBttTimeSec(region_idx);
audit.StrainToBttOffsetSec = offset_sec;
audit.Step04PriorOffsetSec = step04_offset_sec;
audit.AlignmentCandidateOffsetSec = cfg.global_alignment.estimatedOffsetSec;
audit.AlignmentAccepted = cfg.global_alignment.accepted;
audit.AlignmentDecision = string(cfg.global_alignment.decision);
audit.StrainStartSec = strain_start;
audit.StrainEndSec = strain_end;
audit.BundleSource = bundle_source;
audit.PhysicalBladeId = identification.targetBlade;
audit.AnalysisSensors = identification.analysisSensors(:).';
audit.BladeNumberingScheme = string(dynamic_data.DynamicMap.BladeNumbering.scheme);
audit.WindowCount = numel(window_result);
audit.TargetBladePasses = identification.targetBladePasses;
audit.WindowBladePasses = identification.windowBladePasses;
audit.SlidingStepBladePasses = identification.slidingStepBladePasses;
end


function tf = is_path_within_route_local(file_path, route_dir)
file_path = char(string(file_path));
route_dir = char(string(route_dir));
tf = startsWith(lower(file_path), [lower(route_dir), filesep]);
end


function value = string_or_empty_local(S, field_name)
value = "";
...
function print_time_selection_summary_local(audit)
fprintf(['  Time-source audit passed\n' ...
    '    physical blade = B%d | sensors = %s | numbering = %s\n' ...
    '    windows = %d (%d target passes, %d-pass window, step %d)\n' ...
    '    configured BTT start = %.6f s\n' ...
    '    Step07J BTT span = %.6f-%.6f s\n' ...
    '    Step04 resonance region = %.6f-%.6f s | peak = %.6f s\n' ...
    '    applied strainToBttOffsetSec = %.6f s | raw strain span = %.6f-%.6f s\n' ...
    '    alignment candidate = %.6f s | accepted = %d | %s\n' ...
    '    Step07J waveform source = %s\n'], ...
    audit.PhysicalBladeId, mat2str(audit.AnalysisSensors), ...
    audit.BladeNumberingScheme, audit.WindowCount, audit.TargetBladePasses, ...
    audit.WindowBladePasses, audit.SlidingStepBladePasses, ...
    audit.ConfiguredStartSec, audit.ResultStartSec, audit.ResultEndSec, ...
    audit.ResonanceStartSec, audit.ResonanceEndSec, audit.ResonancePeakSec, ...
    audit.StrainToBttOffsetSec, audit.StrainStartSec, audit.StrainEndSec, ...
    audit.AlignmentCandidateOffsetSec, audit.AlignmentAccepted, ...
    audit.AlignmentDecision, ...
    audit.BundleSource);
end


function export_time_selection_audit_local(audit, cfg, result_file)
audit_table = table( ...
    string(result_file), audit.PhysicalBladeId, ...
    string(mat2str(audit.AnalysisSensors)), audit.BladeNumberingScheme, ...
    audit.WindowCount, audit.TargetBladePasses, audit.WindowBladePasses, ...
    audit.SlidingStepBladePasses, audit.ConfiguredStartSec, ...
    audit.ResultStartSec, audit.ResultEndSec, audit.ResonanceStartSec, ...
    audit.ResonanceEndSec, audit.ResonancePeakSec, ...
    audit.Step04PriorOffsetSec, audit.StrainToBttOffsetSec, ...
    audit.AlignmentCandidateOffsetSec, audit.AlignmentAccepted, ...
    audit.AlignmentDecision, audit.StrainStartSec, audit.StrainEndSec, ...
    audit.BundleSource, ...
    'VariableNames', {'ResultFile', 'PhysicalBladeId', 'AnalysisSensors', ...
    'BladeNumberingScheme', 'WindowCount', 'TargetBladePasses', ...
    'WindowBladePasses', 'SlidingStepBladePasses', 'ConfiguredStartSec', ...
    'ResultStartSec', 'ResultEndSec', 'ResonanceStartSec', ...
    'ResonanceEndSec', 'ResonancePeakSec', 'Step04PriorOffsetSec', ...
    'StrainToBttOffsetSec', 'AlignmentCandidateOffsetSec', ...
    'AlignmentAccepted', 'AlignmentDecision', ...
    'StrainStartSec', 'StrainEndSec', 'BundleSource'});
audit_file = fullfile(cfg.figure_dir, sprintf( ...
    'TimeSourceAudit_20241106_B%d.csv', cfg.target_blade));
writetable(audit_table, audit_file);
fprintf('  Saved source audit = %s\n', audit_file);
end
...
elseif isfield(loaded_alignment, 'strain') && ...
        isfield(loaded_alignment, 'strainToBttOffsetSec')
    alignment = struct();
    alignment.strain_file = loaded_alignment.strain.file;
    alignment.best_tau_sec = -loaded_alignment.strainToBttOffsetSec;
else
    StrainRef.message = sprintf('Alignment file has no supported strain alignment: %s', alignment_file);
    warning('%s', StrainRef.message);
    return;
end
alignment.best_tau_sec = -cfg.strain_to_btt_offset_sec;
...
StrainRef.FEModelFrequencyHz = cfg.fe_model_frequency_hz;
StrainRef.FEGaugeLocation = string(cfg.fe_gauge_location);
StrainRef.FEResponseDefinition = string(cfg.fe_response_definition);
StrainRef.FESourceFile = string(cfg.fe_transfer_ratio_source_file);
...
alignment_file = fullfile(cfg.project_dir, 'output', case_name, 'Step3_Spectrum_RPM_Alignment_20241106.mat');
end
...
T = table(StrainRef.WindowID, ...
    ...
    repmat(StrainRef.FEModelFrequencyHz, numel(StrainRef.WindowID), 1), ...
    repmat(StrainRef.FEGaugeLocation, numel(StrainRef.WindowID), 1), ...
    repmat(StrainRef.FEResponseDefinition, numel(StrainRef.WindowID), 1), ...
    repmat(StrainRef.FESourceFile, numel(StrainRef.WindowID), 1), ...
...
function export_step6_dual_strain_reference_csv_local(Strain03, Strain04, cfg)
if ~exist(cfg.figure_dir, 'dir')
    mkdir(cfg.figure_dir);
end
    csv_file = fullfile(cfg.figure_dir, sprintf( ...
        'StrainBTT_B%d_StrainChannelComparison.csv', cfg.target_blade));
T = table(Strain03.WindowID, Strain03.BTTAmpMM, Strain03.BTTFreqHz, ...
    Strain03.AmpTipMM, Strain03.FreqHz, ...
    Strain04.AmpTipMM, Strain04.FreqHz, ...
    Strain03.AmpDifferenceMM, Strain04.AmpDifferenceMM, ...
    Strain03.AmpRatioBTTtoStrain, Strain04.AmpRatioBTTtoStrain, ...
    'VariableNames', {'WindowID', 'WaveformAmp_mm', 'WaveformFreq_Hz', ...
    'AI1_03_StrainDerivedAmp_mm', 'AI1_03_StrainFreq_Hz', ...
    'AI1_04_StrainDerivedAmp_mm', 'AI1_04_StrainFreq_Hz', ...
    'WaveformMinusAI1_03_mm', 'WaveformMinusAI1_04_mm', ...
    'WaveformToAI1_03_Ratio', 'WaveformToAI1_04_Ratio'});
writetable(T, csv_file);
end
...
if isempty(selected_idx)
    selected_idx = find(startsWith(file_names, 'AI1-04', 'IgnoreCase', true), 1, 'first');
end
if isempty(selected_idx)
    selected_idx = 1;
end
...
function fig = plot_strain_channel_comparison_local(Result, cfg, style, Strain03, Strain04)
xv = Result.Trends.WindowID(:);
btt_amp = Result.Trends.Amp(:);
btt_freq = Result.Trends.Freq(:);
color03 = style.colors.strain_ref;
color04 = style.colors.accent4;
...
plot(ax, xv, Strain03.AmpTipMM, '-s', 'Color', color03, ...
    'MarkerFaceColor', 'w', 'LineWidth', style.line_aux, ...
    'MarkerSize', style.marker_small, 'DisplayName', 'AI1-03');
plot(ax, xv, Strain04.AmpTipMM, '-^', 'Color', color04, ...
    'MarkerFaceColor', 'w', 'LineWidth', style.line_aux, ...
    'MarkerSize', style.marker_small, 'DisplayName', 'AI1-04');
...
plot(ax, xv, btt_freq, '-o', 'Color', style.colors.signal, ...
    'MarkerFaceColor', 'w', 'LineWidth', style.line_main, ...
    'MarkerSize', style.marker_main, 'DisplayName', 'BTT');
plot(ax, xv, Strain03.FreqHz, '-s', 'Color', color03, ...
    'MarkerFaceColor', 'w', 'LineWidth', style.line_aux, ...
    'MarkerSize', style.marker_small, 'DisplayName', 'AI1-03');
plot(ax, xv, Strain04.FreqHz, '-^', 'Color', color04, ...
    'MarkerFaceColor', 'w', 'LineWidth', style.line_aux, ...
    'MarkerSize', style.marker_small, 'DisplayName', 'AI1-04');
...
ax = nexttile; hold(ax, 'on');
plot(ax, xv, btt_amp ./ Strain03.AmpTipMM, '-s', 'Color', color03, ...
    'MarkerFaceColor', 'w', 'LineWidth', style.line_aux, ...
    'MarkerSize', style.marker_small, 'DisplayName', 'BTT/AI1-03');
plot(ax, xv, btt_amp ./ Strain04.AmpTipMM, '-^', 'Color', color04, ...
    'MarkerFaceColor', 'w', 'LineWidth', style.line_aux, ...
    'MarkerSize', style.marker_small, 'DisplayName', 'BTT/AI1-04');
...
component_curve.engine_order = Result.Trends.EO(selected_idx);
component_curve.title_text = sprintf('Median-error Window %d EO%d Component', ...
    window_id(selected_idx), round(component_curve.engine_order));
...
function Alignment = load_global_alignment_config_local(alignmentFile, priorOffsetSec)
