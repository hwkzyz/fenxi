...
if strcmpi(templateOverride, packageCfg.files.lowSpeedTemplate) && ~isfile(templateOverride)
    Prepare_AdaptiveSG_Template_20241106(false);
end
if ~isempty(templateOverride)
...
% Rebuild the candidate linearization from the active template and the
% same calibrated forward model used by the complete voltage fit. Cached
