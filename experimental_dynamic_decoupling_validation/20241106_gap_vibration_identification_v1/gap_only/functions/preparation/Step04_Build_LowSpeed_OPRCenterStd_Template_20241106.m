...
if isfield(cfg, 'step04_sensor_ids') && ~isempty(cfg.step04_sensor_ids)
    cfg.sensor_ids = unique(cfg.step04_sensor_ids(:).', 'stable');
end
...
fprintf('Target blades: %s (blade numbering still uses 1..%d)\n', mat2str(cfg.step04_target_blades), cfg.blades_num);
...
if ~isfield(cfg, 'step04_target_blades') || isempty(cfg.step04_target_blades)
    cfg.step04_target_blades = 1:cfg.blades_num;
end
cfg.step04_target_blades = unique(cfg.step04_target_blades(:).', 'stable');
if any(cfg.step04_target_blades < 1) || any(cfg.step04_target_blades > cfg.blades_num)
    error('Step04:InvalidTargetBlades', 'cfg.step04_target_blades must be within 1..cfg.blades_num.');
end
...
if ~isfield(cfg, 'step04_stable_window_lap_counts') || isempty(cfg.step04_stable_window_lap_counts)
    cfg.step04_stable_window_lap_counts = [30 40 50];
end
...
for i = 1:numel(sensor_ids)
    thresholds(i) = get_sensor_threshold_local(cfg, sensor_ids(i));
end
...
case_data.opr_timing_method = 'multi_threshold_width_center';
...
speed_time = opr_times(1:(end - events_per_rev));
...
max_entries = numel(cfg.sensor_ids) * numel(target_blades);
...
for iSensor = 1:numel(cfg.sensor_ids)
    ...
    for iFile = 1:numel(file_ids)
        ...
        for k = 1:numel(pulse_idx_in_file)
            ...
            if ~ismember(blade_id, target_blades)
                continue;
            end
...
if isfield(cfg, 'step04_common_window_lap_range') && ...
        isnumeric(cfg.step04_common_window_lap_range) && ...
        numel(cfg.step04_common_window_lap_range) == 2
    lap_range = round(cfg.step04_common_window_lap_range(:).');
    assert(all(isfinite(lap_range)) && lap_range(2) >= lap_range(1), ...
        'Invalid Step04 common low-speed lap range.');
    fprintf('>>> [Step04] Applying common low-speed lap window %d-%d to every sensor...\n', ...
        lap_range(1), lap_range(2));
    for i = 1:numel(point_cloud)
        pc = point_cloud(i);
        available = unique(pc.lap_index(isfinite(pc.lap_index)));
        assert(all(ismember(lap_range(1):lap_range(2), available)), ...
            'CH%d B%d does not cover the configured common lap window %d-%d.', ...
            pc.sensor_id, pc.blade_id, lap_range(1), lap_range(2));
        point_cloud(i) = subset_point_cloud_by_lap_window_step04_local( ...
            pc, lap_range(1), lap_range(2));
        point_cloud(i).stable_window_enabled = true;
        point_cloud(i).stable_window_start_lap = lap_range(1);
        point_cloud(i).stable_window_end_lap = lap_range(2);
        point_cloud(i).stable_window_lap_count = diff(lap_range) + 1;
        point_cloud(i).stable_window_score = NaN;
        point_cloud(i).stable_window_candidate_count = 1;
        point_cloud(i).stable_window_reason = 'common_window_from_package_config';
        point_cloud(i).stable_window_candidates = table();
    end
    return;
end
...
for i = 1:numel(point_cloud)
    ...
    point_cloud(i).stable_window_candidates = candidates;
    chosen = choose_stable_window_candidate_step04_local(candidates);
...
for lap_count_raw = cfg.step04_stable_window_lap_counts(:).'
    ...
    for start_lap = start_laps
        ...
        row.SelectedPointCount = row.WidePointCount;
        row.xc_mm = median(pulse_stats.xc_mm(valid_center), 'omitnan');
        row.SelectedWidthMm = median(pulse_stats.support_width_mm(valid_center), 'omitnan');
        row.LocalXcMAD = mad(pulse_stats.xc_mm(valid_center), 1);
        row.RMSE = mad(pulse_stats.feature_v(valid_feature), 1);
        row.BoundRatio = 1 - min(row.PulseCount / max(lap_count, 1), 1);
        row.LeftGradientRatio = 0.5;
        row.RightGradientRatio = 0.5;
        ...
        row.Reason = "pulse_stats_scored_candidate";
        row_count = row_count + 1;
        rows(row_count, 1) = row; %#ok<AGROW>
    end
end
...
for i = 1:n
    ...
    if nnz(high) >= 3 && sum(vz(high), 'omitnan') > eps
        xc_mm(i) = sum(x(high) .* vz(high), 'omitnan') ./ sum(vz(high), 'omitnan');
        support_width_mm(i) = prctile(x(high), 95) - prctile(x(high), 5);
    end
end
...
function chosen = choose_stable_window_candidate_step04_local(candidates)
if isempty(candidates)
    chosen = table();
    return;
end
...
row = struct( ...
    ...
    'xc_mm', NaN, 'SelectedWidthMm', NaN, ...
    'RMSE', NaN, 'BoundRatio', NaN, 'LocalXcMAD', NaN, ...
...
blade_pos = find(cfg.step04_target_blades == blade_id, 1, 'first');
if isempty(sensor_pos) || isempty(blade_pos)
    error('Step04:UnexpectedSensorBlade', 'Unexpected Step04 point for CH%d Blade%d.', sid, blade_id);
end
idx = (sensor_pos - 1) * numel(cfg.step04_target_blades) + blade_pos;
end
...
% Match the original 20241106 OPRCenterStd template route: all samples from
...
% Keep the sign when t1 is before t0. This matches the original 20241106
...
Template.Target_Blades = cfg.step04_target_blades;
...
if ~strcmpi(strtrim(Template.Step01_Probe_Arrival_Method), strtrim(Template.Step04_Probe_Arrival_Method))
    warning('Step04:ProbeArrivalMethodMismatch', ...
        'Step01 probe method=%s, Step04 raw extraction probe method=%s.', ...
        Template.Step01_Probe_Arrival_Method, Template.Step04_Probe_Arrival_Method);
end
...
empty = struct( ...
    ...
    'baseline', NaN, 'threshold', NaN, 'amplitude', NaN, ...
...
baseline = NaN;
if nargin >= 5 && isstruct(pc) && isfield(pc, 'baseline_by_pulse') && ~isempty(pc.baseline_by_pulse)
...
if strcmpi(cfg.step04_center_mode, 'sgfit')
    if strcmpi(cfg.step04_template_point_selection_mode, 'wide_window') && nnz(strict_mask) >= 30
        xc = refine_center_by_sg_fit_step04_local(x_wide(strict_mask), v_wide(strict_mask), baseline, xc);
    else
        xc = refine_center_by_sg_fit_step04_local(x_selected, v_selected, baseline, xc);
    end
end
...
% mirrors the 20241106 low-speed rotating calibration route before the
...
metadata.step01_probe_arrival_method = Template.Step01_Probe_Arrival_Method;
metadata.step04_probe_arrival_method = Template.Step04_Probe_Arrival_Method;
...
metadata.target_blades = Template.Target_Blades;
end
...
target_blades = get_template_target_blades_local(Template);
colors = lines(max(Template.Blades_Num, max(target_blades)));
...
for iSensor = 1:numel(sensor_ids)
    ...
    fig_raw = figure('Name', sprintf('Step04 raw template points CH%d', sid), ...
        ...
        'Visible', visibility_state_local(show_plots));
    tile_cols = min(3, max(1, numel(target_blades)));
    tile_rows = ceil(numel(target_blades) / tile_cols);
    tiledlayout(fig_raw, tile_rows, tile_cols, 'TileSpacing', 'compact', 'Padding', 'compact');
...
[pulse_mat, width_mat, amp_mat] = summary_to_matrices_local(S, sensor_ids, target_blades);

nexttile;
imagesc(target_blades, 1:numel(sensor_ids), pulse_mat);
set(gca, 'YTick', 1:numel(sensor_ids), 'YTickLabel', compose('CH%d', sensor_ids), 'XTick', target_blades);
xlabel('Blade ID'); ylabel('Sensor'); title('Pulse count'); colorbar;

nexttile;
imagesc(target_blades, 1:numel(sensor_ids), width_mat);
set(gca, 'YTick', 1:numel(sensor_ids), 'YTickLabel', compose('CH%d', sensor_ids), 'XTick', target_blades);
xlabel('Blade ID'); ylabel('Sensor'); title('Trusted domain width (mm)'); colorbar;

nexttile;
imagesc(target_blades, 1:numel(sensor_ids), amp_mat);
set(gca, 'YTick', 1:numel(sensor_ids), 'YTickLabel', compose('CH%d', sensor_ids), 'XTick', target_blades);
...
function target_blades = get_template_target_blades_local(Template)
...
function [pulse_mat, width_mat, amp_mat] = summary_to_matrices_local(S, sensor_ids, target_blades)
...
for iSensor = 1:numel(sensor_ids)
    ...
    for ib = 1:numel(target_blades)
        ...
        pulse_mat(iSensor, ib) = S.pulse_count(idx);
        width_mat(iSensor, ib) = S.domain_width_mm(idx);
        amp_mat(iSensor, ib) = S.amplitude_v(idx);
    end
end
end
...
required_top = {'SchemaVersion', 'Dataset', 'Case_Name', 'Sensor_IDs', ...
    'Blades_Num', 'Target_Blades', 'R_Tip_mm', 'OPR_Events_Per_Revolution', ...
    'Step01_Probe_Arrival_Method', 'Step04_Probe_Arrival_Method', ...
...
expected_count = numel(cfg.sensor_ids) * numel(cfg.step04_target_blades);
...
for i = 1:numel(Template.SensorBlade)
    ...
    if ~ismember(sid, cfg.sensor_ids) || ~ismember(blade_id, cfg.step04_target_blades)
        error('Step04:TemplateContract', ...
