%% 10: compare the formal fixed-gap and gap-aware results
...
packageEnvState = Apply_Config_Env_20241106(packageCfg);
...
outDir = strtrim(getenv('STEP07J_FVF_OUTPUT_DIR'));
if isempty(outDir)
    outDir = packageCfg.paths.comparison;
end
if exist(outDir, 'dir') ~= 7
    mkdir(outDir);
end
step07jOutDir = packageCfg.paths.gapResults;
...
if exist(foundationFile, 'file') ~= 2
    error('Run program 07 first. Missing exact zero-eta Foundation trend: %s', foundationFile);
end

allowMainFallback = parse_logical_env_local('STEP07J_FVF_ALLOW_MAIN_FALLBACK', false);
step07jFile = resolve_step07j_trend_file_local(step07jOutDir, C0, allowMainFallback);
...
switch char(C.dataset)
    case '20241106'
        timeTag = strrep(sprintf('T%07.3f', ...
            packageCfg.case.analysisStartTimeSec), '.', 'p');
        runTag = sprintf('%s_W%dS%d_F%d_%d', timeTag, ...
            packageCfg.window.windowBladePasses, ...
            packageCfg.window.slidingStepBladePasses, ...
            round(packageCfg.frequency.searchHz(1)), ...
            round(packageCfg.frequency.searchHz(2)));
        file = fullfile(packageCfg.paths.foundationResults, runTag, ...
            packageCfg.case.dynamicCase, sprintf( ...
            'Trend_Step05_FoundationMainPulseAdaptiveZeroEta_%s_%s.csv', ...
            C.caseTag, C.dataset));
    case '20250527'
...
if strcmp(char(C.dataset), '20241106')
    packageCfg = Config_20241106();
    foundationRoot = packageCfg.paths.foundationResults;
else
    foundationRoot = fullfile(rootDir, sprintf('%s_btt_data_foundation', C.dataset), 'output');
end
patterns = {
    sprintf('Trend_Step05_FoundationMainPulseAdaptiveZeroEta_%s_%s.csv', C.caseTag, C.dataset)
    sprintf('Trend_Step05_SingleSyncDirectTemplate_%s_%s.csv', C.caseTag, C.dataset)
    };
file = '';
...
patterns = {
    sprintf('Step07J_NestedStaticWarp_VPFullWave_Trend_%s_%s%s.csv', C.dataset, C.caseTag, resultSuffix)
...
for i = 1:numel(patterns)
    ...
    if ~isempty(hits)
        allHits = [allHits; hits(:)]; %#ok<AGROW>
    end
end
if ~isempty(allHits)
    file = pick_latest_file_local(allHits);
end
end
...
specs = struct( ...
    ...
    'rmse', {'direct_rmse_mV', 'fixed_plain_rmse_mV', 'gap_plain_rmse_mV', 'tilt_plain_rmse_mV'}, ...
    'rmseScale', {1, 1, 1, 1}, ...
