function models = R5_Build_SensorConditionedDynamicModels_20241106(sidecarFile, templateFile, targetBlade, v1Calibration)
...
for i=1:numel(L.sensor)
    ...
    m=struct('sensorId',sid,'bladeId',targetBlade,'gapReferenceMm',st.gap_mm,'isGapSensor',true,...
        ...
        'evaluate',@(dg,xq) eval_one(st.B,L.x_mm,L.gap_mm,scalar_gap(L.gref_mm),tpl,reg,scalar_gap(st.gap_mm),dg,xq));
    models(end+1)=m; %#ok<AGROW>
end

function g=scalar_gap(g)
% A sensor-conditioned model has one effective baseline gap.  Older
% sidecars may store duplicated endpoint values; collapse those here.
g=double(g(:)); g=g(isfinite(g));
assert(~isempty(g),'R5:MissingBaselineGap');
g=mean(g);
end
