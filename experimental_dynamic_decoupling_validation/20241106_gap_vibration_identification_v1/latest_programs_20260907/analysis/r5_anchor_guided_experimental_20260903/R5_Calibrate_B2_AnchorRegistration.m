...
assert(numel(b2GapMm)==numel(sensorIds),'R5:B2GapSensorMismatch', ...
    'One anchor gap is required for each anchor sensor.');
...
for is=1:numel(sensorId)
    gAnchor = b2GapMm(is);
    % The legacy bank reports sensor-specific known gaps that need not lie
    % on the response-surface training grid.  Evaluate the fitted gap law.
    yStatic = F.B0(:,ib) + F.B1(:,ib)./gAnchor + ...
        F.B2(:,ib).*log(gAnchor/F.gref_mm);
    ...
    registration(is)=struct('sensor_id',sensorId(is),'tau_mm',p(1),'x_scale',p(2), ...
        ...
        'point_count',nnz(fitUse),'b2_gap_mm',b2GapMm(is));
