...
%COMPARE_STEP05_NOETA_WITHETA_20241106
% VERSION: ETA_CONTROLLED_COMPARE_20260706
%
% Key logic:
...
%      It runs NoEta and WithEta with the same Step04 template, same dynamic
%      bundle construction, same VP all-EO scan, and same adaptive EO set.
...
%   output/step05_eta_controlled_compare.

cfg = BTTProjectConfig_20241106();

%% Step05 local settings
S = struct();
S.method_name = 'eta_controlled_compare_base';
S.version = 'ETA_CONTROLLED_COMPARE_20260706';
...
RunConfigs = build_eta_compare_run_configs_local(S, cfg);
...
fprintf('\n=== Step05 controlled eta comparison: NoEta vs WithEta ===\n');
fprintf('Dataset: %s\n', cfg.dataset);
...
function RunConfigs = build_eta_compare_run_configs_local(S_base, cfg)
tag = strtrim(getenv('ETA_COMPARE_OUTPUT_TAG'));
root_name = 'step05_eta_controlled_compare';
if ~isempty(tag)
...
noeta = common;
noeta.method_name = 'eta_compare_noeta';
noeta.version = 'ETA_COMPARE_NOETA_20260706';
noeta.eta_compare_label = 'NoEta';
noeta.active_eta_model = 'none';
noeta.eta_models = {'none'};
noeta.fit_sensor_eta = false;
noeta.method_file_tag = 'EtaCompareNoEta';
noeta.output_dir = fullfile(output_root, 'NoEta');
noeta.figure_dir = fullfile(figure_root, 'NoEta');

witheta = common;
witheta.method_name = 'eta_compare_witheta_static_sensor';
witheta.version = 'ETA_COMPARE_WITHETA_STATIC_SENSOR_20260706';
witheta.eta_compare_label = 'WithEta';
witheta.active_eta_model = 'static_sensor_eta';
witheta.eta_models = {'static_sensor_eta'};
witheta.fit_sensor_eta = true;
witheta.method_file_tag = 'EtaCompareWithEta';
witheta.output_dir = fullfile(output_root, 'WithEta');
witheta.figure_dir = fullfile(figure_root, 'WithEta');

RunConfigs = repmat(struct('label', '', 'S', []), 2, 1);
RunConfigs(1).label = noeta.eta_compare_label;
RunConfigs(1).S = noeta;
RunConfigs(2).label = witheta.eta_compare_label;
RunConfigs(2).S = witheta;
end
...
if ~isempty(CompareSet.Runs) && ...
    ...
    output_root = fullfile(cfg.output_root, 'step05_eta_controlled_compare');
end
...
mat_file = fullfile(output_root, sprintf( ...
    'CompareSet_Step05_NoEta_WithEta_%s_%s.mat', sensor_tag, cfg.dataset));
csv_file = fullfile(output_root, sprintf( ...
    'Summary_Step05_NoEta_WithEta_%s_%s.csv', sensor_tag, cfg.dataset));

save(mat_file, 'CompareSet', 'SummaryTable', 'Template', '-v7.3');
writetable(SummaryTable, csv_file);

fprintf('\n=== Eta controlled comparison saved ===\n');
fprintf('Summary CSV:\n  %s\n', csv_file);
fprintf('Compare MAT:\n  %s\n', mat_file);
...
T.noeta_EO = nan(height(T), 1);
T.witheta_EO = nan(height(T), 1);
T.noeta_rmse = nan(height(T), 1);
T.witheta_rmse = nan(height(T), 1);
T.delta_rmse_witheta_minus_noeta = nan(height(T), 1);
T.eo_changed_by_eta = false(height(T), 1);
T.witheta_eta_max_abs_mm = nan(height(T), 1);
...
for i = 1:height(T)
    ...
    idx_noeta = find(same & run_vec == "NoEta", 1, 'first');
    idx_witheta = find(same & run_vec == "WithEta", 1, 'first');

    if isempty(idx_noeta) || isempty(idx_witheta)
        continue;
    end

    noeta_eo = T.EO_id(idx_noeta);
    witheta_eo = T.EO_id(idx_witheta);
    noeta_rmse = T.weighted_voltage_rmse(idx_noeta);
    witheta_rmse = T.weighted_voltage_rmse(idx_witheta);
    delta = witheta_rmse - noeta_rmse;
    eta_max = T.sensor_eta_max_abs_mm(idx_witheta);
    changed = isfinite(noeta_eo) && isfinite(witheta_eo) && noeta_eo ~= witheta_eo;

    T.noeta_EO(i) = noeta_eo;
    T.witheta_EO(i) = witheta_eo;
    T.noeta_rmse(i) = noeta_rmse;
    T.witheta_rmse(i) = witheta_rmse;
    T.delta_rmse_witheta_minus_noeta(i) = delta;
    T.eo_changed_by_eta(i) = changed;
    T.witheta_eta_max_abs_mm(i) = eta_max;
end
end
...
pair_idx = string(T.eta_run) == "NoEta" & isfinite(T.delta_rmse_witheta_minus_noeta);
fprintf('Paired windows: %d; EO changed by eta: %d; median delta RMSE WithEta-NoEta = %.6g V\n', ...
    nnz(pair_idx), nnz(T.eo_changed_by_eta(pair_idx)), ...
    median(T.delta_rmse_witheta_minus_noeta(pair_idx), 'omitnan'));
end
...
if isfield(S, 'fit_sensor_eta') && S.fit_sensor_eta
    method.eta_gauge = sprintf(['controlled WithEta comparison: CH%d eta fixed to 0; ' ...
