function Summary = Compare_Step05_FreeEta_20241106()
%COMPARE_STEP05_FREEETA_20241106
...
sensors = cfg.step05_analysis_sensors(:).';
...
out_dir = fullfile(cfg.output_root, 'diagnostics', ...
    'step05_free_eta_compare_20241106');
if exist(out_dir, 'dir') ~= 7
    mkdir(out_dir);
end

worker_file = fullfile(out_dir, 'DiagWorker_Step05_FreeEta_20241106.m');
...
replacements = {
    ...
    "S.method_file_tag = 'FreeEtaPilot';"
...
summary_csv = fullfile(out_dir, sprintf( ...
    'Step05_FreeEta_Compare_Windows_B%d_%s_20241106.csv', ...
    target_blade, sensor_tag));
...
Summary.summary_mat = fullfile(out_dir, sprintf( ...
    'Step05_FreeEta_Compare_B%d_%s_20241106.mat', ...
    target_blade, sensor_tag));
...
target_blade = cfg.step05_target_blades(1);
...
if ~isempty(env_text)
    ...
    if numel(vals) ~= 1
        error('Compare_Step05_FreeEta_20241106 requires exactly one target blade.');
    end
    target_blade = vals(1);
end
