...
%COMPARE_STEP05_STATICETA_20241106
% VERSION: STATIC_ETA_COMPARE_20260706
%
% Key logic:
...
%   5. This is a controlled comparison entry, not the formal Step05 entry.
%      It runs fixed and bounded static-eta variants with the same Step04
%      template, dynamic bundle construction, VP all-EO scan, and adaptive
%      EO set.
...
%
% Usage:
%   Run this script directly after Step02, Step04, and the formal NoEta
%   Step05 has been validated.  Outputs are isolated under
%   output/step05_static_eta_compare.

cfg = BTTProjectConfig_20241106();

%% Step05 local settings
S = struct();
S.method_name = 'static_eta_compare_base';
S.version = 'STATIC_ETA_COMPARE_20260706';

S.show_plots = false;
S.save_figures = false;
...
S.query_guard_max_mm = S.amplitude_limit_mm + S.dx_c_limit_mm + 0.05;
...
S.method_preset = 'foundation_mainpulse_adaptive_noeta';
...
if ~isempty(method_preset_env)
    error(['STEP05_METHOD_PRESET belongs to archived 20241106 Step05 routes. ' ...
        'The official foundation Step05 uses adaptive_full_fit NoEta.']);
end
...
RunConfigs = build_eta_compare_run_configs_local(S, cfg, Template);

CompareSet = struct();
CompareSet.Dataset = cfg.dataset;
CompareSet.CreatedBy = mfilename;
CompareSet.CreatedOn = datestr(now, 31);
CompareSet.TemplateSourceMode = template_mode;
CompareSet.TargetCases = target_cases;
CompareSet.TargetBlades = S.target_blades;
CompareSet.AnalysisSensors = S.analysis_sensors;
CompareSet.Runs = repmat(struct( ...
    'label', '', ...
    'active_eta_model', '', ...
    'fit_sensor_eta', false, ...
    'ResultSet', []), numel(RunConfigs), 1);

fprintf('\n=== Step05 static eta comparison: fixed vs bounded eta prior ===\n');
fprintf('Dataset: %s\n', cfg.dataset);
...
fprintf('Shared candidate policy: %s (VP all EO, adaptive full-fit set; VP eta disabled).\n', ...
    S.eo_candidate_policy);

for iRun = 1:numel(RunConfigs)
    S_run = RunConfigs(iRun).S;
    print_eta_compare_run_header_local(S_run);

    ResultSet = struct();
    ResultSet.Dataset = cfg.dataset;
    ResultSet.CreatedBy = mfilename;
    ResultSet.CreatedOn = datestr(now, 31);
    ResultSet.CompareLabel = S_run.eta_compare_label;
    ResultSet.RunInfo = build_step05_run_info_local(cfg, S_run);
    ResultSet.Cases = repmat(struct('case_name', '', 'BladeResult', []), ...
        numel(target_cases), 1);

    for iCase = 1:numel(target_cases)
        case_name = char(target_cases{iCase});
        fprintf('\n--- Eta compare [%s] case: %s ---\n', ...
            S_run.eta_compare_label, case_name);

        case_result = process_step05_case_local(case_name, cfg, S_run, Template);

        ResultSet.Cases(iCase).case_name = case_name;
        ResultSet.Cases(iCase).BladeResult = case_result;
    end

    CompareSet.Runs(iRun).label = S_run.eta_compare_label;
    CompareSet.Runs(iRun).active_eta_model = S_run.active_eta_model;
    CompareSet.Runs(iRun).fit_sensor_eta = S_run.fit_sensor_eta;
    CompareSet.Runs(iRun).ResultSet = ResultSet;
end

save_eta_compare_outputs_local(CompareSet, cfg, S, Template);


function RunConfigs = build_eta_compare_run_configs_local(S_base, cfg, Template)
tag = strtrim(getenv('ETA_COMPARE_OUTPUT_TAG'));
root_name = 'step05_static_eta_compare';
if ~isempty(tag)
    tag = regexprep(tag, '[^\w\-]', '_');
    root_name = [root_name, '_', tag];
end

output_root = fullfile(cfg.output_root, root_name);
figure_root = fullfile(cfg.figure_root, root_name);

common = S_base;
common.compare_output_root = output_root;
common.compare_figure_root = figure_root;
common.method_preset = 'eta_controlled_compare';
common.vp_include_sensor_eta = false;
common.eta_model_policy = 'single';
common.static_eta_source_mode = lower(strtrim(getenv('ETA_STATIC_PRIOR_SOURCE')));
if isempty(common.static_eta_source_mode)
    common.static_eta_source_mode = 'step04_xc';
end
common.static_eta_file = strtrim(getenv('ETA_STATIC_PRIOR_FILE'));

[fixed_eta, static_eta_info, static_eta_label] = ...
    resolve_static_eta_prior_local(Template, S_base, cfg, common);
common.static_eta_source = static_eta_label.source;
common.static_eta_info = static_eta_info;
common.fixed_sensor_eta_mm = fixed_eta;
common.sensor_eta_prior_mm = fixed_eta;

bound_env = strtrim(getenv('ETA_STATIC_BOUND_HALFWIDTH_MM'));
bound_half_width = 0.020;
if ~isempty(bound_env)
    parsed_bound = str2double(bound_env);
    if isfinite(parsed_bound) && parsed_bound >= 0
        bound_half_width = parsed_bound;
    end
end
common.sensor_eta_bound_halfwidth_mm = bound_half_width;

force_env = strtrim(getenv('ETA_COMPARE_FORCE_REBUILD'));
if ~isempty(force_env)
    common.force_rebuild = any(strcmpi(force_env, ...
        {'1', 'true', 'yes', 'on'}));
end

fixed = common;
fixed.method_name = static_eta_label.fixed_method_name;
fixed.version = static_eta_label.fixed_version;
fixed.eta_compare_label = static_eta_label.fixed_label;
fixed.active_eta_model = static_eta_label.fixed_eta_model;
fixed.eta_models = {fixed.active_eta_model};
fixed.sensor_eta_mode = 'fixed_static';
fixed.fit_sensor_eta = false;
fixed.method_file_tag = static_eta_label.fixed_file_tag;
fixed.output_dir = fullfile(output_root, fixed.eta_compare_label);
fixed.figure_dir = fullfile(figure_root, fixed.eta_compare_label);

bounded = common;
bounded.method_name = static_eta_label.bounded_method_name;
bounded.version = static_eta_label.bounded_version;
bounded.eta_compare_label = static_eta_label.bounded_label;
bounded.active_eta_model = static_eta_label.bounded_eta_model;
bounded.eta_models = {bounded.active_eta_model};
bounded.sensor_eta_mode = 'bounded_static_prior';
bounded.fit_sensor_eta = true;
bounded.method_file_tag = static_eta_label.bounded_file_tag;
bounded.output_dir = fullfile(output_root, bounded.eta_compare_label);
bounded.figure_dir = fullfile(figure_root, bounded.eta_compare_label);

RunConfigs = repmat(struct('label', '', 'S', []), 2, 1);
RunConfigs(1).label = fixed.eta_compare_label;
RunConfigs(1).S = fixed;
RunConfigs(2).label = bounded.eta_compare_label;
RunConfigs(2).S = bounded;
end


function print_eta_compare_run_header_local(S)
fprintf('\n=== Eta compare run: %s ===\n', S.eta_compare_label);
fprintf('Output dir: %s\n', S.output_dir);
fprintf('Model: V = T_{s,b}(x - dx_c - eta_s - A*sin(EO*theta + phi)).\n');
fprintf('fit_sensor_eta=%d, active_eta_model=%s, reference CH%d eta fixed to 0.\n', ...
    S.fit_sensor_eta, S.active_eta_model, S.eta_reference_sensor);
fprintf('static eta source=%s, eta=[%s] mm\n', ...
    get_string_field_from_struct_local(S, 'static_eta_source', 'unknown'), ...
    sprintf(' %.6g', get_fixed_sensor_eta_local(S)));
if isfield(S, 'sensor_eta_mode') && strcmpi(S.sensor_eta_mode, 'bounded_static_prior')
    fprintf('bounded eta half-width: %.6g mm around prior.\n', ...
        S.sensor_eta_bound_halfwidth_mm);
end
fprintf('VP eta disabled: %d; full-fit EO policy: %s.\n', ...
    ~S.vp_include_sensor_eta, S.eo_candidate_policy);
end


function [eta, info, label] = resolve_static_eta_prior_local(Template, S, cfg, common)
source_mode = lower(strtrim(char(string(common.static_eta_source_mode))));
if is_joint_static_eta_source_local(source_mode)
    [eta, info] = load_joint_static_eta_preview_local(S, cfg, common);
    label = make_static_eta_label_local('joint_preview');
else
    eta = compute_step04_xc_static_eta_local(Template, S);
    info = table(S.analysis_sensors(:), eta(:), ...
        'VariableNames', {'sensor_id', 'eta_prior_mm'});
    label = make_static_eta_label_local('step04_xc');
end
end


function tf = is_joint_static_eta_source_local(source_mode)
source_mode = lower(strtrim(char(string(source_mode))));
tf = any(strcmpi(source_mode, {'joint_preview', ...
    'joint_static_eta_preview', 'joint_dynamic_residual', ...
    'dynamic_residual', 'dynamic_joint', 'joint'}));
end


function label = make_static_eta_label_local(source_key)
switch lower(strtrim(source_key))
    case 'joint_preview'
        label = struct( ...
            'source', 'joint_dynamic_residual_eta_preview', ...
            'fixed_label', 'FixedJointStaticEtaPreview', ...
            'bounded_label', 'BoundedJointStaticEtaPreview', ...
            'fixed_method_name', 'static_eta_compare_fixed_joint_preview', ...
            'bounded_method_name', 'static_eta_compare_bounded_joint_preview', ...
            'fixed_version', 'STATIC_ETA_FIXED_JOINT_PREVIEW_20260706', ...
            'bounded_version', 'STATIC_ETA_BOUNDED_JOINT_PREVIEW_20260706', ...
            'fixed_eta_model', 'fixed_static_eta', ...
            'bounded_eta_model', 'bounded_static_eta', ...
            'fixed_file_tag', 'FixedJointStaticEtaPreview', ...
            'bounded_file_tag', 'BoundedJointStaticEtaPreview');
    otherwise
        label = struct( ...
            'source', 'step04_template_xc_difference', ...
            'fixed_label', 'FixedStep04XcEta', ...
            'bounded_label', 'BoundedStep04XcEta', ...
            'fixed_method_name', 'static_eta_compare_fixed_step04_xc', ...
            'bounded_method_name', 'static_eta_compare_bounded_step04_xc', ...
            'fixed_version', 'STATIC_ETA_FIXED_STEP04_XC_20260706', ...
            'bounded_version', 'STATIC_ETA_BOUNDED_STEP04_XC_20260706', ...
            'fixed_eta_model', 'fixed_step04_xc_static_eta', ...
            'bounded_eta_model', 'bounded_step04_xc_static_eta', ...
            'fixed_file_tag', 'FixedStep04XcEta', ...
            'bounded_file_tag', 'BoundedStep04XcEta');
end
end


function [eta, info] = load_joint_static_eta_preview_local(S, cfg, common)
sensors = S.analysis_sensors(:).';
blade_id = S.target_blades(1);
sensor_tag = ['S', sprintf('%d', sensors)];

eta_file = char(string(common.static_eta_file));
if isempty(strtrim(eta_file))
...
if ~isfile(eta_file)
    error('Joint static eta preview file not found: %s', eta_file);
end

loaded = load(eta_file, 'Summary');
if ~isfield(loaded, 'Summary')
    error('Joint static eta preview file must contain Summary: %s', eta_file);
end
...
if ~isfield(Summary, 'Consensus') || ...
    ...
    error('Joint static eta preview missing Consensus.eta_median_mm: %s', ...
        eta_file);
end
...
info = table(sensors(:), eta(:), plan_iqr(:), repmat(string(eta_file), numel(sensors), 1), ...
    'VariableNames', {'sensor_id', 'eta_prior_mm', ...
    'eta_plan_iqr_mm', 'joint_static_eta_file'});
end


function eta = compute_step04_xc_static_eta_local(Template, S)
eta = zeros(1, numel(S.analysis_sensors));
ref_sensor = S.analysis_sensors(1);
blade_id = S.target_blades(1);

ref_tpl = get_template_entry_local(Template, ref_sensor, blade_id);
if isempty(ref_tpl) || ~isfield(ref_tpl, 'xc') || ~isfinite(ref_tpl.xc)
    error('Cannot compute static eta: missing finite Step04 xc for CH%d B%d.', ...
        ref_sensor, blade_id);
end
...
for i = 1:numel(S.analysis_sensors)
    ...
    if isempty(tpl) || ~isfield(tpl, 'xc') || ~isfinite(tpl.xc)
        error('Cannot compute static eta: missing finite Step04 xc for CH%d B%d.', ...
            sid, blade_id);
    end

    eta(i) = ref_tpl.xc - tpl.xc;
end
...
function eta = get_fixed_sensor_eta_local(S)
eta = zeros(1, numel(S.analysis_sensors));
if isfield(S, 'fixed_sensor_eta_mm') && ...
        isnumeric(S.fixed_sensor_eta_mm) && ...
        numel(S.fixed_sensor_eta_mm) == numel(S.analysis_sensors)
    eta = S.fixed_sensor_eta_mm(:).';
elseif isfield(S, 'sensor_eta_prior_mm') && ...
...
function eta = get_initial_sensor_eta_local(S)
if isfield(S, 'sensor_eta_mode') && any(strcmpi(S.sensor_eta_mode, ...
        {'fixed_static', 'bounded_static_prior'}))
    eta = get_fixed_sensor_eta_local(S);
else
    eta = zeros(1, numel(S.analysis_sensors));
end
...
if isfield(S, 'sensor_eta_mode') && strcmpi(S.sensor_eta_mode, 'fixed_static')
    eta = get_fixed_sensor_eta_local(S);
elseif isfield(S, 'sensor_eta_mode') && strcmpi(S.sensor_eta_mode, 'bounded_static_prior')
    prior = get_fixed_sensor_eta_local(S);
    half_width = get_numeric_field_from_struct_local(S, ...
        'sensor_eta_bound_halfwidth_mm', 0.020);
    lo = prior - half_width;
    hi = prior + half_width;
    lo(1) = 0;
    hi(1) = 0;
    eta = min(max(eta, lo), hi);
else
...
function value = get_numeric_field_from_struct_local(S, name, default_value)
value = default_value;
...
function value = get_string_field_from_struct_local(S, name, default_value)
value = default_value;
...
function save_eta_compare_outputs_local(CompareSet, cfg, S_base, Template)
if ~isempty(CompareSet.Runs) && ...
        isfield(CompareSet.Runs(1).ResultSet, 'RunInfo') && ...
        isfield(CompareSet.Runs(1).ResultSet.RunInfo, 'step_settings') && ...
        isfield(CompareSet.Runs(1).ResultSet.RunInfo.step_settings, 'compare_output_root')
    output_root = CompareSet.Runs(1).ResultSet.RunInfo.step_settings.compare_output_root;
else
    output_root = fullfile(cfg.output_root, 'step05_static_eta_compare');
end
...
SummaryTable = build_eta_compare_summary_table_local(CompareSet);

sensor_tag = ['S', sprintf('%d', S_base.analysis_sensors)];
mat_file = fullfile(output_root, sprintf( ...
    'CompareSet_Step05_StaticEta_%s_%s.mat', sensor_tag, cfg.dataset));
csv_file = fullfile(output_root, sprintf( ...
    'Summary_Step05_StaticEta_%s_%s.csv', sensor_tag, cfg.dataset));

save(mat_file, 'CompareSet', 'SummaryTable', 'Template', '-v7.3');
writetable(SummaryTable, csv_file);

fprintf('\n=== Static eta comparison saved ===\n');
fprintf('Summary CSV:\n  %s\n', csv_file);
fprintf('Compare MAT:\n  %s\n', mat_file);
print_eta_compare_summary_local(SummaryTable);
end


function T = build_eta_compare_summary_table_local(CompareSet)
rows = repmat(make_empty_eta_compare_summary_row_local(), 0, 1);

for iRun = 1:numel(CompareSet.Runs)
    run_label = string(CompareSet.Runs(iRun).label);
    ResultSet = CompareSet.Runs(iRun).ResultSet;

    for iCase = 1:numel(ResultSet.Cases)
        case_name = string(ResultSet.Cases(iCase).case_name);
        blade_results = ResultSet.Cases(iCase).BladeResult;

        for ib = 1:numel(blade_results)
            if ~isfield(blade_results(ib), 'Result') || ...
                    isempty(blade_results(ib).Result)
                continue;
            end

            Result = blade_results(ib).Result;
            Trend = Result.Trend;
            summary = Result.ResonanceSummary;

            for i = 1:height(Trend)
                row = make_empty_eta_compare_summary_row_local();
                row.eta_run = run_label;
                row.case_name = case_name;
                row.blade_id = Result.TargetBlade;
                row.window_id = Trend.window_id(i);
                row.lap_start = Trend.lap_start(i);
                row.lap_end = Trend.lap_end(i);
                row.status = string(Trend.status{i});
                row.fit_stage = string(Trend.fit_stage{i});
                row.EO_id = Trend.EO_id(i);
                row.fn_id = Trend.fn_id(i);
                row.A_id = Trend.A_id(i);
                row.dx_c_id = Trend.dx_c_id(i);
                row.sensor_eta_max_abs_mm = Trend.sensor_eta_max_abs_mm(i);
                row.weighted_voltage_rmse = Trend.weighted_voltage_rmse(i);
                row.plain_voltage_rmse = Trend.plain_voltage_rmse(i);
                row.point_count = Trend.point_count(i);
                row.valid_segment_count = Trend.valid_segment_count(i);
                row.valid_sensor_count = Trend.valid_sensor_count(i);
                row.valid_lap_count = Trend.valid_lap_count(i);
                row.joint_best_eo = get_struct_numeric_scalar_local(summary, 'joint_best_eo');
                row.reported_eo = get_struct_numeric_scalar_local(summary, 'reported_eo');
                row.reported_freq_hz = get_struct_numeric_scalar_local(summary, 'reported_freq_hz');
                row.joint_status = get_struct_string_scalar_local(summary, 'joint_status');
...
T = struct2table(rows);
T = add_eta_compare_pair_columns_local(T);
end


function row = make_empty_eta_compare_summary_row_local()
row = struct( ...
    'eta_run', "", ...
    'case_name', "", ...
    'blade_id', NaN, ...
...
function T = add_eta_compare_pair_columns_local(T)
T.fixed_EO = nan(height(T), 1);
T.bounded_EO = nan(height(T), 1);
T.fixed_rmse = nan(height(T), 1);
T.bounded_rmse = nan(height(T), 1);
T.delta_rmse_bounded_minus_fixed = nan(height(T), 1);
T.eo_changed_by_bounded_eta = false(height(T), 1);
T.fixed_eta_max_abs_mm = nan(height(T), 1);
T.bounded_eta_max_abs_mm = nan(height(T), 1);

case_vec = string(T.case_name);
run_vec = string(T.eta_run);

for i = 1:height(T)
    same = case_vec == case_vec(i) & ...
        T.blade_id == T.blade_id(i) & ...
        T.window_id == T.window_id(i);

    idx_fixed = find(same & run_vec == "FixedStep04XcEta", 1, 'first');
    idx_bounded = find(same & run_vec == "BoundedStep04XcEta", 1, 'first');

    if isempty(idx_fixed) || isempty(idx_bounded)
        continue;
    end

    fixed_eo = T.EO_id(idx_fixed);
    bounded_eo = T.EO_id(idx_bounded);
    fixed_rmse = T.weighted_voltage_rmse(idx_fixed);
    bounded_rmse = T.weighted_voltage_rmse(idx_bounded);
    delta = bounded_rmse - fixed_rmse;
    fixed_eta_max = T.sensor_eta_max_abs_mm(idx_fixed);
    bounded_eta_max = T.sensor_eta_max_abs_mm(idx_bounded);
    changed = isfinite(fixed_eo) && isfinite(bounded_eo) && fixed_eo ~= bounded_eo;

    T.fixed_EO(i) = fixed_eo;
    T.bounded_EO(i) = bounded_eo;
    T.fixed_rmse(i) = fixed_rmse;
    T.bounded_rmse(i) = bounded_rmse;
    T.delta_rmse_bounded_minus_fixed(i) = delta;
    T.eo_changed_by_bounded_eta(i) = changed;
    T.fixed_eta_max_abs_mm(i) = fixed_eta_max;
    T.bounded_eta_max_abs_mm(i) = bounded_eta_max;
end
end


function print_eta_compare_summary_local(T)
if isempty(T)
    fprintf('No comparison rows were produced.\n');
    return;
end

for run_name = unique(string(T.eta_run), 'stable').'
    idx = string(T.eta_run) == run_name & string(T.status) == "ok";
    eos = T.EO_id(idx);
    fprintf('%s ok windows: %d', run_name, nnz(idx));
    if ~isempty(eos)
        u = unique(eos(isfinite(eos)));
        for i = 1:numel(u)
            fprintf(', EO%d=%d', u(i), nnz(eos == u(i)));
        end
    end
    fprintf('\n');
end

pair_idx = string(T.eta_run) == "FixedStep04XcEta" & ...
    isfinite(T.delta_rmse_bounded_minus_fixed);
fprintf('Paired windows: %d; EO changed by bounded eta: %d; median delta RMSE Bounded-Fixed = %.6g V\n', ...
    nnz(pair_idx), nnz(T.eo_changed_by_bounded_eta(pair_idx)), ...
    median(T.delta_rmse_bounded_minus_fixed(pair_idx), 'omitnan'));
end
...
for ib = 1:numel(S.target_blades)
    ...
    method_file_tag = 'FoundationMainPulseAdaptiveNoEta';
    if isfield(S, 'method_file_tag') && ~isempty(S.method_file_tag)
        method_file_tag = char(string(S.method_file_tag));
    end
...
if isfield(S, 'sensor_eta_mode') && strcmpi(S.sensor_eta_mode, 'fixed_static')
    method.eta_gauge = sprintf(['controlled fixed StaticEta: CH%d eta fixed to 0; ' ...
        'eta_s fixed from %s = [%s] mm'], ...
        S.analysis_sensors(1), ...
        get_string_field_from_struct_local(S, 'static_eta_source', 'unknown'), ...
        sprintf(' %.6g', get_fixed_sensor_eta_local(S)));
elseif isfield(S, 'sensor_eta_mode') && strcmpi(S.sensor_eta_mode, 'bounded_static_prior')
    method.eta_gauge = sprintf(['controlled bounded StaticEta: CH%d eta fixed to 0; ' ...
        'eta_s initialized at [%s] mm with half-width %.4g mm'], ...
        S.analysis_sensors(1), ...
        sprintf(' %.6g', get_fixed_sensor_eta_local(S)), ...
        S.sensor_eta_bound_halfwidth_mm);
elseif isfield(S, 'fit_sensor_eta') && S.fit_sensor_eta
    method.eta_gauge = sprintf(['controlled FreeEta comparison: CH%d eta fixed to 0; ' ...
        'other sensor eta_s fitted with bound %.4g mm and regularization %.4g V/mm'], ...
        S.analysis_sensors(1), S.sensor_eta_limit_mm, ...
        S.sensor_eta_reg_weight_v_per_mm);
else
    method.eta_gauge = sprintf(['controlled NoEta comparison: eta_s is fixed to 0 for all CH; ' ...
        'reference CH%d retained for gauge consistency'], S.analysis_sensors(1));
end
...
for iEO = 1:numel(eo_candidates)
    ...
    basis = [-Tp, -Tp .* s, -Tp .* c];
    y = v - T0;
    good = all(isfinite(basis), 2) & isfinite(y) & isfinite(w);
    ...
    seed_table(iEO).linear_vp_rmse = sqrt(nansum(w(good) .* lin_res(good).^2) / ...
        max(nansum(w(good)), eps));
...
for i = 1:numel(final_eos)
    ...
    seed.sensor_eta = get_initial_sensor_eta_local(S);
...
eta = get_initial_sensor_eta_local(S);
...
if nnz(valid) < 5
    ...
    eta_for_reg = eta(:);
    if isfield(S, 'sensor_eta_mode') && strcmpi(S.sensor_eta_mode, 'bounded_static_prior')
        eta_for_reg = eta(:) - get_fixed_sensor_eta_local(S).';
    end
    eta_reg_obj = bundle.point_count * ...
        (S.sensor_eta_reg_weight_v_per_mm * sqrt(mean(eta_for_reg(:).^2))) ^ 2;
