...
alignFile = fullfile(caseOutputDir, 'Step3_Spectrum_RPM_Alignment_20250527.mat');
if ~isfile(alignFile)
...
matFile = fullfile(outDir, 'Step04_BTT_STE_Resonance_Regions_20250527.mat');
csvFile = fullfile(outDir, 'Step04_BTT_STE_Resonance_Regions_20250527.csv');
...
%% 7. Visualization
figure('Name', '20250527 Step04 BTT STE resonance regions', ...
...
figFile = fullfile(outDir, 'Step04_BTT_STE_Resonance_Regions_20250527.png');
saveas(gcf, figFile);

%% 8. Validation visualization: aligned strain STFT
tStrainBtt = alignResult.stft_time_s(:) - alignResult.best_tau_sec;
freqMaskPlot = alignResult.stft_freq_hz >= plotFreqBandHz(1) & ...
    alignResult.stft_freq_hz <= plotFreqBandHz(2);
if ~any(freqMaskPlot)
    error('No strain STFT bins fall inside the plot range %.1f-%.1f Hz.', ...
        plotFreqBandHz(1), plotFreqBandHz(2));
end
plotFreqAxis = alignResult.stft_freq_hz(freqMaskPlot);
strainAmpDbPlot = 20 * log10(alignResult.stft_amp(freqMaskPlot, :) + eps);
[tMeshPlot, fMeshPlot] = meshgrid(tStrainBtt, plotFreqAxis);

stft2dFig = figure('Name', '20250527 Step04 aligned strain STFT 2D', ...
    'Color', 'w', 'Position', [90, 90, 1360, 560], 'NumberTitle', 'off');
ax2d = axes(stft2dFig);
imagesc(ax2d, tStrainBtt, plotFreqAxis, strainAmpDbPlot);
axis(ax2d, 'xy');
grid(ax2d, 'on');
box(ax2d, 'on');
colormap(ax2d, turbo);
cb2d = colorbar(ax2d);
cb2d.Label.String = 'Amplitude (dB)';
hold(ax2d, 'on');
ylim(ax2d, plotFreqBandHz);
xlabel(ax2d, 'BTT-aligned time (s)');
ylabel(ax2d, 'Frequency (Hz)');
title(ax2d, sprintf('Aligned strain STFT (2D), plot range %.0f-%.0f Hz', ...
    plotFreqBandHz(1), plotFreqBandHz(2)));
draw_step04_region_overlays_2d(ax2d, regionTable, alignResult, plotFreqBandHz);
legend(ax2d, 'Location', 'eastoutside');

stft2dFigFile = fullfile(outDir, 'Step04_Aligned_Strain_STFT_2D_20250527.png');
saveas(stft2dFig, stft2dFigFile);

stft3dFig = figure('Name', '20250527 Step04 aligned strain STFT 3D', ...
    'Color', 'w', 'Position', [120, 120, 1360, 760], 'NumberTitle', 'off');
ax3d = axes(stft3dFig);
surf(ax3d, tMeshPlot, fMeshPlot, strainAmpDbPlot, ...
    'EdgeColor', 'none', 'FaceColor', 'interp');
grid(ax3d, 'on');
box(ax3d, 'on');
colormap(ax3d, turbo);
cb3d = colorbar(ax3d);
cb3d.Label.String = 'Amplitude (dB)';
view(ax3d, 46, 34);
axis(ax3d, 'tight');
ylim(ax3d, plotFreqBandHz);
xlabel(ax3d, 'BTT-aligned time (s)');
ylabel(ax3d, 'Frequency (Hz)');
zlabel(ax3d, 'Amplitude (dB)');
title(ax3d, sprintf('Aligned strain STFT (3D), plot range %.0f-%.0f Hz', ...
    plotFreqBandHz(1), plotFreqBandHz(2)));
hold(ax3d, 'on');
zTrackLevel = max(strainAmpDbPlot(:), [], 'omitnan') + 2;
draw_step04_region_overlays_3d(ax3d, regionTable, alignResult, plotFreqBandHz, zTrackLevel);

stft3dFigFile = fullfile(outDir, 'Step04_Aligned_Strain_STFT_3D_20250527.png');
saveas(stft3dFig, stft3dFigFile);

figure('Name', '20250527 Step04 aligned strain STFT validation', ...
    'Color', 'w', 'Position', [90, 90, 1360, 900], 'NumberTitle', 'off');
tiledlayout(3, 1, 'TileSpacing', 'compact', 'Padding', 'compact');

nexttile;
imagesc(tStrainBtt, plotFreqAxis, strainAmpDbPlot);
...
hold on;
ylim(plotFreqBandHz);
draw_step04_region_overlays_2d(gca, regionTable, alignResult, plotFreqBandHz);
xlabel('BTT-aligned time (s)');
ylabel('Frequency (Hz)');
title(sprintf('Aligned strain STFT with Step04 resonance regions, %.0f-%.0f Hz', ...
    plotFreqBandHz(1), plotFreqBandHz(2)));
...
stftFigFile = fullfile(outDir, 'Step04_Aligned_Strain_STFT_Validation_20250527.png');
saveas(gcf, stftFigFile);

fprintf('\nStep04 complete. Candidate resonance regions:\n');
disp(regionTable);
fprintf('Saved:\n  %s\n  %s\n  %s\n  %s\n  %s\n  %s\n', ...
    matFile, csvFile, figFile, stft2dFigFile, stft3dFigFile, stftFigFile);

function draw_step04_region_overlays_2d(ax, regionTable, alignResult, plotFreqBandHz)
hold(ax, 'on');
for i = 1:height(regionTable)
    patch(ax, [regionTable.regionStart(i), regionTable.regionEnd(i), ...
        regionTable.regionEnd(i), regionTable.regionStart(i)], ...
        [plotFreqBandHz(1), plotFreqBandHz(1), plotFreqBandHz(2), plotFreqBandHz(2)], ...
        [1.00, 0.85, 0.30], 'FaceAlpha', 0.16, 'EdgeColor', 'none', ...
        'HandleVisibility', 'off');
    ...
    validLine = isfinite(fLine) & fLine >= plotFreqBandHz(1) & fLine <= plotFreqBandHz(2);
    if any(validLine)
        plot(ax, tLine(validLine), fLine(validLine), 'w-', 'LineWidth', 1.4, ...
            'DisplayName', sprintf('%dX region %d', regionTable.regionPeakOrder(i), i));
    end
    if isfinite(regionTable.regionPeakFreqHz(i)) && ...
            regionTable.regionPeakFreqHz(i) >= plotFreqBandHz(1) && ...
            regionTable.regionPeakFreqHz(i) <= plotFreqBandHz(2)
        plot(ax, regionTable.regionPeakTime(i), regionTable.regionPeakFreqHz(i), ...
            'wo', 'MarkerFaceColor', 'k', 'MarkerSize', 5, 'HandleVisibility', 'off');
    end
end
end

function draw_step04_region_overlays_3d(ax, regionTable, alignResult, plotFreqBandHz, zTrackLevel)
hold(ax, 'on');
for i = 1:height(regionTable)
    tLine = linspace(regionTable.regionStart(i), regionTable.regionEnd(i), 160);
    rpmLine = interp1(alignResult.rpm_time_s, alignResult.rpm_values, ...
    ...
    validLine = isfinite(fLine) & fLine >= plotFreqBandHz(1) & fLine <= plotFreqBandHz(2);
    if any(validLine)
        plot3(ax, tLine(validLine), fLine(validLine), ...
            zTrackLevel * ones(nnz(validLine), 1), 'w-', 'LineWidth', 1.6, ...
            'DisplayName', sprintf('%dX region %d', regionTable.regionPeakOrder(i), i));
    end
    if isfinite(regionTable.regionPeakFreqHz(i)) && ...
            regionTable.regionPeakFreqHz(i) >= plotFreqBandHz(1) && ...
            regionTable.regionPeakFreqHz(i) <= plotFreqBandHz(2)
        plot3(ax, regionTable.regionPeakTime(i), regionTable.regionPeakFreqHz(i), ...
            zTrackLevel, 'wo', 'MarkerFaceColor', 'k', 'MarkerSize', 6, ...
            'HandleVisibility', 'off');
    end
end
end
