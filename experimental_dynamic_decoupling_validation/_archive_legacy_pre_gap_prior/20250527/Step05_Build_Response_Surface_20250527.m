...
targetBladeMode = 'auto_reference';  % auto_reference or explicit blade id below
...
%% 3. Select reference blade and effective response window
peakByBladeGap = squeeze(max(waveforms, [], 1, 'omitnan'));
meanPeakByBlade = mean(peakByBladeGap, 2, 'omitnan');
if strcmpi(targetBladeMode, 'auto_reference')
...
if saveFigures
    ...
    ylabel('Voltage (mV)');
    title(sprintf('Reference blade %d: measured solid, fitted dashed', referenceBladeId));
