%% Step03G_Run_TemplateOnly_FirstOrderVPEOLock_20251222
% Low-speed-template-only identification with data-driven EO locking.
%
% Every integer EO candidate is scored by the first-order model
% V - T(x) ~= -T'(x) * [d0 + a*sin(EO*theta) + b*cos(EO*theta)].
% The first few windows build a trusted EO candidate set by support. Each
% window then refines all trusted EOs with only A, phi, and d0 in the
% waveform domain, and the final EO is selected by waveform RMSE.
...
cfg.debug_max_windows = parse_positive_integer_env_local('STEP03G_MAX_WINDOWS', inf);
cfg.eo_lock_window_count = parse_positive_integer_env_local('STEP03G_EO_LOCK_WINDOWS', 3);
cfg.eo_support_top_k = parse_positive_integer_env_local('STEP03G_EO_SUPPORT_TOP_K', 5);
cfg.trusted_eo_count = parse_positive_integer_env_local('STEP03G_TRUSTED_EO_COUNT', 3);
cfg.diagnostic_eo = parse_optional_integer_env_local('STEP03_DIAGNOSTIC_EO');

method = struct();
method.name = 'template_only_first_order_vp_eo_lock';
method.selection_rule = 'first_order_template_vp_support_candidates_then_fixed_eo_direct_template_waveform_rmse';
method.use_reference_eo_constraint = false;
method.use_vp = true;
method.eo_lock_window_count = cfg.eo_lock_window_count;
method.eo_support_top_k = cfg.eo_support_top_k;
method.trusted_eo_count = cfg.trusted_eo_count;
method.refine_params = {'A', 'phi', 'd0'};
...
method.query_guard_mm = cfg.amplitude_limit_mm + cfg.offset_limit_mm + method.coverage_safety_margin_mm;
...
result_file = fullfile(result_dir, sprintf('Result_TemplateOnlyFirstOrderVPEOLock_B%d_%s_20251222.mat', ...
...
fprintf('\n=== Step03G: template-only first-order VP EO-lock identification ===\n');
...
fprintf('Model: V = T_low(x - A*sin(EO*theta + phi) - d0); no alpha/beta projection.\n');
fprintf('Template coverage guard: dynamic x must preferably stay %.3f mm inside template domain.\n', ...
    method.query_guard_mm);
fprintf('EO candidate lock: first %d window(s), support top %d per window; final fit compares top %d trusted EOs.\n', ...
    method.eo_lock_window_count, method.eo_support_top_k, method.trusted_eo_count);
...
trend_rows = repmat(struct( ...
    ...
    'fn_id', NaN, 'phi_id_wrapped', NaN, 'd0_id', NaN, ...
...
bundle_cache = cell(num_windows, 1);
seed_table_cache = cell(num_windows, 1);

fprintf('\nRunning first-order VP scans for EO lock...\n');
for w_idx = 1:num_windows
    Wmap = DynamicMap.Window(w_idx);
    bundle_cache{w_idx} = build_template_observation_bundle_local(Wmap, Template, cfg.analysis_sensors, method);
    eo_candidates = build_eo_candidates_local(bundle_cache{w_idx}.rot_freq_mean_hz, cfg.freq_search_hz, cfg.eo_pad);
    seed_table_cache{w_idx} = solve_template_seed_eo_scan_local(bundle_cache{w_idx}, eo_candidates);
end

num_lock_windows = min(method.eo_lock_window_count, num_windows);
[trusted_eo_candidates, global_eo_table] = select_trusted_eos_from_seed_tables_local( ...
    seed_table_cache(1:num_lock_windows), DynamicMap.Window(1:num_lock_windows), method);
global_eo = trusted_eo_candidates(1);
fprintf('Trusted EO candidates from first %d window(s): %s\n', ...
    num_lock_windows, mat2str(trusted_eo_candidates));
disp(global_eo_table(:, {'EO','support_count','median_rank','median_rel_rmse','score'}));

for w_idx = 1:num_windows
    Wmap = DynamicMap.Window(w_idx);
    bundle = bundle_cache{w_idx};
    seed_table = seed_table_cache{w_idx};
    result = refine_template_waveform_fit_local(bundle, seed_table, trusted_eo_candidates, cfg);
    result.VPSeedTable = struct2table(seed_table);
    result.TrustedEO = trusted_eo_candidates(:).';
    ...
    trend_rows(w_idx).d0_id = result.d0_id;
...
Result.EOLock = struct( ...
    'Mode', 'initial_windows_first_order_vp_support_candidates_then_fixed_eo_waveform_fit', ...
    'WindowCount', num_lock_windows, ...
    'WindowIDs', [DynamicMap.Window(1:num_lock_windows).window_id], ...
    'LapRanges', vertcat(DynamicMap.Window(1:num_lock_windows).lap_range), ...
    'SupportTopK', method.eo_support_top_k, ...
    'TrustedEOCount', method.trusted_eo_count, ...
    'TrustedEO', trusted_eo_candidates(:).');
...
candidate = fullfile(template_dir, sprintf('Template_LowSpeedRotating_B%d_%s_20251222.mat', target_blade, sensor_tag));
...
sensor_tag = ['S', sprintf('%d', analysis_sensors)];
candidate = fullfile(dynamic_dir, sprintf('DynamicMap_B%d_%s_SlidingWindows_20251222.mat', target_blade, sensor_tag));
if exist(candidate, 'file') == 2
...
for is = 1:numel(analysis_sensors)
    ...
    mask_query_safe = x_raw >= x_domain(1) + method.query_guard_mm & ...
                      x_raw <= x_domain(2) - method.query_guard_mm;
    mask = mask_pulse & mask_domain & isfinite(f0) & isfinite(fx0) & isfinite(v_raw) & isfinite(theta_raw);
    safe_mask = mask & mask_query_safe;
    if nnz(safe_mask) >= 8
        mask = safe_mask;
    end
    if nnz(mask) < 8
        mask = mask_domain & isfinite(f0) & isfinite(fx0) & isfinite(v_raw) & isfinite(theta_raw);
        safe_mask = mask & mask_query_safe;
        if nnz(safe_mask) >= 8
            mask = safe_mask;
        end
    ...
    w_total = max(method.weight_floor, w_edge .* w_template);
    if max(w_total) > 0
    ...
    window_meta(meta_idx).query_guard_mm = method.query_guard_mm;
...
bundle.query_guard_mm = method.query_guard_mm;
bundle.window_meta = window_meta;
bundle.valid_segment_count = numel(window_meta);
...
rows = repmat(struct('EO', NaN, 'A', NaN, 'phi', NaN, 'd0', NaN, ...
...
for i = 1:numel(eo_candidates)
    ...
    [obj, plain_rmse] = template_fixed_eo_objective_local([A, phi, d0], eo, bundle);
    rows(i).EO = eo;
    rows(i).A = A;
    rows(i).phi = phi;
    rows(i).d0 = d0;
...
function [trusted_eos, eo_table] = select_trusted_eos_from_seed_tables_local(seed_tables, windows, method)
all_eo = [];
for iw = 1:numel(seed_tables)
    all_eo = [all_eo, [seed_tables{iw}.EO]]; %#ok<AGROW>
end
eo_candidates = unique(round(all_eo(:)));
...
for ie = 1:numel(eo_candidates)
    eo = eo_candidates(ie);
    ranks = nan(numel(seed_tables), 1);
    rel = nan(numel(seed_tables), 1);
    for iw = 1:numel(seed_tables)
        st = seed_tables{iw};
        eo_list = [st.EO].';
        rmse_list = [st.weighted_voltage_rmse].';
        idx = find(eo_list == eo, 1);
        if isempty(idx)
            continue;
        end
        ranks(iw) = idx;
        rel(iw) = rmse_list(idx) / max(min(rmse_list), eps);
        if nargin >= 2 && numel(windows) >= iw && isfield(windows(iw), 'rot_freq_mean_hz')
            rel(iw) = rel(iw) + 1e-4 * abs(eo * windows(iw).rot_freq_mean_hz - st(idx).EO * windows(iw).rot_freq_mean_hz);
        end
    end
    support_count(ie) = sum(ranks <= method.eo_support_top_k, 'omitnan');
    median_rank(ie) = median(ranks, 'omitnan');
    median_rel_rmse(ie) = median(rel, 'omitnan');
    mean_rel_rmse(ie) = mean(rel, 'omitnan');
    score(ie) = median_rel_rmse(ie) + 0.02 * median_rank(ie) - 0.05 * support_count(ie);
end

eo_table = table(eo_candidates, support_count, median_rank, median_rel_rmse, mean_rel_rmse, score, ...
    'VariableNames', {'EO','support_count','median_rank','median_rel_rmse','mean_rel_rmse','score'});
eo_table = sortrows(eo_table, {'support_count','score','median_rank','EO'}, ...
    {'descend','ascend','ascend','ascend'});
trusted_count = min(method.trusted_eo_count, height(eo_table));
trusted_eos = eo_table.EO(1:trusted_count).';
end
...
offset_limit_mm = abs(cfg.offset_limit_mm);
eo_candidates = unique(round(eo_candidates(:).'));
candidate = repmat(struct('EO', NaN, 'A', NaN, 'phi', NaN, 'd0', NaN, ...
...
for i = 1:numel(eo_candidates)
    ...
    seed_params = [seed.A, seed.phi, seed.d0];
    fun = @(p) template_fixed_eo_objective_local(p, eo, bundle);
    opts = optimset('Display', 'off', 'MaxIter', 400, 'MaxFunEvals', 1200);
    p_opt = fminsearch(@(p) bounded_fixed_eo_objective_local(p, fun, amp_limit_mm, offset_limit_mm), seed_params, opts);
    p_opt(1) = min(abs(p_opt(1)), amp_limit_mm);
    p_opt(2) = wrap_to_pi_local(p_opt(2));
    p_opt(3) = max(min(p_opt(3), offset_limit_mm), -offset_limit_mm);
    [obj, plain_rmse, v_pred, u_est, coverage] = template_fixed_eo_objective_local(p_opt, eo, bundle);

    candidate(i).EO = eo;
    candidate(i).A = p_opt(1);
    candidate(i).phi = p_opt(2);
    candidate(i).d0 = p_opt(3);
...
candidate_table = struct2table(rmfield(candidate, {'V_pred','u_est'}));
...
result.metric_note = 'Template-only fixed-EO waveform fit without sensor affine projection.';
end

function obj = bounded_fixed_eo_objective_local(p, fun, amp_limit_mm, offset_limit_mm)
p_use = p;
p_use(1) = min(abs(p_use(1)), amp_limit_mm);
p_use(2) = wrap_to_pi_local(p_use(2));
p_use(3) = max(min(p_use(3), offset_limit_mm), -offset_limit_mm);
obj = fun(p_use);
end

function [obj, plain_rmse, v_pred, u_est, coverage] = template_fixed_eo_objective_local(params, eo, bundle)
A = params(1);
phi = params(2);
d0 = params(3);
u_est = A .* sin(eo .* bundle.Theta + phi) + d0;
x_in = bundle.X - u_est;
...
obj = sum(bundle.W(valid) .* (res .^ 2)) + 1e6 * nnz(~valid);
