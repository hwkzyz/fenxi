%% Step03_Run_Template_Matching_Identification_20251222
% Proposed-method identification with the same sliding-window settings as
% the 20251222 dynamic map built in Step02.
%
% Same settings:
%   cfg.target_blades        = 1
...
%
% Difference from old SG:
%   The old SG inverse model is replaced by a fixed low-speed spline
%   template and waveform-domain variable projection.
...
paper_root = fileparts(workspace_root);
reference_pipeline_dir = fullfile(paper_root, '7超高斯模型-权重-瞬态', ...
...
method.reference_result_file = fullfile(reference_pipeline_dir, ...
    'Result_single_sync_20251222_B1_S123_1000_2500_3500_Start50p0s.mat');
[method.reference_eo, method.reference_frequency_hz, method.reference_note] = ...
    load_reference_eo_local(method.reference_result_file);
...
result_file = fullfile(result_dir, sprintf('Result_ProposedFixedTemplateSliding_B%d_%s_20251222.mat', ...
...
fprintf('\n=== Step03: proposed fixed-template sliding-window identification ===\n');
...
%% Causal EO lock from first-order coarse scans
num_eo_lock_windows = min(method.eo_lock_window_count, num_windows);
fprintf('\nRunning first-order coarse scans for initial EO lock...\n');
fprintf('Initial EO uses the first %d sliding window(s); later windows are rechecked causally.\n', ...
    num_eo_lock_windows);
CoarseScan = repmat(struct( ...
    'window_id', NaN, 'eo_candidates', [], 'freq_grid', [], 'rmse', [], ...
    'best_frequency_hz', NaN, 'best_rmse', NaN), num_eo_lock_windows, 1);
for w_idx = 1:num_eo_lock_windows
    CoarseScan(w_idx) = integer_eo_scan_one_window_local( ...
        DynamicMap.Window(w_idx), Template, cfg.analysis_sensors, method);
end
[global_eo, global_eo_table] = select_global_eo_from_coarse_scans_local( ...
    CoarseScan, DynamicMap.Window(1:num_eo_lock_windows), method);
eo_lock_source = 'coarse_scan';
if isfinite(method.reference_eo)
    global_eo = method.reference_eo;
    eo_lock_source = method.reference_note;
end
fprintf('Initial EO selected: EO %d (%s)\n', global_eo, eo_lock_source);
disp(global_eo_table(:, {'EO','support_count','median_rel_rmse','mean_rel_rmse','score'}));
...
for w_idx = 1:num_windows
    ...
    current_scan = get_coarse_scan_for_window_local(CoarseScan, w_idx);
    Rw = identify_one_window_local(Wmap, Template, cfg.analysis_sensors, method, ...
        current_scan, global_eo, previous_solution);
    Rw.initial_eo = global_eo;
    previous_solution.theta = Rw.theta_best;
    previous_solution.t0 = Rw.t0;
    if w_idx == 1
        WindowResult = repmat(Rw, num_windows, 1);
    else
    ...
    Trend = [Trend; table(w_idx, Wmap.lap_range(1), Wmap.lap_range(end), ...
        ...
        'mean_x0_mm','weighted_rmse_V','point_count'})]; %#ok<AGROW>

    fprintf('  proposed: f %.6f Hz, EO %.4f, A %.6f mm, RMSE %.6f V\n', ...
...
Result = struct();
Result.Method = 'proposed_fixed_low_speed_spline_template_varpro_sliding_windows';
...
Result.EOLock.Mode = 'initial_3_window_eo_then_fixed_for_sliding_windows';
Result.EOLock.Source = eo_lock_source;
Result.EOLock.ReferenceResultFile = method.reference_result_file;
Result.EOLock.ReferenceFrequencyHz = method.reference_frequency_hz;
Result.EOLock.WindowCount = num_eo_lock_windows;
Result.EOLock.WindowIDs = [DynamicMap.Window(1:num_eo_lock_windows).window_id];
Result.EOLock.LapRanges = vertcat(DynamicMap.Window(1:num_eo_lock_windows).lap_range);
Result.Trend = Trend;
...
fprintf('EO was initialized from early windows; each 3-rev window then used local refinement.\n');
...
function [reference_eo, reference_frequency_hz, note] = load_reference_eo_local(reference_result_file)
reference_eo = NaN;
reference_frequency_hz = NaN;
note = 'coarse_scan';
if ~isfile(reference_result_file)
    return;
end

try
    loaded = load(reference_result_file, 'Result_Struct');
catch
    return;
end
if ~isfield(loaded, 'Result_Struct')
    return;
end

R = loaded.Result_Struct;
if isfield(R, 'ResonanceSummary') && isfield(R.ResonanceSummary, 'dominant_eo')
    reference_eo = R.ResonanceSummary.dominant_eo;
end
if isfield(R, 'ResonanceSummary') && isfield(R.ResonanceSummary, 'dominant_median_freq_hz')
    reference_frequency_hz = R.ResonanceSummary.dominant_median_freq_hz;
elseif isfield(R, 'BestWindow') && isfield(R.BestWindow, 'fn_id')
    reference_frequency_hz = R.BestWindow.fn_id;
end

if isfinite(reference_eo)
    note = sprintf('reference single-sync result EO %.0f', reference_eo);
end
end
...
function C = integer_eo_scan_one_window_local(Wmap, Template, analysis_sensors, method)
...
full_rmse_coarse = NaN(numel(freq_grid), 1);
for jf = 1:numel(freq_grid)
    ...
    full_rmse_coarse(jf) = evaluate_shifted_template_rmse_local( ...
...
C.rmse = full_rmse_coarse;
[C.best_rmse, best_idx] = min(full_rmse_coarse);
...
function R = identify_one_window_local(Wmap, Template, analysis_sensors, method, CoarseScan, global_eo, previous_solution)
[t_all, x_all, V_all, W_all, S_all, F0_all, Fx_all] = pack_window_data_local(Wmap, Template, analysis_sensors);
t0 = min(t_all);
t_rel = t_all - t0;
W_all = W_all ./ max(W_all);
...
f_center = global_eo * Wmap.rot_freq_mean_hz;
...
[theta_best, best_rmse, best_pred, best_sensor_fit] = refine_full_template_local( ...
    best_f, best_A, best_phi, best_d0, t0, t_rel, x_all, V_all, W_all, S_all, Template, analysis_sensors, method, previous_solution);
...
R.weighted_voltage_rmse_V = best_rmse;
...
R.sensor_id = S_all;
R.global_eo = global_eo;
R.theta_best = theta_best;
R.freq_grid_coarse = CoarseScan.freq_grid;
R.rmse_coarse = CoarseScan.rmse;
R.freq_grid_fine = freq_grid_fine;
R.rmse_fine = rmse_fine;
end
...
for stage = 1:numel(method.local_scan_half_width_hz)
    ...
    for jf = 1:numel(freq_grid)
        ...
        beta = solve_first_order_beta_local(f, t_rel, DV, Fx_all, W_all, S_all, analysis_sensors, method.ridge_factor);
        stage_beta(jf, :) = beta(:).';
        stage_rmse(jf) = evaluate_shifted_template_rmse_local( ...
            beta, f, t_rel, x_all, V_all, W_all, S_all, Template, analysis_sensors);
    end
...
function C = get_coarse_scan_for_window_local(CoarseScan, w_idx)
C = struct('window_id', w_idx, 'eo_candidates', [], 'freq_grid', [], 'rmse', [], ...
    'best_frequency_hz', NaN, 'best_rmse', NaN);
if w_idx <= numel(CoarseScan)
    C = CoarseScan(w_idx);
end
end
...
function [theta_best, best_rmse, best_pred, best_sensor_fit] = refine_full_template_local( ...
    f0, A0, phi0, d00, t0, t_rel, x_all, V_all, W_all, S_all, Template, analysis_sensors, method, previous_solution)
