%% Step03F_Run_TemplateOnly_FirstOrderVPTopK_20251222
% First-order template-VP/top-K accelerated low-speed-template-only
% identification using the same final waveform objective as Step03D.
%
% Every integer EO candidate is scored by the first-order model
% V - T(x) ~= -T'(x) * [d0 + a*sin(EO*theta) + b*cos(EO*theta)].
% Top-K EO candidates then enter the full low-speed-template waveform
% refinement. The final EO is selected only by post-refinement RMSE.
...
cfg.debug_max_windows = parse_positive_integer_env_local('STEP03F_MAX_WINDOWS', inf);
cfg.vp_top_k_eo = parse_positive_integer_env_local('STEP03F_TOP_K_EO', 5);
cfg.diagnostic_eo = parse_optional_integer_env_local('STEP03_DIAGNOSTIC_EO');

method = struct();
method.name = 'template_only_first_order_vp_topk';
method.selection_rule = 'first_order_template_vp_topk_then_fixed_eo_direct_template_waveform_rmse';
...
result_file = fullfile(result_dir, sprintf('Result_TemplateOnlyFirstOrderVPTopK_B%d_%s%s_20251222.mat', ...
...
fprintf('\n=== Step03F: template-only first-order VP-topK identification ===\n');
...
fprintf('First-order VP/top-K: keep top %d EO candidates for final waveform optimization.\n', method.vp_top_k_eo);
