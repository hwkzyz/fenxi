...
cfg.allowDynamicMapFallback = parse_logical_env_local('STEP07J_ALLOW_DYNAMIC_MAP_FALLBACK', true);
...
if isempty(staticEtaOverride)
    [cfg.fixedSensorEtaMm, cfg.fixedSensorEtaInfo] = ...
        Step07J_FixedJointEtaPrior_20241106(cfg.targetBlade, cfg.analysisSensors);
else
...
if strcmpi(cfg.localBundleSource, 'foundation_step05_bundle') && ~isempty(FoundationStep05Source)
...
Result.description = ['Current Step07J route uses a folder-local foundation_core no-gap kernel first: fixed joint static eta, gradient-displacement VP seeds, and full-waveform RMSE refinement/ranking. ' ...
    'The default 20241106 route no longer uses the rotating_calibration direct result as the bundle or EO source; direct-result use is retained only behind explicit diagnostic environment switches. ' ...
    ...
    'The default main run computes gap_tilt plus its required fixed kernel. gap_only is computed only in STEP07J_RUN_MODE=comparison. gap_tilt_shift is excluded from the current comparison because the extra sensor-shift freedom is invalid for the paper-facing method.'];
Result.cfg = cfg;
...
templateDir = fullfile(rootDir, '20241106_btt_data_foundation', ...
...
resultFile = '';
sensorTag = ['S', sprintf('%d', sensorIds)];
foundationOutputRoot = fullfile(rootDir, '20241106_btt_data_foundation', 'output');
patterns = {
    fullfile(foundationOutputRoot, 'step05_single_sync_direct_template_fullbundle_for_gap_step07j', '*', sprintf( ...
        'Result_Step05_FoundationMainPulseAdaptiveFixedJointEta_B%d_%s_20241106.mat', ...
        targetBlade, sensorTag))
    fullfile(foundationOutputRoot, 'step05_single_sync_direct_template*', '*', sprintf( ...
        'Result_Step05_FoundationMainPulseAdaptiveFixedJointEta_B%d_%s_20241106.mat', ...
        targetBlade, sensorTag))
    fullfile(foundationOutputRoot, 'step05_single_sync_direct_template*', '*', sprintf( ...
        'Result_Step05_FixedJointStaticEtaPreview_B%d_%s_20241106.mat', ...
        targetBlade, sensorTag))
    fullfile(foundationOutputRoot, 'step05_single_sync_direct_template*', '*', sprintf( ...
        'Result_Step05_SingleSyncDirectTemplate_B%d_%s_20241106.mat', ...
        targetBlade, sensorTag))
    };
for ip = 1:numel(patterns)
