function Sensor_Config = Step1_Build_Sensor_Config_20241106(show_plots)
%STEP1_BUILD_SENSOR_CONFIG_20241106 Build Step1-style fingerprint config for 20241106.
...
case_data = Extract_BTT_Features_20241106(cfg.low_speed_case, cfg);
...
opr_pulses_per_rev = cfg.opr_pulses_per_rev;
if numel(case_data.opr_times) > opr_pulses_per_rev
    spd_t = case_data.opr_times(1:(end - opr_pulses_per_rev));
    spd_v = 360 ./ (case_data.opr_times((opr_pulses_per_rev + 1):end) - case_data.opr_times(1:(end - opr_pulses_per_rev)));
...
save(fullfile(cfg.reference_output_dir, 'Sensor_Config_20241106.mat'), 'Sensor_Config');
save(fullfile(cfg.reference_output_dir, 'Sensor_Config.mat'), 'Sensor_Config');
writetable(summary_table, fullfile(cfg.reference_output_dir, 'Sensor_Config_Summary_20241106.csv'));

fprintf('Saved Sensor_Config to:\n  %s\n', fullfile(cfg.reference_output_dir, 'Sensor_Config_20241106.mat'));
...
fig1 = figure('Name', '20241106 Step1 Fingerprint Alignment', ...
...
fig2 = figure('Name', '20241106 Step1 Standard Relative Angles', ...
...
figure('Name', '20241106 Step1 Low-Speed RPM', ...
...
figure('Name', '20241106 Step1 Summary Table', ...
...
angle_table = array2table(angle_mat.', ...
    'VariableNames', compose('CH%d', sensor_ids));
angle_table = addvars(angle_table, blade_ids, 'Before', 1, ...
    'NewVariableNames', 'BladeID');
disp(angle_table);

ref_sid = cfg.reference_sensor_id;
ref_idx = find(sensor_ids == ref_sid, 1, 'first');
if isempty(ref_idx)
    ref_idx = 1;
    ref_sid = sensor_ids(ref_idx);
end

sector_deg = 360 / cfg.blades_num;
angle_ref = angle_mat - angle_mat(ref_idx, :);
angle_ref = wrap_to_period_local(angle_ref, sector_deg);
angle_ref(ref_idx, :) = 0;

fprintf('Sensor angles referenced to CH%d = 0 deg within one OPR sector (mod %.1f deg)\n', ...
    ref_sid, sector_deg);
ref_table = array2table(angle_ref.', ...
    'VariableNames', compose('CH%d', sensor_ids));
ref_table = addvars(ref_table, blade_ids, 'Before', 1, ...
    'NewVariableNames', 'BladeID');
disp(ref_table);

mean_ref = mean(angle_ref, 2, 'omitnan').';
mean_table = array2table(mean_ref, 'VariableNames', compose('CH%d', sensor_ids));
fprintf('Mean sector-relative sensor angles (not absolute installation spacing)\n');
disp(mean_table);

candidate_text = strings(1, numel(sensor_ids));
for i = 1:numel(sensor_ids)
    base_deg = mean_ref(i);
    candidates = base_deg + (0:(cfg.blades_num - 1)) * sector_deg;
    candidate_text(i) = strjoin(compose('%.3f', candidates), ', ');
end
candidate_table = array2table(candidate_text, 'VariableNames', compose('CH%d', sensor_ids));
fprintf('Equivalent absolute angle candidates relative to CH%d = 0 deg\n', ref_sid);
disp(candidate_table);
fprintf('\n');
end


function y = wrap_to_period_local(x, period)
y = mod(x, period);
y(y < 0) = y(y < 0) + period;
end
...
opr_reference = build_opr_reference_from_center_start_local( ...
    center_times, start_times, cfg.opr_pulses_per_rev, cfg.r_tip_mm);
