%% Step06I: calibrate all-blade gap library for 20241106
...
templateBankFile = fullfile(outDir, P.calibration.templateBankFile);
if ~isfile(responseFile)
    error('Run Step05I first. Missing file: %s', responseFile);
end
if ~isfile(templateBankFile)
    fprintf('Low-speed template bank is missing. Running Step06A first...\n');
    run(fullfile(thisDir, 'Step06A_BuildLowSpeedTemplateBank_20241106.m'));
end
if ~isfile(templateBankFile)
...
bladeIds = parse_int_env_local('STEP06I_BANK_BLADE_IDS', P.calibration.buildBladeIds);
...
fprintf('Blades: %s | gap sensors: %s\n', mat2str(bladeIds), mat2str(gapSensors));
...
bankFile = fullfile(outDir, P.calibration.gapBankFile);
