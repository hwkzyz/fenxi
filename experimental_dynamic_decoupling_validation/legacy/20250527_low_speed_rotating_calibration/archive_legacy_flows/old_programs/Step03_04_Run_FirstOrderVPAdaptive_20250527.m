%% Step03_04_Run_FirstOrderVPAdaptive_20250527
% Adaptive first-order template-VP accelerated low-speed-template-only
...
cfg.vp_top_k_eo = parse_positive_integer_env_local('STEP03_04_TOP_K_EO', 3);
...
method = struct();
method.name = 'template_only_first_order_vp_adaptive';
...
result_file = fullfile(result_dir, sprintf('Result_Step03_04_FirstOrderVPAdaptive_B%d_%s%s_20250527.mat', ...
...
fprintf('\n=== Step03_04: first-order VP adaptive identification ===\n');
...
fprintf('Model: V = T_low(x - dx_c - A*sin(EO*theta + phi)); synchronous f = EO*rot_freq_mean.\n');
...
for is = 1:numel(analysis_sensors)
    ...
    mask = mask_pulse & mask_effective & mask_domain & isfinite(f0) & isfinite(fx0) & isfinite(v_raw) & isfinite(theta_raw);
...
for i = 1:numel(eo_candidates)
    ...
    [obj, plain_rmse] = template_synchronous_objective_local([A, phi, dx_c], eo, bundle);
    rows(i).EO = eo;
    rows(i).A = A;
    rows(i).phi = phi;
    rows(i).dx_c = dx_c;
...
for i = 1:numel(eo_candidates)
    ...
    seed_params = [seed.A, seed.phi, seed.dx_c];
    fun = @(p) template_synchronous_objective_local(p, eo, bundle);
    opts = optimset('Display', 'off', 'MaxIter', 400, 'MaxFunEvals', 1200);
    p_opt = fminsearch(@(p) bounded_synchronous_objective_local(p, fun, amp_limit_mm, dx_c_limit_mm), seed_params, opts);
...
result.metric_note = 'Template-only VP top-K synchronous waveform fit with spatial alignment dx_c.';
end

function obj = bounded_synchronous_objective_local(p, fun, amp_limit_mm, dx_c_limit_mm)
p_use = p;
