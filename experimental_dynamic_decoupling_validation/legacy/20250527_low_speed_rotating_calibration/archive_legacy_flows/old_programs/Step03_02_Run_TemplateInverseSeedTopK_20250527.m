%% Step03_02_Run_TemplateInverseSeedTopK_20250527
% VP/top-K accelerated low-speed-template-only identification using the
% same final waveform objective as Step03_01.
%
...
cfg.debug_max_windows = parse_positive_integer_env_local('STEP03_02_MAX_WINDOWS', inf);
cfg.vp_top_k_eo = parse_positive_integer_env_local('STEP03_02_TOP_K_EO', 5);

method = struct();
method.name = 'template_inverse_seed_topk';
method.selection_rule = 'vp_seed_topk_then_full_waveform_rmse';
...
result_file = fullfile(result_dir, sprintf('Result_Step03_02_TemplateInverseSeedTopK_B%d_%s_20250527.mat', ...
...
fprintf('\n=== Step03_02: template-inverse seed top-K identification ===\n');
...
fprintf('Frequency search: %.1f-%.1f Hz; no reference EO; VP only screens candidates.\n', ...
...
fprintf('VP/top-K: keep top %d EO candidates for final waveform optimization.\n', method.vp_top_k_eo);
...
for w_idx = 1:num_windows
    ...
    final_eo_candidates = select_vp_topk_eo_local(seed_table, method.vp_top_k_eo);
    ...
    eo14_seed_rank = NaN;
    if any([seed_table.EO] == 14)
        eo14_seed_rank = find([seed_table.EO] == 14, 1);
    end
    fprintf('Window %02d/%02d laps %s: EO=%d, f=%.3f Hz, A=%.4f mm, RMSE=%.5f V, clamp=%.2f%%, EO14 seed-rank=%s\n', ...
        ...
        rank_to_string_local(eo14_seed_rank));
end
...
candidate = fullfile(template_dir, sprintf('Template_LowSpeedRotating_B%d_%s_20250527.mat', target_blade, sensor_tag));
...
for is = 1:numel(analysis_sensors)
    ...
    mask = mask_pulse & mask_domain & isfinite(f0) & isfinite(v_raw) & isfinite(theta_raw);
    safe_mask = mask & mask_query_safe;
    if nnz(safe_mask) >= 8
        mask = safe_mask;
    end
    if nnz(mask) < 8
        mask = mask_domain & isfinite(f0) & isfinite(v_raw) & isfinite(theta_raw);
        safe_mask = mask & mask_query_safe;
        if nnz(safe_mask) >= 8
            mask = safe_mask;
        end
