...
P.name.resultSuffix = 'Tmp_Profile_DirectTemplate_OPRCenterStd_Dx035_NoEta_PrevWinPhaseSafe';

P.step03.analysisSensors = [1 2 3];
...
templateFile = fullfile(routeDir, 'output', 'templates', ...
    sprintf('Template_LowSpeedRotating_B1_S123_%s_20251222.mat', P.name.templateSuffix));
dynamicFile = fullfile(routeDir, 'output', 'dynamic_maps', ...
    sprintf('DynamicMap_B1_S123_SlidingWindows_%s_20251222.mat', P.name.dynamicSuffix));
...
try
    save(tmp_result_file, 'Result', '-v7.3');
catch saveErr
    warning('Step03:SaveV73Failed', 'save -v7.3 failed (%s). Retrying with -v7.', saveErr.message);
    if exist(tmp_result_file, 'file') == 2
        delete(tmp_result_file);
    end
