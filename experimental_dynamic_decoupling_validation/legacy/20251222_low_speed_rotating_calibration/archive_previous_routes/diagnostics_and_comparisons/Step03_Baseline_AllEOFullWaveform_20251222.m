%% Step03_Baseline_AllEOFullWaveform_20251222
% Baseline all-EO full waveform identification without VP acceleration.
%
% This script intentionally does not use VP, EO locking, reference EO, or
% continuous frequency refinement. Every integer EO candidate is refined in
% waveform domain with u = A*sin(EO*theta + phi) + d0, and the final EO is
% selected only by the post-refinement weighted waveform RMSE.
...
cfg.debug_max_windows = parse_positive_integer_env_local('STEP03D_MAX_WINDOWS', inf);
cfg.pulse_selection_mode = lower(strtrim(getenv('STEP03D_PULSE_MODE')));
if isempty(cfg.pulse_selection_mode)
    cfg.pulse_selection_mode = 'all';
end
if ~ismember(cfg.pulse_selection_mode, {'all', 'single'})
    error('STEP03D_PULSE_MODE must be "all" or "single".');
end
cfg.sensor_balance_weights = parse_logical_env_local('STEP03D_SENSOR_BALANCE', true);

method = struct();
method.name = 'baseline_all_eo_full_waveform';
method.selection_rule = 'all_eo_full_waveform_rmse';
method.use_reference_eo_constraint = false;
method.use_vp = false;
method.refine_params = {'A', 'phi', 'd0'};
method.template_forward = 'interp1_low_speed_template';
method.template_inverse_seed = 'voltage_crossing_nearest_x';
...
if isempty(method.domain_selection_mode)
    method.domain_selection_mode = 'soft';
end
if ~ismember(method.domain_selection_mode, {'hard', 'soft'})
    error('STEP03D_DOMAIN_SELECTION_MODE must be "hard" or "soft".');
end
method.domain_soft_margin_mm = parse_nonnegative_numeric_env_local('STEP03D_DOMAIN_SOFT_MARGIN_MM', 0.2);
...
method.sensor_balance_weights = cfg.sensor_balance_weights;
...
result_file = fullfile(result_dir, sprintf('Result_Step03_Baseline_AllEOFullWaveform_B%d_%s%s_20251222.mat', ...
...
fprintf('\n=== Step03 baseline: all-EO full waveform identification ===\n');
...
fprintf('Frequency search: %.1f-%.1f Hz; no reference EO; no VP.\n', cfg.freq_search_hz(1), cfg.freq_search_hz(2));
fprintf('Model: V = T_low(x - A*sin(EO*theta + phi) - d0); EO decides frequency.\n');
...
fprintf('Sensor-balanced weights: %d.\n', method.sensor_balance_weights);
...
for w_idx = 1:num_windows
    ...
    result = refine_template_waveform_fit_local(bundle, seed_table, eo_candidates, cfg);
    ...
    eo14_rank = NaN;
    if any(result.CandidateTable.EO == 14)
        eo14_rank = find(result.CandidateTable.EO == 14, 1);
    end
    fprintf('Window %02d/%02d laps %s: EO=%d, f=%.3f Hz, A=%.4f mm, RMSE=%.5f V, clamp=%.2f%%, EO14 rank=%s\n', ...
        w_idx, num_windows, mat2str(Wmap.lap_range), result.EO_id, result.fn_id, ...
        result.A_id, result.weighted_voltage_rmse, 100 * result.Coverage.clamp_fraction, ...
        rank_to_string_local(eo14_rank));
end
...
function v = parse_logical_env_local(name, default_value)
...
elseif ismember(raw, {'0', 'false', 'no', 'off'})
    ...
    error('%s must be true/false or 1/0.', name);
end
end
...
for i = 1:numel(files)
    candidate_file = fullfile(template_dir, files(i).name);
...
for i = 1:numel(files)
    candidate_file = fullfile(dynamic_dir, files(i).name);
...
for is = 1:numel(analysis_sensors)
    ...
    if strcmpi(method.pulse_selection_mode, 'single')
    ...
    mask = mask_pulse & mask_effective & mask_domain & isfinite(f0) & isfinite(v_raw) & isfinite(theta_raw);
    ...
    if nnz(mask) < 8
        mask = mask_effective & mask_domain & isfinite(f0) & isfinite(v_raw) & isfinite(theta_raw);
        if strcmpi(method.domain_selection_mode, 'hard')
            safe_mask = mask & mask_query_safe;
            if nnz(safe_mask) >= 8
                mask = safe_mask;
    ...
    w_total = max(method.weight_floor, w_edge .* w_template .* w_domain);
    if max(w_total) > 0
    ...
    window_meta(meta_idx).selected_theta_range_rad = range(theta_sel);
    window_meta(meta_idx).selected_time_range_s = range(t_sel);
...
if method.sensor_balance_weights
    present_sensor_idx = unique(sensor_index_pt(:)).';
    sensor_weight_total = numel(W) / max(numel(present_sensor_idx), 1);
    for idx_sensor = present_sensor_idx
        idx = sensor_index_pt == idx_sensor;
        w_sum = sum(W(idx), 'omitnan');
        if isfinite(w_sum) && w_sum > 0
            W(idx) = W(idx) ./ w_sum .* sensor_weight_total;
        end
    end
end
...
rows = repmat(struct('EO', NaN, 'A', NaN, 'phi', NaN, 'd0', NaN, ...
    'weighted_voltage_rmse', inf, 'plain_voltage_rmse', inf), numel(eo_candidates), 1);
for i = 1:numel(eo_candidates)
    eo = eo_candidates(i);
    basis = [sin(eo * bundle.Theta), cos(eo * bundle.Theta), ones(bundle.point_count, 1)];
    w_sqrt = sqrt(bundle.W(:));
    coeff = (basis .* w_sqrt) \ (bundle.Y_obs(:) .* w_sqrt);
    A = hypot(coeff(1), coeff(2));
    phi = atan2(coeff(2), coeff(1));
    d0 = coeff(3);
    [obj, plain_rmse] = template_single_sync_objective_local([A, phi, d0], eo, bundle);
    rows(i).EO = eo;
    rows(i).A = A;
    rows(i).phi = phi;
    rows(i).d0 = d0;
...
score = [[rows.weighted_voltage_rmse].', [rows.plain_voltage_rmse].', [rows.EO].'];
...
for i = 1:numel(eo_candidates)
    ...
    if isempty(seed_idx)
        seed_params = [seed_table(1).A, seed_table(1).phi, seed_table(1).d0];
    else
        seed_params = [seed_table(seed_idx).A, seed_table(seed_idx).phi, seed_table(seed_idx).d0];
    end
    fun = @(p) template_single_sync_objective_local(p, eo, bundle);
    opts = optimset('Display', 'off', 'MaxIter', 400, 'MaxFunEvals', 1200);
    p_opt = fminsearch(@(p) bounded_objective_local(p, fun, amp_limit_mm, offset_limit_mm), seed_params, opts);
    ...
    [obj, plain_rmse, v_pred, u_est, coverage] = template_single_sync_objective_local(p_opt, eo, bundle);
...
result.metric_note = 'Template-only single-sync EO-constrained waveform fit.';
end

function obj = bounded_objective_local(p, fun, amp_limit_mm, offset_limit_mm)
p_use = p;
...
function [obj, plain_rmse, v_pred, u_est, coverage] = template_single_sync_objective_local(params, eo, bundle)
A = params(1);
phi = params(2);
d0 = params(3);
...
obj = sum(bundle.W(valid) .* (res .^ 2)) + overshoot_penalty + 1e6 * nnz(~valid);
