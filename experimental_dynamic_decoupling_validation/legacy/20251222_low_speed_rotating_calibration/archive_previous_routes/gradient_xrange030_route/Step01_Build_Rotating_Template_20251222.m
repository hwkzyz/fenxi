%% Step01_Build_Rotating_Template_20251222
...
target_blade = 1;
analysis_sensors = legacy_cfg.sensor_ids(:).';       % build a reusable sensor library; Step03 can use subsets
...
template_file = fullfile(template_dir, sprintf('Template_LowSpeedRotating_B%d_%s%s_20251222.mat', ...
...
for is = 1:numel(analysis_sensors)
    ...
    fprintf(['CH%d: baseline %.5f V, threshold %.3f V, pulses %d, ' ...
        'offset laps %d, selected points %d/%d, domain [%.3f, %.3f] mm, xc %.4f mm, ' ...
        'detected xc %.4f mm, source %s\n'], ...
        sid, baseline, threshold, numel(selected_idx), start_offset_lap, ...
        numel(x_rel), numel(x_wide), x_left, x_right, xc_for_coordinate, P.xc, xc_reference_source);
end
...
for i = 1:numel(file_ids)
    ...
    for sid = sensor_ids
        ...
        raw_stream(sid).T = [raw_stream(sid).T; t_local(:)];
        raw_stream(sid).V = [raw_stream(sid).V; v_local(:)];
    end
end

opr_segments = extract_opr_segments_local(opr_T, opr_V, cfg.opr_threshold, cfg.gap_points);
opr_times = opr_segments.arrival_time(:);
fprintf('Loaded %d OPR pulses from low-speed case.\n', numel(opr_times));
end
