%% Step01L_Build_AllBladeSensor_TemplateLibrary_20251222
...
libraryDir = fullfile(projectDir, 'output', 'template_library');
...
for ib = 1:numel(bladeIds)
    ...
    C = CaseConfig();
    stablePlanFile = fullfile(projectDir, 'output', 'stable_window_plan', sprintf( ...
        'Step01_CoverageFirstStableWindowPlan_%s_%s.csv', C.caseTag, dataset));
    referenceTemplateFile = fullfile(projectDir, 'output', 'templates', sprintf( ...
        'Template_LowSpeedRotating_%s_OPRCenterStdRef_%s.mat', C.caseTag, dataset));
    combinedFile = fullfile(projectDir, 'output', 'templates', sprintf( ...
        ...
        try
            if exist(stablePlanFile, 'file') ~= 2
                if exist(referenceTemplateFile, 'file') ~= 2
                    ...
                    run_script_in_child_matlab_local(projectDir, ...
                        'Step01_Main_Build_OPRCenterStd_Template_20251222.m', bladeId, sensorIds, ...
                        {'STEP01_ALLOW_REFERENCE_ONLY', '1'});
                end
                if ~has_dynamic_map_for_case_local(projectDir, bladeId, sensorIds)
                    fprintf('\nBuild missing bootstrap dynamic map: B%d %s\n', bladeId, C.sensorTag);
                    run_script_in_child_matlab_local(projectDir, ...
                        'Step02_Main_Build_OPRCenterStd_DynamicMap_20251222.m', bladeId, sensorIds, ...
                        {'STEP02_TEMPLATE_SUFFIX', 'OPRCenterStdRef'; ...
                         'STEP02_DYNAMIC_SUFFIX', 'BootstrapFromRef'});
                end
                fprintf('\nBuild missing stable-window plan: B%d %s\n', bladeId, C.sensorTag);
                run_script_in_child_matlab_local(projectDir, ...
                    'Step00_Build_CoverageFirstStableWindowPlan_20251222.m', bladeId, sensorIds);
            end
...
function value = restore_env_local(oldBladeEnv, oldSensorEnv)
