...
if isempty(dynamicFile)
    dynamicFile = find_dynamic_file_for_sensors_script_local(dynamicDir, C.bladeId, C.sensorIds);
...
function dynamic_file = find_dynamic_file_for_sensors_script_local(dynamic_dir, target_blade, analysis_sensors)
...
dynamic_file = fullfile(files(idx).folder, files(idx).name);
end
