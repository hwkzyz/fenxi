function Summary = Step04_Main_Run_AllBlades_DirectTemplate_20241106()
%STEP04_MAIN_RUN_ALLBLADES_DIRECTTEMPLATE_20241106 Run the direct route for B1-B6.
...
runLabel = make_time_label_local(analysisStartTimeSec);
dynamicSuffix = sprintf('Main20L_W3S1_%s_GradientXRange030_OPRCenterStd', runLabel);
if isempty(eoLock)
    resultSuffix = sprintf('Direct_%s', runLabel);
else
    resultSuffix = sprintf('DirectEO%d_%s', eoLock, runLabel);
end
...
fprintf('Route: %s\n', routeDir);
fprintf('Blades: %s, sensors: %s\n', mat2str(bladeIds), mat2str(analysisSensors));
fprintf('Start time: %.6f s (%s), useRegionPlan=%d, regionId=%d, startMode=%s\n', ...
    analysisStartTimeSec, runLabel, useRegionPlan, regionId, regionStartMode);
fprintf('Dynamic suffix: %s\n', dynamicSuffix);
...
for i = 1:numel(bladeIds)
    ...
    set_blade_environment_local(bladeId, analysisSensors, regionId, regionStartMode, ...
        useRegionPlan, analysisStartTimeSec, dynamicSuffix, resultSuffix, ...
        calibrationLaps, eoLock, dxCLimitMM);
    ...
    summaryRows(i) = summarize_blade_result_local(routeDir, bladeId, sensorTag, resultSuffix, dxCLimitMM);
...
outFile = fullfile(outDir, sprintf('Step04_AllBlades_DirectSummary_%s_%s_20241106.csv', sensorTag, resultSuffix));
...
function set_blade_environment_local(bladeId, analysisSensors, regionId, regionStartMode, ...
    useRegionPlan, analysisStartTimeSec, dynamicSuffix, resultSuffix, calibrationLaps, eoLock, dxCLimitMM)
setenv('STEP20241106_USE_ENV_OVERRIDES', '1');
...
setenv('STEP20241106_ANALYSIS_START_TIME', sprintf('%.9f', analysisStartTimeSec));
...
setenv('STEP20241106_DXC_LIMIT_MM', sprintf('%.9f', dxCLimitMM));
if isempty(eoLock)
    setenv('STEP20241106_EO_LOCK', 'free');
else
    setenv('STEP20241106_EO_LOCK', num2str(eoLock));
end
end
...
function row = summarize_blade_result_local(routeDir, bladeId, sensorTag, resultSuffix, dxCLimitMM)
resultDir = fullfile(routeDir, 'output', 'identification');
resultSuffixClean = sanitize_result_suffix_local(resultSuffix);
pattern = sprintf('Result_Step03_Main_VPTop3SynchronousWaveform_B%d_%s%s_20241106.mat', ...
    bladeId, sensorTag, resultSuffixClean);
...
row.dxCAtLimitCount = nnz(abs(Trend.dx_c_id) >= (dxCLimitMM - 1e-3));
