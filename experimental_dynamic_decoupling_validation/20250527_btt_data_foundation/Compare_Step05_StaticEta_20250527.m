...
%   5. This diagnostic script is not the formal Step05 entry.  It uses the
%      same main-pulse / adaptive-EO route, but replaces free per-window
%      eta_s with a static inter-sensor offset derived from Step04 template
%      centers.
...
%
% Forward model:
%   V_i ~= T_{sensor,blade}(x_i - dx_c - A*sin(EO*theta_i + phi) [- eta_s])
%
% Usage:
%   Run after Step02 and Step04.  Set STEP05_STATIC_ETA_MODE to either
%   fixed or bounded.  Outputs are isolated under
...
static_eta_mode_env = lower(strtrim(getenv('STEP05_STATIC_ETA_MODE')));
if isempty(static_eta_mode_env)
    static_eta_mode_env = 'fixed';
end

static_eta_source_env = lower(strtrim(getenv('STEP05_STATIC_ETA_SOURCE')));
if isempty(static_eta_source_env)
    static_eta_source_env = 'step04_xc';
end
S.static_eta_source_mode = static_eta_source_env;
S.static_eta_source = 'step04_template_xc_difference';
S.static_eta_file = strtrim(getenv('STEP05_STATIC_ETA_FILE'));

static_eta_halfwidth_env = strtrim(getenv('STEP05_STATIC_ETA_HALFWIDTH_MM'));
S.static_eta_halfwidth_mm = 0.02;
if ~isempty(static_eta_halfwidth_env)
    parsed_static_eta_halfwidth = str2double(static_eta_halfwidth_env);
    if isfinite(parsed_static_eta_halfwidth) && parsed_static_eta_halfwidth >= 0
        S.static_eta_halfwidth_mm = parsed_static_eta_halfwidth;
    end
end

switch static_eta_mode_env
    case {'fixed', 'fixed_step04_template_center', 'fixed_step04_xc'}
        if is_joint_static_eta_source_local(S.static_eta_source_mode)
            S.eta_compare_label = 'FixedJointStaticEtaPreview';
            S.active_eta_model = 'fixed_static_eta';
            S.static_eta_source = 'joint_dynamic_residual_eta_preview';
        else
            S.eta_compare_label = 'FixedStep04XcEta';
            S.active_eta_model = 'fixed_step04_xc_eta';
        end
        S.eta_models = {S.active_eta_model};
        S.sensor_eta_mode = 'fixed_static';
        S.fit_sensor_eta = false;

    case {'bounded', 'bounded_step04_template_center', 'bounded_step04_xc'}
        if is_joint_static_eta_source_local(S.static_eta_source_mode)
            S.eta_compare_label = sprintf('BoundedJointStaticEtaPreview_pm%03d', ...
                round(1000 * S.static_eta_halfwidth_mm));
            S.active_eta_model = 'bounded_static_eta';
            S.static_eta_source = 'joint_dynamic_residual_eta_preview';
        else
            S.eta_compare_label = sprintf('BoundedStep04XcEta_pm%03d', ...
                round(1000 * S.static_eta_halfwidth_mm));
            S.active_eta_model = 'bounded_step04_xc_eta';
        end
        S.eta_models = {S.active_eta_model};
        S.sensor_eta_mode = 'bounded_static_prior';
        S.fit_sensor_eta = true;

    otherwise
        error('Unsupported STEP05_STATIC_ETA_MODE: %s', static_eta_mode_env);
end

S.output_dir = fullfile(cfg.output_root, 'step05_static_eta_compare', ...
    S.eta_compare_label);
S.figure_dir = fullfile(cfg.figure_root, 'step05_static_eta_compare', ...
    S.eta_compare_label);
...
if ~isempty(run_tag_env)
    ...
    S.output_dir = fullfile(cfg.output_root, ...
        'step05_static_eta_compare', [S.eta_compare_label, '_', run_tag_env]);
...
S.static_case_eta_diagnostic_enable = true;
...
if ~isempty(eta_model_env)
    error(['STEP05_ETA_MODEL is reserved for the formal/eta-model Step05 routes. ' ...
        'Use STEP05_STATIC_ETA_MODE=fixed or bounded for this static-eta comparison.']);
end
...
fprintf('\n=== Step05 static-eta comparison: direct-template identification ===\n');
fprintf('Dataset: %s\n', cfg.dataset);
...
fprintf('Model: V = T_{s,b}(x - dx_c - A*sin(EO*theta + phi) [- eta_s]).\n');
...
[S.sensor_eta_prior_mm, S.static_eta_center_info] = ...
    resolve_static_eta_prior_local(Template, S, cfg);
...
fprintf('Static eta prior (sensors %s, relative to CH%d): %s mm\n', ...
    ...
    mat2str(S.sensor_eta_prior_mm, 6));
if strcmpi(S.sensor_eta_mode, 'bounded_static_prior')
    fprintf('Bounded static eta half-width: %.4f mm\n', ...
        S.static_eta_halfwidth_mm);
end
...
method.forward_model = 'V = T_{s,b}(x - dx_c - A*sin(EO*theta + phi) [- eta_s])';
...
method.eta_compare_label = getfield_default_local(S, 'eta_compare_label', '');
method.sensor_eta_mode = getfield_default_local(S, 'sensor_eta_mode', '');
...
method.static_eta_halfwidth_mm = getfield_default_local(S, 'static_eta_halfwidth_mm', NaN);
method.eta_gauge = sprintf('eta_s fixed to zero for CH%d; other channels use fixed or bounded Step04-derived static eta', ...
...
switch lower(strtrim(eta_model))
    ...
    case {'fixed_step04_template_center_eta', ...
            'fixed_step04_xc_eta', ...
            'fixed_step04_xc_static_eta', ...
            'fixed_static_eta'}
        S_model.fit_sensor_eta = false;
        S_model.sensor_eta_mode = 'fixed_static';

    case {'bounded_step04_template_center_eta', ...
            'bounded_step04_xc_eta', ...
            'bounded_step04_xc_static_eta', ...
            'bounded_static_eta'}
        S_model.fit_sensor_eta = true;
        S_model.sensor_eta_mode = 'bounded_static_prior';
...
function [eta, info] = resolve_static_eta_prior_local(Template, S, cfg)
if isfield(S, 'static_eta_source_mode') && ...
        is_joint_static_eta_source_local(S.static_eta_source_mode)
    [eta, info] = load_joint_static_eta_preview_local(S, cfg);
else
    [eta, info] = compute_step04_static_eta_local(Template, S);
end
end


function [eta, info] = load_joint_static_eta_preview_local(S, cfg)
sensors = S.analysis_sensors(:).';
...
function [eta, info] = compute_step04_static_eta_local(Template, S)
sensors = S.analysis_sensors(:).';

if numel(S.target_blades) ~= 1
    error('Static eta comparison expects one target blade; got %s.', ...
        mat2str(S.target_blades));
end

blade_id = S.target_blades(1);
ref_idx = find(sensors == S.reference_sensor_id, 1, 'first');
if isempty(ref_idx) || ref_idx ~= 1
    error('Static eta gauge expects reference sensor CH%d as first analysis sensor.', ...
        S.reference_sensor_id);
end

centers = nan(1, numel(sensors));
source = strings(1, numel(sensors));

for i = 1:numel(sensors)
    sid = sensors(i);
    tpl = get_template_entry_local(Template, sid, blade_id);
    if isempty(tpl)
        error('Cannot compute static eta: missing Step04 template for CH%d B%d.', ...
            sid, blade_id);
    end

    [centers(i), source(i)] = estimate_step04_template_center_local(tpl);
    if ~isfinite(centers(i))
        error('Cannot compute static eta: invalid Step04 center for CH%d B%d.', ...
            sid, blade_id);
    end
end

eta = centers(ref_idx) - centers;
eta(ref_idx) = 0;

info = table(sensors(:), centers(:), source(:), eta(:), ...
    'VariableNames', {'sensor_id', 'template_center_mm', ...
    'center_source', 'eta_prior_mm'});
end


function [xc, source] = estimate_step04_template_center_local(tpl)
...
xc = NaN;
source = "template_top85_centroid";
...
if isfield(tpl, 'v_grid_baseline_removed') && ...
    ...
    v = tpl.v_grid_baseline_removed(:);
elseif isfield(tpl, 'v_grid') && numel(tpl.v_grid) == numel(x)
    v = tpl.v_grid(:);
    if isfield(tpl, 'baseline') && isscalar(tpl.baseline) && isfinite(tpl.baseline)
        v = v - tpl.baseline;
    else
...
elseif isfield(tpl, 'valid_grid_mask') && ...
        numel(tpl.valid_grid_mask) == numel(x)
    mask = mask & tpl.valid_grid_mask(:);
end
...
if den <= eps
    ...
    xc = sum(x(high) .* w(high), 'omitnan') ./ den;
end
end
...
if isfield(S, 'sensor_eta_mode') && strcmpi(S.sensor_eta_mode, 'bounded_static_prior')
    prior = resolve_sensor_eta_prior_local(S);
    halfwidth = getfield_default_local(S, 'static_eta_halfwidth_mm', 0.02);
    halfwidth = max(abs(halfwidth), 0);
    eta = max(min(eta, prior + halfwidth), prior - halfwidth);
    eta(1) = 0;
    return;
end
...
if isfield(S, 'fit_sensor_eta') && S.fit_sensor_eta && numel(p) > 3
    if isfield(S, 'sensor_eta_mode') && strcmpi(S.sensor_eta_mode, 'bounded_static_prior')
        prior = resolve_sensor_eta_prior_local(S);
        halfwidth = max(abs(getfield_default_local(S, ...
            'static_eta_halfwidth_mm', 0.02)), 0);
        raw_eta = p(4:end);
        prior_eta = prior(2:end);
        penalty = penalty + 100 * sum(max(0, raw_eta - (prior_eta + halfwidth)).^2);
        penalty = penalty + 100 * sum(max(0, (prior_eta - halfwidth) - raw_eta).^2);
    else
...
for iw = 1:numel(WindowResult)
    ...
    target_eta_model = getfield_default_local(S, 'active_eta_model', 'window_bounded');
    idx = find(strcmpi({model_results.eta_model}, target_eta_model), 1, 'first');
    if isempty(idx)
...
if isempty(eta_rows)
    ...
    Summary.note = 'No valid eta results were available for the active/static eta model.';
    return;
end
