%% Step06B_Validate_Step05_With_Strain_TARC_20250527.m
...
target_blades = 1;
analysis_sensors = cfg.step3_default_btt_sensor_ids;
...
step06b_output_root = fullfile(cfg.output_root, 'step06b_step05_strain_tarc_validation');
step06b_figure_root = fullfile(cfg.figure_root, 'step06b_step05_strain_tarc_validation');
...
ValidationSetting.strain_peak_search_hz = [520 640];
...
ValidationSetting.channel_file_patterns = {'AI1-*.mat'};
...
for iCase = 1:numel(target_cases)
    ...
    for ib = 1:numel(target_blades)
        ...
        outMat = fullfile(outDir, sprintf( ...
            'Step06B_Validate_Step05_With_Strain_TARC_B%d_%s_20250527.mat', ...
            bladeId, sensor_tag));
        ...
        writetable(WindowStrainStatsTable, fullfile(outDir, sprintf( ...
            'Step06B_WindowStrainStats_B%d_%s_20250527.csv', bladeId, sensor_tag)));

        writetable(TARC_Table, fullfile(outDir, sprintf( ...
            'Step06B_TARC_Table_B%d_%s_20250527.csv', bladeId, sensor_tag)));

        writetable(ResponseAmpComparisonTable, fullfile(outDir, sprintf( ...
            'Step06B_ResponseAmp_Comparison_B%d_%s_20250527.csv', bladeId, sensor_tag)));

        writetable(ChannelSensitivityTable, fullfile(outDir, sprintf( ...
            'Step06B_ChannelSensitivity_B%d_%s_20250527.csv', bladeId, sensor_tag)));

        writetable(ValidationSummary, fullfile(outDir, sprintf( ...
            'Step06B_ValidationSummary_B%d_%s_20250527.csv', bladeId, sensor_tag)));
...
if ~isfile(evidenceFile)
    dd = dir(fullfile(step06aRoot, '**', 'Step06A_StrainRPM_ResonanceEvidence_20250527.mat'));
...
patterns = { ...
    ...
    sprintf('*B%d*%s*20250527*.mat', bladeId, sensorTag), ...
    '*Step05*.mat'};
    ...
    for ip = 1:numel(patterns)
        ...
        for k = 1:numel(dd)
            ...
            if ~contains(f, caseName) && ~contains(dd(k).folder, caseName)
                % Do not reject immediately, because some Step05 outputs may not
                % include case name in file path. Keep only if pattern is specific.
                if strcmp(patterns{ip}, '*Step05*.mat')
                    continue;
                end
            end
...
[~, order] = sort(T.phase_opt_TARC, 'descend');
T = T(order, :);

tl = tiledlayout(fig, 2, 1, 'TileSpacing', 'compact', 'Padding', 'compact');

ax1 = nexttile(tl);
bar(ax1, categorical(T.strain_channel), T.phase_opt_TARC);
grid(ax1, 'on');
...
if ismember('strain_sync_amp_eq_mm', T.Properties.VariableNames)
    bar(ax2, categorical(T.strain_channel), T.strain_sync_amp_eq_mm);
    ylabel(ax2, 'Sync equivalent displacement (mm)');
else
    bar(ax2, categorical(T.strain_channel), T.strain_sync_amp_microstrain);
    ylabel(ax2, 'Sync strain amp. (\mue)');
end
