%% Step03_Baseline_AllEOFullWaveform_20250527
% Baseline low-speed-template-only identification using the same single-sync
% logic as the super-Gaussian Step5 route.
%
...
cfg.debug_max_windows = parse_positive_integer_env_local('STEP03_01_MAX_WINDOWS', inf);
...
method = struct();
method.name = 'template_only_baseline_all_eo_full_waveform';
...
result_file = fullfile(result_dir, sprintf('Result_Step03_Baseline_AllEOFullWaveform_B%d_%s%s_20250527.mat', ...
...
fprintf('\n=== Step03 baseline: all-EO full waveform optimization ===\n');
...
for w_idx = 1:num_windows
    ...
    fprintf('Window %02d/%02d laps %s: EO=%d, f=%.3f Hz, A=%.4f mm, RMSE=%.5f V, clamp=%.2f%%, candidates=%d\n', ...
...
for is = 1:numel(analysis_sensors)
    ...
    w_query = build_query_guard_soft_weight_local( ...
        x_sel, x_domain, method.query_guard_mm, method.domain_soft_margin_mm, method.weight_floor);
