function report = Plot_B1_StaticDynamicDecomposition(resultFile,windowIndex,outputDir,bundleFile)
...
S=load(resultFile); R=pickResult(S);
if isfield(R,'WindowResult')
    W=R.WindowResult; assert(windowIndex<=numel(W),'Window index out of range.'); w=W(windowIndex); assert(isfield(w,'bundle'),'Result window has no bundle.'); b=w.bundle;
else
    assert(nargin>=4 && ~isempty(bundleFile),'A bundle file is required for row-only results.'); Q=load(bundleFile); Q=pickResult(Q); W=Q.WindowResult; assert(windowIndex<=numel(W),'Bundle window index out of range.'); w=R.rows(windowIndex); wb=W(windowIndex); b=wb.CoreBundlePreview;
end
if ~isfield(b,'sensorIds')
    ids0=unique(double(b.sensor_id(:)),'stable'); b.sensorIds=ids0(:).';
    b.X=double(b.x(:)); b.T=double(b.t(:)); b.V=1000*double(b.v(:));
    [~,b.sensorIndex]=ismember(double(b.sensor_id(:)),ids0);
    b.F0=1000*double(b.template_v(:));
end
...
if isempty(fixed), fixed=vecField(f,'VPredNoGap',numel(y)); end
...
function f=pickFit(w), f=struct; if isfield(w,'modelFits') && isfield(w.modelFits,'gap_only') && isstruct(w.modelFits.gap_only), f=w.modelFits.gap_only; elseif isfield(w,'VPred'), f=w; end, end
