...
%
% Paper-facing main output uses gap_tilt. fixed and gap_only are retained
% only as comparison/ablation methods. The old gap_tilt_shift branch remains
% in the file for legacy checks but is off by default.
%
...
if isempty(staticEtaOverride)
    [cfg.fixedSensorEtaMm, cfg.fixedSensorEtaInfo] = ...
        Step07J_FixedJointEtaPrior_20251222(cfg.targetBlade, cfg.analysisSensors);
else
...
if strcmpi(cfg.localBundleSource, 'foundation_step05_bundle')
    if isempty(FoundationStep05Source)
        error(['STEP07J_LOCAL_BUNDLE_SOURCE=foundation_step05_bundle requires a Step05 foundation result. ' ...
            'Run Build_Step07J_FoundationFullBundle_20251222 first or set STEP07J_FOUNDATION_STEP05_RESULT_FILE.']);
    end
...
for iw = 1:numWindows
    ...
    for im = 1:numel(fitModelNames)
        ...
        modelFits.(modeName) = refine_static_warp_fit_local( ...
            bundle, seedTable, fitEO, cfgWindow, modeName, modelFits.fixed);
    end
...
Result.description = ['Main result uses gap_tilt as a nested update of a folder-local foundation_core no-gap kernel. ' ...
    'Step07J builds the Step03-style direct-template point set locally from Template and DynamicMap; no rotating_calibration Step03 result, EO trend, or stored bundle is loaded as prior information. ' ...
    'The no-gap kernel uses fixed joint static eta, gradient-displacement VP seeds, and full-waveform RMSE refinement/ranking. ' ...
    'gap_only and gap_tilt start from the fixed solution, lock to the fixed EO by default, and add only physically bounded gap/tilt increments inside the same full waveform objective. ' ...
    'The legacy gap_tilt_shift branch is retained only when STEP07J_INCLUDE_GAP_TILT_SHIFT=1.'];
Result.cfg = cfg;
...
templateDir = fullfile(rootDir, '20251222_btt_data_foundation', ...
...
resultFile = '';
sensorTag = ['S', sprintf('%d', sensorIds)];
foundationOutputRoot = fullfile(rootDir, '20251222_btt_data_foundation', 'output');
patterns = {
    fullfile(foundationOutputRoot, 'step05_single_sync_direct_template_fullb_T50p2_L20', '*', sprintf( ...
        'Result_Step05_FixedEtaGradDispVP_B%d_%s_20251222.mat', targetBlade, sensorTag))
    ...
    fullfile(foundationOutputRoot, 'step05_single_sync_direct_template*', '*', sprintf( ...
        'Result_Step05_FixedEtaGradDispVP_B%d_%s_20251222.mat', targetBlade, sensorTag))
    fullfile(foundationOutputRoot, 'step05_single_sync_direct_template*', '*', sprintf( ...
        'Result_Step05_FoundationMainPulseAdaptiveFixedJointEta_B%d_%s_20251222.mat', targetBlade, sensorTag))
    fullfile(foundationOutputRoot, 'step05_single_sync_direct_template*', '*', sprintf( ...
        'Result_Step05_SingleSyncDirectTemplate_B%d_%s_20251222.mat', targetBlade, sensorTag))
    };
for ip = 1:numel(patterns)
...
for is = 1:numel(cfg.analysisSensors)
    ...
    mask = bundle.sensorIndex == is;
    eta = 0;
    if isfield(fit, 'sensorEtaMm') && numel(fit.sensorEtaMm) >= is
        eta = fit.sensorEtaMm(is);
    end
