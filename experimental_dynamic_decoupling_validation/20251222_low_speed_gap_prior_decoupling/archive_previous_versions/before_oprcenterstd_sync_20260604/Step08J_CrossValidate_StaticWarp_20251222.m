...
cfg.maxTrainPointsPerFold = parse_positive_integer_env_local('STEP08J_MAX_TRAIN_POINTS', 700);
cfg.maxValidPointsPerFold = parse_positive_integer_env_local('STEP08J_MAX_VALID_POINTS', 2500);
...
if ~isempty(step07jFileOverride)
    ...
    step07jFile = fullfile(outDir, sprintf('Step07J_NestedStaticWarp_VPFullWave_20251222_B%d_%s.mat', ...
        cfg.targetBlade, [sensorTag step07jSuffix]));
...
outputTag = [sensorTag outputSuffix];
matFile = fullfile(outDir, sprintf('Step08J_CrossValidate_StaticWarp_20251222_B%d_%s.mat', ...
    cfg.targetBlade, outputTag));
cvCsv = fullfile(outDir, sprintf('Step08J_CrossValidate_StaticWarp_CV_20251222_B%d_%s.csv', ...
    cfg.targetBlade, outputTag));
paramCsv = fullfile(outDir, sprintf('Step08J_CrossValidate_StaticWarp_Params_20251222_B%d_%s.csv', ...
    cfg.targetBlade, outputTag));
windowCsv = fullfile(outDir, sprintf('Step08J_CrossValidate_StaticWarp_WindowSummary_20251222_B%d_%s.csv', ...
    cfg.targetBlade, outputTag));
summaryCsv = fullfile(outDir, sprintf('Step08J_CrossValidate_StaticWarp_Summary_20251222_B%d_%s.csv', ...
    cfg.targetBlade, outputTag));
decisionCsv = fullfile(outDir, sprintf('Step08J_CrossValidate_StaticWarp_Decision_20251222_B%d_%s.csv', ...
    cfg.targetBlade, outputTag));
...
plot_cv_rmse_local(CV, Summary, fullfile(figDir, sprintf('Step08J_CV_RMSE_20251222_B%d_%s.png', ...
    cfg.targetBlade, outputTag)));
plot_validation_trend_local(WindowSummary, fullfile(figDir, sprintf('Step08J_Validation_Trend_20251222_B%d_%s.png', ...
    cfg.targetBlade, outputTag)));
plot_dmu_stability_local(SensorParams, cfg, fullfile(figDir, sprintf('Step08J_Dmu_Stability_20251222_B%d_%s.png', ...
    cfg.targetBlade, outputTag)));
