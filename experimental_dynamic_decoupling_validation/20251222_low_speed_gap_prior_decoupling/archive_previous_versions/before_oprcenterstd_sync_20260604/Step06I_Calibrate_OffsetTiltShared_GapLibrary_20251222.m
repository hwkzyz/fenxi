...
matFile = fullfile(outDir, sprintf('Step06I_OffsetTiltShared_GapLibrary_20251222_B%d_%s.mat', ...
    cfg.targetBlade, outputTag));
csvFile = fullfile(outDir, sprintf('Step06I_OffsetTiltShared_GapLibrary_Calibration_20251222_B%d_%s.csv', ...
    cfg.targetBlade, outputTag));
highCsvFile = fullfile(outDir, sprintf('Step06I_OffsetTiltShared_GapLibrary_HighStaticCheck_20251222_B%d_%s.csv', ...
    cfg.targetBlade, outputTag));
figLow = fullfile(figDir, sprintf('Step06I_LowTemplate_OffsetTiltShared_Fit_20251222_B%d_%s.png', ...
    cfg.targetBlade, outputTag));
figPath = fullfile(figDir, sprintf('Step06I_TiltPath_In_OffsetTiltShared_Library_20251222_B%d_%s.png', ...
    cfg.targetBlade, outputTag));
figHigh = fullfile(figDir, sprintf('Step06I_HighStatic_OffsetTiltShared_Check_20251222_B%d_%s.png', ...
    cfg.targetBlade, outputTag));
...
for i = 1:numel(files)
    token = regexp(files(i).name, '_S([0-9]+)(?:_GradientXRange030(?:_OPRAnchored)?)?_20251222\.mat$', 'tokens', 'once');
    if isempty(token)
        continue;
    end
