function Step09_Compare_Step07J_Methods_20251222
%% Compare Step07J methods, 20251222
% This is the separate comparison entry. The main Step07J program computes
% only gap_tilt by default; this script uses STEP07J_RUN_MODE=comparison to
...
if ~isfile(resultFile)
    ...
    evalin('base', 'Step07J_NestedStaticWarp_VPFullWave_20251222');
    delete(cleanupEnv);
...
if ~is_step07j_comparison_result_local(Result, Trend)
    error(['This is not a usable comparison result. Expected fixed/gap_only/gap_tilt columns, ' ...
        'or Result.cfg.runMode="comparison". Delete/regenerate or set STEP07J_COMPARE_RESULT_FILE:\n  %s'], ...
        resultFile);
end
...
if makeWaveformFigures
    ...
    restore_env_local('STEP07J_METHOD_RESULT_FILE', previousMethodResultFile);
end
...
function tf = is_step07j_comparison_result_local(Result, Trend)
tf = false;
if isfield(Result, 'cfg') && isfield(Result.cfg, 'runMode') && strcmpi(Result.cfg.runMode, 'comparison')
    tf = true;
    return;
end
requiredTrendVars = {'fixed_EO','fixed_frequency_hz','fixed_rmse_mV', ...
    'gap_EO','gap_frequency_hz','gap_rmse_mV', ...
    'tilt_EO','tilt_frequency_hz','tilt_rmse_mV'};
if istable(Trend) && all(ismember(requiredTrendVars, Trend.Properties.VariableNames))
    tf = true;
    return;
end
if isfield(Result, 'WindowResult') && ~isempty(Result.WindowResult) && ...
        isfield(Result.WindowResult(1), 'modelFits')
    fitNames = fieldnames(Result.WindowResult(1).modelFits);
    tf = all(ismember({'fixed','gap_only','gap_tilt'}, fitNames));
end
end
...
restore_env_local('STEP07J_RUN_MODE', previousRunMode);
restore_env_local('STEP07J_RESULT_SUFFIX', previousSuffix);
end

function restore_env_local(name, previousValue)
