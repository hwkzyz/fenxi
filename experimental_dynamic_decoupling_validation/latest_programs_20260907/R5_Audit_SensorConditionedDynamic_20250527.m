...
n=numel(R); ids=double(O.sensorIds(:).'); assert(isequal(ids,double(cfg.case.analysisSensors(:).')),'R5:AuditSensorRole');
dg=reshape([R.delta_gap_mm],numel(ids),n).'; eo=[R.EO].'; Aamp=[R.amplitude_mm].'; dx=[R.dx_mm].'; rmse=[R.rmse_mv].';
...
A=struct('schema','R5_SENSOR_CONDITIONED_DYNAMIC_AUDIT_V1','rows',sensorRows,...
 'sensorIds',ids,'gapSensorIds',double(cfg.case.gapSensors(:).'),...
