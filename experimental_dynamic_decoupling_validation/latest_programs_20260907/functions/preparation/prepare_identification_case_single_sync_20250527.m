function experiment_case = prepare_identification_case_single_sync_20250527(cfg, show_plots, save_output)
%PREPARE_IDENTIFICATION_CASE_SINGLE_SYNC_20250527
...
if ~isfile(fullfile(cfg.case_output_dir, sprintf('jilublade_probe%d.mat', cfg.analysis_sensors(1))))
    Step2_Extract_JiluBlade_20250527(cfg.dynamic_case_name, false);
end
...
slot_cfg = load_formal_slot_calibration_local();
...
for i = 1:numel(cfg.analysis_sensors)
    ...
    laps = struct('lap_id', {}, 'row_id', {}, 'x_points', {}, ...
        'theta_points', {}, 't_points', {}, 'v_points', {}, ...
        'peak_val', {}, 'peak_time', {});
    ...
    for k = 1:numel(blade_rows)
        ...
        [theta_points, theta_rot] = map_adjacent_slot_times_local( ...
            opr_times, t_seg, slot_cfg, cfg.opr_pulses_per_rev);
        ...
        laps(end).theta_points = theta_rot(:);
...
experiment_case.Diagnostics.angle_mapping = ...
    'adjacent_opr_unequal_physical_slot';
experiment_case.Diagnostics.slot_angle_deg = slot_cfg.slotAngleDeg;
experiment_case.Diagnostics.slot_index_offset = slot_cfg.slotOffset;
experiment_case.Diagnostics.speed_diagnostics = compute_opr_speed_diagnostics_20250527( ...
...
function cfg = load_formal_slot_calibration_local()
...
cfg = struct('slotAngleDeg', double(C.intervalAngleDeg(:).'), ...
    'slotOffset', double(C.slotIndexOffsetHighToLow), 'file', file);
assert(numel(cfg.slotAngleDeg) == 6 && ...
    abs(sum(cfg.slotAngleDeg) - 360) < 1e-9, ...
    'Invalid formal OPR6 slot calibration.');
end
...
selection = repmat(struct( ...
    ...
    'window_end', NaN, ...
    'selection_mode', '', ...
    'requested_start_time', cfg.analysis_start_time, ...
    'available_time_start', NaN, ...
    'available_time_end', NaN), numel(cfg.analysis_sensors), 1);
...
for i = 1:numel(cfg.analysis_sensors)
    ...
    target_mask = jilublade(:, 4) == cfg.target_blade;
    target_rows = find(target_mask);
    target_times = jilublade(target_rows, 3);
    selection(i).available_time_start = min(target_times, [], 'omitnan');
    selection(i).available_time_end = max(target_times, [], 'omitnan');

    blade_mask = target_mask & jilublade(:, 3) >= cfg.analysis_start_time;
    blade_rows = find(blade_mask);
    if isempty(blade_rows)
        if isempty(target_rows)
            error('No target-blade rows exist for CH%d in %s.', sid, probe_path);
        end
        error(['No target-blade rows found for CH%d after %.3f s. ' ...
            'Available target rows span %.3f to %.3f s.'], ...
            sid, cfg.analysis_start_time, ...
            selection(i).available_time_start, selection(i).available_time_end);
    end
    selection(i).selection_mode = 'requested_window';
    if numel(blade_rows) < cfg.target_laps
        warning('CH%d only has %d target rows after %.3f s; requested %d.', ...
            sid, numel(blade_rows), cfg.analysis_start_time, cfg.target_laps);
    end
...
if isempty(file_ranges)
    error('No dynamic files overlap the requested identification window [%.6f, %.6f] s.', ...
