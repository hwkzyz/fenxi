...
if ~isfield(R,'cfg') || ~isfield(R.cfg,'gapActiveSensorMask') || ...
        ...
        any(~isfinite(double(R.cfg.gapActiveSensorMask(:))))
    reasons(end+1)="inactive_formal_gap_sensor"; %#ok<AGROW>
end
if isfield(R,'cfg') && isfield(R.cfg,'gapSensors') && ~isequal(logical(R.cfg.gapActiveSensorMask(:)).', ...
        ismember(double(R.cfg.analysisSensors(:).'), double(R.cfg.gapSensors(:).')))
    reasons(end+1)="gap_sensor_role_mismatch"; %#ok<AGROW>
end
