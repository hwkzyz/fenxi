...
load(familyFile,'F'); load(localizationFile,'T','latentMode');
if exist('latentMode','var') && strcmpi(latentMode,'sensor_conditioned')
    error('R5:SensorConditionedExportRequired', ...
        ['Localization is sensor-conditioned. Main10 sidecar export requires ', ...
        'a sensor-specific response-surface interface; do not collapse z_s,b into one z.']);
end
S=load(baseLibraryFile,'CorrectedGapLibrary');
