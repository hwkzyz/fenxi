function C = R5_CompareSharedIndependentLatent_20250527(localizationFile, outputFile)
%R5_COMPARESHAREDINDEPENDENTLATENT_20250527 Diagnostic only.
% Compare independent sensor-wise latent minima with the joint shared latent.
% No pass/fail threshold is imposed here; the result must be interpreted
% against low-speed repeatability/noise evidence.
cfg = Config_20250527();
...
required = {'blade_id','sensor_id','latent_coordinate','anchor_rmse_mv'};
assert(all(ismember(required, T.Properties.VariableNames)), ...
    'R5:LocalizationSchema', 'Localization table lacks required columns.');
