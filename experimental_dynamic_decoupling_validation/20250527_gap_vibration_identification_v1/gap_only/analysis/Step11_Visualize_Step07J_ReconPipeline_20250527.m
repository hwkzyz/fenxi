%% Visualize Step07J waveform reconstruction pipeline, 20250527
...
C = current_case_local(thisDir, explicitResultFile);
...
function C = current_case_local(thisDir, explicitResultFile)
C = CaseConfig();
C.bladeTag = C.caseTag;
C.caseDir = thisDir;
if ~isempty(explicitResultFile) && isfile(explicitResultFile)
    C.resultFile = explicitResultFile;
    return;
end
C.resultFile = resolve_step07j_result_local(fullfile(C.caseDir, 'outputs'), ...
    sprintf('Step07J_NestedStaticWarp_VPFullWave_%s_%s_main_gaponly.mat', C.dataset, C.caseTag), ...
    sprintf('Step07J_NestedStaticWarp_VPFullWave_%s_%s_method_compare.mat', C.dataset, C.caseTag), ...
    'Run Step00 first for _main_gaponly, or Step09 for _method_compare.');
end

function resultFile = resolve_step07j_result_local(outDir, primaryName, fallbackName, hint)
primaryFile = fullfile(outDir, primaryName);
fallbackFile = fullfile(outDir, fallbackName);
if isfile(primaryFile)
    resultFile = primaryFile;
elseif ~isempty(dir(fullfile(outDir, strrep(primaryName, '_main_gaponly.mat', '_*main_gaponly.mat'))))
    hits = dir(fullfile(outDir, strrep(primaryName, '_main_gaponly.mat', '_*main_gaponly.mat')));
    [~, latestIdx] = max([hits.datenum]);
    resultFile = fullfile(hits(latestIdx).folder, hits(latestIdx).name);
elseif isfile(fallbackFile)
    resultFile = fallbackFile;
else
    tokens = regexp(primaryName, 'Step07J_NestedStaticWarp_VPFullWave_(\d+)_B(\d+)_([A-Z0-9_]+)_(.+)\.mat$', 'tokens', 'once');
    if ~isempty(tokens)
        legacyPrimary = fullfile(outDir, sprintf('Step07J_NestedStaticWarp_VPFullWave_%s_B%s_%s_%s.mat', ...
            tokens{1}, tokens{2}, tokens{3}, tokens{4}));
        if isfile(legacyPrimary)
            resultFile = legacyPrimary;
            return;
        end
    end
    tokens = regexp(fallbackName, 'Step07J_NestedStaticWarp_VPFullWave_(\d+)_B(\d+)_([A-Z0-9_]+)_(.+)\.mat$', 'tokens', 'once');
    if ~isempty(tokens)
        legacyFallback = fullfile(outDir, sprintf('Step07J_NestedStaticWarp_VPFullWave_%s_B%s_%s_%s.mat', ...
            tokens{1}, tokens{2}, tokens{3}, tokens{4}));
        if isfile(legacyFallback)
            resultFile = legacyFallback;
            return;
        end
    end
    error('Missing current Step07J result. Expected:\n  %s\nor:\n  %s\n%s', ...
        primaryFile, fallbackFile, hint);
end
end
