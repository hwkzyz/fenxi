...
step07File = fullfile(routeDir, 'outputs', sprintf('Step07J_NestedStaticWarp_VPFullWave_%s_%s_main_gaponly.mat', C0.dataset, C0.caseTag));
if ~isfile(step07File)
    step07File = fullfile(routeDir, 'outputs', sprintf('Step07J_NestedStaticWarp_VPFullWave_%s_B%d_%s_main_gaponly.mat', C0.dataset, C0.bladeId, C0.sensorTag));
end
if ~isfile(step07File)
    hits = dir(fullfile(routeDir, 'outputs', sprintf( ...
        'Step07J_NestedStaticWarp_VPFullWave_%s_%s_*main_gaponly.mat', C0.dataset, C0.caseTag)));
    if ~isempty(hits)
        [~, latestIdx] = max([hits.datenum]);
        step07File = fullfile(hits(latestIdx).folder, hits(latestIdx).name);
    end
end
