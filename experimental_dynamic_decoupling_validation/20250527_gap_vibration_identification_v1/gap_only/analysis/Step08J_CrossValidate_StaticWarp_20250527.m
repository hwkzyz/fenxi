...
if ~isempty(step07jFileOverride)
    ...
    if ~isfile(step07jFile) && isempty(step07jSuffix)
        step07jFile = fullfile(outDir, sprintf('Step07J_NestedStaticWarp_VPFullWave_%s_%s_main_gaponly.mat', ...
            C0.dataset, C0.caseTag));
    end
    if ~isfile(step07jFile) && isempty(step07jSuffix)
        step07jFile = fullfile(outDir, sprintf('Step07J_NestedStaticWarp_VPFullWave_%s_B%d_%s_main_gaponly.mat', ...
            C0.dataset, cfg.targetBlade, sensorTag));
    end
    if ~isfile(step07jFile) && isempty(step07jSuffix)
        hits = dir(fullfile(outDir, sprintf( ...
            'Step07J_NestedStaticWarp_VPFullWave_%s_%s_*main_gaponly.mat', ...
            C0.dataset, C0.caseTag)));
        if ~isempty(hits)
            [~, latestIdx] = max([hits.datenum]);
            step07jFile = fullfile(hits(latestIdx).folder, hits(latestIdx).name);
        end
    end
