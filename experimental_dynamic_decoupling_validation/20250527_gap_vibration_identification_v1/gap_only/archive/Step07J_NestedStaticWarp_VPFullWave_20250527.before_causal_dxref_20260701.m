...
if ~ismember(cfg.dxReferenceMode, {'seed_best', 'seed_median'})
    error('STEP07J_DX_REFERENCE_MODE must be "seed_best" or "seed_median".');
end
...
for iw = 1:numWindows
    ...
    for im = 1:numel(cfg.modelNames)
        ...
        modelFits.(modeName) = refine_static_warp_fit_local(bundle, seedTable, fitEO, cfgFit, modeName);
    end
...
Result.description = ['Current Step07J route builds the Step03-style direct-template point set locally from Template and DynamicMap, then converts those selected points to the gap-prior response model. No rotating_calibration Step03 result, EO trend, or stored bundle is loaded as prior information. ' ...
    'The unified paper-facing gap-aware model is gap_tilt: VP Top-K plus optional previous-window continuity provide only the EO candidate set, while gap_tilt jointly identifies EO, vibration, gap increment and a small x-dependent tilt increment with the full waveform objective. ' ...
