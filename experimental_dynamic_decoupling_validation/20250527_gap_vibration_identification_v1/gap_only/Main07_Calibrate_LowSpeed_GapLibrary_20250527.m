%% Step06I: Calibrate low-speed template with offset-tilt shared gap library
% The Step05I gap library F_raw(g,x) is generated by using all static blades
% after applying waveform-inverted blade clearance offsets and learned
% blade-tilt corrections. In the
...
% template:
%
%   T_low,s^c(x) ~= a_s * F_raw^c(g0_s + mu_s*(x-tau_s), k_s*(x-tau_s))
%
...
figDir = fullfile(outDir, 'figures_step06i_offset_tilt_shared_gap_library');
...
cfg.lowEffectiveWindowThreshold = 0.12;
cfg.highCheckMaxPointsPerSensor = 320;
cfg.highCheckGapGridCount = 61;
cfg.highCheckDxMode = 'adaptive_sensor';   % fixed | adaptive_sensor
cfg.highCheckDxFixedGridMm = -0.08:0.01:0.08;
cfg.highCheckDxStepMm = 0.01;
cfg.highCheckDxRangeFraction = 0.10;       % relative to each sensor's robust x span
cfg.highCheckDxMinHalfRangeMm = 0.08;
cfg.highCheckDxMaxHalfRangeMm = 0.60;
cfg.lowFitMode = 'slope_only';
cfg.domainMarginMm = 0;
cfg.domainToleranceMm = 1e-8;
cfg.fminOptions = optimoptions('fmincon', 'Display', 'off', 'Algorithm', 'sqp', ...
    'MaxIterations', 500, 'MaxFunctionEvaluations', 4000, ...
    'StepTolerance', 1e-8, 'OptimalityTolerance', 1e-8, ...
    'ConstraintTolerance', cfg.domainToleranceMm);

sensorOverride = strtrim(getenv('STEP06I_ANALYSIS_SENSORS'));
if ~isempty(sensorOverride)
    parsedSensors = sscanf(sensorOverride, '%d').';
    if isempty(parsedSensors)
        error('STEP06I_ANALYSIS_SENSORS must contain integer sensor IDs, for example "1 3 6".');
    end
    cfg.analysisSensors = parsedSensors;
end
sensorTag = ['S', sprintf('%d', cfg.analysisSensors)];
resultSuffix = sanitize_suffix_local(strtrim(getenv('STEP06I_RESULT_SUFFIX')));

responseFile = fullfile(outDir, 'Step05I_OffsetTilt_Shared_Response_Surface_20250527.mat');
highMapFile = fullfile(outDir, sprintf('Step06_HighMap_%s_%s.mat', C0.dataset, C0.caseTag));
if ~isfile(highMapFile)
    highMapFile = fullfile(outDir, sprintf('Step06_HighMap_%s_B%d_%s.mat', ...
        C0.dataset, cfg.targetBlade, sensorTag));
end
templateOverride = strtrim(getenv('STEP06I_TEMPLATE_FILE'));
if ~isempty(templateOverride)
    ...
    templateSourceMode = 'bundled_foundation_opr_center_template';
end

if ~isfile(responseFile)
    error('Run Step05 and Step05I first. Missing file: %s', responseFile);
end
...
Template = normalize_template_for_target_blade_local(Template, cfg.targetBlade, cfg.analysisSensors);
...
fprintf('\n=== Step06I: offset-tilt shared gap-library calibration ===\n');
fprintf('Response surface: %s\n', responseFile);
fprintf('Low-speed template: %s [%s]\n', templateFile, templateSourceMode);
fprintf('Sensors: %s | low-speed fit: %s\n', mat2str(cfg.analysisSensors), cfg.lowFitMode);
...
sensorCorr = repmat(struct('sensorId', NaN, 'g0Mm', NaN, 'tauMm', NaN, ...
    ...
    'domainFeasible', false, 'gDomainMarginMm', NaN, 'xDomainMarginMm', NaN, ...
    'activeBoundary', false, 'pointCount', NaN, 'x', [], 'vLowMv', [], ...
    'vFitMv', [], 'xLib', [], 'gEff', [], 'residualMv', [], ...
    'etaMedianMm', NaN, 'etaIqrMm', NaN, 'etaLimitMm', NaN), numel(cfg.analysisSensors), 1);
...
for is = 1:numel(cfg.analysisSensors)
    ...
    rows{is} = table(sid, fit.g0Mm, fit.tauMm, fit.xScale, fit.muGapPerXMm, ...
        ...
        fit.domainFeasible, fit.gDomainMarginMm, fit.xDomainMarginMm, ...
        fit.activeBoundary, fit.pointCount, fit.etaMedianMm, fit.etaIqrMm, fit.etaLimitMm, ...
        'VariableNames', {'sensorId','g0Mm','tauMm','xScale','muGapPerXMm', ...
        'tiltAngleDeg','voltageGain','voltageOffsetMv','lowFitRmseMv', ...
        'domainFeasible','gDomainMarginMm','xDomainMarginMm','activeBoundary', ...
        'pointCount','etaMedianMm','etaIqrMm','etaLimitMm'});
    ...
    fprintf('       feasible %d, g margin %.6f mm, x margin %.6f mm, active boundary %d\n', ...
        fit.domainFeasible, fit.gDomainMarginMm, fit.xDomainMarginMm, fit.activeBoundary);
    ...
    if hasHighMap
        ...
        highRows{end+1, 1} = table(sid, hfit.gMm, hfit.dxCorrectionMm, ...
            hfit.dxSearchMinMm, hfit.dxSearchMaxMm, hfit.dxAtBoundary, ...
            hfit.weightedRmseMv, hfit.pointCount, ...
            'VariableNames', {'sensorId','highStaticGMm','dxCorrectionMm', ...
            'dxSearchMinMm','dxSearchMaxMm','dxAtBoundary', ...
...
CorrectedGapLibrary.method = 'low_speed_template_offset_tilt_shared_incremental_gap_library';
...
outputTag = [C0.caseTag resultSuffix];
matFile = fullfile(outDir, sprintf('Step06I_OffsetTiltShared_GapLibrary_%s_%s.mat', ...
    C0.dataset, outputTag));
csvFile = fullfile(outDir, sprintf('Step06I_OffsetTiltShared_GapLibrary_Calibration_%s_%s.csv', ...
    C0.dataset, outputTag));
highCsvFile = fullfile(outDir, sprintf('Step06I_OffsetTiltShared_GapLibrary_HighStaticCheck_%s_%s.csv', ...
    C0.dataset, outputTag));
figLow = fullfile(figDir, sprintf('Step06I_LowTemplate_OffsetTiltShared_Fit_%s_%s.png', ...
    C0.dataset, outputTag));
figPath = fullfile(figDir, sprintf('Step06I_TiltPath_In_OffsetTiltShared_Library_%s_%s.png', ...
    C0.dataset, outputTag));
figHigh = fullfile(figDir, sprintf('Step06I_HighStatic_OffsetTiltShared_Check_%s_%s.png', ...
    C0.dataset, outputTag));
...
[~, calibIndexFile] = publish_gap_calibration_entries_local( ...
    CorrectedGapLibrary, calibrationTable, highCheckTable, outDir, C0, matFile, resultSuffix);
...
fprintf('Published blade-sensor calibration index:\n  %s\n', calibIndexFile);

%% Local functions
function [indexTable, indexFile] = publish_gap_calibration_entries_local( ...
        CorrectedGapLibrary, calibrationTable, highCheckTable, outDir, C0, sourceFile, resultSuffix)
...
for i = 1:numel(CorrectedGapLibrary.sensor)
    ...
    sensorSuffix = sprintf('%s_%s_B%d_S%d%s', ...
        filePrefix, C0.dataset, CorrectedGapLibrary.targetBlade, sid, suffix);
    sensorFile = fullfile(libDir, [sensorSuffix '.mat']);
...
indexFile = fullfile(libDir, sprintf('%s_Index_%s_%s%s.csv', ...
    filePrefix, C0.dataset, C0.caseTag, suffix));
...
caseTag = sprintf('B%d_%s', targetBlade, exactTag);
patterns = {
    ...
    sprintf('Template_LowSpeedRotating_%s_GradientXRange030_BaseFrame_20250527.mat', caseTag)
    sprintf('Template_LowSpeedRotating_B%d_%s_GradientXRange030_OPRCenterStd_20250527.mat', targetBlade, exactTag)
    sprintf('Template_LowSpeedRotating_B%d_%s_GradientXRange030_OPRAnchored_20250527.mat', targetBlade, exactTag)
    ...
    sprintf('Template_LowSpeedRotating_B%d_%s_GradientXRange030_BaseFrame_20250527.mat', targetBlade, exactTag)
    };
...
for i = 1:numel(files)
    token = regexp(files(i).name, '_S([0-9]+)(?:_GradientXRange030(?:_[A-Za-z0-9]+)?)?_20250527\.mat$', 'tokens', 'once');
    if isempty(token)
        continue;
    end
...
if ~isfield(Template, 'SensorBlade')
    error('Unsupported low-speed template: missing Sensor and SensorBlade.');
end
...
for sid = sensorIds(:).'
    ...
    if isempty(idx)
        error('Low-speed template does not contain B%d CH%d.', targetBlade, sid);
    end
...
[responseXMin, responseXMax] = response_x_domain_local(responseSurface);
baseMask = isfinite(x) & isfinite(vLow);
effectiveMask = false(size(baseMask));
thresholdMv = cfg.lowEffectiveWindowThreshold * max(vLow(baseMask));
hits = find(baseMask & vLow >= thresholdMv);
if isempty(hits)
    error('CH%d has no points in the fixed low-speed effective ROI.', Tsen.sensor_id);
end
effectiveMask(hits(1):hits(end)) = true;
mask = baseMask & effectiveMask & x >= responseXMin & x <= responseXMax;
x = x(mask);
vLow = vLow(mask);
...
lb = [cfg.gBoundsMm(1), cfg.tauBoundsMm(1), cfg.xScaleBounds(1), cfg.muBounds(1)];
ub = [cfg.gBoundsMm(2), cfg.tauBoundsMm(2), cfg.xScaleBounds(2), cfg.muBounds(2)];
nonlcon = @(p) tilt_path_domain_constraints_local(p, x, responseSurface, cfg);
for g0 = gStarts(:).'
    for tau = tauStarts
        for k = kStarts
            for mu = muStarts
...
if ~isfinite(best.objective)
    error('CH%d has no feasible low-speed calibration solution.', Tsen.sensor_id);
end
fit = struct('sensorId', Tsen.sensor_id, 'g0Mm', best.g0Mm, 'tauMm', best.tauMm, ...
    ...
    'lowFitRmseMv', best.lowFitRmseMv, 'domainFeasible', best.domainFeasible, ...
    'gDomainMarginMm', best.gDomainMarginMm, 'xDomainMarginMm', best.xDomainMarginMm, ...
    'activeBoundary', best.activeBoundary, ...
    'pointCount', best.pointCount, 'x', x, 'vLowMv', vLow, 'vFitMv', best.vFitMv, ...
    'xLib', best.xLib, 'gEff', best.gEff, 'residualMv', vLow - best.vFitMv, ...
    'etaMedianMm', get_eta_scalar_local(Tsen, 'eta_median_mm'), ...
    'etaIqrMm', get_eta_scalar_local(Tsen, 'eta_iqr_mm'), ...
    'etaLimitMm', get_eta_scalar_local(Tsen, 'eta_limit_mm'));
end
...
p = pRaw(:).';
[g0, tau, k, mu] = deal(p(1), p(2), p(3), p(4));
[modelRaw, xLib, gEff, overshoot] = eval_raw_tilt_path_local(responseSurface, g0, tau, k, mu, x);
[rmse, vFit, affine] = slope_rmse_local(vLow, modelRaw, ones(size(vLow)));
[gMargin, xMargin] = tilt_path_domain_margins_local(p, x, responseSurface, cfg);
domainFeasible = gMargin >= -cfg.domainToleranceMm && xMargin >= -cfg.domainToleranceMm;
obj = rmse;
detail = struct('g0Mm', g0, 'tauMm', tau, 'xScale', k, 'muGapPerXMm', mu, ...
    'voltageGain', affine.gain, 'voltageOffsetMv', affine.offsetMv, ...
    'lowFitRmseMv', rmse, 'domainFeasible', domainFeasible, ...
    'gDomainMarginMm', gMargin, 'xDomainMarginMm', xMargin, ...
...
function [c, ceq] = tilt_path_domain_constraints_local(p, x, responseSurface, cfg)
g0 = p(1); tau = p(2); k = p(3); mu = p(4);
xLibRaw = k .* (x(:) - tau);
gEffRaw = g0 + mu .* (x(:) - tau);
[xMin, xMax] = response_x_domain_local(responseSurface);
xMin = xMin + cfg.domainMarginMm;
xMax = xMax - cfg.domainMarginMm;
gMin = min(responseSurface.gTrainMm(:)) + cfg.domainMarginMm;
gMax = max(responseSurface.gTrainMm(:)) - cfg.domainMarginMm;
c = [xMin - xLibRaw; xLibRaw - xMax; gMin - gEffRaw; gEffRaw - gMax];
ceq = [];
end

function [gMargin, xMargin] = tilt_path_domain_margins_local(p, x, responseSurface, cfg)
g0 = p(1); tau = p(2); k = p(3); mu = p(4);
xLibRaw = k .* (x(:) - tau);
gEffRaw = g0 + mu .* (x(:) - tau);
[xMin, xMax] = response_x_domain_local(responseSurface);
...
gMin = min(responseSurface.gTrainMm(:)) + cfg.domainMarginMm;
gMax = max(responseSurface.gTrainMm(:)) - cfg.domainMarginMm;
xMargin = min([xLibRaw - xMin; xMax - xLibRaw]);
gMargin = min([gEffRaw - gMin; gMax - gEffRaw]);
end
...
function [xMin, xMax] = response_x_domain_local(responseSurface)
finiteRows = all(isfinite(responseSurface.coeff), 2);
if ~any(finiteRows)
    error('Response surface has no finite coefficient support.');
end
xFinite = responseSurface.xGrid(finiteRows);
xMin = min(xFinite(:));
xMax = max(xFinite(:));
end
...
value = NaN;
if isstruct(Tsen) && isfield(Tsen, fieldName)
    raw = Tsen.(fieldName);
    if isempty(raw)
        return;
    end
...
dxGrid = build_high_check_dx_grid_local(x, Tsen, cfg);
fit = struct('gMm', NaN, 'dxCorrectionMm', NaN, 'weightedRmseMv', inf, ...
    'pointCount', 0, 'pred', [], 'dxSearchMinMm', min(dxGrid), ...
    'dxSearchMaxMm', max(dxGrid), 'dxAtBoundary', false);
for g = gGrid(:).'
    for dx = dxGrid(:).'
        ...
        if rmse < fit.weightedRmseMv
            fit = struct('gMm', g, 'dxCorrectionMm', dx, ...
                ...
                'dxSearchMinMm', min(dxGrid), 'dxSearchMaxMm', max(dxGrid), ...
                'dxAtBoundary', false);
        end
    end
end
dxTol = max(cfg.highCheckDxStepMm, eps) * 0.5;
fit.dxAtBoundary = abs(fit.dxCorrectionMm - fit.dxSearchMinMm) <= dxTol || ...
    abs(fit.dxCorrectionMm - fit.dxSearchMaxMm) <= dxTol;
cache = struct('x', x, 'v', v, 'w', w, 'pred', fit.pred, 'dxGrid', dxGrid);
end

function dxGrid = build_high_check_dx_grid_local(x, Tsen, cfg)
if isfield(cfg, 'highCheckDxMode') && strcmpi(cfg.highCheckDxMode, 'fixed')
    dxGrid = cfg.highCheckDxFixedGridMm(:).';
    return;
end
...
xSpan = percentile_local(finiteX, 95) - percentile_local(finiteX, 5);
tplSpan = max(Tsen.x_grid(:)) - min(Tsen.x_grid(:));
spanForRange = max([xSpan, 0.5 * tplSpan, cfg.highCheckDxMinHalfRangeMm]);
halfRange = cfg.highCheckDxRangeFraction * spanForRange;
halfRange = min(max(halfRange, cfg.highCheckDxMinHalfRangeMm), cfg.highCheckDxMaxHalfRangeMm);
step = max(cfg.highCheckDxStepMm, eps);
dxGrid = -halfRange:step:halfRange;
if dxGrid(end) < halfRange
    dxGrid = [dxGrid, halfRange];
end
dxGrid = unique(round(dxGrid / step) * step);
end
...
function [rmse, pred, affine] = slope_rmse_local(v, model, w)
pred = nan(size(v));
valid = isfinite(v) & isfinite(model) & isfinite(w);
if nnz(valid) < 3
    rmse = inf; affine = struct('gain', NaN, 'offsetMv', 0); return;
end
sw = sqrt(max(w(valid), eps));
gain = (model(valid).*sw) \ (v(valid).*sw);
pred(valid) = gain .* model(valid);
res = v(valid) - pred(valid);
rmse = sqrt(sum(w(valid).*res.^2)/max(sum(w(valid)),eps));
affine = struct('gain', gain, 'offsetMv', 0);
end
...
fig = figure('Name', 'Step06I low-speed offset-tilt shared fit', 'Color', 'w', ...
...
fig = figure('Name', 'Step06I tilt path in offset-tilt shared library', 'Color', 'w', ...
...
fig = figure('Name', 'Step06I high-speed static check with offset-tilt shared library', ...
...
for i = 1:numel(highCache)
    ...
    title(sprintf('CH%d high-speed static check | g %.4f | dx %.4f mm | search [%.2f, %.2f]%s', ...
        highCache(i).sensorId, highCache(i).fit.gMm, highCache(i).fit.dxCorrectionMm, ...
        highCache(i).fit.dxSearchMinMm, highCache(i).fit.dxSearchMaxMm, ...
        ternary_local(highCache(i).fit.dxAtBoundary, ' boundary', '')));
    xlabel('x_{OPR} (mm)'); ylabel('mV'); legend('Location', 'best');
end
exportgraphics(fig, figFile, 'Resolution', 220);
end

function out = ternary_local(flag, trueText, falseText)
if flag
    out = trueText;
else
    out = falseText;
