...
S.version = 'FOUNDATION_MAINPULSE_ADAPTIVE_ZEROETA_V1';
angleMappingCfg = angle_mapping_config_local();
S.angle_map_mode = angleMappingCfg.mode;
S.slot_angle_file = angleMappingCfg.file;
S.slot_angle_deg = angleMappingCfg.slotAngleDeg;
S.slot_anchor_deg = angleMappingCfg.slotAnchorDeg;
S.slot_index_offset = angleMappingCfg.slotOffset;
S.slot_alignment_score = angleMappingCfg.alignmentScore;
S.slot_alignment_margin = angleMappingCfg.alignmentMargin;

S.show_plots = packageCfg.run.showPlots;
S.save_figures = cfg.save_figures;
S.target_cases = {packageCfg.case.dynamicCase};
S.target_blades = packageCfg.case.targetBlade;
S.analysis_sensors = packageCfg.case.analysisSensors;
S.reference_sensor_id = S.analysis_sensors(1);

S.analysis_start_time_s = packageCfg.case.analysisStartTimeSec;
S.target_laps = packageCfg.window.targetBladePasses;
S.window_laps = packageCfg.window.windowBladePasses;
S.sliding_step_laps = packageCfg.window.slidingStepBladePasses;
S.debug_max_windows = inf;

S.output_dir = packageCfg.paths.foundationResults;
S.figure_dir = fullfile(packageCfg.paths.figures, 'foundation_no_gap');
S.formal_result_file = packageCfg.files.formalFoundationResult;

run_tag_env = strtrim(getenv('STEP05_OUTPUT_TAG'));
S.run_tag = run_tag_env;
if ~isempty(run_tag_env)
    run_tag_env = regexprep(run_tag_env, '[^\w\-]', '_');
    S.run_tag = run_tag_env;
    S.output_dir = fullfile(packageCfg.paths.foundationResults, run_tag_env);
    S.figure_dir = fullfile(packageCfg.paths.figures, ...
...
S.freq_search_hz = packageCfg.frequency.searchHz;
S.continuous_frequency_refine = packageCfg.frequency.refineEachCandidate;
S.frequency_refine_half_width_hz = packageCfg.frequency.refineHalfWidthHz;
S.eo_pad = 2;

S.eo_candidate_policy = 'adaptive_full_fit';
S.vp_top_k_min = packageCfg.frequency.vpTopK;
S.vp_top_k_max = packageCfg.frequency.vpTopK;
...
if ~isempty(static_eta_file_env)
    error('Formal V1 enforces eta=0; STEP05_STATIC_ETA_FILE is not accepted.');
end
...
for ib = 1:numel(S.target_blades)
    ...
    if isempty(S.run_tag)
        result_file = S.formal_result_file;
    else
        result_file = fullfile(out_dir, sprintf('FixedGap_B%d_%s_%s.mat', ...
            blade_id, sensor_tag, S.run_tag));
    end

    trend_file = fullfile(out_dir, sprintf('FixedGap_Trend_B%d_%s.csv', ...
        blade_id, sensor_tag));
...
angleCfg = angle_mapping_config_local();
if strcmp(angleCfg.mode, 'original_full_revolution')
    ...
    [theta_actual_deg, theta_rad] = map_adjacent_slot_times_local( ...
        opr_times, t, angleCfg, opr_events_per_revolution);
end
...
function cfg = angle_mapping_config_local()
persistent cached key
modeName = lower(strtrim(getenv('OPR_ANGLE_MODE')));
if isempty(modeName)
    modeName = 'original_full_revolution';
end
file = strtrim(getenv('OPR_SLOT_CAL_FILE'));
newKey = [modeName, '|', file];
if ~isempty(cached) && strcmp(key, newKey)
    cfg = cached;
    return;
end
cfg = struct('mode', modeName, 'file', file, 'slotAngleDeg', 60 * ones(1, 6), ...
    'slotAnchorDeg', 0:60:300, 'slotOffset', 0, ...
    'alignmentScore', NaN, 'alignmentMargin', NaN);
if ~strcmp(modeName, 'original_full_revolution')
    assert(isfile(file), 'Diagnostic slot calibration file is missing: %s', file);
    S = load(file, 'SlotCalibration');
    C = S.SlotCalibration;
    cfg.slotAngleDeg = double(C.intervalAngleDeg(:).');
    cfg.slotAnchorDeg = double(C.anchorAngleDeg(:).');
    cfg.slotOffset = double(C.slotIndexOffsetHighToLow);
    cfg.alignmentScore = double(C.alignmentScoreDeg);
    cfg.alignmentMargin = double(C.alignmentMarginDeg);
    if strcmp(modeName, 'adjacent_uniform_60')
        cfg.slotAngleDeg = 60 * ones(1, 6);
        cfg.slotAnchorDeg = 0:60:300;
    elseif ~strcmp(modeName, 'unequal_physical_slot')
        error('Unsupported OPR_ANGLE_MODE: %s', modeName);
    end
end
assert(numel(cfg.slotAngleDeg) == 6 && abs(sum(cfg.slotAngleDeg) - 360) < 1e-9);
cached = cfg;
key = newKey;
end


function [localAngleDeg, thetaRot] = map_adjacent_slot_times_local( ...
    oprTimes, sampleTimes, cfg, epr)
oprTimes = oprTimes(:);
sampleTimes = sampleTimes(:);
prev = discretize(sampleTimes, [-inf; oprTimes(2:end); inf]);
prev = max(1, min(prev, numel(oprTimes) - 1));
frac = (sampleTimes - oprTimes(prev)) ./ ...
    (oprTimes(prev + 1) - oprTimes(prev));
eventInRev = mod((1:numel(oprTimes)).' - 1, epr) + 1;
physicalSlot = mod(eventInRev - 1 + cfg.slotOffset, epr) + 1;
intervalByEvent = cfg.slotAngleDeg(physicalSlot).';
eventPhase = zeros(numel(oprTimes), 1);
eventPhase(2:end) = cumsum(intervalByEvent(1:end-1)) * pi / 180;
intervalDeg = intervalByEvent(prev);
localAngleDeg = frac .* intervalDeg;
thetaRot = eventPhase(prev) + frac .* intervalDeg * pi / 180;
invalid = sampleTimes < oprTimes(1) | sampleTimes > oprTimes(end) | ~isfinite(frac);
localAngleDeg(invalid) = NaN;
...
switch lower(strtrim(eta_model))
    case {'none', 'noeta', 'eta_none', 'theoretical_zero_eta'}
        S_model.fit_sensor_eta = false;
        S_model.sensor_eta_mode = 'fixed_zero';
        S_model.sensor_eta_prior_mm = zeros(1, numel(S.analysis_sensors));
...
for i = 1:numel(final_eos)
    ...
    if S.continuous_frequency_refine && S.frequency_refine_half_width_hz > 0
        z0 = [p_best(:).', frequency_hz];
        obj_freq = @(z) direct_template_contfreq_objective_local(z, EO, bundle, S);
        try
            [z_best, ~, exitflag_freq] = fminsearch(obj_freq, z0, opts);
            frequency_hz = project_frequency_local(z_best(end), EO, bundle, S);
            p_best = z_best(1:end-1);
            exitflag = min(exitflag, exitflag_freq);
            [A, phi, dx_c, eta] = unpack_params_local(p_best, S);
        catch
            frequency_hz = EO * bundle.rot_freq_mean_hz;
        end
    ...
    Candidate(i).EO = EO;
    Candidate(i).frequency_hz = frequency_hz;
    Candidate(i).frequency_offset_hz = frequency_hz - EO * bundle.rot_freq_mean_hz;
    Candidate(i).A = A;
...
result.fn_id = best.frequency_hz;
...
function value = direct_template_contfreq_objective_local(z, EO, bundle, S)
z = z(:).';
if numel(z) < 4
    value = 1e12;
    return;
...
p = z(1:end-1);
frequency_raw_hz = z(end);
frequency_hz = project_frequency_local(frequency_raw_hz, EO, bundle, S);
[A, phi, dx_c, eta] = unpack_params_local(p, S);

[~, ~, ~, coverage, objective_score] = evaluate_full_template_model_local( ...
    bundle, EO, A, phi, dx_c, eta, S, frequency_hz);
if ~isfinite(objective_score)
    objective_score = 1e12;
end
...
% fminsearch is unconstrained. Penalize the raw frequency outside the
% candidate-local interval while evaluating the projected frequency.
penalty = penalty + 100 * (frequency_raw_hz - frequency_hz).^2;
eta_penalty = S.sensor_eta_reg_weight_v_per_mm * sqrt(mean(eta(:).^2));
overshoot_penalty = S.overshoot_penalty_weight * coverage.overshoot_rms_mm;
value = objective_score + eta_penalty + overshoot_penalty + penalty;
end


function frequency_hz = project_frequency_local(frequency_hz, EO, bundle, S)
f0_hz = EO * bundle.rot_freq_mean_hz;
half_width_hz = max(0, S.frequency_refine_half_width_hz);
lower_hz = f0_hz - half_width_hz;
upper_hz = f0_hz + half_width_hz;
if isfield(S, 'search_freq_hz') && numel(S.search_freq_hz) == 2
    lower_hz = max(lower_hz, min(S.search_freq_hz));
    upper_hz = min(upper_hz, max(S.search_freq_hz));
end
...
frequency_hz = min(max(frequency_hz, lower_hz), upper_hz);
end
...
t_rel = bundle.t(:) - mean(bundle.t(:), 'omitnan');
phase = EO .* bundle.theta(:) + ...
    2 * pi * (frequency_hz - f0_hz) .* t_rel;
y = A .* sin(phase + phi);
...
result.fn_id = NaN;
result.frequency_offset_hz = NaN;
