...
% Isolated production-resolution diagnostic. Formal Step04 is untouched.
diagnostic_script_dir = fileparts(mfilename('fullpath'));
experimental_dir = fileparts(fileparts(fileparts(diagnostic_script_dir)));
foundation_dir = fullfile(experimental_dir, '20250527_btt_data_foundation');
...
slot_loaded = load(fullfile(diagnostic_script_dir, ...
    'OPR6_SlotCalibration_20250527.mat'), 'SlotCalibration');
slot_cal = slot_loaded.SlotCalibration;
assert(numel(slot_cal.intervalAngleDeg) == 6 && ...
    abs(sum(slot_cal.intervalAngleDeg) - 360) < 1e-8, ...
    'Invalid six-slot angle calibration.');
cfg.step04_slot_angle_deg = double(slot_cal.intervalAngleDeg(:).');
cfg.step04_slot_anchor_deg = double(slot_cal.anchorAngleDeg(:).');
cfg.step04_slot_index_offset_high_to_low = ...
    double(slot_cal.slotIndexOffsetHighToLow);
cfg.step04_template_start_lap = 2;
cfg.step04_template_end_lap = 21;
cfg.step04_output_dir = fullfile(diagnostic_script_dir, ...
    'step04_production_unequal_slot_laps2_21');
cfg.step04_figure_dir = fullfile(cfg.step04_output_dir, 'figures');
...
fprintf('Angle mapping: adjacent OPR unequal physical slot.\n');
fprintf('Template common laps: %d-%d.\n', ...
    cfg.step04_template_start_lap, cfg.step04_template_end_lap);
...
for iSensor = 1:numel(cfg.sensor_ids)
    ...
    for iFile = 1:numel(file_ids)
        ...
        for k = 1:numel(pulse_idx_in_file)
            ...
            if lap_index < cfg.step04_template_start_lap || ...
                    lap_index > cfg.step04_template_end_lap
                continue;
            end
            ...
            t_seg = t_global(idx);
            v_seg = v(idx);
            [x_mm, keep_mask] = map_time_to_x_local(t_seg, opr_times, ...
                F_omega_deg_s, std_angles(sid, blade_id), cfg, arrival_t);
            if ~any(keep_mask)
                continue;
            end
...
function [x_mm, keep_mask] = map_time_to_x_local(t_seg, opr_times, F_omega_deg_s, theta_std_deg, cfg, t_arrival) %#ok<INUSD>
t_seg = t_seg(:);
idx_ref = find(opr_times < t_arrival, 1, 'last');
keep_mask = false(size(t_seg));
x_mm = nan(size(t_seg));
if isempty(idx_ref) || idx_ref >= numel(opr_times) || ~isfinite(t_arrival)
    return;
end
t_ref = opr_times(idx_ref);
t_next = opr_times(idx_ref + 1);
local_period_s = t_next - t_ref;
if ~isfinite(local_period_s) || local_period_s <= 0
    return;
end
event_slot = mod(idx_ref - 1, numel(cfg.step04_slot_angle_deg)) + 1;
interval_angle_deg = cfg.step04_slot_angle_deg(event_slot);
for i = 1:numel(t_seg)
    t_now = t_seg(i);
    if ~isfinite(t_now)
        continue;
    end
    theta_actual = interval_angle_deg .* (t_now - t_ref) ./ local_period_s;
