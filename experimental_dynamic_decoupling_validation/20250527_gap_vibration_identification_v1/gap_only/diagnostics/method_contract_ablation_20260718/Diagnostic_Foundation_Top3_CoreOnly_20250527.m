clc; close all;
% Diagnostic copy only: strict Top-3 plus core-only fitting; formal source unchanged.
...
angleMappingCfg = formal_angle_mapping_config_local();
S.angle_map_mode = 'adjacent_opr_unequal_physical_slot';
...
% Main10 consumes these stored bundles; retain all points in the formal run.
...
angleCfg = formal_angle_mapping_config_local();
...
function cfg = formal_angle_mapping_config_local()
persistent cached
if ~isempty(cached)
    cfg = cached;
    return;
end
projectCfg = Config_20250527();
file = fullfile(projectCfg.paths.calibrationInputs, ...
    'OPR6_SlotCalibration_20250527.mat');
assert(isfile(file), 'Missing formal OPR6 slot calibration: %s', file);
loaded = load(file, 'SlotCalibration');
assert(isfield(loaded, 'SlotCalibration'), ...
    'OPR6 slot calibration file does not contain SlotCalibration.');
C = loaded.SlotCalibration;
cfg = struct();
cfg.file = file;
...
assert(numel(cfg.slotAngleDeg) == 6 && ...
    abs(sum(cfg.slotAngleDeg) - 360) < 1e-9, ...
    'Invalid formal OPR6 slot-angle calibration.');
cached = cfg;
end
...
invalid = sampleTimes < oprTimes(1) | sampleTimes > oprTimes(end) | ...
    ~isfinite(frac);
