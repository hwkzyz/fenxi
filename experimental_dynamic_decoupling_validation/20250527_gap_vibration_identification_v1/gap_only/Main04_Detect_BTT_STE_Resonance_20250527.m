%% Step04: resonance detection and visualization dashboard for 20250527
...
targetBladeId = packageCfg.case.targetBlade; % Must match the blade with the strain gauge.
baselineTimeSec = 3.0;
...
step3ResultFile = fullfile(caseOutputDir, 'Step3_Result_20250527.mat');
...
%% 4. Print summary
fprintf('\n=== 20250527 Step04 resonance visualization dashboard ===\n');
fprintf('Case: %s\n', caseName);
...
if ~isempty(preferredRegionTimeSec)
    fprintf('Preferred downstream segment priority: %.2f-%.2f s\n\n', preferredRegionTimeSec);
else
    fprintf('Preferred downstream segment priority: none\n\n');
end
...
scoreCsvFile = fullfile(outDir, 'Step04_DirectStrainResonance_Score_20250527.csv');
localFftCsvFile = fullfile(outDir, 'Step04_LocalStrainFFT_20250527.csv');
...
strainSource = struct('step3_file', step3ResultFile, 'strain_file', '', ...
    'strain_time_offset_total_sec', NaN);
...
% Step03 defines strain_time = BTT_time + best_tau_sec. Convert every
% time-domain strain sample to the same BTT clock used by the STFT map.
timeOffsetSec = D.result.strain_time_offset_total_sec - bestTauSec;
strainTimeBtt = S.Datas(:, 1) + timeOffsetSec;
...
figure('Name', '20250527 Step04 resonance visualization dashboard', ...
...
[T, F] = meshgrid(t, freq);
figure('Name', '20250527 Step04 auxiliary raw segmented FFT', ...
