function cfg = resolve_single_sync_experiment_paths_20251222(cfg)
%RESOLVE_SINGLE_SYNC_EXPERIMENT_PATHS_20251222 Resolve output paths.
...
cfg.case_file = fullfile(cfg.project_dir, ...
    sprintf('ExperimentCase_20251222_B%d_%s_%s_Start%ss.mat', ...
...
if isfield(cfg, 'custom_calib_file') && ~isempty(cfg.custom_calib_file)
    ...
    cfg.calib_file = fullfile(cfg.project_dir, ...
        sprintf('SGCalib_20251222_B%d_%s.mat', cfg.target_blade, sensor_tag));
end

cfg.per_sensor_calib_files = cell(numel(cfg.analysis_sensors), 1);
for i = 1:numel(cfg.analysis_sensors)
    sid = cfg.analysis_sensors(i);
    cfg.per_sensor_calib_files{i} = fullfile(cfg.project_dir, ...
        sprintf('SGCalib_20251222_B%d_S%d.mat', cfg.target_blade, sid));
end

cfg.legacy_calib_file = fullfile(fileparts(cfg.project_dir), ...
    sprintf('Blade_%d_SG_Static_Config.mat', cfg.target_blade));
cfg.calib_summary_csv = fullfile(cfg.project_dir, ...
    sprintf('SGCalib_20251222_B%d_%s_Summary.csv', cfg.target_blade, sensor_tag));
cfg.result_file = fullfile(cfg.project_dir, ...
    sprintf('Result_single_sync_20251222_B%d_%s_%s_Start%ss.mat', ...
    cfg.target_blade, sensor_tag, case_tag, time_tag));
cfg.report_file = fullfile(cfg.project_dir, ...
    sprintf('Result_single_sync_20251222_B%d_%s_%s_Start%ss.txt', ...
    cfg.target_blade, sensor_tag, case_tag, time_tag));
end
