%% Step05: Build static gap response surface for 20251222 dynamic validation
...
%   outputs/Step05_Response_Surface_20251222.mat
...
targetBladeMode = 'explicit_reference';
explicitReferenceBladeId = 2; % reference blade: B2 (known nominal-gap blade)
...
anchorDiagnostics = nan(numel(trueGaps), numel(bladeIds));
referenceCenters = nan(numel(trueGaps), 1);
refIdx = find(bladeIds == explicitReferenceBladeId, 1);
if isempty(refIdx), error('Reference blade B%d is missing.', explicitReferenceBladeId); end
for ig = 1:numel(trueGaps)
    T = rawRows{ig};
    speedNow = referenceTangentialSpeedMmS;
    baselineNow = staticBaselineMvByGap(ig);
    keys = unique(T(:, {'bladeId','eventNo','peakCenterSample'}), 'rows');
    refRows = keys.bladeId == explicitReferenceBladeId;
    if ~any(refRows), error('No B%d event in gap row %d.', explicitReferenceBladeId, ig); end
    refCenter = median(keys.peakCenterSample(refRows), 'omitnan');
    referenceCenters(ig) = refCenter;
    % Unwrap each blade passage around the B2 reference.  The staged files
    % may start at different rotor phases, so a blade can legitimately wrap
    % across the end of the sampled revolution.  Align its event to the
    % nearest expected slot before applying the fixed kinematic offset.
    slotPitchMm = 2*pi*rTipMm/numel(bladeIds);
    slotIntervalSamples = slotPitchMm / speedNow * sampleRateHz;
    revPeriodSamples = slotIntervalSamples * numel(bladeIds);
    bladeCenters = nan(numel(bladeIds),1);
    bladeUnwrapShiftSamples = zeros(numel(bladeIds),1);
    for ib = 1:numel(bladeIds)
        q = keys.bladeId == bladeIds(ib);
        rawCenter = median(keys.peakCenterSample(q), 'omitnan');
        expectedCenter = refCenter + (bladeIds(ib) - explicitReferenceBladeId) * slotIntervalSamples;
        shiftSamples = round((expectedCenter - rawCenter) / revPeriodSamples) * revPeriodSamples;
        bladeUnwrapShiftSamples(ib) = shiftSamples;
        bladeCenters(ib) = rawCenter + shiftSamples;
        anchorDiagnostics(ig,ib) = (bladeCenters(ib)-expectedCenter) / sampleRateHz * speedNow;
    end
    for ib = 1:numel(bladeIds)
        bid = bladeIds(ib);
        rows = T.bladeId == bid;
        ...
        % Convert each sample to the B2-anchored coordinate, then remove
        % the measured blade passage offset so x=0 is its kinematic center.
        sampleIndex = T.sampleIndex(rows) + bladeUnwrapShiftSamples(ib);
        yy = T.voltageMv(rows);
        frozenOffsetMm = (bladeIds(ib) - explicitReferenceBladeId) * slotPitchMm;
        tt = ((sampleIndex(:) - referenceCenters(ig)) * speedNow / sampleRateHz ...
            - frozenOffsetMm) / speedNow;
        [tt, idxSort] = sort(tt(:));
        yy = yy(idxSort);
...
responseSurface.dataset = '20251222';
...
responseSurface.xUnit = 'mm circumferential displacement from B2-anchored virtual phase';
...
responseSurface.referenceBladeAnchor = 'B2 peak passage; other blades use frozen kinematic offsets';
responseSurface.bladeOffsetMmFromB2 = (bladeIds(:) - explicitReferenceBladeId) * (2*pi*rTipMm/numel(bladeIds));
responseSurface.anchorDiagnosticsMm = anchorDiagnostics;
...
%% 5. Save outputs
matFile = fullfile(outDir, 'Step05_Response_Surface_20251222_RefBladeAnchor.mat');
csvFile = fullfile(outDir, 'Step05_Response_Surface_Fit_20251222_RefBladeAnchor.csv');
save(matFile, 'responseSurface', '-v7.3');
motionCsvFile = fullfile(outDir, 'Step05_Calibration_Motion_20251222_RefBladeAnchor.csv');
baselineCsvFile = fullfile(outDir, 'Step05_Static_Gap_Continuous_Baseline_20251222_RefBladeAnchor.csv');
...
writematrix([trueGaps(:), anchorDiagnostics], fullfile(outDir, ...
    'Step05_B2_Anchor_Offsets_20251222_RefBladeAnchor.csv'));

%% 6. Diagnostics
if saveFigures
    fig = figure('Name', '20251222 Step05 response surface', ...
    ...
    figFile = fullfile(outDir, 'Step05_Response_Surface_20251222.png');
    saveas(fig, figFile);

    % All-blade waveform clusters in the B2-anchored coordinate.  Each
    % subplot keeps the fixed slot offset, so blade-to-blade peak shifts
    % remain visible instead of being removed by per-blade peak alignment.
    figAll = figure('Name', '20251222 all blade waveform clusters', ...
        'Color', 'w', 'Position', [80, 60, 1450, 920], 'NumberTitle', 'off');
    tiledlayout(3, 2, 'TileSpacing', 'compact', 'Padding', 'compact');
    clusterColors = turbo(numel(gTrain));
    for ib = 1:numel(bladeIds)
        nexttile; hold on; grid on; box on;
        for ig = 1:numel(trueGaps)
            y = waveforms(:, ib, ig);
            if any(isfinite(y))
                plot(xGrid, y, '-', 'Color', clusterColors(ig, :), 'LineWidth', 0.8);
            end
        end
        xline(0, 'k-', 'LineWidth', 0.7, 'HandleVisibility', 'off');
        xlabel('B2-anchored circumferential x (mm)');
        ylabel('Baseline-subtracted voltage (mV)');
        title(sprintf('Blade %d', bladeIds(ib)));
    end
    lg = gobjects(numel(gTrain), 1);
    for ig = 1:numel(gTrain)
        lg(ig) = plot(nan, nan, '-', 'Color', clusterColors(ig, :), 'LineWidth', 1.5);
    end
    legend(lg, compose('%.2f mm', gTrain), 'NumColumns', 4, ...
        'Location', 'southoutside', 'Box', 'off');
    sgtitle('Static calibration waveform clusters: all blades, B2 reference anchor');
    saveas(figAll, fullfile(outDir, 'Step05_AllBlade_WaveformClusters_20251222_RefBladeAnchor.png'));

    % Direct blade-to-blade comparison at low, middle, and high nominal gaps.
    figRep = figure('Name', '20251222 representative gap overlays', ...
        'Color', 'w', 'Position', [80, 100, 1500, 470], 'NumberTitle', 'off');
    tiledlayout(1, 3, 'TileSpacing', 'compact', 'Padding', 'compact');
    repIdx = unique([1, max(1, round(numel(trueGaps)/2)), numel(trueGaps)]);
    bladeColors = lines(numel(bladeIds));
    repHandles = gobjects(numel(bladeIds), 1);
    for ir = 1:numel(repIdx)
        ig = repIdx(ir);
        nexttile; hold on; grid on; box on;
        for ib = 1:numel(bladeIds)
            y = waveforms(:, ib, ig);
            if any(isfinite(y))
                h = plot(xGrid, y, 'Color', bladeColors(ib, :), 'LineWidth', 1.2);
                if ir == 1
                    repHandles(ib) = h;
                end
            end
        ...
        xlabel('B2-anchored circumferential x (mm)');
        ylabel('Voltage (mV)');
        title(sprintf('Nominal gap = %.2f mm', trueGaps(ig)));
    end
    legend(repHandles, compose('B%d', bladeIds), 'NumColumns', 6, ...
        'Location', 'southoutside', 'Box', 'off');
    sgtitle('Representative static-gap waveforms across blades');
    saveas(figRep, fullfile(outDir, 'Step05_RepresentativeGap_BladeOverlays_20251222_RefBladeAnchor.png'));

    % Peak offsets are a compact diagnostic of the retained blade-to-blade
    % phase differences after anchoring B2.
    figDiag = figure('Name', '20251222 B2 anchor diagnostics', ...
        'Color', 'w', 'Position', [120, 120, 1050, 620], 'NumberTitle', 'off');
    imagesc(trueGaps, bladeIds, anchorDiagnostics.');
    axis xy; colorbar; grid on; box on;
    xlabel('Nominal gap (mm)'); ylabel('Blade ID');
    title('Peak-center offsets relative to the B2 anchor (mm)');
    saveas(figDiag, fullfile(outDir, 'Step05_B2Anchor_PeakOffsets_20251222_RefBladeAnchor.png'));
end
