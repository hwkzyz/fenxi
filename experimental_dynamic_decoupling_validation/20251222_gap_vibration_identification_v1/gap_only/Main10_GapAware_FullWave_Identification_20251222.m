...
if ~isempty(correctedLibOverride)
    ...
    if isfile(r5SidecarFile)
        ...
        allowLegacy = strcmp(strtrim(getenv('ALLOW_LEGACY_FALLBACK')),'1');
        if ~allowLegacy
            error('R5:MissingSidecar', ['Formal R5 run requires the sidecar for B%d/%s. ' ...
                'Run Main05_R5_PC1_FrontEnd_20251222 first. Set ALLOW_LEGACY_FALLBACK=1 only for historical reruns.'], ...
                cfg.targetBlade,sensorTag);
        end
...
if contains(correctedLibFile, 'R5_Main10_CompatibleLibrary_')
    if ~isfield(Sc.CorrectedGapLibrary,'r5')
        error('R5:InvalidSidecar','R5 sidecar is missing its provenance contract.');
    end
...
Result.MethodContract = struct( ...
    'schema_version', 'unified_structured_voltage_contract_v2', ...
    'stage', 'gap_aware', ...
