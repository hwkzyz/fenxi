function G = R5_Calibrate_B2_AnchorRegistration(familyFile,templateFile,outputFile,b2GapMm)
...
ib=find(F.blade_id==2,1);
assert(~isempty(ib),'R5:B2Missing','Static family does not contain B2.');
...
sensorId=unique([Template.SensorBlade.sensor_id]);
...
for is=1:numel(sensorId)
    a=Template.SensorBlade([Template.SensorBlade.blade_id]==2 & [Template.SensorBlade.sensor_id]==sensorId(is));
    assert(numel(a)==1,'R5:B2AnchorMissing','B2/S%d anchor missing.',sensorId(is));
...
G=struct('schema','R5_B2_ANCHOR_COORDINATE_REGISTRATION_V1', ...
    'family_file',familyFile,'template_file',templateFile,'b2_gap_mm',b2GapMm, ...
    'registration',registration,'summary',vertcat(row{:}), ...
    'note','B2 known-gap anchor fixes spatial and affine-voltage registration before target localization.');
