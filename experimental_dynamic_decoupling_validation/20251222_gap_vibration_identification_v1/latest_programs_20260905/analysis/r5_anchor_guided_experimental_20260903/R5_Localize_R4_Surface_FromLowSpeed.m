...
% Enforce the R4 contract: one shared surface coordinate per blade, while
% retaining an independent nuisance gap for each sensor. The previous
% sensor-wise minima are retained only as diagnostics in details.
for bid=unique(T.blade_id(:)).'
    idx=find(T.blade_id==bid);
    if numel(idx)<2, continue; end
    z0=details(idx(1)).profile_z(:);
    J=zeros(size(z0));
    for ii=1:numel(idx)
        p=details(idx(ii)).profile_rmse;
        [~,ig]=min(p,[],2);
    ...
    for ii=1:numel(idx)
        ...
        details(idx(ii)).latent=zShared;
        details(idx(ii)).gap=details(idx(ii)).profile_gap(ig);
        qShared=interp1(z,qord.',zShared,'linear').';
        details(idx(ii)).B=reshape(qShared,numel(x),3);
    end
end
...
save(fullfile(outputDir,'loc.mat'),'T','details','familyFile','templateFile','b2GapMm','registrationFile','-v7.3');
