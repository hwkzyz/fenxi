...
cfg.strain_reference_blade = 1;
if cfg.target_blade == cfg.strain_reference_blade
    cfg.strain_validation_scope = 'same_target_blade_direct_reference';
else
    cfg.strain_validation_scope = 'cross_modal_operating_condition_reference';
end
...
% Internal result branch. The formal public method name is stored in
% Result.method / Result.MethodContract.formal_method_name.
cfg.method_name = 'gap_only';
cfg.public_method_name = 'R5_PC1_AnchorSurface_SensorWiseDg_FullWave';
cfg.primary_strain_channel = 'AI1-03';
cfg.method_result_file = strtrim(getenv('R5_VALIDATED_RESULT_FILE'));
if isempty(cfg.method_result_file)
    error('R5:MissingValidatedResult', ...
        ['Formal Main07 requires the exact Main06 result already accepted by ' ...
         'R5_PreMain07_HardGate via R5_VALIDATED_RESULT_FILE.']);
end
cfg.method_result_dir = packageCfg.paths.gapResults;
cfg.show_figures = true;
cfg.save_figures = true;
cfg.figure_dir = packageCfg.paths.strainValidation;
...
cfg.strain_reference_freq_half_band_hz = 3;
cfg.strain_reference_freq_step_hz = 0.1;
cfg.strain_reference_frequency_mode = 'global_fixed_band_then_local';
% Registered physical bands, selected before the BTT result is read.
% R01 is the approximately 580-Hz resonance; R04 is the approximately
% 630-Hz resonance. These bands are used for the strain-side reference
% only and are never centred on a BTT-identification frequency.
if cfg.target_blade == 5
    cfg.strain_reference_freq_band_hz = [600, 650];
    cfg.strain_expected_frequency_hz = 629.94;
else
    cfg.strain_reference_freq_band_hz = [560, 610];
    cfg.strain_expected_frequency_hz = 582.22;
end
cfg.strain_expected_frequency_tolerance_hz = 15;
...
if ~isfield(MethodResult,'method') || ...
        ~strcmp(string(MethodResult.method),'R5_PC1_AnchorSurface_SensorWiseDg_FullWave')
    error('R5:WrongMethodResult','Main07 received a result from a different method contract.');
end
if ~isfield(MethodResult,'cfg') || ~isfield(MethodResult.cfg,'targetBlade') || ...
        double(MethodResult.cfg.targetBlade) ~= cfg.target_blade
    error('R5:ResultBladeMismatch','Main07 result target does not match the B1 strain reference.');
end
...
if ~isfile(result_file)
    error('R5:MissingValidatedResult','Validated Main06 result does not exist: %s',result_file);
end
end
...
Result.Config = struct('dynamic_case_name', cfg.dynamic_case_name, ...
    'strain_reference_blade', cfg.strain_reference_blade, ...
    'strain_validation_scope', cfg.strain_validation_scope, ...
    'strain_channel', cfg.primary_strain_channel);
...
fprintf(['Strain-BTT validation using the local decoupling method\n' ...
    ...
    '  Scope = %s | strain reference blade = B%d | channel = %s\n' ...
    '  Dominant EO = %d | mean frequency = %.4f Hz | mean amplitude = %.6f mm\n'], ...
    result_file, cfg.public_method_name, numel(Result.Trends.WindowID), best_index, ...
    cfg.strain_validation_scope, cfg.strain_reference_blade, cfg.primary_strain_channel, ...
...
elseif isfield(loaded_alignment, 'strainSource')
    ...
    if isfield(loaded_alignment, 'alignResult') && ...
            isfield(loaded_alignment.alignResult, 'best_tau_sec')
        alignment.best_tau_sec = loaded_alignment.alignResult.best_tau_sec;
    elseif isfield(source, 'best_tau_sec')
        alignment.best_tau_sec = source.best_tau_sec;
    else
        alignment.best_tau_sec = -source.strain_time_offset_total_sec;
    end
    alignment.strain_time_offset_total_sec = source.strain_time_offset_total_sec;
else
...
t_strain_raw = loaded_strain.Datas(:, 1);
v_strain = loaded_strain.Datas(:, 2);
if isfield(alignment, 'strain_time_offset_total_sec') && ...
        isfinite(alignment.strain_time_offset_total_sec)
    strain_time_offset_sec = alignment.strain_time_offset_total_sec;
else
    strain_time_offset_sec = 0;
end
t_strain = t_strain_raw + strain_time_offset_sec - alignment.best_tau_sec;
...
strain_center_time = btt_center_time;
...
% Establish one strain-side frequency reference from the complete selected
% BTT span.  This is intentionally done before any per-window BTT frequency
% is used, following the audited 20241106 validation chain.
selected_time_sec = [min(btt_center_time - half_window_sec), ...
    max(btt_center_time + half_window_sec)];
[global_peak_hz, global_peak_amp_raw] = estimate_strain_band_peak_local( ...
    t_strain, v_strain, selected_time_sec, cfg.strain_reference_freq_band_hz);
global_reference_fit = fit_strain_frequency_local(t_strain, v_strain, ...
    selected_time_sec, global_peak_hz, cfg.strain_reference_freq_half_band_hz, ...
    cfg.strain_reference_freq_step_hz);
global_reference_hz = global_reference_fit.frequencyHz;
global_fit_r2 = global_reference_fit.rSquared;
global_fit_rmse = global_reference_fit.rmseMm * cfg.strain_to_tip_mm;
global_reference_boundary = global_reference_fit.hitFrequencyBoundary;
global_reference_band_boundary = global_peak_hz <= cfg.strain_reference_freq_band_hz(1) + 1 || ...
    global_peak_hz >= cfg.strain_reference_freq_band_hz(2) - 1;
global_reference_physical_mismatch = abs(global_reference_hz - ...
    cfg.strain_expected_frequency_hz) > cfg.strain_expected_frequency_tolerance_hz;
if global_reference_boundary
    error('R5:StrainReferenceBoundary', ...
        'Global strain reference reached its local search boundary (%.3f Hz).', ...
        global_reference_hz);
end
if global_reference_band_boundary
    warning('R5:StrainReferenceBandBoundary', ...
        'Global strain FFT peak %.3f Hz is at the edge of the registered band [%g,%g] Hz.', ...
        global_peak_hz, cfg.strain_reference_freq_band_hz(1), cfg.strain_reference_freq_band_hz(2));
end
if global_reference_physical_mismatch
    warning('R5:StrainReferencePhysicalMismatch', ...
        'Independent strain reference %.3f Hz differs from registered resonance %.3f Hz by %.3f Hz.', ...
        global_reference_hz, cfg.strain_expected_frequency_hz, ...
        global_reference_hz - cfg.strain_expected_frequency_hz);
end
...
for iWin = 1:num_windows
    ...
    freq_grid = build_step6_strain_freq_grid_local(global_reference_hz, cfg);
...
StrainRef.StrainTimeOffsetSec = strain_time_offset_sec;
StrainRef.WindowID = xv;
StrainRef.WindowCenterTimeBTT = btt_center_time;
StrainRef.WindowCenterTimeStrain = strain_center_time;
StrainRef.WindowCenterTimeStrainRaw = btt_center_time - strain_time_offset_sec + ...
    alignment.best_tau_sec;
...
StrainRef.FrequencySearchMode = string(cfg.strain_reference_frequency_mode);
StrainRef.FrequencySearchBandHz = cfg.strain_reference_freq_band_hz;
StrainRef.GlobalReferenceHz = global_reference_hz;
StrainRef.GlobalReferencePeakHz = global_peak_hz;
StrainRef.GlobalReferencePeakAmplitudeRaw = global_peak_amp_raw;
StrainRef.GlobalReferenceFitR2 = global_fit_r2;
StrainRef.GlobalReferenceFitRMSE = global_fit_rmse;
StrainRef.GlobalReferenceBoundary = global_reference_boundary;
StrainRef.GlobalReferenceBandBoundary = global_reference_band_boundary;
StrainRef.ExpectedFrequencyHz = cfg.strain_expected_frequency_hz;
StrainRef.GlobalReferencePhysicalMismatch = global_reference_physical_mismatch;
...
function [peak_hz, peak_amp] = estimate_strain_band_peak_local(t, y, time_span, band_hz)
mask = isfinite(t) & isfinite(y) & t >= time_span(1) & t <= time_span(2);
t = t(mask); y = detrend(y(mask));
if numel(t) < 32, error('Selected strain span has too few samples.'); end
dt = median(diff(t), 'omitnan');
n = numel(y); nfft = 2^nextpow2(n * 4);
f = (0:floor(nfft/2)).' / (dt*nfft);
amp = abs(fft(y .* hann_window_local(n), nfft)); amp = amp(1:numel(f));
use = f >= band_hz(1) & f <= band_hz(2);
if ~any(use), error('Strain reference band is outside the available spectrum.'); end
[peak_amp, k] = max(amp(use)); ff = f(use); peak_hz = ff(k);
end

function fit = fit_strain_frequency_local(t, y, time_span, reference_hz, half_band_hz, step_hz)
mask = isfinite(t) & isfinite(y) & t >= time_span(1) & t <= time_span(2);
t = t(mask); y = detrend(y(mask));
if numel(t) < 32, error('Selected strain span has too few valid samples.'); end
t = t(:) - mean(t, 'omitnan'); y = y(:);
grid = (reference_hz-half_band_hz):step_hz:(reference_hz+half_band_hz);
sse = inf(size(grid)); coef = nan(numel(grid),3);
for k = 1:numel(grid)
    H = [cos(2*pi*grid(k)*t), sin(2*pi*grid(k)*t), ones(size(t))];
    c = H\y; e = y-H*c; sse(k)=sum(e.^2); coef(k,:)=c(:).';
end
[best_sse, k] = min(sse); c=coef(k,:); total=sum((y-mean(y)).^2);
fit.frequencyHz=grid(k); fit.amplitudeMm=hypot(c(1),c(2));
fit.rmseMm=sqrt(best_sse/numel(y)); fit.rSquared=1-best_sse/max(total,eps);
fit.hitFrequencyBoundary=(k==1 || k==numel(grid));
end
...
T = table(StrainRef.WindowID, ...
    ...
    StrainRef.WindowCenterTimeStrainRaw, ...
    repmat(StrainRef.AlignmentTauSec, numel(StrainRef.WindowID), 1), ...
    repmat(StrainRef.StrainTimeOffsetSec, numel(StrainRef.WindowID), 1), ...
    ...
    'WindowCenterTimeStrain_BTTAligned_s', ...
    'WindowCenterTimeStrainRaw_s', ...
    'AlignmentTau_s', ...
    'StrainTimeOffset_s', ...
...
% Convert the strain time axis exactly as in Main04.
time_offset_sec = 0;
if isfield(StrainRef, 'StrainTimeOffsetSec') && isfinite(StrainRef.StrainTimeOffsetSec)
    time_offset_sec = StrainRef.StrainTimeOffsetSec;
end
t_strain_btt = t_strain + time_offset_sec - StrainRef.AlignmentTauSec;
end
