...
% Internal result branch; not the public method name.
...
if isempty(cfg.method_result_file)
    error('R5:MissingValidatedResult', ...
        'Main08 requires the exact hard-gated Main06 result via R5_VALIDATED_RESULT_FILE.');
end
...
if ~isfield(MethodResult,'method') || ...
    ...
    error('R5:WrongMethodResult','Main08 received a result from a different method contract.');
end
if ~isfield(MethodResult,'cfg') || ~isfield(MethodResult.cfg,'targetBlade') || ...
        double(MethodResult.cfg.targetBlade) ~= cfg.target_blade
    error('R5:ResultBladeMismatch','Main08 result target does not match the B1 strain reference.');
end
...
dt = median(diff(t), 'omitnan'); n = numel(y); nfft = 2^nextpow2(n*4);
...
t=t(:)-mean(t,'omitnan'); y=y(:); grid=(reference_hz-half_band_hz):step_hz:(reference_hz+half_band_hz);
sse=inf(size(grid)); coef=nan(numel(grid),3);
for k=1:numel(grid), H=[cos(2*pi*grid(k)*t),sin(2*pi*grid(k)*t),ones(size(t))]; c=H\y; e=y-H*c; sse(k)=sum(e.^2); coef(k,:)=c(:).'; end
[best_sse,k]=min(sse); c=coef(k,:); total=sum((y-mean(y)).^2);
fit.frequencyHz=grid(k); fit.amplitudeMm=hypot(c(1),c(2)); fit.rmseMm=sqrt(best_sse/numel(y));
fit.rSquared=1-best_sse/max(total,eps); fit.hitFrequencyBoundary=(k==1||k==numel(grid));
end
