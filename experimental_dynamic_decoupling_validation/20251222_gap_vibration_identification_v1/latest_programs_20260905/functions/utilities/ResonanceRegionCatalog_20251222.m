...
catalog = make_region(1, 44.7418, 59.4098, 50.2000, 20, 14, 582.22, 1, ...
    'R01_EO14_main_50p2', 'Main EO14 region; keeps the historical 50.2 s start.');
catalog(end+1) = make_region(2, 19.8062, 21.2730, 20.0000, 20, 20, 580.86, 1, ...
    'R02_EO20_20p0', 'Short EO20 region near 20.17 s.');
catalog(end+1) = make_region(3, 13.9390, 14.6724, 13.9400, 20, 20, 629.94, 5, ...
    'R03_EO20_13p94', 'Short EO20 region near 14.31 s.');
catalog(end+1) = make_region(4, 31.5406, 33.0074, 31.8712, 20, 18, 629.94, 5, ...
    'R04_EO18_31p8712', ['BTT-aligned 630-Hz region. Raw strain clock is about ', ...
    '30.0459 s later; keep the frozen BTT start 31.8712 s.']);
catalog(end+1) = make_region(5, 143.7508, 145.9510, 144.2000, 20, 20, 579.49, 1, ...
    'R05_EO20_144p2', 'Low-speed EO20 region near 145.58 s.');
catalog(end+1) = make_region(6, 16.8726, 18.3394, 17.0000, 20, 20, 579.49, 1, ...
    'R06_EO20_17p0', 'EO20 region near 17.24 s.');
catalog(end+1) = make_region(7, 38.8746, 40.3414, 39.2000, 20, 15, 579.49, 1, ...
    'R07_EO15_39p2', 'EO15 region near 39.24 s; use representative blade B1.');
...
function r = make_region(regionId, regionStartSec, regionEndSec, analysisStartTimeSec, ...
    targetBladePasses, dominantOrder, dominantFreqHz, representativeBladeId, tag, note)
r = struct();
r.regionId = regionId;
r.tag = tag;
...
r.representativeBladeId = representativeBladeId;
r.note = note;
end
