%% Unified structured-voltage single-sync gap/vibration identification
...
%
% The single-sync structured branch scans every integer EO from 300 to
% 1000 Hz and applies an all-sensor voltage-gradient linearized
...
%
% The support-aware grouped-lap adaptive-SG template and PCHIP interpolation
% match the low-template + static-gap-increment theory used by simulation.
% The no-gap method is maintained separately as a comparison.
...
bladeOverride = strtrim(getenv('STEP07J_TARGET_BLADE'));
if ~isempty(bladeOverride)
    b = str2double(bladeOverride);
    if ~isfinite(b) || b ~= round(b) || b < 1
        error('STEP07J_TARGET_BLADE must be a positive integer.');
    end
    if round(b) ~= C0.bladeId
        error('R5:TargetBladeRegionMismatch', ...
            ['STEP07J_TARGET_BLADE=%d conflicts with the selected resonance-region blade B%d. ' ...
             'Select the region through BLADE_RESONANCE_REGION_ID instead.'], ...
            round(b), C0.bladeId);
    end
end
...
cfg.eoPad = 2;
cfg.frequencyStructureMode = packageCfg.frequency.structureMode;
cfg.candidateMethod = packageCfg.frequency.candidateMethod;
cfg.structureSelection = packageCfg.frequency.structureSelection;
cfg.unknownStructureCriterion = packageCfg.frequency.unknownStructureCriterion;
cfg.vpTopK = packageCfg.frequency.candidateTopK;
...
% The frozen R5 production model keeps the low-speed calibration fixed and
% fits independent dg_s for the active sensors.  Residual dmu/dtau belongs
% only to historical diagnostic branches and must not enter production.
cfg.deltaMuLimit = 0;
cfg.deltaTauLimitMm = 0;
...
cfg.formalMethodName = 'R5_PC1_AnchorSurface_SensorWiseDg_FullWave';
...
if strcmpi(cfg.runMode, 'main')
    if isfinite(cfg.expectedFoundationStartTimeSec) && ...
            abs(cfg.expectedFoundationStartTimeSec - Pflow.identification.analysisStartTimeSec) > 1e-9
        error('R5:RegionStartOverride', ...
            ['STEP05_ANALYSIS_START_TIME conflicts with the selected resonance region %s. ' ...
             'Select BLADE_RESONANCE_REGION_ID instead.'], ...
            Pflow.identification.resonanceRegionShortTag);
    end
    if isfinite(cfg.expectedFoundationTargetLaps) && ...
            cfg.expectedFoundationTargetLaps ~= Pflow.identification.targetBladePasses
        error('R5:RegionTargetLapsOverride', ...
            ['STEP05_TARGET_LAPS conflicts with the selected resonance region %s. ' ...
             'Select BLADE_RESONANCE_REGION_ID instead.'], ...
            Pflow.identification.resonanceRegionShortTag);
    end
end
cfg.includeGapTiltShift = parse_logical_env_local('STEP07J_INCLUDE_GAP_TILT_SHIFT', false);
if strcmpi(cfg.runMode, 'main') && cfg.includeGapTiltShift
    error('R5:FrozenContractViolation', ...
        'Formal R5 production forbids the legacy delta-mu/delta-tau gap-tilt branch.');
end
...
assert(strcmpi(cfg.frequencyStructureMode, 'single_sync') && ...
    strcmpi(cfg.candidateMethod, 'structured_voltage_vp'), ...
    'Step07K:StructuredContractViolation', ...
    'The formal experiment must use the single_sync structured-voltage branch.');
...
if isempty(resultSuffix)
    resultSuffix = ['_', lower(Pflow.identification.resonanceRegionShortTag), '_gapaware'];
end
...
if ~isempty(correctedLibOverride)
    ...
    % The frozen R5 front-end owns the target-blade sidecar. Prefer it when
    % present; the legacy Step06I library remains only as an explicit
    % compatibility fallback for historical reruns.
    r5SidecarFile = fullfile(packageCfg.paths.analysis, ...
        'r5_anchor_guided_experimental_20260903', 'results', ...
        'r4_frontend_only', sprintf('R5_Main10_CompatibleLibrary_B%d_%s.mat', ...
        cfg.targetBlade, sensorTag));
    if isfile(r5SidecarFile)
        correctedLibFile = r5SidecarFile;
    else
        error('R5:MissingSidecar', ['Formal latest-program run requires the LOBO R5 sidecar for B%d/%s. ' ...
            'Run Main05_R5_PC1_FrontEnd_20251222 first. Historical fallback is not supported in this directory.'], ...
            cfg.targetBlade,sensorTag);
    end
end
...
if strcmpi(templateOverride, packageCfg.files.lowSpeedTemplate) && ~isfile(templateOverride)
    Prepare_AdaptiveSG_Template_20251222(false);
end
if ~isempty(templateOverride)
...
if ~isfield(Sc.CorrectedGapLibrary,'r5')
    error('R5:InvalidSidecar','Formal latest-program run requires an R5 sidecar with provenance.');
end
if ~isfield(Sc.CorrectedGapLibrary.r5,'target_blade') || ...
        double(Sc.CorrectedGapLibrary.r5.target_blade) ~= double(cfg.targetBlade)
    error('R5:TargetMismatch','R5 sidecar target blade does not match the current case.');
end
if ~isfield(Sc.CorrectedGapLibrary.r5,'sensor_latent_coordinates')
    error('R5:InvalidSidecar','R5 sidecar lacks shared-anchor provenance.');
end
manifestFile = fullfile(fileparts(correctedLibFile),'R5_Formal_Manifest.mat');
if ~isfile(manifestFile)
    error('R5:MissingManifest','Formal R5 manifest is missing: %s',manifestFile);
end
M = load(manifestFile,'manifest');
assert(strcmp(M.manifest.methodFreezeId,'R4R5_PC1_ANCHOR_SIDECAR_20260905'), ...
    'R5:ManifestMismatch','R5 manifest freeze ID does not match the current method.');
assert(isfield(M.manifest,'entries'), ...
    'R5:ManifestMismatch','R5 manifest has no target-specific LOBO entry.');
entryIndex = find(arrayfun(@(e) double(e.targetBlade)==double(cfg.targetBlade), M.manifest.entries),1);
assert(~isempty(entryIndex),'R5:ManifestMismatch','R5 manifest has no target-specific LOBO entry.');
entry = M.manifest.entries(entryIndex);
assert(isfield(entry,'excludedTargetBlade') && ...
    double(entry.excludedTargetBlade)==double(cfg.targetBlade), ...
    'R5:LOBOContract','Manifest entry does not exclude the current target blade.');
assert(isfield(entry,'trainingBladeIds') && ...
    ~ismember(double(cfg.targetBlade),double(entry.trainingBladeIds)), ...
    'R5:LOBOContract','Current target blade appears in the LOBO training set.');
manifestSidecar = entry.sidecarFile;
if ~is_absolute_path_local(manifestSidecar)
    manifestSidecar = fullfile(rootDir,manifestSidecar);
end
assert(double(entry.targetBlade)==double(cfg.targetBlade) && ...
    isfile(manifestSidecar) && strcmpi(manifestSidecar,correctedLibFile), ...
    'R5:ManifestMismatch','Current sidecar is not the manifest-bound target sidecar.');
baseForContract = fullfile(packageCfg.paths.calibrationInputs, sprintf( ...
    'Step06I_OffsetTiltShared_GapLibrary_20251222_B%d_%s.mat',cfg.targetBlade,sensorTag));
if isfile(baseForContract)
    Bc = load(baseForContract,'CorrectedGapLibrary');
    assert(isequaln(Sc.CorrectedGapLibrary.sensor,Bc.CorrectedGapLibrary.sensor), ...
        'R5:SensorContractChanged','R5 sidecar sensor calibration differs from frozen base library.');
end
...
if strcmpi(cfg.localBundleSource, 'foundation_step05_bundle')
    ...
    if ~isempty(templateOverride)
        [Template, templateFile] = load_step04_template_file_for_blade_local( ...
            templateOverride, cfg.targetBlade, cfg.analysisSensors, sensorTag);
...
fprintf('\n=== Unified structured-voltage method: single-sync NoEta gap/full-wave ===\n');
fprintf('Template: %s\n', templateFile);
...
if strcmpi(cfg.localBundleSource, 'foundation_step05_bundle')
    ...
    expectedFoundationWindows = floor((double(Aset.target_laps) - double(Aset.window_laps)) / ...
        double(Aset.sliding_step_laps)) + 1;
    actualFoundationWindows = numel(FoundationStep05Source.Window);
    fprintf('Foundation window coverage: %d/%d expected windows.\n', ...
        actualFoundationWindows, expectedFoundationWindows);
    if isinf(cfg.maxWindows)
        assert(actualFoundationWindows == expectedFoundationWindows, ...
            'R5:IncompleteFoundationRun', ...
            ['Formal run requires the complete Foundation Step05 bundle (%d windows); ' ...
             'found %d. Re-run Main05_Foundation_NoGap_VPTopK_20251222 without STEP05_DEBUG_MAX_WINDOWS.'], ...
            expectedFoundationWindows, actualFoundationWindows);
    elseif actualFoundationWindows < min(expectedFoundationWindows,cfg.maxWindows)
        error('R5:IncompleteFoundationRun','Foundation bundle has %d windows; at least %d are required for this run.', ...
            actualFoundationWindows,min(expectedFoundationWindows,cfg.maxWindows));
    end
...
displayMainModel = cfg.mainModel;
displayModelNames = strjoin(cfg.modelNames, ', ');
if strcmpi(cfg.runMode,'main')
    displayMainModel = cfg.formalMethodName;
    displayModelNames = cfg.formalMethodName;
end
fprintf(['Models: %s | main %s, EO screen %s | dx limit %.3f mm, ' ...
    'dg bound %.3f mm, dg prior scale %.3f mm, dmu %.3f, dtau %.3f mm\n'], ...
    displayModelNames, displayMainModel, cfg.mainEoScreenModel, ...
...
for iw = 1:numWindows
    ...
    for im = 1:numel(fitModelNames)
        ...
        % Store a same-parameter no-gap prediction for waveform ablation.
        % This is diagnostic only: it never participates in EO selection or
        % optimization.  Keeping it beside VPred makes gap contribution
        % auditable without re-fitting a second model.
        if strcmpi(modeName,'gap_only') && isstruct(modelFits.(modeName)) && ...
                isfield(modelFits.(modeName),'VPred')
            Fg = modelFits.(modeName);
            nS = numel(bundle.sensorIds);
            thetaNg = initial_theta_local(Fg.amplitudeMm, Fg.phaseRad, ...
                Fg.dxMm, cfgWindow, modeName, nS);
            [idxDg, idxDmu, idxDtau] = mode_param_indices_local(modeName,nS);
            if ~isempty(idxDg), thetaNg(idxDg)=0; end
            if ~isempty(idxDmu), thetaNg(idxDmu)=0; end
            if ~isempty(idxDtau), thetaNg(idxDtau)=0; end
            noGapDetail = evaluate_static_warp_waveform_local( ...
                thetaNg, round(Fg.EO), bundle, cfgWindow, modeName);
            modelFits.(modeName).VPredNoGap = noGapDetail.VPred;
            modelFits.(modeName).NoGapDiagnostic = struct( ...
                'sameEO',round(Fg.EO),'sameFrequencyHz',Fg.freqHz, ...
                'sameAmplitudeMm',Fg.amplitudeMm,'samePhaseRad',Fg.phaseRad, ...
                'sameDxMm',Fg.dxMm,'deltaGapForcedZero',true, ...
                'rmseMv',noGapDetail.plainRmseMv);
        end
    end
    ...
    elseif strcmpi(cfg.runMode, 'comparison')
        ...
        fprintf('Window %02d/%02d laps %s: proposed R5 %.2f mV\n', ...
            iw, numWindows, mat2str(Wmap.lap_range), ...
...
Result.method = 'R5_PC1_AnchorSurface_SensorWiseDg_FullWave';
Result.PublicMethodName = cfg.formalMethodName;
Result.InternalModelBranch = cfg.mainModel;
Result.description = ['Each window independently identifies EO, A, phi, dx_c and one dg per active sensor. ' ...
    ...
    'The known single-sync structured branch scans 300-1000 Hz and retains exactly the all-sensor voltage-gradient VP Top-3 for full nonlinear waveform refinement. No neighboring EO or NoGap VP candidate enters the final candidate set. Candidate screening is weighted, while the final full-wave voltage residual is unweighted. ' ...
    'The forward model uses a support-aware grouped-lap adaptive-SG low-speed template, PCHIP interpolation and the calibrated static gap-response increment. ' ...
    ...
    'The Foundation result supplies only the common raw fitting bundle; candidate and final evaluations reload the configured adaptive-SG template. ' ...
    'The program verifies in every window that dg=0 exactly reproduces the corresponding NoEta no-gap forward model.'];
Result.cfg = cfg;
% The internal model branch remains gap_only for data-structure
% compatibility; it is not the public name of the proposed method.
Result.MethodContract = struct( ...
    'schema_version', 'R5_PC1_SENSORWISE_DG_CONTRACT_v1', ...
    'formal_method_name', 'R5_PC1_AnchorSurface_SensorWiseDg_FullWave', ...
    'front_end', 'target_excluded_PC1_family_B2_registration_low_speed_anchor', ...
    'pc1_dynamic_state', false, ...
    'sensor_wise_dynamic_gap', true, ...
    'delta_mu_enabled', logical(cfg.includeGapTiltShift), ...
    'delta_tau_enabled', logical(cfg.includeGapTiltShift), ...
    'stage', 'gap_aware', ...
    'frequency_structure', cfg.frequencyStructureMode, ...
    'structure_selection', cfg.structureSelection, ...
    'unknown_structure_criterion', cfg.unknownStructureCriterion, ...
    'candidate_method', cfg.candidateMethod, ...
    'low_speed_template_method', packageCfg.lowSpeed.templateMethod, ...
    'forward_interpolation', packageCfg.lowSpeed.interpolationMethod, ...
    ...
    'data_residual', 'plain_unweighted_voltage_SSE', ...
    'optimization_objective', ...
        'voltage_SSE_plus_overshoot_penalty_plus_dg_regularization_plus_invalid_penalty', ...
    'plain_rmse_field', 'plainRmseMv_is_unpenalized_waveform_RMSE', ...
    'weighted_rmse_field', 'weightedRmseMv_is_objective_equivalent_RMSE', ...
    'eta_policy', 'zero', ...
    'model', 'gap_only', ...
...
if strcmpi(cfg.localBundleSource,'foundation_step05_bundle')
    Result.FoundationWindowCoverage = [actualFoundationWindows, expectedFoundationWindows];
    Result.Main06WindowLimit = cfg.maxWindows;
end
...
preferred = fullfile(templateDir, 'Template_AdaptiveSG.mat');
...
[Template, templateFile] = load_step04_template_file_for_blade_local( ...
    templateFile, targetBlade, sensorIds, sensorTag);
end

function [Template, templateFile] = load_step04_template_file_for_blade_local( ...
        templateFile, targetBlade, sensorIds, sensorTag)
if ~isfile(templateFile)
    error('Configured low-speed template does not exist: %s', templateFile);
end
...
resultFile = '';
sensorTag = ['S', sprintf('%d', sensorIds)];
foundationRoots = {fullfile(rootDir, 'results', 'fixed_gap'), ...
    fullfile(rootDir, 'inputs', 'prepared', 'foundation')};
files = [];
for ir = 1:numel(foundationRoots)
    pattern = fullfile(foundationRoots{ir}, 'step05*', '*', sprintf( ...
    'Result_Step05_NoEtaVPTopKDirectTemplate_B%d_%s_20251222.mat', targetBlade, sensorTag));
    files = [files; dir(pattern)]; %#ok<AGROW>
end
if isempty(files)
    return;
end
...
if isfield(S.Result, 'TargetBlade')
    Source.TargetBlade = S.Result.TargetBlade;
else
    Source.TargetBlade = NaN;
end
...
A = Source.AnalysisSettings;
if ~isfield(Source, 'TargetBlade') || double(Source.TargetBlade) ~= double(cfg.targetBlade)
    sourceBlade = NaN;
    if isfield(Source, 'TargetBlade'), sourceBlade = double(Source.TargetBlade); end
    error('Step07K:FoundationBladeMismatch', ...
        'Foundation blade B%g does not match the selected resonance-region blade B%d.', ...
        sourceBlade, cfg.targetBlade);
end
...
for is = 1:numel(sensorIds)
    ...
    % Candidate generation and final optimization must call the same chosen
    % low-speed template. Never inherit stale template_v/template_dv_dx
    % arrays cached by the Foundation comparison bundle.
    F0(mask) = eval_corrected_template_local( ...
        Tpl, responseSurface, corr, corr.g0Mm, X(mask));
    Fx(mask) = eval_corrected_template_derivative_local( ...
        Tpl, responseSurface, corr, corr.g0Mm, X(mask), cfg.derivativeStepMm);
...
% Keep the fixed nested baseline structurally compatible with the refined
% gap-aware fit record.  Continuous-frequency refinement is intentionally
% applied only to gap models, but downstream comparison tables require the
% integer-frequency fields for every model.
detail.integerFrequencyHz = detail.freqHz;
detail.frequencyOffsetHz = 0;
detail.integerPlainRmseMv = plainRmse;
detail.plainRmseDeltaMv = 0;
detail.frequencyRefinementExitFlag = NaN;
detail.frequencyRefinementIterations = 0;
...
rows = allRows(keep);
internalModel = string({rows.method}).';
method = internalModel;
method(method == "gap_only") = string(cfg.formalMethodName);
...
plot(T.window_id, T.gap_EO, '-', 'Color', style.red, 'LineWidth', 1.0, ...
    'Marker', 's', 'MarkerSize', 4.2, 'DisplayName', 'Proposed R5');
...
tf = ischar(p) || isstring(p);
if ~tf, return; end
p = char(p);
tf = (numel(p) >= 2 && p(2) == ':') || startsWith(p,'\\') || startsWith(p,'/');
end
