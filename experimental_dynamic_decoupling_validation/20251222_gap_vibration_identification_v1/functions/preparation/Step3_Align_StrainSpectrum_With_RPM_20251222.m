function result = Step3_Align_StrainSpectrum_With_RPM_20251222(case_name, base_alignment_offset_sec, order_candidates, freq_plot_hz, show_plots)
...
strain_preprocess_method = 'linear';

fprintf('>>> [Step3Align][%s] Preparing Step3 products...\n', case_name);
step3_result = Step3_Displacement_Calculation_20251222(case_name, base_alignment_offset_sec, false);
...
v_strain_for_stft = preprocess_strain_signal_local(v_strain, Fs_strain, strain_preprocess_method);

fprintf('>>> [Step3Align][%s] Strain file: %s\n', case_name, step3_result.strain_file);
fprintf('>>> [Step3Align][%s] Strain STFT preprocess: %s\n', case_name, strain_preprocess_method);
fprintf('>>> [Step3Align][%s] RPM points: %d | order candidates: [%s]\n', ...
    case_name, numel(rpm_values), num2str(order_candidates));

[stft_time_s, stft_freq_hz, stft_amp] = compute_strain_stft_local(v_strain_for_stft, Fs_strain, ...
...
frot_strain_hz = postprocess_frot_trace_local(frot_strain_hz, cfg);
focus_info = build_focus_info_local(stft_time_s, frot_strain_hz, frot_confidence, step3_result, cfg);

tau_candidates = build_tau_candidates_local(stft_time_s, rpm_time_s, cfg.step3_align_tau_step_sec);
[tau_rmse_hz, tau_overlap_count, tau_focus_rmse_hz, tau_focus_count] = scan_tau_alignment_local( ...
    stft_time_s, frot_strain_hz, frot_confidence, rpm_time_s, rpm_values / 60, ...
    tau_candidates, focus_info.focus_mask, cfg.step3_align_min_valid_points, case_name);
[best_tau_sec, best_tau_rmse_hz, best_tau_overlap_mask, best_tau_focus_rmse_hz, ...
    best_tau_focus_mask, best_tau_mode] = pick_best_tau_local( ...
    stft_time_s, frot_strain_hz, rpm_time_s, rpm_values / 60, tau_candidates, tau_rmse_hz, ...
    tau_focus_rmse_hz, tau_focus_count, focus_info.focus_mask, cfg.step3_align_min_valid_points);
...
result.strain_preprocess_method = strain_preprocess_method;
...
result.tau_focus_rmse_hz = tau_focus_rmse_hz;
result.tau_focus_count = tau_focus_count;
...
result.best_tau_focus_rmse_hz = best_tau_focus_rmse_hz;
result.best_tau_focus_mask = best_tau_focus_mask;
result.best_tau_mode = best_tau_mode;
...
result.focus_info = focus_info;

save(fullfile(step3_result.btt_output_dir, 'Step3_Spectrum_RPM_Alignment_20251222.mat'), 'result');

fprintf('>>> [Step3Align][%s] Best tau = %.3f s | mode = %s | global RMSE = %.3f Hz', ...
    case_name, best_tau_sec, best_tau_mode, best_tau_rmse_hz);
if isfinite(best_tau_focus_rmse_hz)
    fprintf(' | focus RMSE = %.3f Hz', best_tau_focus_rmse_hz);
end
fprintf('\n');
...
if show_plots
    plot_alignment_results_local(result, t_strain_aligned, v_strain_for_stft, freq_plot_hz);
end
end
...
function y = preprocess_strain_signal_local(x, Fs, method)
x = x(:);
finiteX = isfinite(x);
if any(finiteX)
...
switch lower(string(method))
    ...
    otherwise
        error('Unknown strain_preprocess_method: %s', method);
end
...
[S, f_axis, t_local] = spectrogram(v_strain, window, noverlap, nfft, Fs_strain);
...
function frot_out = postprocess_frot_trace_local(frot_in, cfg)
frot_out = frot_in(:);
if isfield(cfg, 'step3_align_frot_median_span')
    med_span = max(3, round(cfg.step3_align_frot_median_span));
    if mod(med_span, 2) == 0
        med_span = med_span + 1;
    end
    frot_out = smoothdata(frot_out, 'movmedian', med_span);
end
if isfield(cfg, 'step3_align_frot_sgolay_span')
    sg_span = max(5, round(cfg.step3_align_frot_sgolay_span));
    if mod(sg_span, 2) == 0
        sg_span = sg_span + 1;
    end
    sg_span = min(sg_span, numel(frot_out) - 1 + mod(numel(frot_out), 2));
    if sg_span >= 5 && sg_span <= numel(frot_out)
        try
            frot_out = smoothdata(frot_out, 'sgolay', sg_span);
        catch
        end
    end
end
...
function focus_info = build_focus_info_local(stft_time_s, frot_strain_hz, frot_confidence, step3_result, cfg)
focus_mask = isfinite(stft_time_s) & isfinite(frot_strain_hz) & isfinite(frot_confidence);
dt = median(diff(stft_time_s), 'omitnan');
if ~isfinite(dt) || dt <= 0
    dt = 1;
end

conf_norm = normalize_01_local(frot_confidence);
slope = abs([0; diff(frot_strain_hz)]) / dt;
slope_norm = normalize_01_local(slope);
conf_thr = quantile(conf_norm(isfinite(conf_norm)), 0.55);
slope_thr = quantile(slope_norm(isfinite(slope_norm)), 0.55);

focus_mask = focus_mask & (conf_norm >= conf_thr) & (slope_norm >= slope_thr);
focus_mode = "confidence_and_slope";
activity_window = [NaN, NaN];

if isfield(cfg, 'step3_align_use_activity_window') && cfg.step3_align_use_activity_window ...
        && isfield(step3_result, 'activity_info') && isfield(step3_result.activity_info, 'active_window') ...
        && numel(step3_result.activity_info.active_window) == 2
    activity_window = step3_result.activity_info.active_window(:).';
    pad_sec = 0;
    if isfield(cfg, 'step3_align_activity_pad_sec')
        pad_sec = cfg.step3_align_activity_pad_sec;
    end
    activity_window = activity_window + [-pad_sec, pad_sec];
    in_window = stft_time_s >= activity_window(1) & stft_time_s <= activity_window(2);
    if nnz(focus_mask & in_window) >= max(20, floor(cfg.step3_align_min_valid_points / 2))
        focus_mask = focus_mask & in_window;
        focus_mode = "confidence_and_slope+activity_window";
    elseif nnz(in_window) >= max(20, floor(cfg.step3_align_min_valid_points / 2))
        focus_mask = in_window;
        focus_mode = "activity_window_fallback";
    end
end

if nnz(focus_mask) < max(20, floor(cfg.step3_align_min_valid_points / 2))
    focus_mask = isfinite(stft_time_s) & isfinite(frot_strain_hz);
    focus_mode = "global_fallback";
end

focus_info = struct();
focus_info.focus_mask = focus_mask(:);
focus_info.focus_mode = char(focus_mode);
focus_info.activity_window = activity_window;
focus_info.focus_count = nnz(focus_mask);
end


function [tau_rmse_hz, tau_overlap_count, tau_focus_rmse_hz, tau_focus_count] = scan_tau_alignment_local(stft_time_s, frot_strain_hz, frot_confidence, rpm_time_s, btt_frot_hz, tau_candidates, focus_mask_base, min_valid_points, case_name)
num_taus = numel(tau_candidates);
tau_rmse_hz = nan(num_taus, 1);
tau_overlap_count = zeros(num_taus, 1);
tau_focus_rmse_hz = nan(num_taus, 1);
tau_focus_count = zeros(num_taus, 1);
...
for iTau = 1:num_taus
    ...
    mask = isfinite(btt_interp) & isfinite(frot_strain_hz);
    ...
    focus_mask = mask & focus_mask_base;
    tau_focus_count(iTau) = nnz(focus_mask);
    if tau_focus_count(iTau) >= max(20, floor(min_valid_points / 2))
        err_focus = frot_strain_hz(focus_mask) - btt_interp(focus_mask);
        w_focus = frot_confidence(focus_mask) - min(frot_confidence(focus_mask));
        if sum(w_focus) < eps
            w_focus = ones(size(err_focus));
        end
        tau_focus_rmse_hz(iTau) = sqrt(sum(w_focus .* err_focus .^ 2) / sum(w_focus));
    end
end
end


function [best_tau_sec, best_tau_rmse_hz, best_overlap_mask, best_tau_focus_rmse_hz, best_focus_mask, best_tau_mode] = pick_best_tau_local(stft_time_s, frot_strain_hz, rpm_time_s, btt_frot_hz, tau_candidates, tau_rmse_hz, tau_focus_rmse_hz, tau_focus_count, focus_mask_base, min_valid_points)
focus_min = max(20, floor(min_valid_points / 2));
valid_focus = isfinite(tau_focus_rmse_hz) & tau_focus_count >= focus_min;
if any(valid_focus)
    idx_candidates = find(valid_focus);
    [~, rel_idx] = min(tau_focus_rmse_hz(valid_focus));
    idx = idx_candidates(rel_idx);
    best_tau_mode = 'focus';
else
...
best_focus_mask = best_overlap_mask & focus_mask_base;
best_tau_rmse_hz = tau_rmse_hz(idx);
best_tau_focus_rmse_hz = tau_focus_rmse_hz(idx);
end
...
fig1 = figure('Name', sprintf('20251222 Spectrum-RPM Alignment - %s', result.case_name), ...
...
fig2 = figure('Name', sprintf('20251222 Black-Line Extraction Detail - %s', result.case_name), ...
...
if isfield(result, 'best_tau_focus_mask') && any(result.best_tau_focus_mask)
    plot(result.stft_time_s(result.best_tau_focus_mask), result.frot_strain_hz(result.best_tau_focus_mask), '.', ...
        'Color', [0.20 0.20 1.00], 'MarkerSize', 8, 'DisplayName', 'Focus region');
end
xlabel('Aligned time (s)');
ylabel('Frequency (Hz)');
if isfield(result, 'best_tau_focus_rmse_hz') && isfinite(result.best_tau_focus_rmse_hz)
    title(sprintf('Synchronous base frequency alignment | tau = %.3f s | focus RMSE = %.2f Hz', ...
        result.best_tau_sec, result.best_tau_focus_rmse_hz));
else
...
function y = normalize_01_local(x)
x = x(:);
...
if xmax <= xmin + eps
    ...
    y(finite_mask) = (x(finite_mask) - xmin) ./ (xmax - xmin);
end
end
