...
%   4. Weighted VP scans all EO values without sensor eta; its Top-K enters
%      full nonlinear refinement, whose final voltage residual is unweighted.
...
gapRouteDir = packageCfg.paths.utilities;
if exist(fullfile(gapRouteDir, 'ResonanceRegionCatalog_20251222.m'), 'file') == 2
    [regionSelection, ~] = ResonanceRegionCatalog_20251222();
else
    regionSelection = struct('representativeBladeId', 1, ...
        'analysisStartTimeSec', 50.2, 'targetBladePasses', 20, ...
        'windowBladePasses', 3, 'slidingStepBladePasses', 1);
end
...
S.output_dir = fullfile(packageCfg.paths.foundationResults, ...
    'step05_noeta_vptopk_direct_template');
S.figure_dir = fullfile(packageCfg.paths.figures, ...
    'foundation_noeta_vptopk_direct_template');
...
if ~isempty(run_tag_env)
    ...
    S.output_dir = fullfile(packageCfg.paths.foundationResults, ...
        ['step05_noeta_vptopk_direct_template_', run_tag_env]);
    S.figure_dir = fullfile(packageCfg.paths.figures, ...
        ['step05_noeta_vptopk_direct_template_', run_tag_env]);
end
...
S.freq_search_hz = [300 1000];
S.eo_pad = 2;

S.eo_candidate_policy = 'adaptive_full_fit';
S.vp_top_k_min = 3;
S.vp_top_k_max = 3;
S.vp_top_rank_keep = 3;
...
if nnz(valid) < 5
    ...
    residual_sse = nansum(res(valid).^2);
