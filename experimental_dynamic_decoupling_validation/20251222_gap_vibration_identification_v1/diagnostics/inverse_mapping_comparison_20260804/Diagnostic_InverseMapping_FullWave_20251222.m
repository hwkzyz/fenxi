...
cfg.eoCandidateMode = 'inverse_mapping';
cfg.inverseTopK = 3;
cfg.inverseMaxTopK = 3;
cfg.inverseExpandRhoThreshold = 0.05;
...
for iw = 1:numWindows
    ...
    if strcmpi(cfg.localBundleSource, 'foundation_step05_bundle')
        localDirectCoreSeedTable = solve_inverse_seed_scan_local(coreBundleFull, eoCandidates, cfg);
    else
    ...
    seedTable = solve_inverse_seed_scan_local(bundle, eoCandidates, cfg);
    ...
    selectedEO = select_inverse_candidates_local(seedTable, cfg);
    if ~isempty(cfg.diagnosticForcedEO)
    ...
    expectedTop3 = seedTable.EO(1:min(numel(selectedEO), height(seedTable))).';
    assert(isequal(selectedEO(:).', expectedTop3(:).'), ...
        'Step07K:InverseCandidateViolation', ...
...
function seedTable = solve_inverse_seed_scan_local(bundle, eoCandidates, cfg)
...
for is = 1:nSensor
    ...
    model(is).evaluate = @(dg,x) inverse_forward_local(Tpl, rs, corr, dg, x);
end
[inv, invSummary] = step07jcore.inverse_map_experimental_displacement( ...
    bundle, zeros(nSensor,1), 0, model, cfg);
[seedTable, invInfo] = step07jcore.solve_inverse_mapping_seed(inv, bundle, cfg);
seedTable.linearRmseMm = seedTable.weightedRmseMm;
seedTable.linearRmseMv = seedTable.weightedRmseMm;
seedTable.pointCount = repmat(bundle.pointCount, height(seedTable), 1);
seedTable.gradientRef = repmat(median(abs(inv.dVoltageDx(inv.valid)),'omitnan'), height(seedTable), 1);
seedTable.fixedSensorEtaMm = repmat(zeros(1,nSensor), height(seedTable), 1);
seedTable.noGapVPRank = nan(height(seedTable),1);
gapScore = inf(height(seedTable),1);
for i=1:height(seedTable)
    theta0 = initial_theta_local(seedTable.A(i), seedTable.phi(i), seedTable.dx(i), cfg, 'gap_only', nSensor);
    detail = evaluate_static_warp_waveform_local(theta0, seedTable.EO(i), bundle, cfg, 'gap_only');
    gapScore(i) = detail.weightedRmseMv;
end
seedTable.gapScreenRmseMv = gapScore;
seedTable.fallbackRmseMv = gapScore;
seedTable.inverseValidFraction = repmat(invSummary.inverse_valid_fraction,height(seedTable),1);
seedTable.inverseVoltageRmseMv = repmat(invSummary.inverse_voltage_rmse,height(seedTable),1);
seedTable.candidateRho = repmat(invInfo.candidate_rho,height(seedTable),1);
end

function selectedEO = select_inverse_candidates_local(seedTable, cfg)
topK = min(cfg.inverseTopK,height(seedTable));
selectedEO=seedTable.EO(1:topK).';
end

function [v,aux] = inverse_forward_local(Tpl,rs,corr,dg,x)
[v,baseInfo] = step07jcore.evaluate_experimental_sensor_voltage(Tpl,rs,corr,dg,x,0,0);
h=1e-4;
