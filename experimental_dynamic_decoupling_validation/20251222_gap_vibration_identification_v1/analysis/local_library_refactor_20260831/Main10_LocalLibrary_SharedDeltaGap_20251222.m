...
% increment dg_s for every active sensor. eta_s is identically zero. Static
% calibration and low-speed localization are frozen upstream in a local
% no-vibration waveform library Tnv(x,dg).
%
...
%
% The high-speed forward model only interpolates Tnv at the candidate dg and
% evaluates it at x-dx-A*sin(EO*theta+phi). It does not re-evaluate Fcal.

clear; clc;

%% 1. Paths and settings
scriptDir = fileparts(mfilename('fullpath'));
thisDir = fileparts(fileparts(scriptDir));
addpath(thisDir);
...
localLibraryFile = strtrim(getenv('STEP07J_LOCAL_GAP_LIBRARY_FILE'));
if isempty(localLibraryFile)
    localLibraryFile = fullfile(scriptDir, ...
        'LocalNoVibrationGapLibrary_20251222_B1_S123_nob_20260831.mat');
end
assert(isfile(localLibraryFile), 'Missing local no-vibration gap library: %s', localLibraryFile);
Sloc = load(localLibraryFile, 'LocalNoVibrationGapLibrary');
[CorrectedGapLibrary, localGapLower, localGapUpper] = ...
    attach_local_no_vibration_library_local(CorrectedGapLibrary, ...
    Sloc.LocalNoVibrationGapLibrary, cfg.analysisSensors);
cfg.localDeltaGapLowerMm = localGapLower;
cfg.localDeltaGapUpperMm = localGapUpper;
...
fprintf('\n=== Local no-vibration library: single-sync NoEta gap/full-wave ===\n');
fprintf('Template: %s\n', templateFile);
fprintf('DynamicMap: %s\n', dynamicFile);
fprintf('Corrected gap library: %s\n', correctedLibFile);
fprintf('Local no-vibration gap library: %s\n', localLibraryFile);
if strcmpi(cfg.localBundleSource, 'foundation_step05_bundle')
...
Result.method = 'local_no_vibration_library_single_sync_full_wave';
Result.description = ['Each window independently identifies EO, A, phi, dx_c and one dg per active sensor. ' ...
    'eta_s is identically zero; the local no-vibration gap library is frozen upstream. ' ...
    ...
    'The forward model interpolates the precomputed Tnv(x,dg) library and applies vibration only through the x coordinate. ' ...
...
Result.MethodContract = struct( ...
    'schema_version', 'local_no_vibration_library_contract_v1', ...
    'stage', 'gap_aware', ...
    ...
    'forward_interpolation', 'linear_delta_pchip_x', ...
    'forward_model', 'Tnv(x-dx-A*sin(EO*theta+phi),deltaG)', ...
    'eo_top_k', 3, ...
...
Result.localNoVibrationGapLibraryFile = localLibraryFile;
...
if isfield(cfg, 'localDeltaGapLowerMm') && isfield(cfg, 'localDeltaGapUpperMm')
    cfgOut.deltaGapLowerMm = max(cfgOut.deltaGapLowerMm, ...
        expand_vector_local(cfg.localDeltaGapLowerMm, nSensor));
    cfgOut.deltaGapUpperMm = min(cfgOut.deltaGapUpperMm, ...
        expand_vector_local(cfg.localDeltaGapUpperMm, nSensor));
end
...
for is = 1:nSensor
    ...
    if all(isfinite(values)) && values(2) <= values(1) && values(1) <= values(3)
        ...
        if isfield(cfg, 'localDeltaGapLowerMm') && isfield(cfg, 'localDeltaGapUpperMm')
            cfgOut.deltaGapLowerMm(is) = max(cfgOut.deltaGapLowerMm(is), ...
                cfg.localDeltaGapLowerMm(is));
            cfgOut.deltaGapUpperMm(is) = min(cfgOut.deltaGapUpperMm(is), ...
                cfg.localDeltaGapUpperMm(is));
            cfgOut.deltaGapCenterMm(is) = min(max(cfgOut.deltaGapCenterMm(is), ...
                cfgOut.deltaGapLowerMm(is)), cfgOut.deltaGapUpperMm(is));
        end
    end
end
end
...
if isfield(cfg, 'deltaGapCenterMm')
    if is_shared_gap_mode_local(modeName)
        theta0(idxDg) = mean(cfg.deltaGapCenterMm, 'omitnan');
    else
...
if is_shared_gap_mode_local(modeName)
    theta0(idxDg) = max(min(mean(dgSeed, 'omitnan'), cfg.deltaGapLimitMm), -cfg.deltaGapLimitMm);
else
...
if ~isempty(idxDg)
    if is_shared_gap_mode_local(modeName)
        theta0(idxDg) = mean(integerFit.deltaGapMm(1:nSensor), 'omitnan');
    else
...
if ~isempty(idxDg)
    if isfield(cfg, 'deltaGapLowerMm') && isfield(cfg, 'deltaGapUpperMm')
        if is_shared_gap_mode_local(modeName)
            lbTheta(idxDg) = mean(cfg.deltaGapLowerMm, 'omitnan');
            ubTheta(idxDg) = mean(cfg.deltaGapUpperMm, 'omitnan');
        else
    ...
    if ~is_shared_gap_mode_local(modeName) && isfield(cfg, 'gapActiveSensorMask') && numel(cfg.gapActiveSensorMask) == nSensor
...
if ~isempty(idxDg)
    ...
    if is_shared_gap_mode_local(modeName)
        patterns(9, idxDg) = theta0(idxDg) + 0.02;
    else
...
need = 3 + mode_parameter_count_local(modeName, nSensor);
if numel(theta) < need
...
if ~isempty(idxDg)
    if isfield(cfg, 'deltaGapLowerMm') && isfield(cfg, 'deltaGapUpperMm')
        if is_shared_gap_mode_local(modeName)
            lower = mean(cfg.deltaGapLowerMm, 'omitnan');
            upper = mean(cfg.deltaGapUpperMm, 'omitnan');
        else
...
if is_shared_gap_mode_local(modeName) && ~isempty(dg)
    dg = repmat(dg(1), nSensor, 1);
end
...
function n = mode_parameter_count_local(modeName, nSensor)
if is_shared_gap_mode_local(modeName)
    n = 1;
else
    n = mode_extra_count_local(modeName) * nSensor;
end
end

function tf = mode_has_gap_local(modeName)
tf = any(strcmpi(modeName, {'gap_only','gap_shared','gap_tilt','gap_tilt_shift'}));
end

function tf = is_shared_gap_mode_local(modeName)
tf = strcmpi(modeName, 'gap_shared');
end
...
xWarpRaw = xOpr(:) - dtau;
if isfield(corr, 'localNoVibration')
    if abs(dmu) > eps
        model = nan(size(xWarpRaw));
        overshoot = inf(size(xWarpRaw));
        return;
    end
    [model, overshoot] = eval_local_no_vibration_template_local( ...
        corr.localNoVibration, dg, xWarpRaw);
    return;
end
xLo = min(Tpl.x_grid(:));
xHi = max(Tpl.x_grid(:));
...
xDyn = xOpr(:) - dtau;
if isfield(corr, 'localNoVibration')
    if abs(dmu) > eps || abs(dtau) > eps
        deltaV = nan(size(xDyn));
        overshoot = inf(size(xDyn));
        return;
    end
    [vDyn, overDyn] = eval_local_no_vibration_template_local( ...
        corr.localNoVibration, dg, xDyn);
    [vBase, overBase] = eval_local_no_vibration_template_local( ...
        corr.localNoVibration, 0, xDyn);
    deltaV = vDyn - vBase;
...
if isfield(corr, 'localNoVibration')
    [model, overshoot] = eval_local_no_vibration_template_local( ...
        corr.localNoVibration, g - corr.g0Mm, xLowRaw);
    return;
end
xLo = min(Tpl.x_grid(:));
xHi = max(Tpl.x_grid(:));
...
function [C, lower, upper] = attach_local_no_vibration_library_local(C, Lall, sensorIds)
assert(C.targetBlade == Lall.targetBlade, ...
    'The local library target blade does not match the corrected library.');
lower = nan(1, numel(sensorIds));
upper = nan(1, numel(sensorIds));
for is = 1:numel(sensorIds)
    sid = sensorIds(is);
    iCorr = find([C.sensor.sensorId] == sid, 1, 'first');
    iLocal = find([Lall.sensor.sensorId] == sid, 1, 'first');
    assert(~isempty(iCorr) && ~isempty(iLocal), ...
        'Sensor %d is missing from a required gap library.', sid);
    L = Lall.sensor(iLocal);
    assert(abs(L.zeroGapEquivalenceErrorMv) <= 1e-9, ...
        'Sensor %d local library does not reproduce the low-speed anchor at deltaG=0.', sid);
    C.sensor(iCorr).localNoVibration = L;
    lower(is) = L.deltaGMinMm;
    upper(is) = L.deltaGMaxMm;
end
end

function [model, overshoot] = eval_local_no_vibration_template_local(L, dg, xq)
xq = xq(:);
xGrid = L.xGridMm(:);
...
gLo = L.deltaGMinMm;
gHi = L.deltaGMaxMm;
xOver = max(xLo - xq, 0) + max(xq - xHi, 0);
gOver = max(gLo - dg, 0) + max(dg - gHi, 0);
overshoot = hypot(xOver, gOver);
if ~isfinite(dg) || dg < gLo - 1e-12 || dg > gHi + 1e-12
    model = nan(size(xq));
    return;
end
dg = min(max(dg, gLo), gHi);
upperIndex = find(gGrid >= dg, 1, 'first');
lowerIndex = find(gGrid <= dg, 1, 'last');
if isempty(lowerIndex) || isempty(upperIndex)
    model = nan(size(xq));
    return;
end
yLower = interp1(xGrid, L.noVibrationWaveformMv(:, lowerIndex), ...
    xq, 'pchip', NaN);
if lowerIndex == upperIndex
    model = yLower;
    return;
end
yUpper = interp1(xGrid, L.noVibrationWaveformMv(:, upperIndex), ...
    xq, 'pchip', NaN);
alpha = (dg - gGrid(lowerIndex)) / ...
    (gGrid(upperIndex) - gGrid(lowerIndex));
model = (1 - alpha) .* yLower + alpha .* yUpper;
end
