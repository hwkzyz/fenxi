...
manifest = struct();
manifest.schema = 'R5_FORMAL_MANIFEST_V1';
manifest.method = 'B2 registration + PC1 ordered coefficient manifold + low-speed anchor';
manifest.methodFreezeId = 'R4R5_PC1_ANCHOR_SIDECAR_20260905';
manifest.targetBlades = [1 5];
manifest.familyFile = familyFile;
manifest.registrationFile = registrationFile;
manifest.localizationFile = localizationFile;
manifest.sidecarFiles = main10Libraries;
manifest.main10Unmodified = true;
manifest.sidecarContract = 'responseSurface.coeff only; sensor calibration preserved';
save(fullfile(outRoot,'R5_Formal_Manifest.mat'),'manifest','-v7.3');
end
