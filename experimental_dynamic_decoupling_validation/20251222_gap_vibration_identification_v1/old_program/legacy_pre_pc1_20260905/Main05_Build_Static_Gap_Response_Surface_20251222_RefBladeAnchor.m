...
for ig = 1:numel(trueGaps)
    ...
    % Measured passage offsets define the fixed blade-local registration.
    bladeCenters = nan(numel(bladeIds),1);
    for ib = 1:numel(bladeIds)
        q = keys.bladeId == bladeIds(ib);
        bladeCenters(ib) = median(keys.peakCenterSample(q), 'omitnan');
        anchorDiagnostics(ig,ib) = (bladeCenters(ib)-refCenter) / sampleRateHz * speedNow;
    end
    for ib = 1:numel(bladeIds)
        bid = bladeIds(ib);
        rows = T.bladeId == bid;
        ...
        tt = ((sampleIndex(:) - referenceCenters(ig)) * speedNow / sampleRateHz ...
            - median(anchorDiagnostics(:,ib), 'omitnan')) / speedNow;
        [tt, idxSort] = sort(tt(:));
        yy = yy(idxSort);
...
responseSurface.bladeOffsetMmFromB2 = median(anchorDiagnostics, 1, 'omitnan').';
