...
responseFile = packageCfg.files.responseSurface;
speedSummaryFile = fullfile(packageCfg.paths.calibrationWork, ...
    'Step05A_single_gap_speed_summary', ...
    'Step05A_Average_Calibration_Speed.csv');
regionFile = fullfile(packageCfg.paths.prepared, ...
    ...
    if is_highmap_cache_match_local(highMap, C0, resonanceRegionShortTag, ...
        ...
        if ~isfile(dynamicMapCacheFile)
            if isfield(highMap, 'lowSpeedTemplateFile') && isfile(highMap.lowSpeedTemplateFile)
                ...
                templateFile = packageCfg.files.lowSpeedTemplate;
            end
...
templateFile = packageCfg.files.lowSpeedTemplate;
templateSourceMode = 'bundled_foundation_step04_all_blades';
