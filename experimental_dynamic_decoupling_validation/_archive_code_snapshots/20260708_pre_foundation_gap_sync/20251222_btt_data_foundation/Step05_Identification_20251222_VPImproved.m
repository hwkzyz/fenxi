...
% VERSION: FOUNDATION_FULLRAW_TOP3_RMSE_20260705
%
% Key logic:
...
%   6. VP screening uses a separate high-information bundle
%      (main-pulse/dynamic-effective/query-safe). Final full-waveform RMSE
%      still uses the full query-safe or phase-safe waveform bundle.
%
% Forward model:
...
S = struct();
S.method_name = 'foundation_fullraw_vpscreen_top3_rmse';
S.version = 'FOUNDATION_FULLRAW_VPSCREEN_TOP3_RMSE_20260705';
...
S.output_dir = get_cfg_or_default_local(cfg, ...
    'step05_direct_template_output_dir', ...
...
S.eo_pad = 2;
% Foundation route: VP ranks EO candidates, then the top candidates enter
% the full-waveform bounded RMSE refinement.
S.top_k_eo = 3;
S.vp_include_sensor_eta = false;
S.vp_bundle_mode = 'legacy_screening_query_safe';
...
if ~isempty(top_k_eo_env)
    ...
    if isfinite(parsed_top_k_eo) && parsed_top_k_eo >= 1
        ...
        error('STEP05_TOP_K_EO must be a positive finite integer in the foundation Step05.');
    end
end
...
if ~isempty(core_mask_mode_env)
    error('STEP05_CORE_MASK_MODE changes the method route; official foundation Step05 uses query_safe_full_wave.');
end
...
if ~isempty(fit_sensor_eta_env)
    error('STEP05_FIT_SENSOR_ETA changes the method route; official foundation Step05 fits sensor eta.');
end

static_eta_env = strtrim(getenv('STEP05_STATIC_ETA_ENABLE'));
if ~isempty(static_eta_env)
    error('STEP05_STATIC_ETA_ENABLE changes the method route; use diagnostics for static-eta checks.');
end

phase_safe_env = strtrim(getenv('STEP05_PHASE_SAFE_EXPANSION'));
if ~isempty(phase_safe_env)
    error('STEP05_PHASE_SAFE_EXPANSION changes the method route; official foundation Step05 enables it.');
end
...
for ib = 1:numel(S.target_blades)
    ...
    method_file_tag = 'FoundationFullRawVPScreenTop3RMSE';
...
for iw = 1:height(WindowPlan)
    ...
    if S.phase_safe_expansion && strcmpi(fit_core.status, 'ok')
        ...
        if expanded_bundle.point_count >= S.min_fit_points && ...
            ...
            [expanded_vp_bundle, expanded_vp_info] = build_vp_screening_bundle_local( ...
                expanded_bundle, S, 'phase_safe_expanded');
            ...
            expand_info.expanded_eo_policy = 'phase_reference_selects_points_only_vp_topk_full_rmse';
    ...
    WindowResult(iw).CoreVPBundlePreview = ...
        downsample_bundle_for_storage_local(core_vp_bundle, ...
        S.store_bundle_preview_points);
    ...
    if ~isempty(expanded_vp_bundle)
        WindowResult(iw).ExpandedVPBundlePreview = ...
            downsample_bundle_for_storage_local(expanded_vp_bundle, ...
            S.store_bundle_preview_points);
    end
...
method.frequency_decision = ['single-window EO is selected by VP top-k plus full-waveform RMSE refinement; ' ...
    'candidate ambiguity is reported through gap metrics and joint EO summary, not automatically rejected'];
...
method.core_point_policy = 'finite & inside_domain & inside_query_guard';
method.vp_bundle_mode = string(getfield_default_local(S, 'vp_bundle_mode', 'legacy_screening_query_safe'));
method.vp_point_policy = ['VP EO screening uses a separate high-information bundle ' ...
    '(finite/domain + dynamic_effective + main_pulse within the current query-safe or phase-safe fit bundle); ' ...
    'final RMSE still uses the full selected waveform bundle including peak points.'];
method.frequency_policy = 'VP voltage-residual top-3, full waveform RMSE refinement';
method.parameter_bound_policy = 'formal A, dx_c, and sensor eta bounds are enforced during objective evaluation and reporting';
method.phase_safe_policy = ['phase reference only selects phase-safe expanded points; ' ...
    'expanded pass reruns VP top-k and full-waveform RMSE, so phase reference is not an EO prior'];
...
function [vp_bundle, info] = build_vp_screening_bundle_local(fit_bundle, S, stage_name)
%BUILD_VP_SCREENING_BUNDLE_LOCAL Build a high-information bundle for VP EO screening.
%
% The verified 20251222_low_speed_rotating_calibration route does not use
% the whole waveform window for first-order VP screening. It first keeps the
% single main pulse, dynamically informative points, finite/template-domain
% points, then applies the current query-safe or phase-safe selection. In
% this foundation Step05, the input fit_bundle is already the query-safe core
% or the phase-safe expanded full-waveform bundle. This helper only creates
% the VP screening subset; the final nonlinear waveform RMSE still uses the
% original fit_bundle, including peak points.
if nargin < 3 || isempty(stage_name)
    stage_name = 'unknown';
end

info = make_empty_vp_screening_info_local(stage_name);
info.requested_policy = string(getfield_default_local(S, 'vp_bundle_mode', 'legacy_screening_query_safe'));

if isempty(fit_bundle) || ~isstruct(fit_bundle) || ~isfield(fit_bundle, 'x') || isempty(fit_bundle.x)
    vp_bundle = fit_bundle;
    info.policy = 'empty_fit_bundle';
    info.fallback_reason = 'empty_fit_bundle';
    return;
end

n = numel(fit_bundle.x);
finite = true(n, 1);
required_numeric = {'x','v','theta','template_v','template_dv_dx','fit_weight'};
for i = 1:numel(required_numeric)
    f = required_numeric{i};
    if isfield(fit_bundle, f) && numel(fit_bundle.(f)) == n
        finite = finite & isfinite(fit_bundle.(f)(:));
    end
end
if isfield(fit_bundle, 'finite_mask') && numel(fit_bundle.finite_mask) == n
    finite = finite & logical(fit_bundle.finite_mask(:));
end

inside_domain = true(n, 1);
if isfield(fit_bundle, 'inside_domain_mask') && numel(fit_bundle.inside_domain_mask) == n
    inside_domain = logical(fit_bundle.inside_domain_mask(:));
end

dynamic_effective = true(n, 1);
if isfield(fit_bundle, 'dynamic_effective_mask') && numel(fit_bundle.dynamic_effective_mask) == n
    dynamic_effective = logical(fit_bundle.dynamic_effective_mask(:));
end

main_pulse = true(n, 1);
if isfield(fit_bundle, 'main_pulse_mask') && numel(fit_bundle.main_pulse_mask) == n
    main_pulse = logical(fit_bundle.main_pulse_mask(:));
end

base_mask = finite & inside_domain;
legacy_screen_mask = base_mask & dynamic_effective & main_pulse;

info.fit_point_count = get_bundle_point_count_local(fit_bundle);
info.finite_domain_count = nnz(base_mask);
info.dynamic_effective_count = nnz(base_mask & dynamic_effective);
info.main_pulse_count = nnz(base_mask & main_pulse);
info.legacy_screening_count = nnz(legacy_screen_mask);

policy = lower(strtrim(char(info.requested_policy)));
switch policy
    case {'legacy_screening_query_safe','legacy_screening','legacy_dynamic_main','dynamic_main'}
        mask = legacy_screen_mask;
        info.policy = 'legacy_screening_query_safe';
    case {'dynamic_effective_query_safe','dynamic_effective'}
        mask = base_mask & dynamic_effective;
        info.policy = 'dynamic_effective_query_safe';
    case {'main_pulse_query_safe','main_pulse'}
        mask = base_mask & main_pulse;
        info.policy = 'main_pulse_query_safe';
    case {'full_fit_bundle','fit_bundle','none'}
        mask = base_mask;
        info.policy = 'full_fit_bundle';
    otherwise
        mask = legacy_screen_mask;
        info.policy = 'legacy_screening_query_safe';
        info.fallback_reason = sprintf('unknown_policy_%s', policy);
end

[min_points, min_passes] = get_vp_screening_minima_local(S);
if ~vp_mask_has_enough_support_local(fit_bundle, mask, min_points, min_passes)
    fallback_mask = base_mask;
    if vp_mask_has_enough_support_local(fit_bundle, fallback_mask, min_points, min_passes)
        if strlength(string(info.fallback_reason)) == 0
            info.fallback_reason = 'legacy_screening_too_few_points_fallback_to_full_fit';
        else
            info.fallback_reason = sprintf('%s|legacy_screening_too_few_points_fallback_to_full_fit', info.fallback_reason);
        end
        mask = fallback_mask;
        info.policy = sprintf('%s__fallback_full_fit_bundle', char(string(info.policy)));
    else
        if strlength(string(info.fallback_reason)) == 0
            info.fallback_reason = 'all_screening_masks_too_few_points';
        else
            info.fallback_reason = sprintf('%s|all_screening_masks_too_few_points', info.fallback_reason);
        end
        mask = finite;
        info.policy = sprintf('%s__fallback_finite_only', char(string(info.policy)));
    end
end

vp_bundle = select_bundle_points_local(fit_bundle, mask, S);
vp_bundle.selection_pass = sprintf('%s_vp_screening', stage_name);
vp_bundle.vp_screening_policy = info.policy;
vp_bundle.vp_screening_fallback = info.fallback_reason;
vp_bundle.full_fit_point_count = get_bundle_point_count_local(fit_bundle);

info.point_count = get_bundle_point_count_local(vp_bundle);
info.valid_pass_count = get_bundle_valid_pass_count_local(vp_bundle);
info.point_fraction_of_fit = info.point_count ./ max(info.fit_point_count, 1);
end

function info = make_empty_vp_screening_info_local(stage_name)
if nargin < 1 || isempty(stage_name)
    stage_name = 'unknown';
end
info = struct();
info.stage = string(stage_name);
info.requested_policy = "";
info.policy = "";
info.fallback_reason = "";
info.fit_point_count = NaN;
info.point_count = NaN;
info.valid_pass_count = NaN;
info.point_fraction_of_fit = NaN;
info.finite_domain_count = NaN;
info.dynamic_effective_count = NaN;
info.main_pulse_count = NaN;
info.legacy_screening_count = NaN;
end

function [min_points, min_passes] = get_vp_screening_minima_local(S)
if isfield(S, 'vp_min_fit_points') && isfinite(S.vp_min_fit_points)
    min_points = S.vp_min_fit_points;
else
    min_points = S.min_fit_points;
end
if isfield(S, 'vp_min_valid_pass_count') && isfinite(S.vp_min_valid_pass_count)
    min_passes = S.vp_min_valid_pass_count;
else
    min_passes = S.min_valid_pass_count;
end
end

function ok = vp_mask_has_enough_support_local(bundle, mask, min_points, min_passes)
mask = mask(:);
ok = nnz(mask) >= min_points;
if ~ok
    return;
end
if isfield(bundle, 'pass_id') && numel(bundle.pass_id) == numel(mask)
    pass_ids = bundle.pass_id(mask);
    pass_ids = pass_ids(isfinite(pass_ids));
    ok = numel(unique(pass_ids)) >= min_passes;
end
end
...
row = struct( ...
    ...
    'final_vp_point_count', NaN, ...
    'core_vp_policy', "", ...
    'expanded_vp_policy', "", ...
    'final_vp_policy', "", ...
    'core_vp_fallback', "", ...
    'expanded_vp_fallback', "", ...
    'final_vp_fallback', "", ...
...
row.final_vp_point_count = getfield_default_local(result, 'VPPointCount', NaN);
row.core_vp_policy = string(getfield_default_local(result, 'CoreVPPolicy', ""));
row.expanded_vp_policy = string(getfield_default_local(result, 'ExpandedVPPolicy', ""));
row.final_vp_policy = string(getfield_default_local(result, 'VPPolicy', ""));
row.core_vp_fallback = string(getfield_default_local(result, 'CoreVPFallback', ""));
row.expanded_vp_fallback = string(getfield_default_local(result, 'ExpandedVPFallback', ""));
row.final_vp_fallback = string(getfield_default_local(result, 'VPFallback', ""));
