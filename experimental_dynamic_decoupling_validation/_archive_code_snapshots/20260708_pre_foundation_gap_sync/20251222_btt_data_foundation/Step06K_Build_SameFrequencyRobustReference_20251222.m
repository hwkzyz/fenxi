%% Step06K_Build_SameFrequencyRobustReference_20251222.m
...
for iCase = 1:numel(targetCases)
    ...
    Obs = load_step03_observation_table_step06k_local(files.step03_bundle_mat);
    ...
    FESpectrumReference = build_fe_spectrum_reference_step06k_local( ...
        TrefTarget, FEWidthSensitivity, FEReference, cfg);
    BttCorrectionPoints = select_btt_correction_points_step06k_local( ...
        TargetPoints, FESpectrumReference, cfg);
    [PrimaryFit, PrimaryGrid] = build_fe_btt_grid_reference_step06k_local( ...
        BttCorrectionPoints, TrefTarget, TargetOnlyFit, FEReference, ...
        RegionTable, cfg);
    ...
    BootstrapTable = bootstrap_primary_grid_reference_step06k_local( ...
        BttCorrectionPoints, TrefTarget, TrefTarget, TargetOnlyFit, ...
    ...
    Summary = build_summary_step06k_local(caseName, targetEO, ...
        ...
        SensorResiduals, FEReference, FEWidthSensitivity, ...
        FESpectrumReference, BttCorrectionPoints, S06I, cfg);
    ...
    summaryCsv = fullfile(outDir, ...
        'Step06K_SameFrequencyRobustReference_Summary_20251222.csv');
    regionCsv = fullfile(outDir, ...
        'Step06K_SameFrequencyRobustReference_RegionFits_20251222.csv');
    eoCsv = fullfile(outDir, ...
        'Step06K_SameFrequencyRobustReference_EOFits_20251222.csv');
    modelCsv = fullfile(outDir, ...
        'Step06K_SameFrequencyRobustReference_ModelComparison_20251222.csv');
    asyncRegionCsv = fullfile(outDir, ...
        'Step06K_SameFrequencyRobustReference_AsyncRegionFits_20251222.csv');
    targetPointsCsv = fullfile(outDir, ...
        'Step06K_SameFrequencyRobustReference_TargetFitPoints_20251222.csv');
    bttCorrectionCsv = fullfile(outDir, ...
        'Step06K_SameFrequencyRobustReference_BTTCorrectionPoints_20251222.csv');
    targetTsCsv = fullfile(outDir, ...
        'Step06K_SameFrequencyRobustReference_TargetTimeSeries_20251222.csv');
    bootCsv = fullfile(outDir, ...
        'Step06K_SameFrequencyRobustReference_PrimaryGridBootstrap_20251222.csv');
    sensorCsv = fullfile(outDir, ...
        'Step06K_SameFrequencyRobustReference_SensorResiduals_20251222.csv');
    feCsv = fullfile(outDir, ...
        'Step06K_SameFrequencyRobustReference_FEReference_20251222.csv');
    feWidthCsv = fullfile(outDir, ...
        'Step06K_SameFrequencyRobustReference_FEWidthSensitivity_20251222.csv');
    feSpectrumCsv = fullfile(outDir, ...
        'Step06K_SameFrequencyRobustReference_FESpectrumReference_20251222.csv');
    gridCsv = fullfile(outDir, ...
        'Step06K_SameFrequencyRobustReference_PrimaryGridSearch_20251222.csv');
    feBandSensitivityCsv = fullfile(outDir, ...
        'Step06K_SameFrequencyRobustReference_FEBandSensitivity_20251222.csv');
    matFile = fullfile(outDir, ...
        'Step06K_SameFrequencyRobustReference_20251222.mat');
    ...
    writetable(BttCorrectionPoints, bttCorrectionCsv);
    ...
    writetable(FESpectrumReference, feSpectrumCsv);
    ...
    save(matFile, 'Summary', 'RegionFits', 'EOFits', 'AsyncRegionFits', ...
        'ModelComparison', 'TargetFitPoints', 'BttCorrectionPoints', ...
        'TargetTimeSeries', ...
        'BootstrapTable', 'SensorResiduals', 'FEReference', ...
        'FEWidthSensitivity', 'FESpectrumReference', 'PrimaryGrid', 'FEBandSensitivity', ...
    ...
    [figFile, feFigFile] = plot_step06k_reference_local(TargetTimeSeries, ...
        ...
        BootstrapTable, FEWidthSensitivity, FESpectrumReference, ...
        Summary, figDir, cfg);
    ...
    fprintf(['Saved:\n  %s\n  %s\n  %s\n  %s\n  %s\n  %s\n', ...
        ...
        bttCorrectionCsv, targetTsCsv, bootCsv, sensorCsv, feCsv, feWidthCsv, ...
        feSpectrumCsv, gridCsv, feBandSensitivityCsv, matFile, figFile, feFigFile);
end
...
if ~isfield(cfg, 'step06i_sensor_ids') || isempty(cfg.step06i_sensor_ids)
    if isfield(cfg, 'optical_fiber_ids') && ~isempty(cfg.optical_fiber_ids)
        cfg.step06i_sensor_ids = cfg.optical_fiber_ids(:).';
    else
...
if ~isfield(cfg, 'step06i_random_seed') || isempty(cfg.step06i_random_seed)
    cfg.step06i_random_seed = 20251222;
end
...
if ~isfield(cfg, 'step06k_core_trim_fraction') || ...
        isempty(cfg.step06k_core_trim_fraction)
    cfg.step06k_core_trim_fraction = 0.15;
end
if ~isfield(cfg, 'step06k_core_trim_min_s') || ...
        isempty(cfg.step06k_core_trim_min_s)
    cfg.step06k_core_trim_min_s = 0.5;
end
if ~isfield(cfg, 'step06k_core_trim_max_s') || ...
        isempty(cfg.step06k_core_trim_max_s)
...
if ~isfield(cfg, 'step06k_hide_btt_only_from_main_plots') || ...
        isempty(cfg.step06k_hide_btt_only_from_main_plots)
    cfg.step06k_hide_btt_only_from_main_plots = true;
end
end
...
files.step06i_mat = fullfile(cfg.output_root, ...
    ...
    'Step06I_EODependentReference_20251222.mat');
...
function Obs = load_step03_observation_table_step06k_local(file)
S = load(file);
if isfield(S, 'bundle')
    B = S.bundle;
...
if ~any(mask)
    regionIds = unique(FitPoints.region_id(isfinite(FitPoints.region_id)));
    bestRegion = NaN;
    bestScore = -inf;
    for i = 1:numel(regionIds)
        rmask = FitPoints.region_id == regionIds(i) & ...
            FitPoints.blade_id == cfg.step06i_target_blade & ...
            ismember(FitPoints.sensor_id, cfg.step06i_sensor_ids) & ...
            isfinite(FitPoints.displacement_mm) & ...
            isfinite(FitPoints.strain_basis_real_microstrain) & ...
        ...
        if score > bestScore
            bestScore = score;
            bestRegion = regionIds(i);
        end
    end
    if ~isfinite(bestRegion)
        error('No target-window BTT fit points in [%.3f, %.3f] s, and no fallback resonance region is available.', ...
            tw(1), tw(2));
    end
    mask = FitPoints.region_id == bestRegion & ...
        FitPoints.blade_id == cfg.step06i_target_blade & ...
        ismember(FitPoints.sensor_id, cfg.step06i_sensor_ids) & ...
        isfinite(FitPoints.displacement_mm) & ...
        isfinite(FitPoints.strain_basis_real_microstrain) & ...
        isfinite(FitPoints.strain_basis_imag_microstrain);
    tw = [min(FitPoints.arrival_time_s(mask)), max(FitPoints.arrival_time_s(mask))];
    fprintf(['Step06K target window [%.3f, %.3f] s has no BTT fit points; ', ...
        'using strongest same-frequency resonance region %g [%.3f, %.3f] s instead.\n'], ...
        cfg.step06i_target_time_range_s(1), cfg.step06i_target_time_range_s(2), ...
        bestRegion, tw(1), tw(2));
end
...
function Tcorr = select_btt_correction_points_step06k_local( ...
    TargetPoints, FESpectrumReference, cfg)
Tcorr = TargetPoints;
Tcorr.is_used_for_primary_BTT_correction = true(height(Tcorr), 1);
if cfg.step06k_btt_correction_use_core_only && ...
        ~isempty(FESpectrumReference) && ...
        ismember('core_time_start_s', FESpectrumReference.Properties.VariableNames)
    t0 = FESpectrumReference.core_time_start_s(1);
    t1 = FESpectrumReference.core_time_end_s(1);
    m = Tcorr.arrival_time_s >= t0 & Tcorr.arrival_time_s <= t1;
    if nnz(m) >= max(30, ceil(0.20 * height(Tcorr)))
        Tcorr = Tcorr(m, :);
    end
end
Tcorr.is_used_for_primary_BTT_correction = true(height(Tcorr), 1);
Tcorr = sortrows(Tcorr, {'arrival_time_s', 'sensor_id'});
end
...
function Fit = add_primary_fit_point_predictions_step06k_local(Fit, T, hAtBtt, C)
if isempty(T)
    Fit.fit_point_time_s = [];
    Fit.fit_point_sensor_id = [];
    Fit.fit_point_region_id = [];
    Fit.dynamic_BTT_at_fit_points_mm = [];
    Fit.u_ref_at_fit_points_mm = [];
    Fit.total_fit_at_fit_points_mm = [];
    Fit.residual_at_fit_points_mm = [];
    return;
end
Fit.fit_point_time_s = T.arrival_time_s(:);
Fit.fit_point_sensor_id = T.sensor_id(:);
Fit.fit_point_region_id = T.region_id(:);
Fit.u_ref_at_fit_points_mm = real(C .* hAtBtt(:));
Fit.total_fit_at_fit_points_mm = Fit.u_ref_at_fit_points_mm + ...
    Fit.offset_at_points_mm(:);
Fit.dynamic_BTT_at_fit_points_mm = T.displacement_mm(:) - ...
    Fit.offset_at_points_mm(:);
Fit.residual_at_fit_points_mm = T.displacement_mm(:) - ...
    Fit.total_fit_at_fit_points_mm;
end
...
Fit = add_primary_fit_point_predictions_step06k_local(Fit, T, ...
    hBestAtBtt, Cbest);
end
...
function FESpectrumReference = build_fe_spectrum_reference_step06k_local( ...
    TrefTarget, FEWidthSensitivity, FEReference, cfg)
coreMask = select_core_region_mask_step06k_local(TrefTarget.time_s, cfg);
if ~any(coreMask)
    coreMask = true(height(TrefTarget), 1);
end
h = TrefTarget.strain_basis_real_microstrain + ...
    1i*TrefTarget.strain_basis_imag_microstrain;
hAbs = abs(h);
strainAmp = median(hAbs(coreMask), 'omitnan');
strainRmsReal = std(real(h(coreMask)), 'omitnan');
strainAReal = sqrt(2) * strainRmsReal;
if ~isfinite(strainAmp) || strainAmp <= 0
    strainAmp = strainAReal;
end
if ~isfinite(strainAmp) || strainAmp <= 0
    strainAmp = sqrt(2) * std(abs(h(coreMask)), 'omitnan');
end

if ~isempty(FEWidthSensitivity) && ...
        ismember('in_nominal_uncertainty_band', FEWidthSensitivity.Properties.VariableNames)
    F = FEWidthSensitivity(FEWidthSensitivity.in_nominal_uncertainty_band, :);
else
    F = table();
end
if isempty(F)
    F = table();
    F.z_mm = FEReference.nominal_z_mm;
    F.K_um_per_microstrain = FEReference.K_um_per_microstrain;
    F.K_mm_per_microstrain = FEReference.K_mm_per_microstrain;
end

A = F.K_mm_per_microstrain .* strainAmp;
[Amin, iMin] = min(A, [], 'omitnan');
[Amax, iMax] = max(A, [], 'omitnan');
Amed = median(A, 'omitnan');

FESpectrumReference = table();
FESpectrumReference.reference_name = "FE_spectrum_only_core_region";
FESpectrumReference.region_id = mode(TrefTarget.region_id);
FESpectrumReference.dominant_order = mode(TrefTarget.dominant_order);
FESpectrumReference.time_start_s = min(TrefTarget.time_s, [], 'omitnan');
FESpectrumReference.time_end_s = max(TrefTarget.time_s, [], 'omitnan');
FESpectrumReference.core_time_start_s = min(TrefTarget.time_s(coreMask), [], 'omitnan');
FESpectrumReference.core_time_end_s = max(TrefTarget.time_s(coreMask), [], 'omitnan');
FESpectrumReference.trim_fraction = cfg.step06k_core_trim_fraction;
FESpectrumReference.strain_spectrum_amp_microstrain = strainAmp;
FESpectrumReference.strain_real_A_peak_equiv_microstrain = strainAReal;
FESpectrumReference.FE_K_min_um_per_microstrain = min(F.K_um_per_microstrain, [], 'omitnan');
FESpectrumReference.FE_K_median_um_per_microstrain = median(F.K_um_per_microstrain, 'omitnan');
FESpectrumReference.FE_K_max_um_per_microstrain = max(F.K_um_per_microstrain, [], 'omitnan');
FESpectrumReference.A_tip_min_mm = Amin;
FESpectrumReference.A_tip_median_mm = Amed;
FESpectrumReference.A_tip_max_mm = Amax;
FESpectrumReference.z_at_A_min_mm = F.z_mm(iMin);
FESpectrumReference.z_at_A_max_mm = F.z_mm(iMax);
FESpectrumReference.core_sample_count = nnz(coreMask);
FESpectrumReference.total_sample_count = height(TrefTarget);
FESpectrumReference.note = "Branch 1: direct FE conversion of the trimmed resonance-core strain spectrum/envelope amplitude; BTT is not used.";
end


function coreMask = select_core_region_mask_step06k_local(t, cfg)
t = t(:);
...
if cfg.step06k_hide_btt_only_from_main_plots
...
function Summary = build_summary_step06k_local(caseName, targetEO, ...
    ...
    SensorResiduals, FEReference, FEWidthSensitivity, FESpectrumReference, ...
    BttCorrectionPoints, S06I, cfg)
...
Summary.reference_definition = ...
    "Branch1_FE_strain_spectrum_range_plus_Branch2_FE_strain_BTT_residual_correction";
...
Summary.primary_BTT_correction_point_count = height(BttCorrectionPoints);
...
if ~isempty(FESpectrumReference)
    Summary.FE_spectrum_core_time_start_s = ...
        FESpectrumReference.core_time_start_s(1);
    Summary.FE_spectrum_core_time_end_s = ...
        FESpectrumReference.core_time_end_s(1);
    Summary.FE_spectrum_strain_amp_microstrain = ...
        FESpectrumReference.strain_spectrum_amp_microstrain(1);
    Summary.FE_spectrum_A_tip_min_mm = FESpectrumReference.A_tip_min_mm(1);
    Summary.FE_spectrum_A_tip_median_mm = ...
        FESpectrumReference.A_tip_median_mm(1);
    Summary.FE_spectrum_A_tip_max_mm = FESpectrumReference.A_tip_max_mm(1);
    Summary.FE_spectrum_reference_note = string(FESpectrumReference.note(1));
else
    Summary.FE_spectrum_core_time_start_s = NaN;
    Summary.FE_spectrum_core_time_end_s = NaN;
    Summary.FE_spectrum_strain_amp_microstrain = NaN;
    Summary.FE_spectrum_A_tip_min_mm = NaN;
    Summary.FE_spectrum_A_tip_median_mm = NaN;
    Summary.FE_spectrum_A_tip_max_mm = NaN;
    Summary.FE_spectrum_reference_note = "";
end
Summary.FE_BTT_corrected_A_tip_mm = main.target_A_peak_equiv_mm;
Summary.FE_BTT_corrected_minus_FE_spectrum_median_mm = ...
    Summary.FE_BTT_corrected_A_tip_mm - Summary.FE_spectrum_A_tip_median_mm;
Summary.FE_BTT_corrected_within_FE_spectrum_range = ...
    Summary.FE_BTT_corrected_A_tip_mm >= Summary.FE_spectrum_A_tip_min_mm & ...
    Summary.FE_BTT_corrected_A_tip_mm <= Summary.FE_spectrum_A_tip_max_mm;
...
if ~isempty(bttOnly)
    Summary.btt_only_diagnostic_target_A_peak_equiv_mm = ...
        bttOnly.target_A_peak_equiv_mm;
else
    Summary.btt_only_diagnostic_target_A_peak_equiv_mm = ...
        BttOnlyDiagnosticA_step06k_local(TS);
end
...
function A = BttOnlyDiagnosticA_step06k_local(TS)
if istable(TS) && ismember('u_ref_btt_only_mm', TS.Properties.VariableNames)
    A = sqrt(2) * std(TS.u_ref_btt_only_mm, 'omitnan');
else
    A = NaN;
end
end
...
function [figFile, feFigFile] = plot_step06k_reference_local(TS, TargetPoints, SameFreqPoints, ...
    ...
    Boot, FEWidthSensitivity, FESpectrumReference, Summary, figDir, cfg)
...
plot_downsampled_step06k_local(ax4, TS.time_s, TS.u_ref_main_EOall_mm, ...
    blue, 'Branch 2: FE+strain+BTT correction', 1.15);
plot_downsampled_step06k_local(ax4, TS.time_s, TS.u_ref_FE_mode_shape_mm, ...
    red, 'Branch 1 median: FE+strain only', 0.95);
if isfinite(Summary.FE_spectrum_A_tip_min_mm) && ...
        isfinite(Summary.FE_spectrum_A_tip_max_mm)
    yline(ax4, Summary.FE_spectrum_A_tip_min_mm, ':', 'Color', red, ...
        'LineWidth', 0.8, 'DisplayName', 'Branch 1 A range');
    yline(ax4, -Summary.FE_spectrum_A_tip_min_mm, ':', 'Color', red, ...
        'LineWidth', 0.8, 'HandleVisibility', 'off');
    yline(ax4, Summary.FE_spectrum_A_tip_max_mm, ':', 'Color', red, ...
        'LineWidth', 0.8, 'HandleVisibility', 'off');
    yline(ax4, -Summary.FE_spectrum_A_tip_max_mm, ':', 'Color', red, ...
        'LineWidth', 0.8, 'HandleVisibility', 'off');
end
...
title(ax4, '4 Complete tip displacement reference');
...
if isfield(Summary, 'FE_spectrum_core_time_start_s') && ...
        isfinite(Summary.FE_spectrum_core_time_start_s)
    yTmp = [-1 1] * max(abs([Summary.FE_spectrum_A_tip_max_mm, ...
        Summary.target_A_peak_equiv_mm]), [], 'omitnan') * 1.35;
    if all(isfinite(yTmp)) && diff(yTmp) > 0
        patch(ax6, [Summary.FE_spectrum_core_time_start_s ...
            Summary.FE_spectrum_core_time_end_s ...
            Summary.FE_spectrum_core_time_end_s ...
            Summary.FE_spectrum_core_time_start_s], ...
            [yTmp(1) yTmp(1) yTmp(2) yTmp(2)], ...
            [0.92 0.95 1.00], 'EdgeColor', 'none', ...
            'FaceAlpha', 0.62, 'DisplayName', 'trimmed core');
    end
end
...
if isfield(Summary, 'FE_spectrum_A_tip_min_mm') && ...
        isfinite(Summary.FE_spectrum_A_tip_min_mm)
    yline(ax6, Summary.FE_spectrum_A_tip_min_mm, ':', 'Color', red, ...
        'LineWidth', 0.8, 'DisplayName', 'FE-strain A range');
    yline(ax6, -Summary.FE_spectrum_A_tip_min_mm, ':', 'Color', red, ...
        'LineWidth', 0.8, 'HandleVisibility', 'off');
    yline(ax6, Summary.FE_spectrum_A_tip_max_mm, ':', 'Color', red, ...
        'LineWidth', 0.8, 'HandleVisibility', 'off');
    yline(ax6, -Summary.FE_spectrum_A_tip_max_mm, ':', 'Color', red, ...
        'LineWidth', 0.8, 'HandleVisibility', 'off');
end
...
title(ax6, '8 BTT correction anchors');
...
ampNames = categorical(["FE+strain only", "FE+strain+BTT"]);
ampNames = reordercats(ampNames, ["FE+strain only", "FE+strain+BTT"]);
ampVals = [Summary.FE_spectrum_A_tip_median_mm, ...
    Summary.target_A_peak_equiv_mm];
bar(ax9, ampNames, ampVals, 0.58, 'FaceColor', [0.70 0.70 0.70]);
if isfinite(Summary.FE_spectrum_A_tip_min_mm) && ...
        isfinite(Summary.FE_spectrum_A_tip_max_mm)
    errorbar(ax9, ampNames(1), Summary.FE_spectrum_A_tip_median_mm, ...
        Summary.FE_spectrum_A_tip_median_mm - ...
        Summary.FE_spectrum_A_tip_min_mm, ...
        Summary.FE_spectrum_A_tip_max_mm - ...
        Summary.FE_spectrum_A_tip_median_mm, 'k.', ...
        'LineWidth', 1.0, 'CapSize', 9, ...
        'DisplayName', 'FE Z uncertainty');
end
ylabel(ax9, 'Target A_{peak,eq} (mm)');
title(ax9, '11 Benchmark amplitude branches');
legend(ax9, 'Location', 'best');
xtickangle(ax9, 15);

statusText = strrep(string(Summary.reference_status(1)), '_', ' ');
sg = sgtitle(fig, sprintf(['Step06K: Branch 1 FE+strain gives A range; ', ...
    'Branch 2 uses BTT only as residual correction\n', ...
    'EO%d, %.2f-%.2f s, core %.2f-%.2f s, K=%.3f um/microstrain, A=%.3f mm, status=%s'], ...
    targetEO, targetRange(1), targetRange(2), ...
    Summary.FE_spectrum_core_time_start_s(1), ...
    Summary.FE_spectrum_core_time_end_s(1), ...
...
figFile = fullfile(figDir, ...
    'Step06K_SameFrequencyRobustReference_20251222.png');
...
feFigFile = fullfile(figDir, ...
    'Step06K_FEWidthSensitivity_20251222.png');
