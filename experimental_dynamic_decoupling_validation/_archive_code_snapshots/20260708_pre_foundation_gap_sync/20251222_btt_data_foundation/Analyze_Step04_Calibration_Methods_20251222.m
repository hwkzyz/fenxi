function Analyze_Step04_Calibration_Methods_20251222(action)
%ANALYZE_STEP04_CALIBRATION_METHODS_20251222 Compare Step04 calibration policies.
%
% This utility keeps the 20251222 data, OPR semantics, Step05 windowing, and
...
%
% Usage:
%   Analyze_Step04_Calibration_Methods_20251222('build')
%   % run Step05 once per generated template with STEP05_TEMPLATE_OVERRIDE_FILE
%   Analyze_Step04_Calibration_Methods_20251222('summarize')
...
for im = 1:numel(methods)
    ...
    for it = 1:numel(T.SensorBlade)
        ...
        switch M.policy
            ...
            case "trusted_strict"
                entry = base_entry;
                entry.comparison_policy = 'trusted_strict_current_20251222';
            otherwise
...
dynamic_map_file = fullfile(repo_root, '20251222_low_speed_rotating_calibration', ...
    'output', 'dynamic_maps', ...
...
Analysis.extra_step05_env = { ...
    ...
    'STEP05_METHOD_PRESET', 'legacy_oprcenterstd_noeta', ...
    'STEP05_DYNAMIC_MAP_OVERRIDE_FILE', dynamic_map_file, ...
    'STEP05_FORCE_REBUILD', '1'};
end
...
methods(3).tag = 'm20251222_trusted_strict';
methods(3).label = '20251222 current: xc + strict trusted-domain recalibration';
methods(3).policy = "trusted_strict";
methods(3).source_mode = 'method_compare_20251222_trusted_strict';
...
methods(3).step05_output_tag = 'c22curr';
end
...
for i = 1:numel(methods)
    ...
    row = summarize_one_trend_local(M, trend_file, Analysis);
    rows = [rows; row]; %#ok<AGROW>
end
...
function row = summarize_one_trend_local(M, trend_file, Analysis)
row = struct();
row.method_tag = M.tag;
row.method_label = M.label;
...
row.window_count = 0;
row.ok_count = 0;
row.expected_eo = Analysis.expected_eo;
row.expected_eo_count = 0;
row.eo_mode = NaN;
row.eo_unique = "";
...
row.ok_count = nnz(ok);
row.expected_eo_count = nnz(ok & T.EO_id == Analysis.expected_eo);
if any(ok)
    eos = T.EO_id(ok);
    ...
    eo_fraction = row.expected_eo_count / max(row.window_count, 1);
...
nexttile;
bar(x, T.expected_eo_count ./ max(T.window_count, 1));
ylabel('expected EO fraction');
ylim([0, 1.05]);
...
for i = 1:numel(methods)
    M = methods(i);
    extra_env_text = '';
    for ie = 1:2:numel(Analysis.extra_step05_env)
        extra_env_text = [extra_env_text, sprintf( ...
            'setenv(''%s'',''%s''); ', ...
            Analysis.extra_step05_env{ie}, Analysis.extra_step05_env{ie + 1})]; %#ok<AGROW>
    end
    fprintf(fid, ['matlab -batch "cd(''%s''); setenv(''STEP05_OUTPUT_TAG'',''%s''); ', ...
        ...
        extra_env_text, ...
        'setenv(''STEP05_SHOW_PLOTS'',''false''); Step05_SingleSync_DirectTemplate_Identification_20251222"\n'], ...
        project_dir, M.step05_output_tag, M.template_file);
end
fprintf(fid, '\nThen run:\n');
fprintf(fid, 'matlab -batch "cd(''%s''); Analyze_Step04_Calibration_Methods_20251222(''summarize'')"\n', project_dir);
...
for i = 1:numel(tags)
    switch char(tags(i))
        ...
        case 'm20251222_trusted_strict'
            labels(i) = "20251222-strict";
        otherwise
            labels(i) = tags(i);
    end
end
