...
if isfield(loaded, 'SensorConfigSummary')
    SensorConfigSummary = loaded.SensorConfigSummary;
else
    SensorConfigSummary = build_sensor_config_summary_local(Sensor_Config, cfg, source_file);
end

if isfield(loaded, 'OPRCenterAngleTable')
    OPRCenterAngleTable = loaded.OPRCenterAngleTable;
else
...
Sensor_Config.OPR_Timing_Method = get_field_or_default_local( ...
    Sensor_Config, 'OPR_Timing_Method', 'existing_opr_center_reference');
...
for i = 1:numel(cfg.sensor_ids)
    ...
    angles = force_row_vector_local(Sensor_Config.Standard_Relative_Angles_OPRCenter(sid, 1:cfg.blades_num));
    rows(i).SensorID = sid;
