...
Step03_Setting.standard_angle_source = 'Step01.Standard_Relative_Angles_OPRCenter';
...
step02_center_method = string(get_field_or_default_local(Step02.Metadata, 'OPRCenterMethod', 'unknown'));
rows = [rows; consistency_row_local('Step01 standard-angle reference', ...
    step01_ref, 'contains opr/pulse center semantics', contains(lower(step01_ref), 'opr') || contains(lower(step01_ref), 'center'))]; %#ok<AGROW>
rows = [rows; consistency_row_local('Step02 OPR center method', ...
    step02_center_method, 'known center method', step02_center_method ~= "unknown")]; %#ok<AGROW>
...
opr_t = OPRTable.opr_center_time_s;
...
for i = 1:n
    ...
    rev_period = OPRTable.opr_center_time_s(idx_next) - OPRTable.opr_center_time_s(idx);
    if rev_period <= 0
        continue;
    end
    theta_local = 360 * (t - OPRTable.opr_center_time_s(idx)) / rev_period;
