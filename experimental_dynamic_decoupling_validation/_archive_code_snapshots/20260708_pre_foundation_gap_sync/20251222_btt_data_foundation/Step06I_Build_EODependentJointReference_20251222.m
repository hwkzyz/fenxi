%% Step06I_Build_EODependentJointReference_20251222.m
...
for iCase = 1:numel(targetCases)
    ...
    summaryCsv = fullfile(outDir, ...
        'Step06I_EODependentReference_Summary_20251222.csv');
    regionsCsv = fullfile(outDir, ...
        'Step06I_EODependentReference_SelectedRegions_20251222.csv');
    eoCsv = fullfile(outDir, ...
        'Step06I_EODependentReference_EOGainTable_20251222.csv');
    pointsCsv = fullfile(outDir, ...
        'Step06I_EODependentReference_FitPoints_20251222.csv');
    tsCsv = fullfile(outDir, ...
        'Step06I_EODependentReference_TimeSeries_20251222.csv');
    sensorCsv = fullfile(outDir, ...
        'Step06I_EODependentReference_SensorResiduals_20251222.csv');
    cvCsv = fullfile(outDir, ...
        'Step06I_EODependentReference_CrossValidation_20251222.csv');
    bootCsv = fullfile(outDir, ...
        'Step06I_EODependentReference_Bootstrap_20251222.csv');
    windowCsv = fullfile(outDir, ...
        'Step06I_EODependentReference_Step05WindowComparison_20251222.csv');
...
files.step06a_evidence_mat = fullfile(cfg.output_root, ...
    ...
    'Step06A_StrainRPM_ResonanceEvidence_20251222.mat');
files.step05_trend_csv = resolve_step05_trend_csv_local(cfg, caseName);
files.jiluopr_mat = fullfile(cfg.step02_output_dir, caseName, 'jiluOPR.mat');
end


function file = resolve_step05_trend_csv_local(cfg, caseName)
namePattern = sprintf('Trend_Step05_*_B%d_%s_%s.csv', ...
    cfg.step06i_target_blade, cfg.step06i_sensor_group, cfg.dataset);
preferredRoots = { ...
    fullfile(cfg.output_root, 'step05_single_sync_direct_template', caseName), ...
    fullfile(cfg.step05_direct_template_output_dir, caseName), ...
    fullfile(cfg.output_root, 'step05_single_sync_direct_template_official_default_unconstrained_w18', caseName), ...
    fullfile(cfg.output_root, 'step05_single_sync_direct_template_smoke_allblades_w3_summarydiag', caseName)};

for i = 1:numel(preferredRoots)
    dd = dir(fullfile(preferredRoots{i}, namePattern));
    if ~isempty(dd)
        file = fullfile(dd(1).folder, dd(1).name);
        return;
    end
end

dd = dir(fullfile(cfg.output_root, '**', namePattern));
if ~isempty(dd)
    [~, order] = sort([dd.datenum], 'descend');
    dd = dd(order);
    file = fullfile(dd(1).folder, dd(1).name);
else
    file = fullfile(cfg.step05_direct_template_output_dir, caseName, sprintf( ...
...
if ~isfield(B, 'Observation_Table') && ~isfield(B, 'ObservationTable')
...
if isempty(eoRow)
    okEo = strcmp(string(EOT.status), "ok");
    if any(okEo)
        [~, ii] = max(EOT.point_count(okEo));
        eoRows = find(okEo);
        eoRow = EOT(eoRows(ii), :);
    elseif ~isempty(EOT)
        [~, ii] = max(EOT.point_count);
        eoRow = EOT(ii, :);
    else
        eoRow = empty_eo_summary_row_local(targetEO);
    end
    targetEO = eoRow.dominant_order(1);
end
...
function T = empty_eo_summary_row_local(targetEO)
T = table();
T.dominant_order = targetEO;
T.status = "no_target_eo_fit";
T.K_mm_per_microstrain = NaN;
T.K_ratio_to_fixed = NaN;
T.phase_lag_rad = NaN;
T.phase_coverage_fraction = NaN;
T.point_count = 0;
T.rmse_mm = NaN;
end
...
figFile = fullfile(figDir, ...
    'Step06I_EODependentReference_20251222.png');
