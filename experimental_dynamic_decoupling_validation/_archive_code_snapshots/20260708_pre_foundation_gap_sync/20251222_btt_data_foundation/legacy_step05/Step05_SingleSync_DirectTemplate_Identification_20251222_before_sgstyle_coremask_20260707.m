...
% VERSION: FOUNDATION_MAINPULSE_ADAPTIVE_NOETA_VOLTAGEDECISION_20260707
%
% Key logic:
...
%   5. The formal 20251222 foundation route is NoEta. Static/free eta
%      variants belong in comparison/diagnostic scripts until their
%      calibration is independently validated.
...
S = struct();
S.method_name = 'foundation_mainpulse_adaptive_noeta_voltage_decision';
S.version = 'FOUNDATION_MAINPULSE_ADAPTIVE_NOETA_VOLTAGEDECISION_20260707';
S.method_file_tag = 'FoundationMainPulseAdaptiveNoEtaVoltageDecision';
...
if ~isempty(fit_sensor_eta_env)
    error('STEP05_FIT_SENSOR_ETA changes the method route; the formal foundation Step05 is NoEta.');
end

static_eta_env = strtrim(getenv('STEP05_STATIC_ETA_ENABLE'));
if ~isempty(static_eta_env)
    error('STEP05_STATIC_ETA_ENABLE changes the method route; use comparison scripts for static-eta checks.');
end

static_eta_file_env = strtrim(getenv('STEP05_STATIC_ETA_FILE'));
if ~isempty(static_eta_file_env)
    error('STEP05_STATIC_ETA_FILE changes the method route; use comparison scripts for static-eta checks.');
end
...
elseif ~isfield(S, 'active_eta_model') || strcmpi(S.active_eta_model, 'none')
    ...
    method.vp_seed_model = 'VP excludes eta_s; full fit uses the configured static eta prior';
end
...
method.eo_decision_score_mode = getfield_default_local( ...
    S, 'eo_decision_score_mode', 'weighted_voltage_rmse');
method.frequency_decision = ['single-window EO uses SG-style template-inversion displacement seeds, ' ...
