...
% identification observation. A legacy DynamicMap can optionally be supplied
% through STEP05_DYNAMIC_MAP_OVERRIDE_FILE for legacy-comparable diagnostics.
...
S.dynamic_map_override_file = strtrim(getenv('STEP05_DYNAMIC_MAP_OVERRIDE_FILE'));
...
S.method_preset = 'all_eo_unconstrained_corr';
...
if ~isempty(method_preset_env)
    ...
    switch S.method_preset
        case 'legacy_oprcenterstd_noeta'
            % Legacy-comparable 20251222 route:
            % Step03_Compare_Run_OPRCenterStd_DirectTemplate_NoEta_20251222.
            % This preset is intentionally conservative: no sensor eta,
            % VP top-3 screening, bounded A/dx, template-edge clipping
            % with overshoot penalty, and no phase-safe expansion.
            S.top_k_eo = 3;
            S.vp_include_sensor_eta = false;
            S.fit_sensor_eta = false;
            S.static_eta_enable = false;
            ...
            S.fminsearch_max_iter = 400;
            S.fminsearch_max_fun = 1200;
        case 'legacy_oprcenterstd_witheta'
            % Legacy main 20251222 route:
            % Step03_Main_Run_OPRCenterStd_DirectTemplate_WithEta_20251222.
            % This is the verified WithEta020_Reg002_PrevWinPhaseSafe path:
            % VP top-3 screening, bounded A/dx/eta, adaptive query guard,
            % hard query-safe core, previous-window phase-safe expansion,
            % and template-edge clipping with overshoot penalty.
            S.top_k_eo = 3;
            S.vp_include_sensor_eta = false;
            ...
            S.phase_safe_prev_min_final_gap_ratio = 0.005;
            S.choose_better_pass = true;
            S.better_pass_rmse_tolerance_v = -inf;
            S.core_mask_mode = 'legacy_base_query_safe';
            S.domain_selection_mode = 'hard';
            S.domain_soft_margin_mm = 0;
            S.main_pulse_mode = 'legacy_threshold';
            S.default_sensor_threshold = 0.5;
            S.enforce_legacy_parameter_bounds = true;
            S.use_template_eta_limit = true;
            S.overshoot_penalty_weight = 100;
            S.extrapolation_mode = 'clip_to_grid';
            S.objective_mode = 'rmse';
            S.reject_ambiguous_frequency = false;
        ...
        case 'vp_top3_bounded_rmse'
            ...
            S.sensor_eta_limit_mm = 0.20;
            S.sensor_eta_reg_weight_v_per_mm = 0.02;
            % Match the verified 20251222 OPRCenterStd VP route: cap the
            % adaptive guard by the bounded vibration/alignment range,
            % while eta remains an optimized sensor-zero correction.
            S.query_guard_max_mm = S.amplitude_limit_mm + ...
                S.dx_c_limit_mm + 0.05;
            S.phase_safe_expansion = true;
            S.overshoot_penalty_weight = 100;
...
DynamicMapOverride = [];
if ~isempty(S.dynamic_map_override_file)
    if ~isfile(S.dynamic_map_override_file)
        error('STEP05_DYNAMIC_MAP_OVERRIDE_FILE not found:\n  %s', ...
            S.dynamic_map_override_file);
    end
    loaded_dynamic_map = load(S.dynamic_map_override_file, 'DynamicMap');
    if ~isfield(loaded_dynamic_map, 'DynamicMap')
        error('Dynamic map override lacks variable DynamicMap:\n  %s', ...
            S.dynamic_map_override_file);
    end
    DynamicMapOverride = loaded_dynamic_map.DynamicMap;
    fprintf('Step05 dynamic map override: %s\n', S.dynamic_map_override_file);
end
...
for ib = 1:numel(S.target_blades)
    ...
    if isempty(DynamicMapOverride)
        WindowPlan = build_window_plan_local(ref_passes, S);
    else
        WindowPlan = build_window_plan_from_dynamic_map_local( ...
            DynamicMapOverride, S);
    end
...
for iw = 1:height(WindowPlan)
    ...
    if isempty(DynamicMapOverride)
        ...
        bundle_all = build_direct_bundle_from_dynamic_map_window_local( ...
            DynamicMapOverride.Window(iw), case_name, Template, cfg, S, blade_id);
    end
...
function WindowPlan = build_window_plan_from_dynamic_map_local(DynamicMap, S)
if ~isstruct(DynamicMap) || ~isfield(DynamicMap, 'Window') || isempty(DynamicMap.Window)
    error('DynamicMap override has no Window entries.');
end

n_win = numel(DynamicMap.Window);
if isfinite(S.debug_max_windows)
    n_win = min(n_win, S.debug_max_windows);
end

wins = NaN(n_win, 6);
for iw = 1:n_win
    W = DynamicMap.Window(iw);
    lap_range = W.lap_range(:).';
    if isempty(lap_range)
        lap_range = [iw, iw + S.window_laps - 1];
    end
    time_window = W.time_window(:).';
    if isempty(time_window) || numel(time_window) < 2
        time_window = [NaN NaN];
    end
    wins(iw, :) = [ ...
        W.window_id, ...
...
function bundle = build_direct_bundle_from_dynamic_map_window_local( ...
    Wmap, case_name, Template, cfg, S, blade_id)

bundle = make_empty_bundle_local();
bundle.case_name = case_name;
bundle.blade_id = blade_id;
bundle.lap_range = [Wmap.lap_range(1), Wmap.lap_range(end)];
bundle.time_window_s = Wmap.time_window(:).';

pass_id_global = 0;
for sid = S.analysis_sensors
    idx_sensor = find([Wmap.Sensor.sensor_id] == sid, 1, 'first');
    if isempty(idx_sensor)
        continue;
    end

    D = Wmap.Sensor(idx_sensor);
    tpl = get_template_entry_local(Template, sid, blade_id);
    if isempty(tpl)
        continue;
    end
    ...
    x_abs_mm = D.x_abs(:);
    x_center_for_template = get_template_xcenter_local(tpl);
    x_mm = x_abs_mm - x_center_for_template;
    t_seg = D.t(:);
    v_seg = D.V(:);
    theta_rad = D.theta(:);

    n = min([numel(x_mm), numel(x_abs_mm), numel(t_seg), numel(v_seg), numel(theta_rad)]);
    if n < 5
        continue;
    end
    x_mm = x_mm(1:n);
    x_abs_mm = x_abs_mm(1:n);
    t_seg = t_seg(1:n);
    v_seg = v_seg(1:n);
    theta_rad = theta_rad(1:n);
    ...
    main_pulse_mask = build_main_pulse_mask_local(t_seg, v_seg, S);
    ...
    switch lower(strtrim(S.core_mask_mode))
        case 'legacy_base'
            core_mask = finite_mask & inside_domain & ...
                        dynamic_effective & main_pulse_mask;
        case 'legacy_base_query_safe'
            core_mask = finite_mask & inside_domain & ...
                        dynamic_effective & main_pulse_mask & inside_guard;
        otherwise
            core_mask = finite_mask & inside_domain & inside_guard;
    end

    lap_ids = assign_dynamic_map_lap_ids_local(n, Wmap.lap_range(:).');
    pass_ids = NaN(n, 1);
    unique_laps = unique(lap_ids(:).', 'stable');
    for il = 1:numel(unique_laps)
        pass_id_global = pass_id_global + 1;
        pass_ids(lap_ids == unique_laps(il)) = pass_id_global;
    end

    x_center_mm = repmat(x_center_for_template, n, 1);

    bundle.x = [bundle.x; x_mm]; %#ok<AGROW>
    bundle.x_abs = [bundle.x_abs; x_abs_mm]; %#ok<AGROW>
    bundle.x_center = [bundle.x_center; x_center_mm]; %#ok<AGROW>
    ...
    bundle.analysis_lap = [bundle.analysis_lap; lap_ids(:)]; %#ok<AGROW>
    bundle.pass_id = [bundle.pass_id; pass_ids(:)]; %#ok<AGROW>
    bundle.prev_opr_index = [bundle.prev_opr_index; NaN(n, 1)]; %#ok<AGROW>
...
bundle.valid_pass_count = numel(unique(bundle.pass_id(bundle.core_mask & isfinite(bundle.pass_id))));
bundle.rot_freq_mean_hz = Wmap.rot_freq_mean_hz;
bundle.rot_rpm_mean = Wmap.rot_rpm_mean;

if ~isfinite(bundle.rot_rpm_mean) && isfinite(bundle.rot_freq_mean_hz)
    bundle.rot_rpm_mean = 60 * bundle.rot_freq_mean_hz;
end
if ~isfinite(bundle.rot_freq_mean_hz) && isfinite(bundle.rot_rpm_mean)
    bundle.rot_freq_mean_hz = bundle.rot_rpm_mean / 60;
end

bundle.source_mode = 'legacy_dynamic_map_override';
end


function lap_ids = assign_dynamic_map_lap_ids_local(n, lap_range)
lap_range = lap_range(:).';
if isempty(lap_range)
    lap_range = 1;
end
lap_count = numel(lap_range);
edge = round(linspace(1, n + 1, lap_count + 1));
lap_ids = NaN(n, 1);
for il = 1:lap_count
    idx = edge(il):max(edge(il + 1) - 1, edge(il));
    idx = idx(idx >= 1 & idx <= n);
    lap_ids(idx) = lap_range(il);
end
lap_ids(~isfinite(lap_ids)) = lap_range(end);
end
...
if isfield(S, 'enforce_legacy_parameter_bounds') && ...
...
mode = lower(strtrim(getfield_default_local(S, ...
    'phase_safe_reference_mode', 'core_current')));
