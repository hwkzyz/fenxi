...
if ~isempty(step07jFileOverride)
    ...
    step07jFile = resolve_step07j_result_local(outDir, C0, cfg.targetBlade, sensorTag, step07jSuffix);
...
if isempty(outputSuffix)
    outputSuffix = auto_output_suffix_local(cfg);
end
...
%% Local functions
function suffix = auto_output_suffix_local(cfg)
parts = {};
if isfinite(cfg.maxWindows)
    parts{end+1} = sprintf('win%d', cfg.maxWindows); %#ok<AGROW>
end
if cfg.kFold ~= 3
    parts{end+1} = sprintf('k%d', cfg.kFold); %#ok<AGROW>
end
if isempty(parts)
    suffix = '';
else
...
function step07jFile = resolve_step07j_result_local(outDir, C0, targetBlade, sensorTag, step07jSuffix)
if ~isempty(step07jSuffix)
    candidates = {
        sprintf('Step07J_NestedStaticWarp_VPFullWave_%s_%s%s.mat', C0.dataset, C0.caseTag, step07jSuffix)
        sprintf('Step07J_NestedStaticWarp_VPFullWave_%s_B%d_%s%s.mat', C0.dataset, targetBlade, sensorTag, step07jSuffix)
        };
else
    regionTag = '';
    ...
    candidates = {
        ...
        sprintf('Step07J_NestedStaticWarp_VPFullWave_%s_B%d_%s_%s_main_gaptilt.mat', C0.dataset, targetBlade, sensorTag, regionTag)
        sprintf('Step07J_NestedStaticWarp_VPFullWave_%s_B%d_%s_main_gaptilt.mat', C0.dataset, targetBlade, sensorTag)
        sprintf('Step07J_NestedStaticWarp_VPFullWave_%s_B%d_%s_method_compare.mat', C0.dataset, targetBlade, sensorTag)
        };
end
...
if isempty(files)
    files = dir(fullfile(outDir, sprintf( ...
        ...
        C0.dataset, targetBlade, sensorTag)));
end
if ~isempty(files)
...
error('Run Step07J first. Missing Step07J result for %s under:\n  %s', C0.caseTag, outDir);
end
