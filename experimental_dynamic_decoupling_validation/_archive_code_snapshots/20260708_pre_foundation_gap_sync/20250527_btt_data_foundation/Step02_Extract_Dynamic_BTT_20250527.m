...
%STEP02_EXTRACT_DYNAMIC_BTT_20250527 Method-neutral dynamic BTT extraction.
%
% Improved version for the 20250527 data-foundation route.
%
% Main improvements over the previous draft:
%   1. Dynamic OPR timing uses the same configurable definition as Step01
%      by default: multi-threshold pulse center.
...
%   3. Probe arrival time uses baseline-corrected half-area integration by
%      default, with optional methods kept for comparison.
%   4. Gap, OPR timing, probe-arrival method, and time-index interpretation
%      are controlled by BTTDataConfig_20250527 rather than hard-coded.
...
config_path = fullfile(cfg.step01_output_dir, 'Sensor_Config_20250527.mat');
if ~isfile(config_path)
    error('Step01 reference not found. Please run Step01_Build_LowSpeed_Reference_20250527.m first:\n  %s', config_path);
end
...
for iCase = 1:numel(target_cases)
    ...
    writetable(case_summary, fullfile(output_dir, 'Step02_Summary_20250527.csv'));
    save(fullfile(output_dir, 'Step02_Summary_20250527.mat'), 'case_summary', 'metadata');

    cfg_snapshot = cfg; %#ok<NASGU>
    save(fullfile(output_dir, 'Step02_Dynamic_BTT_Extraction_20250527.mat'), ...
...
% versions of BTTDataConfig_20250527.
...
switch method
    case {'rising_edge', 'threshold_rising_edge', 'start_edge'}
    ...
    otherwise
        warning('Unknown cfg.opr_timing_method=%s. Falling back to rising_edge.', cfg.opr_timing_method);
        t_arrival = threshold_crossing_start_local(raw, a, cfg.opr_threshold, fs);
end
...
switch method
    ...
    case {'polynomial_centroid', 'poly_centroid'}
        [t_arrival, ~] = polynomial_centroid_arrival_local(time_s, data, threshold);
    otherwise
        warning('Unknown cfg.probe_arrival_method=%s. Falling back to half_area.', cfg.probe_arrival_method);
        t_arrival = baseline_corrected_half_area_local(time_s, data, threshold, cfg);
end
...
fig1 = figure('Name', sprintf('20250527 Step02 RPM - %s', case_name), ...
...
fig2 = figure('Name', sprintf('20250527 Step02 Blade ID Sequence - %s', case_name), ...
...
fig3 = figure('Name', sprintf('20250527 Step02 Fingerprint - %s', case_name), ...
...
fig4 = figure('Name', sprintf('20250527 Step02 Revolution Shift Path - %s', case_name), ...
...
fig5 = figure('Name', sprintf('20250527 Step02 Summary Table - %s', case_name), ...
