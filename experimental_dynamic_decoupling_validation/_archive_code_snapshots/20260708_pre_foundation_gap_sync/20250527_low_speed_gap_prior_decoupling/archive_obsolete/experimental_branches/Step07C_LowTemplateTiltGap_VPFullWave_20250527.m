%% Step07C: Low-speed template baseline + tilt-corrected gap increment
% The first-order VP step only screens EO candidates. Final scoring uses the
% full waveform model:
%
%   V_s(t) = M_s(g0_s + dg_s, x_s(t) - dx_c - u(t))
%
% where M_s is the Step06F low-speed-template anchored incremental gap
...
figDir = fullfile(outDir, 'figures_step07c_low_template_tilt_gap_vp_full_wave');
...
cfg.eoPad = 2;
cfg.vpTopK = parse_positive_integer_env_local('STEP07C_TOP_K_EO', 3);
cfg.maxWindows = parse_positive_integer_env_local('STEP07C_MAX_WINDOWS', inf);
cfg.maxPointsPerWindow = parse_positive_integer_env_local('STEP07C_MAX_POINTS_PER_WINDOW', 900);
cfg.amplitudeLimitMm = parse_nonnegative_numeric_env_local('STEP07C_AMP_LIMIT_MM', 0.50);
cfg.dxLimitMm = parse_nonnegative_numeric_env_local('STEP07C_DX_LIMIT_MM', 0.20);
cfg.deltaGapLimitMm = parse_nonnegative_numeric_env_local('STEP07C_DELTA_G_LIMIT_MM', 0.25);
cfg.derivativeStepMm = 1e-3;
cfg.weightFloor = 0.05;
cfg.defaultSensorThreshold = 0.5;
cfg.pulseSelectionMode = lower(strtrim(getenv('STEP07C_PULSE_MODE')));
if isempty(cfg.pulseSelectionMode)
    cfg.pulseSelectionMode = 'all';
end
if ~ismember(cfg.pulseSelectionMode, {'single', 'all'})
    error('STEP07C_PULSE_MODE must be "single" or "all".');
end
cfg.dynamicGradientMinRatio = parse_nonnegative_numeric_env_local('STEP07C_DYNAMIC_GRADIENT_MIN_RATIO', 0.08);
cfg.domainSoftMarginMm = parse_nonnegative_numeric_env_local('STEP07C_DOMAIN_SOFT_MARGIN_MM', 0.20);
cfg.overshootPenaltyWeight = parse_nonnegative_numeric_env_local('STEP07C_OVERSHOOT_PENALTY_WEIGHT', 100);
cfg.fminOptions = optimset('Display', 'off', 'MaxIter', 450, 'MaxFunEvals', 1800, ...
    'TolX', 1e-5, 'TolFun', 1e-5);

sensorOverride = strtrim(getenv('STEP07C_ANALYSIS_SENSORS'));
if ~isempty(sensorOverride)
    parsedSensors = sscanf(sensorOverride, '%d').';
    if isempty(parsedSensors)
        error('STEP07C_ANALYSIS_SENSORS must contain integer sensor IDs, for example "1 3 6".');
    end
...
fprintf('\n=== Step07C: low-template anchored tilt-gap VP/full-wave ===\n');
fprintf('Template: %s\n', templateFile);
...
Result.method = 'low_template_anchored_tilt_gap_vp_full_wave';
Result.description = ['Step06F corrected gap library is used only as a clearance-change increment. ' ...
...
matFile = fullfile(outDir, sprintf('Step07C_LowTemplateTiltGap_VPFullWave_20250527_B%d_%s.mat', ...
    cfg.targetBlade, sensorTag));
trendCsv = fullfile(outDir, sprintf('Step07C_LowTemplateTiltGap_VPFullWave_Trend_20250527_B%d_%s.csv', ...
    cfg.targetBlade, sensorTag));
summaryCsv = fullfile(outDir, sprintf('Step07C_LowTemplateTiltGap_VPFullWave_Summary_20250527_B%d_%s.csv', ...
    cfg.targetBlade, sensorTag));
figTrend = fullfile(figDir, sprintf('Step07C_Trend_20250527_B%d_%s.png', cfg.targetBlade, sensorTag));
figBest = fullfile(figDir, sprintf('Step07C_BestWindow_Collapse_20250527_B%d_%s.png', cfg.targetBlade, sensorTag));
figGap = fullfile(figDir, sprintf('Step07C_GapIdentification_20250527_B%d_%s.png', cfg.targetBlade, sensorTag));
...
fig = figure('Name', 'Step07C trend', 'Color', 'w', ...
...
fig = figure('Name', 'Step07C best-window dynamic collapse', 'Color', 'w', ...
...
style = paper_style_local();
fig = figure('Name', 'Step07C gap identification', 'Color', 'w', ...
