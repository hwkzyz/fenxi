...
%STEP03_BUILD_DYNAMIC_OBSERVATION_BUNDLE_20241106 Build method-neutral BTT observations.
%
% Improved version for the 20241106 BTT data-foundation route.
%
...
if ~isfile(sensor_config_path)
    error('Missing Step01 sensor config. Please run Step01_Build_LowSpeed_Reference_20241106.m first:\n  %s', sensor_config_path);
end
...
for iCase = 1:numel(target_cases)
    ...
    if ~isfolder(step02_case_dir) || ~isfile(fullfile(step02_case_dir, 'jiluOPR.mat'))
        error('Missing Step02 products for %s. Please run Step02_Extract_Dynamic_BTT_20241106.m first.', case_name);
    end
    ...
    save(fullfile(case_output_dir, 'BTT_Observation_Bundle_20241106.mat'), 'bundle', 'metadata', '-v7.3');
    writetable(observation_table, fullfile(case_output_dir, 'BTT_Observation_LongTable_20241106.csv'));
    writetable(summary_table, fullfile(case_output_dir, 'BTT_Observation_Summary_20241106.csv'));
...
if ~isfield(cfg, 'step03_standard_angle_preference')
    cfg.step03_standard_angle_preference = 'opr_center'; % 'opr_center', 'selected', 'mean', or 'median'
end
...
[omega_diag_time_s, omega_diag_rpm, omega_diag_rad_s, omega_source] = ...
    load_or_compute_rpm_local(fullfile(step02_case_dir, 'omega.mat'), opr_times, cfg.opr_pulses_per_rev);
...
if cfg.step03_use_current_opr_for_speed
    [omega_time_s, omega_rpm, omega_rad_s] = compute_rpm_from_opr_local(opr_times, cfg.opr_pulses_per_rev);
...
[F_omega_deg_s, F_rpm] = build_speed_interpolants_local(opr_times, omega_time_s, omega_rpm, cfg.opr_pulses_per_rev);
...
summary_table = summarize_observations_local(case_name, observation_table, angle_ref, cfg, step02_diag);
...
if numel(opr_times) <= cfg.opr_pulses_per_rev
...
full_file = fullfile(step02_case_dir, 'Step02_Dynamic_BTT_Extraction_20241106.mat');
if isfile(full_file)
    S = load(full_file);
...
summary_mat = fullfile(step02_case_dir, 'Step02_Summary_20241106.mat');
...
summary_csv = fullfile(step02_case_dir, 'Step02_Summary_20241106.csv');
...
value = [];
value_name = '';
if strcmp(preference, 'opr_center') && isfield(Sensor_Config, 'Standard_Relative_Angles_OPRCenter')
    value = Sensor_Config.Standard_Relative_Angles_OPRCenter;
    value_name = 'opr_center';
elseif strcmp(preference, 'start_edge') && isfield(Sensor_Config, 'Standard_Relative_Angles_StartEdge')
    value = Sensor_Config.Standard_Relative_Angles_StartEdge;
    value_name = 'start_edge';
elseif strcmp(preference, 'mean') && isfield(Sensor_Config, 'Standard_Relative_Angles_Mean')
...
angle_ref.opr_center = get_optional_field_local(Sensor_Config, 'Standard_Relative_Angles_OPRCenter', nan(size(value)));
angle_ref.start_edge = get_optional_field_local(Sensor_Config, 'Standard_Relative_Angles_StartEdge', nan(size(value)));
...
revolution_index(valid_prev) = floor((prev_opr_index(valid_prev) - 1) / cfg.opr_pulses_per_rev) + 1;
...
function summary_table = summarize_observations_local(case_name, observation_table, angle_ref, cfg, step02_diag)
rows = repmat(struct( ...
    'case_name', '', ...
    'sensor_id', NaN, ...
    'blade_id', NaN, ...
...
for iSensor = 1:numel(cfg.sensor_ids)
    ...
    for blade_id = 1:cfg.blades_num
        ...
        rows(row).std_angle_std_deg = get_angle_matrix_value_local(angle_ref.std, sid, blade_id);
        rows(row).std_angle_iqr_deg = get_angle_matrix_value_local(angle_ref.iqr, sid, blade_id);
        rows(row).std_angle_count = get_angle_matrix_value_local(angle_ref.count, sid, blade_id);
...
function [omega_time_s, omega_rpm, omega_rad_s, source] = load_or_compute_rpm_local(omega_path, opr_times, opr_pulses_per_rev)
omega_time_s = [];
omega_rpm = [];
omega_rad_s = [];
...
[omega_time_s, omega_rpm, omega_rad_s] = compute_rpm_from_opr_local(opr_times, opr_pulses_per_rev);
end


function [omega_time_s, omega_rpm, omega_rad_s] = compute_rpm_from_opr_local(opr_times, opr_pulses_per_rev)
...
omega_time_s = opr_times(1:(end - opr_pulses_per_rev));
...
expected_n = numel(opr_times) - cfg.opr_pulses_per_rev;
...
function [F_omega_deg_s, F_rpm] = build_speed_interpolants_local(opr_times, omega_time_s, omega_rpm, opr_pulses_per_rev)
speed_time = opr_times(1:(end - opr_pulses_per_rev));
speed_deg_s = 360 ./ max(opr_times((opr_pulses_per_rev + 1):end) - opr_times(1:(end - opr_pulses_per_rev)), eps);
...
fig1 = figure('Name', sprintf('20241106 Step03 Displacement Time Series - %s', bundle.Case_Name), ...
...
fig2 = figure('Name', sprintf('20241106 Step03 Sensor-Blade Summary - %s', bundle.Case_Name), ...
...
fig3 = figure('Name', sprintf('20241106 Step03 RPM and Coverage - %s', bundle.Case_Name), ...
...
fig4 = figure('Name', sprintf('20241106 Step03 Displacement Distribution - %s', bundle.Case_Name), ...
...
fig5 = figure('Name', sprintf('20241106 Step03 Summary Table - %s', bundle.Case_Name), ...
...
fig6 = figure('Name', sprintf('20241106 Step03 Quality Flags - %s', bundle.Case_Name), ...
...
xtickangle(30);
set(gca, 'YScale', 'log');
ylim([1, max(counts) * 1.8]);
for i = 1:numel(counts)
    text(c(i), counts(i), sprintf(' %d', counts(i)), ...
