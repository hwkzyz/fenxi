function Analyze_Step04_Calibration_Methods_20241106(action)
%ANALYZE_STEP04_CALIBRATION_METHODS_20241106 Compare Step04 calibration policies.
%
% This utility keeps the 20241106 data, OPR semantics, Step05 windowing, and
...
%
% Usage:
%   Analyze_Step04_Calibration_Methods_20241106('build')
%   % run Step05 once per generated template with STEP05_TEMPLATE_OVERRIDE_FILE
%   Analyze_Step04_Calibration_Methods_20241106('summarize')
...
switch action
    case "build"
        build_template_variants_local(cfg, cmp_dir, fig_dir);
    case "summarize"
        summarize_step05_comparison_local(cfg, cmp_dir, fig_dir);
    case "all"
        build_template_variants_local(cfg, cmp_dir, fig_dir);
        summarize_step05_comparison_local(cfg, cmp_dir, fig_dir);
    otherwise
...
function build_template_variants_local(cfg, cmp_dir, fig_dir)
...
for im = 1:numel(methods)
    ...
    for it = 1:numel(T.SensorBlade)
        ...
        switch M.policy
            ...
            case "trusted_strict"
                entry = base_entry;
                entry.comparison_policy = 'trusted_strict_current_20241106';
            otherwise
...
write_step05_run_commands_local(methods, cfg, cmp_dir);
end
...
methods(3).tag = 'm20241106_trusted_strict';
methods(3).label = '20241106 current: xc + strict trusted-domain recalibration';
methods(3).policy = "trusted_strict";
methods(3).source_mode = 'method_compare_20241106_trusted_strict';
...
for i = 1:numel(methods)
    ...
    methods(i).step05_output_tag = ['calibcmp_', methods(i).tag];
end
end
...
function summarize_step05_comparison_local(cfg, cmp_dir, fig_dir)
methods = method_defs_local(cfg, cmp_dir);
rows = struct([]);
for i = 1:numel(methods)
    M = methods(i);
    trend_file = fullfile(cfg.output_root, ...
        ...
        cfg.step05_target_blades(1), sensor_tag_local(cfg.step05_analysis_sensors), cfg.dataset));
...
function write_step05_run_commands_local(methods, cfg, cmp_dir)
...
for i = 1:numel(methods)
    ...
    fprintf(fid, ['matlab -batch "cd(''%s''); setenv(''STEP05_OUTPUT_TAG'',''%s''); ', ...
        ...
        'setenv(''STEP05_SHOW_PLOTS'',''false''); Step05_SingleSync_DirectTemplate_Identification_20241106"\n'], ...
        project_dir, M.step05_output_tag, M.template_file);
end
fprintf(fid, '\nThen run:\n');
fprintf(fid, 'matlab -batch "cd(''%s''); Analyze_Step04_Calibration_Methods_20241106(''summarize'')"\n', project_dir);
...
for i = 1:numel(tags)
    switch char(tags(i))
        ...
        case 'm20241106_trusted_strict'
            labels(i) = "20241106-strict";
        otherwise
            labels(i) = tags(i);
    end
end
