%% Step06A_Build_StrainRPM_ResonanceEvidence_20241106.m
% Build method-neutral strain/RPM/resonance evidence for the 20241106 BTT route.
%
% This script follows the old Step04_Detect_Resonance_By_BTT_STE_20241106 logic:
...
Step06A_Revision = '2026-07-04-20241106-foundation-strain-rpm-v1';
...
force_rebuild_env = strtrim(getenv('STEP06A_FORCE_REBUILD'));
...
step06a_figure_root = cfg.step06a_figure_dir;

%% 3. Strain/RPM/BTT settings
if ~isfield(cfg, 'step06_target_blade') || isempty(cfg.step06_target_blade)
    cfg.step06_target_blade = cfg.step05_target_blades(1);
end
targetBladeId = cfg.step06_target_blade;

% Only used for BTT displacement display, not for resonance detection.
btt_sensor_ids_for_plot = cfg.sensor_ids;

baselineTimeSec = 3.0;

% Explicit strain file override.
% This is the strain gauge channel used by Step06A.
strain_file_override = cfg.step06_default_strain_file;
...
strain_primary_keyword = cfg.step06_default_strain_channel;
...
% Explicit BTT-to-strain offset used by the 20241106 validation route.
manual_strain_time_offset_total_sec = cfg.step06_strain_to_btt_offset_s;

% Old Step04 style.
detrendMethod = 'linear';       % 'linear', 'movingmedian', 'none'

%% 4. Resonance detection settings
orders = cfg.step06_align_order_candidates(:).';
...
rawFftPeakBandHz = cfg.step06_raw_fft_peak_band_hz;

% Display frequency band.
dashboardFreqBandHz = cfg.step06_fft_freq_band_hz;
...
for iCase = 1:numel(target_cases)
    ...
    evidenceMat = fullfile(outDir, 'Step06A_StrainRPM_ResonanceEvidence_20241106.mat');
    ...
    legacy = struct('hasAlignResult', false, 'hasStep3Result', false);
    alignResult = struct();
    ...
    writetable(regionTable, fullfile(outDir, 'Step06A_RegionTable_20241106.csv'));
    writetable(scoreTable, fullfile(outDir, 'Step06A_ScoreTable_20241106.csv'));
    writetable(localFftTable, fullfile(outDir, 'Step06A_LocalStrainFFT_20241106.csv'));
...
if isfield(cfg, 'step03_output_dir') && ~isempty(cfg.step03_output_dir)
    candidateFiles{end+1} = fullfile(cfg.step03_output_dir, caseName, 'BTT_Observation_Bundle_20241106.mat');
end

candidateFiles{end+1} = fullfile(cfg.output_root, caseName, 'BTT_Observation_Bundle_20241106.mat');
...
if isempty(bundleFile)
    dd = dir(fullfile(cfg.output_root, '**', 'BTT_Observation_Bundle_20241106.mat'));
    for k = 1:numel(dd)
...
if isempty(bundleFile)
    error('Missing Step03 BTT_Observation_Bundle_20241106.mat for case %s.', caseName);
end
...
alignPatterns = { ...
    'Step3_Spectrum_RPM_Alignment_20241106.mat', ...
    '*Spectrum_RPM_Alignment*.mat'};

resultPatterns = { ...
    'Step3_Result_20241106.mat'};
...
[allTime, allVib, bttSource] = load_btt_vibration_from_step03_compat_local( ...
    cfg, caseName, legacy, sensorIds, targetBladeId, baselineTimeSec);

end


function [allTime, allVib, bttSource] = load_btt_vibration_from_step03_compat_local( ...
    cfg, caseName, legacy, sensorIds, targetBladeId, baselineTimeSec)
...
bttSource = struct();
bttSource.source = 'Step03 compatible jilublade displacement';
...
if isfield(cfg, 'step03_output_dir') && ~isempty(cfg.step03_output_dir)
    candidateDirs{end+1} = fullfile(cfg.step03_output_dir, caseName);
end

if isfield(cfg, 'step02_output_dir') && ~isempty(cfg.step02_output_dir)
    candidateDirs{end+1} = fullfile(cfg.step02_output_dir, caseName);
end
...
for sid = sensorIds(:).'
    ...
    if ~isfile(vibFile)
        warning('Missing Step03-compatible BTT displacement file: %s', vibFile);
        continue;
    end
    ...
    if ~isfield(S, 'jilublade') || size(S.jilublade, 2) < 6
        warning('Invalid Step03-compatible BTT displacement file: %s', vibFile);
        continue;
    end
