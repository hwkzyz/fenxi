...
primaryRegion = S4.regionTable(1, :);
selectedTimeSec = [primaryRegion.bttStartSec, primaryRegion.bttEndSec];
if ismember('syncFreqAtRegionOrderHz', S4.regionTable.Properties.VariableNames)
    strainOrderFreqHz = S4.regionTable.syncFreqAtRegionOrderHz(1);
else
    strainOrderFreqHz = S4.localFftTable.regionPeakFreqHz(1);
end
...
auditTable = build_step07j_order_audit_local(outDir, C0, step07jFile, ...
    strainFftPeakHz, strainOrderFreqHz);
auditCsvFile = fullfile(outDir, sprintf( ...
    'Step12_S257_vs_S57_OrderAudit_%s_%s.csv', C0.dataset, C0.caseTag));
...
fprintf('Saved:\n  %s\n  %s\n  %s\n  %s\n  %s\n  %s\n  %s\n', ...
    ...
    natureFigFile, auditFigFile, channelTimeFigFile);
...
function primarySource = make_primary_strain_source_local(S4)
primarySource = struct();
primarySource.strain_file = S4.strain.file;
primarySource.channel = string(S4.strain.label);
primarySource.strain_time_offset_total_sec = S4.strainToBttOffsetSec;
...
function T = build_step07j_order_audit_local(outDir, C0, primaryFile, ...
    strainPeakHz, orderLineHz)
files = {primaryFile};
labels = {'S257 selected'};
s57File = fullfile(outDir, sprintf( ...
    'Step07J_NestedStaticWarp_VPFullWave_%s_B%d_S57_main_gaptilt.mat', ...
    C0.dataset, C0.bladeId));
if isfile(s57File) && ~strcmpi(s57File, primaryFile)
    files{end + 1} = s57File; %#ok<AGROW>
    labels{end + 1} = 'S57 main'; %#ok<AGROW>
end
...
T = cell2table(rows, 'VariableNames', {'result_label', 'result_file', ...
    ...
    'amplitude_mm', 'rmse_mV', 'freq_error_vs_strain_peak_hz', ...
    'freq_error_vs_order_line_hz'});
end

function pngFile = plot_step12_order_audit_local(auditTable, figDir, C0, ...
    strainPeakHz, orderLineHz)
baseName = sprintf('Step12_S257_vs_S57_OrderAudit_%s_%s', ...
...
candidates = {
    ...
    sprintf('Step07J_NestedStaticWarp_VPFullWave_%s_%s_hybrid_s2_aligned_compare.mat', C0.dataset, C0.caseTag)
    sprintf('Step07J_NestedStaticWarp_VPFullWave_%s_%s_hybrid_s2_main_compare.mat', C0.dataset, C0.caseTag)
