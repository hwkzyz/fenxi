...
%
% On top of that, this script now records the associated rotating template,
% corrected gap library, rotating-calibration region/numbering references,
% and the direct baseline result that define the unified event basis.

clear; clc; close all;

routeDir = fileparts(mfilename('fullpath'));
outDir = fullfile(routeDir, 'outputs');
figDir = fullfile(outDir, 'figures_step06_gapaware_dynamic_map_20241106');
if ~exist(figDir, 'dir')
    mkdir(figDir);
end
...
P.templateFile = '';
P.correctedLibFile = '';
P.templateBankFile = fullfile(outDir, Flow.calibration.templateBankFile);
P.gapBankFile = fullfile(outDir, Flow.calibration.gapBankFile);
...
if isempty(P.templateFile)
    P.templateFile = fullfile(fileparts(routeDir), ...
        '20241106_low_speed_rotating_calibration', 'output', 'templates', ...
        sprintf('Template_LowSpeedRotating_B%d_%s_GradientXRange030_OPRCenterStd_20241106.mat', ...
        P.targetBlade, sensorTag));
end
if isempty(P.correctedLibFile)
    P.correctedLibFile = fullfile(outDir, sprintf( ...
        'Step06I_OffsetTiltShared_GapLibrary_20241106_B%d_%s.mat', ...
        P.targetBlade, gapSensorTag));
end
if isempty(P.rotatingDynamicMapFile)
    P.rotatingDynamicMapFile = fullfile(fileparts(routeDir), ...
        '20241106_low_speed_rotating_calibration', 'output', 'dynamic_maps', ...
        sprintf('DynamicMap_B%d_%s_SlidingWindows_Main20L_W3S1_T075s_GradientXRange030_OPRCenterStd_20241106.mat', ...
        P.targetBlade, sensorTag));
end
if isempty(P.directReferenceFile)
    P.directReferenceFile = fullfile(fileparts(routeDir), ...
        '20241106_low_speed_rotating_calibration', 'output', 'new_flow', ...
        '06_identification', sensorTag, ...
        sprintf('IdentificationResult_T075s_20241106_B%d_only.mat', P.targetBlade));
end
...
DynamicMap.description = ['Continuous raw high-speed waveform windows for the 20241106 gap-aware main route. ' ...
    'Event organization keeps the mature direct-style DynamicMap layout, while metadata now points to the unified rotating template, direct reference and corrected gap library.'];
DynamicMap.TemplateFile = P.templateFile;
DynamicMap.CorrectedGapLibraryFile = P.correctedLibFile;
DynamicMap.DirectReferenceFile = P.directReferenceFile;
DynamicMap.RotatingDynamicMapFile = P.rotatingDynamicMapFile;
...
DynamicMap.TargetBlade = P.targetBlade;
DynamicMap.SourceResultFile = P.directReferenceFile;
DynamicMap.SourceResultSensorTag = sensorTag;
DynamicMap.TemplateFileForXRel = P.templateFile;
DynamicMap.TemplateSuffixForXRel = 'Template_LowSpeedRotating_20241106';
...
matFile = fullfile(outDir, sprintf( ...
    'Step06_BuildGapAwareDynamicMap_20241106_B%d_%s.mat', P.targetBlade, sensorTag));
...
csvFile = fullfile(outDir, sprintf( ...
    'Step06_BuildGapAwareDynamicMap_WindowSummary_20241106_B%d_%s.csv', ...
    P.targetBlade, sensorTag));
writetable(summary, csvFile);

figFile = fullfile(figDir, sprintf( ...
    'Step06_BuildGapAwareDynamicMap_20241106_B%d_%s.png', P.targetBlade, sensorTag));
