...
% increment and a small high-speed tilt increment inside the direct-template + VP Top-K EO candidate set with the
% full waveform objective. VP narrows the EO candidates but does not fix the
% final EO. The default main route computes only the paper-facing gap_tilt
% model. In comparison mode this script also computes nested static
...
cfg.deltaMuLimit = parse_nonnegative_numeric_env_local('STEP07J_DELTA_MU_LIMIT', flowCfg.model.deltaMuLimit);
...
if isempty(cfg.localBundleSource)
    cfg.localBundleSource = 'compact_case';
end
if ~ismember(cfg.localBundleSource, {'compact_case', 'dynamic_map'})
    error('STEP07J_LOCAL_BUNDLE_SOURCE must be "compact_case" or "dynamic_map".');
end
...
if strcmpi(cfg.localBundleSource, 'dynamic_map') && ~cfg.allowDynamicMapFallback
    error(['STEP07J_LOCAL_BUNDLE_SOURCE=dynamic_map is disabled by default because this fallback ' ...
        'can change the analysis bundle. Set STEP07J_ALLOW_DYNAMIC_MAP_FALLBACK=1 only for debugging.']);
end
...
templateBankFile = fullfile(outDir, flowCfg.calibration.templateBankFile);
gapBankFile = fullfile(outDir, flowCfg.calibration.gapBankFile);
...
if ~isempty(dynamicOverride)
    ...
    dynamicFile = find_dynamic_map_file_local({gapDynamicDir, rotDynamicDir}, cfg.targetBlade, cfg.analysisSensors);
end
...
Template = [];
if isempty(templateOverride) && isfile(templateBankFile)
...
fprintf('Local fallback bundle source: %s (DynamicMap debug fallback: %d).\n', ...
    cfg.localBundleSource, cfg.allowDynamicMapFallback);
fprintf('Fixed baseline EO mode: %s.\n', cfg.fixedEoMode);
...
for iw = 1:numWindows
    ...
    elseif strcmpi(cfg.localBundleSource, 'compact_case')
        error(['Compact waveform case is required for local bundle reconstruction but is missing. ' ...
            'Generate the matching CompactWaveformCase, use direct bundle auto mode with a valid direct result, ' ...
            'or set STEP07J_ALLOW_DYNAMIC_MAP_FALLBACK=1 and STEP07J_LOCAL_BUNDLE_SOURCE=dynamic_map for debugging only.']);
    elseif strcmpi(cfg.localBundleSource, 'dynamic_map') && cfg.allowDynamicMapFallback
        ...
        coreBundleFull.localSource = 'dynamic_map_debug_core';
        bundleSource = 'dynamic_map_debug';
    else
    ...
    if cfg.phaseSafeExpansion && phaseInfo.usable
        ...
        elseif strcmpi(bundleSource, 'dynamic_map_debug') && cfg.allowDynamicMapFallback
            expandedBundleFull = build_gap_observation_bundle_local( ...
                Wmap, Template, CorrectedGapLibrary, responseSurface, cfg, phaseInfo);
            expandedBundleFull.localSource = 'dynamic_map_debug_phase_safe';
        end
    ...
    for im = 1:numel(cfg.modelNames)
        ...
        modelFits.(modeName) = refine_static_warp_fit_local(bundle, seedTable, modelCandidateEO, cfgWindow, modeName);
    end
    if strcmpi(cfg.runMode, 'comparison') && isfield(modelFits, 'fixed')
...
if strcmpi(cfg.runMode, 'comparison')
    ...
    Result.method = 'gap_tilt_main_vp_full_wave';
end
...
etaRegWeightMv = cfg.etaRegWeightMv;
etaPenalty = pointCount * (etaRegWeightMv * sqrt(sum((eta ./ max(etaLimit, eps)).^2) / max(nSensor, 1))) ^ 2;
...
grid = responseSurface.xGrid(:);
finite1 = isfinite(grid) & isfinite(responseSurface.coeff(:, 1));
finite2 = isfinite(grid) & isfinite(responseSurface.coeff(:, 2));
finite3 = isfinite(grid) & isfinite(responseSurface.coeff(:, 3));
B0 = interp1(grid(finite1), responseSurface.coeff(finite1, 1), xq, 'linear', NaN);
B1 = interp1(grid(finite2), responseSurface.coeff(finite2, 2), xq, 'linear', NaN);
B2 = interp1(grid(finite3), responseSurface.coeff(finite3, 3), xq, 'linear', NaN);
...
if isfield(F, 'fixed')
    ...
    row.fixed_rmse_mV = residual_rmse_mv_local(F.fixed, wr.bundle);
...
if isfield(F, 'gap_only')
    ...
    row.gap_rmse_mV = residual_rmse_mv_local(F.gap_only, wr.bundle);
...
if isfield(F, 'gap_tilt')
    ...
    row.tilt_rmse_mV = residual_rmse_mv_local(F.gap_tilt, wr.bundle);
