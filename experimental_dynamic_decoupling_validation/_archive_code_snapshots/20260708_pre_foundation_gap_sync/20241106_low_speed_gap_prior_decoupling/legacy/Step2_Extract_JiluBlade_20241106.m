function summary_table = Step2_Extract_JiluBlade_20241106(target_cases, show_plots)
%STEP2_EXTRACT_JILUBLADE_20241106 Original-like Step2 for 20241106 dynamic cases.
% This version follows the 20241106 Step2 idea more closely:
...
if isfield(cfg, 'step2_sensor_ids') && ~isempty(cfg.step2_sensor_ids)
    step2_sensor_ids = cfg.step2_sensor_ids(:).';
else
    step2_sensor_ids = cfg.sensor_ids(:).';
end
...
config_path = fullfile(cfg.reference_output_dir, 'Sensor_Config_20241106.mat');
if ~isfile(config_path)
    Step1_Build_Sensor_Config_20241106();
end
...
for iCase = 1:numel(target_cases)
    ...
    case_rows = repmat(struct( ...
        ...
        'best_shift', NaN, ...
        'best_corr', NaN), numel(step2_sensor_ids), 1);

    for iSensor = 1:numel(step2_sensor_ids)
        sid = step2_sensor_ids(iSensor);
    ...
    writetable(case_summary, fullfile(output_dir, 'Step2_Summary_20241106.csv'));
    save(fullfile(output_dir, 'Step2_Summary_20241106.mat'), 'case_summary');
    all_rows = [all_rows; case_rows]; %#ok<AGROW>

    fprintf('\nCase %s completed. Output dir:\n  %s\n', case_name, output_dir);

    if show_plots
        plot_step2_case_local(case_data, cfg, case_name, case_summary, step2_sensor_ids);
    end
end
...
sensor_ids = get_step2_sensor_ids_local(cfg);
...
case_data.jiluOPR = [opr_result.start_times, opr_result.end_times];
...
for iFile = 1:numel(file_ids)
    ...
    arrival_times = [arrival_times; segments.start_sample / cfg.pinlv]; %#ok<AGROW>
end
...
if isempty(arrival_times) || isempty(peak_features) || numel(opr_times) <= cfg.opr_pulses_per_rev
    return;
end
...
spd_t = opr_times(1:(end - opr_pulses_per_rev));
spd_v = 360 ./ (opr_times((opr_pulses_per_rev + 1):end) - opr_times(1:(end - opr_pulses_per_rev)));
...
function plot_step2_case_local(case_data, cfg, case_name, case_summary, sensor_ids)
if nargin < 5 || isempty(sensor_ids)
    sensor_ids = get_step2_sensor_ids_local(cfg);
end
blades_num = cfg.blades_num;
colors = lines(blades_num);

figure('Name', sprintf('20241106 Step2 RPM - %s', case_name), ...
...
fig2 = figure('Name', sprintf('20241106 Step2 Blade ID Sequence - %s', case_name), ...
...
fig3 = figure('Name', sprintf('20241106 Step2 First-Lap Fingerprint - %s', case_name), ...
...
fig4 = figure('Name', sprintf('20241106 Step2 Revolution Shift Path - %s', case_name), ...
...
figure('Name', sprintf('20241106 Step2 Summary Table - %s', case_name), ...
...
function sensor_ids = get_step2_sensor_ids_local(cfg)
if isfield(cfg, 'step2_sensor_ids') && ~isempty(cfg.step2_sensor_ids)
    sensor_ids = cfg.step2_sensor_ids(:).';
else
