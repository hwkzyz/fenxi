function experiment_case = Prepare_experiment_case_single_sync_20241106(cfg, show_plots, save_output)
%PREPARE_EXPERIMENT_CASE_SINGLE_SYNC_20241106
% Build a unified dynamic experiment case from the Step2 outputs.
...
if nargin < 3
    if isfield(cfg, 'save_experiment_case')
        save_output = cfg.save_experiment_case;
    else
        save_output = true;
    end
end
...
if ~isfile(fullfile(cfg.case_output_dir, sprintf('jilublade_probe%d.mat', cfg.analysis_sensors(1))))
    Step2_Extract_JiluBlade_20241106(cfg.dynamic_case_name, false);
end
...
raw_stream = load_case_raw_streams_local(cfg);
...
for i = 1:numel(cfg.analysis_sensors)
    ...
    if isempty(blade_rows)
        warning('No target-blade rows found for CH%d after %.3f s.', sid, cfg.analysis_start_time);
        continue;
    end
...
if isfield(omega_loaded, 'omega_rpm')
    experiment_case.Diagnostics.rot_freq_mean_hz = median(omega_loaded.omega_rpm, 'omitnan') / 60;
else
    experiment_case.Diagnostics.rot_freq_mean_hz = median(1 ./ (cfg.opr_pulses_per_rev .* diff(opr_times)), 'omitnan');
end

if save_output
    save(cfg.case_file, 'experiment_case', '-v7.3');
    fprintf('Saved 20241106 experiment case to %s\n', cfg.case_file);
else
    fprintf('Prepared 20241106 experiment case in memory for Blade %d.\n', cfg.target_blade);
end

if show_plots
    plot_experiment_case_local(experiment_case, cfg);
end
end


function raw_stream = load_case_raw_streams_local(cfg)
...
for i = 1:numel(file_ids)
    ...
    for sid = cfg.analysis_sensors
        if ~isfile(fullfile(cfg.dynamic_data_dir, sprintf('4-%d-%d.mat', sid, file_id)))
            continue;
        end
        [t_local, v_local] = load_raw_case_channel_local(cfg.dynamic_data_dir, sid, file_id, cfg.pinlv);
        raw_stream(sid).T = [raw_stream(sid).T; t_local(:) + offset]; %#ok<AGROW>
...
function plot_experiment_case_local(experiment_case, cfg)
fig = figure('Name', sprintf('20241106 Experiment Case - %s', cfg.dynamic_case_name), ...
...
for i = 1:numel(cfg.analysis_sensors)
    ...
    plot(experiment_case.Raw_Stream(sid).T, experiment_case.Raw_Stream(sid).V, '-', ...
        'Color', [0.75 0.75 0.75], 'LineWidth', 0.5, 'DisplayName', 'Raw');
    if ~isempty(data_idx)
        rows = experiment_case.Extracted_Data(data_idx).selected_rows;
        probe_path = fullfile(cfg.case_output_dir, sprintf('jilublade_probe%d.mat', sid));
        loaded_probe = load(probe_path, 'jilublade');
        selected = loaded_probe.jilublade(rows, :);
        scatter(selected(:, 3), zeros(size(rows)), 20, [0.85 0.33 0.10], 'filled', 'DisplayName', 'Target blade peaks');
    end
    xlabel('Time (s)');
    ylabel('Voltage (V)');
    title(sprintf('CH%d raw window selection', sid));
    xlim([cfg.analysis_start_time - 0.2, cfg.analysis_start_time + 2.0]);
    ...
    if ~isempty(data_idx)
        ...
        for k = 1:numel(laps)
            plot(laps(k).x_points, laps(k).v_points, '-', 'Color', cmap(k, :), ...
                'LineWidth', 1.2, 'DisplayName', sprintf('Lap %d', k));
        end
    end
    xlabel('Restored position (mm)');
    ylabel('Voltage (V)');
    title(sprintf('CH%d mapped target-blade pulses', sid));
    legend('Location', 'best');
end
end
