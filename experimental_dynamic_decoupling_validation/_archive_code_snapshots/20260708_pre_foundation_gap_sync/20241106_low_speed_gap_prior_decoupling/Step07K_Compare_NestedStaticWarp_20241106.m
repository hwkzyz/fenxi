function Step07K_Compare_NestedStaticWarp_20241106
%% Compare Step07J methods, 20241106
...
if ~isfile(resultFile)
    ...
    evalin('base', 'Step07J_NestedStaticWarp_VPFullWave_20241106');
    delete(cleanupEnv);
...
Result = S.Result;
Trend = Result.Trend;
if isfield(Result, 'cfg') && isfield(Result.cfg, 'targetBlade') && isfield(Result.cfg, 'analysisSensors')
    bladeTag = sprintf('B%d_S%s', Result.cfg.targetBlade, sprintf('%d', Result.cfg.analysisSensors));
end

fprintf('\n=== Step07K method comparison: %s %s ===\n', dataset, bladeTag);
fprintf('Result: %s\n', resultFile);
...
summaryFile = fullfile(outDir, sprintf( ...
    'Step07K_Compare_NestedStaticWarp_Summary_%s_%s_%s.csv', ...
    dataset, bladeTag, resultSuffix));
writetable(Summary, summaryFile);

windowFile = fullfile(outDir, sprintf( ...
    'Step07K_Compare_NestedStaticWarp_Window_%s_%s_%s.csv', ...
    dataset, bladeTag, resultSuffix));
...
figFile = fullfile(figDir, sprintf( ...
    'Step07K_Compare_NestedStaticWarp_Trend_%s_%s_%s.png', ...
    dataset, bladeTag, resultSuffix));
...
makeWaveformFigures = parse_logical_env_local('STEP07J_COMPARE_WAVEFORM_FIGURES', false);
if makeWaveformFigures
    fprintf('\nWaveform-figure auto-generation is disabled by default for 20241106.\n');
end
