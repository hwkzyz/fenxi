...
% The main gap_only route jointly identifies EO, vibration and static gap
% increment inside the VP Top-K EO candidate set with the full waveform
% objective. VP narrows the EO candidates but does not fix the final EO.
% This script compares nested static correction models on top of the
% low-speed template:
%
...
if isempty(cfg.eoCandidateMode)
    cfg.eoCandidateMode = 'direct_plus_vp';
end
...
for iw = 1:numWindows
    ...
    if numel(directRef.WindowResult) >= iw && isfield(directRef.WindowResult(iw), 'bundle') && ...
    ...
    for im = 1:numel(cfg.modelNames)
        ...
        fitEO = selectedEO;
        modelFits.(modeName) = refine_static_warp_fit_local(bundle, seedTable, fitEO, cfg, modeName);
    end
    wr = pack_window_result_local(iw, Wmap, bundleFull, seedTable, selectedEO, mainCandidateEO, modelFits);
    if iw == 1
...
Result.description = ['Main result uses the scalar gap-only high-speed static increment. ' ...
    'VP Top-K provides only the EO candidate set; gap_only jointly identifies EO, vibration and gap increment inside that set with the full waveform objective. ' ...
    'Fixed, gap+tilt and gap+tilt+shift remain diagnostic ablation models.'];
Result.cfg = cfg;
...
matFile = fullfile(outDir, sprintf('Step07J_NestedStaticWarp_VPFullWave_20250527_B%d_%s.mat', ...
    cfg.targetBlade, [sensorTag resultSuffix]));
trendCsv = fullfile(outDir, sprintf('Step07J_NestedStaticWarp_VPFullWave_Trend_20250527_B%d_%s.csv', ...
    cfg.targetBlade, [sensorTag resultSuffix]));
summaryCsv = fullfile(outDir, sprintf('Step07J_NestedStaticWarp_VPFullWave_Summary_20250527_B%d_%s.csv', ...
    cfg.targetBlade, [sensorTag resultSuffix]));
figTrend = fullfile(figDir, sprintf('Step07J_Trend_20250527_B%d_%s.png', cfg.targetBlade, [sensorTag resultSuffix]));
figBest = fullfile(figDir, sprintf('Step07J_BestWindow_Collapse_20250527_B%d_%s.png', cfg.targetBlade, [sensorTag resultSuffix]));
figGap = fullfile(figDir, sprintf('Step07J_StaticWarp_20250527_B%d_%s.png', cfg.targetBlade, [sensorTag resultSuffix]));
...
sensorTag = ['S', sprintf('%d', sensorIds)];
preferred = {
    sprintf('Result_Step03_Main_VPTop3SynchronousWaveform_B%d_%s_Main_DirectTemplate_OPRCenterStd_20250527.mat', targetBlade, sensorTag)
    sprintf('Result_Step03_Main_VPTop3SynchronousWaveform_B%d_%s*DirectTemplate_OPRCenterStd*_20250527.mat', targetBlade, sensorTag)
    sprintf('Result_Step03_Main_VPTop3SynchronousWaveform_B%d_%s*OPRCenterStd*_20250527.mat', targetBlade, sensorTag)
    sprintf('Result_Step03_Main_VPTop3SynchronousWaveform_B%d_%s_Diag80L_W3S1_BaseXc_GradientTpl_20250527.mat', targetBlade, sensorTag)
    sprintf('Result_Step03_Main_VPTop3SynchronousWaveform_B%d_%s*BaseXc_GradientTpl*_20250527.mat', targetBlade, sensorTag)
    sprintf('Result_Step03_Main_VPTop3SynchronousWaveform_B%d_%s_Main_GradientXRange030_20250527.mat', targetBlade, sensorTag)
    sprintf('Result_Step03_Main_VPTop3SynchronousWaveform_B%d_%s*GradientXRange030*_20250527.mat', targetBlade, sensorTag)
    sprintf('Result_Step03_Main_VPTop3SynchronousWaveform_B%d_%s_Main_GradientXRange030_OPRAnchored_20250527.mat', targetBlade, sensorTag)
    sprintf('Result_Step03_Main_VPTop3SynchronousWaveform_B%d_%s_Main_GradientXRange030_OPRAnchored*_20250527.mat', targetBlade, sensorTag)
    };
...
function wr = pack_window_result_local(iw, Wmap, bundle, seedTable, selectedEO, mainCandidateEO, modelFits)
wr = struct();
wr.windowId = iw;
