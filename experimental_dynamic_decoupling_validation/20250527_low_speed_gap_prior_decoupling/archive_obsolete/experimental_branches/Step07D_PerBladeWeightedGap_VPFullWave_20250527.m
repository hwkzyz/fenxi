%% Step07D: Low-speed template baseline + per-blade weighted gap increment
% The first-order VP step only screens EO candidates. Final scoring uses the
% full waveform model:
%
%   V_s(t) = M_s(g0_s + dg_s, x_s(t) - dx_c - u(t))
%
% where M_s is the Step06G low-speed-template anchored per-blade weighted
% incremental gap library. When dg_s = 0, M_s reduces to the measured
% low-speed template. This experimental branch does not overwrite Step07C.
...
figDir = fullfile(outDir, 'figures_step07d_per_blade_weighted_gap_vp_full_wave');
...
cfg.eoPad = 2;
cfg.vpTopK = parse_positive_integer_env_local('STEP07D_TOP_K_EO', 3);
cfg.maxWindows = parse_positive_integer_env_local('STEP07D_MAX_WINDOWS', inf);
cfg.maxPointsPerWindow = parse_positive_integer_env_local('STEP07D_MAX_POINTS_PER_WINDOW', 900);
cfg.amplitudeLimitMm = parse_nonnegative_numeric_env_local('STEP07D_AMP_LIMIT_MM', 0.50);
cfg.dxLimitMm = parse_nonnegative_numeric_env_local('STEP07D_DX_LIMIT_MM', 0.20);
cfg.deltaGapLimitMm = parse_nonnegative_numeric_env_local('STEP07D_DELTA_G_LIMIT_MM', 0.25);
cfg.derivativeStepMm = 1e-3;
cfg.weightFloor = 0.05;
cfg.defaultSensorThreshold = 0.5;
cfg.pulseSelectionMode = lower(strtrim(getenv('STEP07D_PULSE_MODE')));
if isempty(cfg.pulseSelectionMode)
    cfg.pulseSelectionMode = 'all';
end
if ~ismember(cfg.pulseSelectionMode, {'single', 'all'})
    error('STEP07D_PULSE_MODE must be "single" or "all".');
end
cfg.dynamicGradientMinRatio = parse_nonnegative_numeric_env_local('STEP07D_DYNAMIC_GRADIENT_MIN_RATIO', 0.08);
cfg.domainSoftMarginMm = parse_nonnegative_numeric_env_local('STEP07D_DOMAIN_SOFT_MARGIN_MM', 0.20);
cfg.overshootPenaltyWeight = parse_nonnegative_numeric_env_local('STEP07D_OVERSHOOT_PENALTY_WEIGHT', 100);
cfg.fminOptions = optimset('Display', 'off', 'MaxIter', 450, 'MaxFunEvals', 1800, ...
    'TolX', 1e-5, 'TolFun', 1e-5);

sensorOverride = strtrim(getenv('STEP07D_ANALYSIS_SENSORS'));
if ~isempty(sensorOverride)
    parsedSensors = sscanf(sensorOverride, '%d').';
    if isempty(parsedSensors)
        error('STEP07D_ANALYSIS_SENSORS must contain integer sensor IDs, for example "1 3 6".');
    end
...
correctedLibFile = fullfile(outDir, sprintf('Step06G_PerBladeWeighted_GapLibrary_20250527_B%d_%s.mat', ...
...
if ~isfile(correctedLibFile)
    error('Run Step06G first. Missing file: %s', correctedLibFile);
end

St = load(templateFile, 'Template');
Sd = load(dynamicFile, 'DynamicMap');
Sc = load(correctedLibFile, 'PerBladeWeightedGapLibrary');
...
CorrectedGapLibrary = Sc.PerBladeWeightedGapLibrary;
responseSurface = CorrectedGapLibrary.PerBladeResponseLibrary;
...
fprintf('\n=== Step07D: per-blade weighted gap VP/full-wave ===\n');
fprintf('Template: %s\n', templateFile);
fprintf('DynamicMap: %s\n', dynamicFile);
fprintf('Per-blade weighted gap library: %s\n', correctedLibFile);
...
Result.method = 'low_template_anchored_per_blade_weighted_gap_vp_full_wave';
Result.description = ['Step06G per-blade weighted gap library is used only as a clearance-change increment. ' ...
...
matFile = fullfile(outDir, sprintf('Step07D_PerBladeWeightedGap_VPFullWave_20250527_B%d_%s.mat', ...
    cfg.targetBlade, sensorTag));
trendCsv = fullfile(outDir, sprintf('Step07D_PerBladeWeightedGap_VPFullWave_Trend_20250527_B%d_%s.csv', ...
    cfg.targetBlade, sensorTag));
summaryCsv = fullfile(outDir, sprintf('Step07D_PerBladeWeightedGap_VPFullWave_Summary_20250527_B%d_%s.csv', ...
    cfg.targetBlade, sensorTag));
figTrend = fullfile(figDir, sprintf('Step07D_Trend_20250527_B%d_%s.png', cfg.targetBlade, sensorTag));
figBest = fullfile(figDir, sprintf('Step07D_BestWindow_Collapse_20250527_B%d_%s.png', cfg.targetBlade, sensorTag));
figGap = fullfile(figDir, sprintf('Step07D_GapIdentification_20250527_B%d_%s.png', cfg.targetBlade, sensorTag));
...
sensorTag = ['S', sprintf('%d', sensorIds)];
candidate = fullfile(dynamicDir, sprintf('DynamicMap_B%d_%s_SlidingWindows_20250527.mat', targetBlade, sensorTag));
if isfile(candidate)
...
sensorTag = ['S', sprintf('%d', sensorIds)];
files = dir(fullfile(resultDir, sprintf('Result_Step03_Main_VPTop3SynchronousWaveform_B%d_%s*_20250527.mat', ...
...
for is = 1:numel(sensorIds)
    ...
    F0(mask) = eval_corrected_template_local(Tpl, responseSurface, corr, 0, X(mask));
    Fx(mask) = eval_corrected_template_derivative_local(Tpl, responseSurface, corr, 0, X(mask), cfg.derivativeStepMm);
    g0BySensor(is) = weighted_sensor_g0_local(corr);
...
for is = 1:numel(cfg.analysisSensors)
    ...
    f0Raw = eval_corrected_template_local(Tpl, responseSurface, corr, 0, xRaw);
    fxRaw = eval_corrected_template_derivative_local(Tpl, responseSurface, corr, 0, xRaw, cfg.derivativeStepMm);
...
for is = 1:numel(bundle.sensorIds)
    ...
    [vPred(mask), overshoot(mask)] = eval_corrected_template_local(Tpl, ...
        bundle.responseSurface, corr, dgNow, xEval);
end
...
function [model, overshoot] = eval_corrected_template_local(Tpl, responseSurface, corr, dg, xOpr)
xLowRaw = xOpr(:);
xLo = min(Tpl.x_grid(:));
xHi = max(Tpl.x_grid(:));
...
delta = zeros(size(vLow));
overshoot = overshootLow;
for ib = 1:numel(corr.candidate)
    C = corr.candidate(ib);
    if ~isfinite(C.bladeWeight) || C.bladeWeight <= 0
        continue;
    end
    bladeSurface = responseSurface.blade(C.bladeIndex);
    [Fdyn, overDyn] = eval_raw_tilt_path_local(bladeSurface, C.g0Mm + dg, C.tauMm, ...
        C.xScale, C.muGapPerXMm, xOpr);
    [Fbase, overBase] = eval_raw_tilt_path_local(bladeSurface, C.g0Mm, C.tauMm, ...
        C.xScale, C.muGapPerXMm, xOpr);
    delta = delta + C.bladeWeight .* C.voltageGain .* (Fdyn - Fbase);
    overshoot = max(overshoot, max(overDyn, overBase));
end
model = vLow + delta;
end

function dMdx = eval_corrected_template_derivative_local(Tpl, responseSurface, corr, dg, xOpr, h)
[mp, ~] = eval_corrected_template_local(Tpl, responseSurface, corr, dg, xOpr + h);
[mm, ~] = eval_corrected_template_local(Tpl, responseSurface, corr, dg, xOpr - h);
dMdx = (mp - mm) ./ (2 * h);
end

function [model, overshoot] = eval_raw_tilt_path_local(bladeSurface, g0, tau, k, mu, xOpr)
xLibRaw = k .* (xOpr(:) - tau);
...
function g0 = weighted_sensor_g0_local(corr)
C = corr.candidate;
w = [C.bladeWeight].';
g = [C.g0Mm].';
valid = isfinite(w) & isfinite(g) & w > 0;
if ~any(valid)
    g0 = NaN;
else
    g0 = sum(w(valid) .* g(valid)) / sum(w(valid));
end
end
...
fig = figure('Name', 'Step07D trend', 'Color', 'w', ...
...
fig = figure('Name', 'Step07D best-window dynamic collapse', 'Color', 'w', ...
...
for is = 1:numel(cfg.analysisSensors)
    ...
    Tpl = get_template_sensor_local(bundle.Template, sid);
    corr = get_corrected_sensor_local(bundle.CorrectedGapLibrary, sid);
    dgNow = fit.deltaGapMm(is);
    [modelNo, ~] = eval_corrected_template_local(Tpl, bundle.responseSurface, corr, dgNow, xRaw);
    [modelDyn, ~] = eval_corrected_template_local(Tpl, bundle.responseSurface, corr, dgNow, xComp);
    [xs1, o1] = sort(xRaw);
    [xs2, o2] = sort(xComp);
    vObs = bundle.V(mask);
...
style = paper_style_local();
fig = figure('Name', 'Step07D gap identification', 'Color', 'w', ...
