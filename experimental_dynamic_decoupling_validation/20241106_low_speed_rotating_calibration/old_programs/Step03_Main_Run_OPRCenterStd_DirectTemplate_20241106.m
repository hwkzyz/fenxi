%% Step03_Main_Run_OPRCenterStd_DirectTemplate_20241106
% Identify vibration directly from the OPRCenterStd low-speed template.
...
P.name.analysisStartTime = 75.0;
runLabelTimeMs = round(1000 * P.name.analysisStartTime);
...
P.step03.targetBlade = 6;
P.step03.analysisSensors = [2 3 5 7];
...
P.step03.eoLock = [];
if isempty(P.step03.eoLock)
    P.name.resultSuffix = sprintf('Direct_%s', P.name.runLabel);
else
    P.name.resultSuffix = sprintf('DirectEO%d_%s', P.step03.eoLock, P.name.runLabel);
end

envOverridesEnabled = use_env_overrides_local();
if envOverridesEnabled
    ...
    resultSuffixEnv = strtrim(getenv('STEP20241106_RESULT_SUFFIX'));
    if ~isempty(resultSuffixEnv)
        P.name.resultSuffix = resultSuffixEnv;
    end
    eoLockEnv = str2double(strtrim(getenv('STEP20241106_EO_LOCK')));
    eoLockText = lower(strtrim(getenv('STEP20241106_EO_LOCK')));
    if ~isempty(eoLockText)
        if ismember(eoLockText, {'free', 'none', 'all', '[]'})
            P.step03.eoLock = [];
        elseif isfinite(eoLockEnv) && eoLockEnv >= 1
            P.step03.eoLock = round(eoLockEnv);
        end
    end
    if isempty(resultSuffixEnv)
        if isempty(P.step03.eoLock)
    ...
    dxCLimitEnv = str2double(strtrim(getenv('STEP20241106_DXC_LIMIT_MM')));
    if isfinite(dxCLimitEnv) && dxCLimitEnv > 0
        P.step03.dxCLimitMM = dxCLimitEnv;
    end
end

%% Files
sensorTag = ['S', sprintf('%d', P.step03.analysisSensors)];
templateFile = fullfile(routeDir, 'output', 'templates', ...
    sprintf('TemplateBundle_LowSpeedRotating_B%d_%s_%s_20241106.mat', ...
    P.step03.targetBlade, sensorTag, P.name.templateSuffix));
dynamicFile = fullfile(routeDir, 'output', 'dynamic_maps', ...
    sprintf('DynamicMap_B%d_%s_SlidingWindows_%s_20241106.mat', ...
    P.step03.targetBlade, sensorTag, P.name.dynamicSuffix));

fprintf('\n=== Step03: direct-template identification ===\n');
fprintf('Environment overrides: %s\n', logical_to_text_local(envOverridesEnabled));
step03_identify_direct_template_embedded(routeDir, P, templateFile, dynamicFile);
...
function text = logical_to_text_local(tf)
if tf
    text = 'on';
else
    text = 'off';
...
function step03_identify_direct_template_embedded(rootDir, P, template_file, dynamic_file)
%% Step03_Main_VPTop3SynchronousWaveform_20241106
...
cfg = struct();
cfg.target_blades = P.step03.targetBlade;
cfg.analysis_sensors = P.step03.analysisSensors;
cfg.analysis_start_time = NaN;
cfg.target_laps = NaN;
cfg.analysis_win_size = NaN;
cfg.sliding_step = NaN;
...
result_file = fullfile(result_dir, sprintf('Result_Step03_Main_VPTop3SynchronousWaveform_B%d_%s%s_20241106.mat', ...
    cfg.target_blades, sensor_tag, result_suffix));

loaded_dynamic = load(dynamic_file, 'DynamicMap');
Template = load_template_for_sensors_local(template_file, cfg.analysis_sensors, sensor_tag);
...
cfg.target_blades = DynamicMap.TargetBlade;
...
if isfield(DynamicMap, 'SelectionInfo') && isstruct(DynamicMap.SelectionInfo) && ...
        isfield(DynamicMap.SelectionInfo, 'use_region_plan') && DynamicMap.SelectionInfo.use_region_plan
    fprintf(['Selected high-speed region: region %d, time [%.6f, %.6f] s, ' ...
        'peak %.6f s, EO %.0f, dominant freq %.3f Hz.\n'], ...
        DynamicMap.SelectionInfo.region_id, DynamicMap.SelectionInfo.start_time_sec, ...
        DynamicMap.SelectionInfo.end_time_sec, DynamicMap.SelectionInfo.peak_time_sec, ...
        DynamicMap.SelectionInfo.dominant_order, DynamicMap.SelectionInfo.dominant_freq_hz);
end
...
if ~coordinate_check.is_consistent && legacy_base_xc_gradient_tpl
    fprintf('Coordinate note: legacy 20241106 route uses base-xc DynamicMap with GradientXRange030 template.\n');
...
if ~isempty(cfg.eo_lock)
    fprintf('EO lock enabled: only EO%d is refined in the final waveform objective.\n', cfg.eo_lock);
end
...
patterns = {
    sprintf('TemplateBundle_LowSpeedRotating_B%d_%s_GradientXRange030_OPRCenterStd_20241106.mat', target_blade, sensor_tag)
    sprintf('TemplateBundle_LowSpeedRotating_B%d_%s_GradientXRange030_20241106.mat', target_blade, sensor_tag)
    sprintf('TemplateBundle_LowSpeedRotating_B%d_%s_20241106.mat', target_blade, sensor_tag)
    sprintf('Template_LowSpeedRotating_B%d_%s_GradientXRange030_OPRCenterStd_20241106.mat', target_blade, sensor_tag)
    sprintf('Template_LowSpeedRotating_B%d_%s_GradientXRange030_20241106.mat', target_blade, sensor_tag)
    sprintf('Template_LowSpeedRotating_B%d_%s_20241106.mat', target_blade, sensor_tag)
    };
...
files = [ ...
    dir(fullfile(template_dir, sprintf('TemplateBundle_LowSpeedRotating_B%d_*.mat', target_blade))); ...
    dir(fullfile(template_dir, sprintf('Template_LowSpeedRotating_B%d_*.mat', target_blade)))];
if isempty(files)
    error('No low-speed template file found under %s.', template_dir);
end
template_file = find_sensor_superset_file_local(files, analysis_sensors, 'Template');
if ~isempty(template_file)
    return;
end
error('Template for sensor tag %s not found, and no superset template contains sensors %s.', ...
    sensor_tag, mat2str(analysis_sensors));
end

function dynamic_file = find_dynamic_file_for_sensors_local(dynamic_dir, target_blade, analysis_sensors)
sensor_tag = ['S', sprintf('%d', analysis_sensors)];
patterns = {
    sprintf('DynamicMap_B%d_%s_SlidingWindows_Main20L_W3S1_GradientXRange030_OPRCenterStd_20241106.mat', target_blade, sensor_tag)
    sprintf('DynamicMap_B%d_%s_SlidingWindows_20241106.mat', target_blade, sensor_tag)
    sprintf('DynamicMap_B%d_%s_SlidingWindows_GradientXRange030_20241106.mat', target_blade, sensor_tag)
    };
for ip = 1:numel(patterns)
...
dynamic_file = find_sensor_superset_file_local(files, analysis_sensors, 'DynamicMap');
if ~isempty(dynamic_file)
    return;
end
error('DynamicMap for sensor tag %s not found, and no superset DynamicMap contains sensors %s.', ...
    sensor_tag, mat2str(analysis_sensors));
end

function selected_file = find_sensor_superset_file_local(files, requested_sensors, variable_name)
selected_file = '';
best_rank = inf;
best_time = -inf;
for iFile = 1:numel(files)
    file_path = fullfile(files(iFile).folder, files(iFile).name);
    sensor_ids = load_sensor_ids_from_file_local(file_path, variable_name);
    if isempty(sensor_ids) || ~all(ismember(requested_sensors, sensor_ids))
        continue;
    end
    rank = numel(sensor_ids) - numel(requested_sensors);
    if rank < best_rank || (rank == best_rank && files(iFile).datenum > best_time)
        best_rank = rank;
        best_time = files(iFile).datenum;
        selected_file = file_path;
    end
end
end

function sensor_ids = load_sensor_ids_from_file_local(file_path, variable_name)
sensor_ids = [];
loaded = load(file_path);
if strcmp(variable_name, 'Template') && isfield(loaded, 'TemplateBundle')
    sensor_ids = loaded.TemplateBundle.SensorIDs(:).';
    return;
end
if isfield(loaded, variable_name)
    S = loaded.(variable_name);
    if isfield(S, 'SensorIDs') && ~isempty(S.SensorIDs)
        sensor_ids = S.SensorIDs(:).';
        return;
    end
    if isfield(S, 'Sensor') && ~isempty(S.Sensor)
        sensor_ids = [S.Sensor.sensor_id];
    end
    return;
end
if strcmp(variable_name, 'Template') && isfield(loaded, 'sensor_template')
    S = loaded.sensor_template;
    if isfield(S, 'SensorIDs') && ~isempty(S.SensorIDs)
        sensor_ids = S.SensorIDs(:).';
        return;
    end
...
if isfield(loaded, 'Sensor') && ~isempty(loaded.Sensor)
    S = loaded;
    sensor_ids = [S.Sensor.sensor_id];
end
end

function Template = load_template_for_sensors_local(template_file, analysis_sensors, sensor_tag)
loaded = load(template_file);
if isfield(loaded, 'TemplateBundle')
    Template = load_template_from_bundle_local(loaded.TemplateBundle, analysis_sensors, sensor_tag);
    return;
end
if isfield(loaded, 'sensor_template')
    Template = filter_template_sensors_local(loaded.sensor_template, analysis_sensors, sensor_tag);
    return;
end
if isfield(loaded, 'Template')
    Template = filter_template_sensors_local(loaded.Template, analysis_sensors, sensor_tag);
    return;
end
error('Unsupported template file format: %s', template_file);
end

function Template = load_template_from_bundle_local(bundle, analysis_sensors, sensor_tag)
for is = 1:numel(analysis_sensors)
    ...
    sensor_template = load_template_for_sensors_local(char(row.template_file(1)), sid, sprintf('CH%d', sid));
    if is == 1
...
if ~isempty(cfg.eo_lock)
    eo_candidates = unique(round(cfg.eo_lock(:).'));
end
core_bundle = bundle;
