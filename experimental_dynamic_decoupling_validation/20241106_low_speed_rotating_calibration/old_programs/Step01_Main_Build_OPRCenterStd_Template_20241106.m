...
P.step01.targetBlade = 6;
P.step01.analysisSensors = [2 3 5 7];
P.step01.calibrationLaps = 30;
...
if use_env_overrides_local()
    ...
    if isfinite(targetBladeEnv) && targetBladeEnv >= 1
        P.step01.targetBlade = round(targetBladeEnv);
    end
    analysisSensorsEnv = sscanf(strtrim(getenv('STEP20241106_ANALYSIS_SENSORS')), '%d').';
    if ~isempty(analysisSensorsEnv)
        P.step01.analysisSensors = unique(analysisSensorsEnv, 'stable');
    end
    calibrationLapsEnv = str2double(strtrim(getenv('STEP20241106_CALIBRATION_LAPS')));
    if isfinite(calibrationLapsEnv) && calibrationLapsEnv >= 1
        P.step01.calibrationLaps = round(calibrationLapsEnv);
    end
end

%% Files
step01SensorTag = ['S', sprintf('%d', P.step01.analysisSensors)];
stablePlanFile = fullfile(routeDir, 'output', 'stable_window_plan', ...
    sprintf('Step01_CoverageFirstStableWindowPlan_B%d_%s_20241106.csv', ...
    P.step01.targetBlade, step01SensorTag));
referenceCenterTemplateFile = fullfile(routeDir, 'output', 'templates', ...
    ...
    P.step01.targetBlade, step01SensorTag, P.name.referenceTemplateSuffix));
...
if exist(stablePlanFile, 'file') ~= 2
    error(['Stable-window plan not found:\n  %s\n' ...
        'The old planning script is archived under archive_previous_routes/old_programs.'], ...
        stablePlanFile);
end
...
setenv('STEP01_TARGET_BLADE', num2str(P.step01.targetBlade));
setenv('STEP01_ANALYSIS_SENSORS', sprintf('%d ', P.step01.analysisSensors));
setenv('STEP01_CALIBRATION_LAPS', num2str(P.step01.calibrationLaps));
end
...
mainLegacyDir = fullfile(validationRoot, '20241106_low_speed_gap_prior_decoupling', 'legacy');
...
requiredFiles = {
    ...
    fullfile(caseOutputDir, sprintf('jilublade_probe%d.mat', cfg.reference_sensor_id))
    };
...
if hasOutputs && ~forceLegacyRebuild
    ...
    Step2_Extract_JiluBlade_20241106(caseName, showPlots);
end
...
if needsOprCenter
    ...
    [omega_time_s, omega_rad_s, omega_rpm] = compute_speed_local(centerTimes, cfg.opr_pulses_per_rev);
    ...
    save(omegaFile, 'omega_zong', 'omega_time_s', 'omega_rad_s', 'omega_rpm', 'metadata');
else
...
fig = figure('Name', '20241106 Step01 OPR-center and blade timing', ...
...
if P.view.saveFigures
    ...
    saveas(fig, fullfile(outDir, 'Step01_OPRCenter_Blade_Timing_20241106.png'));
end
...
%% Step01_Build_Rotating_Template_20241106
...
legacy_dir = fullfile(validation_root, '20241106_low_speed_gap_prior_decoupling', 'legacy');
if exist(legacy_dir, 'dir') ~= 7
    error('Legacy 20241106 helper folder not found: %s', legacy_dir);
end
addpath(legacy_dir);

legacy_cfg = Get_20241106_BTT_Config();
low_speed_case = legacy_cfg.low_speed_case;
low_speed_data_dir = fullfile(legacy_cfg.dataset_root, low_speed_case);
sensor_config_file = fullfile(legacy_cfg.reference_output_dir, 'Sensor_Config_20241106.mat');
target_blade = parse_positive_integer_env_local('STEP01_TARGET_BLADE', 2);
analysis_sensors = parse_integer_vector_env_local('STEP01_ANALYSIS_SENSORS', [5 7]);
sensor_tag = ['S', sprintf('%d', analysis_sensors)];
calibration_laps = parse_positive_integer_env_local('STEP01_CALIBRATION_LAPS', 30);
...
template_bundle_file = fullfile(template_dir, sprintf('TemplateBundle_LowSpeedRotating_B%d_%s%s_20241106.mat', ...
...
fprintf('Template save mode: one file per sensor + one lightweight bundle index.\n');
...
if ~isfile(sensor_config_file)
    fprintf('Sensor_Config missing. Rebuilding low-speed fingerprint config first...\n');
    Step1_Build_Sensor_Config_20241106(false);
end
if ~isfile(sensor_config_file)
    error('Sensor_Config not found after rebuild attempt: %s', sensor_config_file);
end
...
F_omega_deg = build_phase_speed_local(opr_times, legacy_cfg.opr_pulses_per_rev);
...
sensor_template_files = cell(numel(analysis_sensors), 1);
for is = 1:numel(analysis_sensors)
    sid = analysis_sensors(is);
    sensor_template = build_single_sensor_template_local(Template, Template.Sensor(is));
    sensor_template_file = make_sensor_template_file_local(template_dir, target_blade, sid, template_suffix);
    save(sensor_template_file, 'sensor_template', '-v7.3');
    sensor_template_files{is} = sensor_template_file;
    fprintf('Saved CH%d template: %s\n', sid, sensor_template_file);
end

TemplateBundle = build_template_bundle_local(Template, sensor_template_files, template_bundle_file); %#ok<NASGU>
save(template_bundle_file, 'TemplateBundle', '-v7.3');
fprintf('Saved template bundle index: %s\n', template_bundle_file);
...
function sensor_template_file = make_sensor_template_file_local(template_dir, target_blade, sid, template_suffix)
sensor_template_file = fullfile(template_dir, sprintf( ...
    'Template_LowSpeedRotating_B%d_CH%d%s_20241106.mat', target_blade, sid, template_suffix));
end

function sensor_template = build_single_sensor_template_local(template, sensor_struct)
sensor_template = template;
sensor_template.SensorIDs = sensor_struct.sensor_id;
sensor_template.SensorTag = sprintf('CH%d', sensor_struct.sensor_id);
sensor_template.Sensor = sensor_struct;
end

function bundle = build_template_bundle_local(template, sensor_template_files, bundle_file)
bundle = struct();
...
bundle.SensorFiles = table(template.SensorIDs(:), string(sensor_template_files(:)), ...
...
function vec = parse_integer_vector_env_local(name, default_value)
raw = strtrim(getenv(name));
if isempty(raw)
    vec = default_value(:).';
    return;
...
vec = sscanf(raw, '%d').';
if isempty(vec)
    error('%s must contain one or more integer sensor IDs.', name);
end
...
bundle_loaded = load(template_file);
if isfield(bundle_loaded, 'TemplateBundle')
    for i = 1:numel(sensor_ids)
        row = bundle_loaded.TemplateBundle.SensorFiles( ...
            bundle_loaded.TemplateBundle.SensorFiles.sensor_id == sensor_ids(i), :);
        if height(row) ~= 1
            error('Reference center bundle missing CH%d entry: %s', sensor_ids(i), template_file);
        end
        sensor_loaded = load(char(row.template_file(1)));
        reference_center.xc_mm(i) = extract_sensor_xc_local(sensor_loaded, sensor_ids(i), char(row.template_file(1)));
    end
    return;
end
for i = 1:numel(sensor_ids)
    reference_center.xc_mm(i) = extract_sensor_xc_local(bundle_loaded, sensor_ids(i), template_file);
end
end

function xc = extract_sensor_xc_local(loaded_template, sensor_id, source_file)
xc = NaN;
if isfield(loaded_template, 'sensor_template')
    T = loaded_template.sensor_template;
    if isfield(T, 'Sensor') && isstruct(T.Sensor) && isfield(T.Sensor, 'xc') && ...
            isfield(T.Sensor, 'sensor_id') && T.Sensor.sensor_id == sensor_id
        xc = T.Sensor.xc;
        return;
    end
end
if isfield(loaded_template, 'Template')
    T = loaded_template.Template;
    if isfield(T, 'Sensor') && ~isempty(T.Sensor)
        sensor_struct = T.Sensor;
        if numel(sensor_struct) == 1 && isfield(sensor_struct, 'sensor_id') && sensor_struct.sensor_id == sensor_id
            xc = sensor_struct.xc;
            return;
        end
        available = [sensor_struct.sensor_id];
        idx = find(available == sensor_id, 1);
        if ~isempty(idx) && isfield(sensor_struct(idx), 'xc')
            xc = sensor_struct(idx).xc;
            return;
        end
    end
end
error('Reference center template missing xc for CH%d: %s', sensor_id, source_file);
end
...
opr_reference = build_opr_reference_local(opr_segments, cfg.opr_pulses_per_rev, cfg.r_tip_mm);
