...
% VERSION: FOUNDATION_MAINPULSE_STATICRESIDUAL_EODECISION_FIXEDJOINTETA_20260707
%
% Key logic:
...
%      variants belong in comparison/diagnostic scripts, not this entry.
...
S = struct();
S.method_name = 'foundation_mainpulse_static_residual_adaptive_fixed_joint_static_eta';
S.version = 'FOUNDATION_MAINPULSE_STATICRESIDUAL_EODECISION_FIXEDJOINTETA_20260707';
S.method_file_tag = 'FoundationMainPulseStaticResidualFixedJointEta';
...
S.vp_rank_score_mode = 'static_residual_mm';
S.eo_decision_score_mode = 'static_residual_mm';
...
if ~isempty(fit_sensor_eta_env)
    error(['STEP05_FIT_SENSOR_ETA changes the method route; the formal ' ...
        'foundation Step05 uses fixed joint static eta. Use comparison scripts for free-eta checks.']);
end

static_eta_env = strtrim(getenv('STEP05_STATIC_ETA_ENABLE'));
if ~isempty(static_eta_env)
    error(['STEP05_STATIC_ETA_ENABLE is obsolete for the formal route; ' ...
        'fixed joint static eta is always loaded from the calibration preview.']);
end
...
if numel(S.target_blades) ~= 1
    error(['Fixed joint static eta is validated one target blade at a time. ' ...
        'Set STEP05_TARGET_BLADES to a single blade for the formal route.']);
end
...
if isfield(Summary, 'Consensus') && isfield(Summary.Consensus, 'diagnostic_status') && ...
        ~strcmpi(string(Summary.Consensus.diagnostic_status), "ok")
    warning('Step05:StaticEtaPriorSpread', ...
        'Joint static eta diagnostic status is %s; inspect plan spread before interpreting EO.', ...
        char(string(Summary.Consensus.diagnostic_status)));
end
...
for ib = 1:numel(S.target_blades)
    ...
    method_file_tag = getfield_default_local(S, ...
        'method_file_tag', 'FoundationMainPulseAdaptiveFixedJointEta');
...
for iw = 1:height(WindowPlan)
    ...
    static_eta = get_fixed_joint_static_eta_local(S);
...
method.frequency_decision = ['single-window EO is selected by static-residual template-inversion score; ' ...
    'full-waveform voltage RMSE refines A/phi/dx and is reported as a reconstruction diagnostic'];
...
method.frequency_policy = 'VP voltage-residual all-EO scan, adaptive full nonlinear refinement';
method.parameter_bound_policy = 'formal A and dx_c bounds are enforced; eta_s is a fixed dynamic-residual prior and is not fitted window-by-window';
...
elseif any(strcmp(T.Properties.VariableNames, 'static_residual_rmse_mm'))
    T = sortrows(T, {'static_residual_rmse_mm', 'weighted_voltage_rmse', 'EO'}, ...
        {'ascend', 'ascend', 'ascend'});
