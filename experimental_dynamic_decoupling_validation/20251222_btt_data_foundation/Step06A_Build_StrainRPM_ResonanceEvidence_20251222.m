%% Step06A_Build_StrainRPM_ResonanceEvidence_20251222.m
% Build method-neutral strain/RPM/resonance evidence for the 20251222 BTT route.
%
% This script follows the old strain/RPM resonance-evidence logic:
...
%% 0. Config
if exist('BTTDataConfig_20251222', 'file') == 2
    cfg = BTTDataConfig_20251222();
else
    error('Cannot find BTTDataConfig_20251222.m.');
end

script_dir = fileparts(mfilename('fullpath'));
Step06A_Revision = '2026-07-08-20251222-optical-fiber-AI1-01';
...
if ~isfield(cfg, 'step04_reference_legacy_output_root') || ...
    ...
    cfg.step04_reference_legacy_output_root = fullfile(route_parent_dir, ...
        '20251222_low_speed_gap_prior_decoupling', 'legacy', 'output');
end
...
if ~isfield(cfg, 'step3_default_btt_sensor_ids') || isempty(cfg.step3_default_btt_sensor_ids)
    if isfield(cfg, 'optical_fiber_ids') && ~isempty(cfg.optical_fiber_ids)
        cfg.step3_default_btt_sensor_ids = cfg.optical_fiber_ids;
    elseif isfield(cfg, 'fiber_sensor_ids') && ~isempty(cfg.fiber_sensor_ids)
        cfg.step3_default_btt_sensor_ids = cfg.fiber_sensor_ids;
...
if ~isfield(cfg, 'step3_default_strain_channel') || isempty(cfg.step3_default_strain_channel)
    cfg.step3_default_strain_channel = 'AI1-01';
end
...
% Explicit strain file override. Leave empty to use cfg.strain_root search.
strain_file_override = fullfile(cfg.strain_root, 'AI1-01_20251222204737.mat');
...
for iCase = 1:numel(target_cases)
    ...
    evidenceMat = fullfile(outDir, 'Step06A_StrainRPM_ResonanceEvidence_20251222.mat');
    ...
    writetable(regionTable, fullfile(outDir, 'Step06A_RegionTable_20251222.csv'));
    writetable(scoreTable, fullfile(outDir, 'Step06A_ScoreTable_20251222.csv'));
    writetable(localFftTable, fullfile(outDir, 'Step06A_LocalStrainFFT_20251222.csv'));
...
if isfield(cfg, 'step03_output_dir') && ~isempty(cfg.step03_output_dir)
    candidateFiles{end+1} = fullfile(cfg.step03_output_dir, caseName, 'BTT_Observation_Bundle_20251222.mat');
end

candidateFiles{end+1} = fullfile(cfg.output_root, 'step03_dynamic_observation_bundle', caseName, 'BTT_Observation_Bundle_20251222.mat');
candidateFiles{end+1} = fullfile(cfg.output_root, caseName, 'BTT_Observation_Bundle_20251222.mat');
...
if isempty(bundleFile)
    dd = dir(fullfile(cfg.output_root, '**', 'BTT_Observation_Bundle_20251222.mat'));
    for k = 1:numel(dd)
...
if isempty(bundleFile)
    error('Missing Step03 BTT_Observation_Bundle_20251222.mat for case %s.', caseName);
end
...
alignPatterns = { ...
    'Step3_Spectrum_RPM_Alignment_20251222.mat', ...
    '*Spectrum_RPM_Alignment*.mat'};

resultPatterns = { ...
    'Step3_Result_20251222.mat'};
