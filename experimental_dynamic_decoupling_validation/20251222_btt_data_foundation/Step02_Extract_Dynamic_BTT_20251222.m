...
% computes OPR pulse centers, recomputes speed, extracts probe arrivals,
...
for iCase = 1:numel(target_cases)
    ...
    [OPRSemanticsTable, opr_events_per_revolution] = infer_opr_events_per_revolution_local( ...
        OPRRaw.center_time_s, cfg, Step02_Setting);
...
event_id = (1:numel(OPRRaw.center_time_s)).';
...
rev_period_s(valid) = OPRRaw.center_time_s(event_id(valid) + events_per_rev) - OPRRaw.center_time_s(valid);
...
[omega_time_s, omega_rad_s, omega_rpm, omega_period_s] = compute_speed_from_opr_local( ...
    OPRRaw.center_time_s, events_per_rev);
Omega = struct();
...
Omega.metadata = struct( ...
    ...
    'rpm_recomputed_from_raw_opr_centers', true);
...
Meta.Note = ['Independent Step02 raw extraction. Old dynamic jiluOPR/omega/jilublade ', ...
...
jiluOPR = [OPRRaw.center_time_s(:), OPRRaw.start_time_s(:), OPRRaw.end_time_s(:)]; %#ok<NASGU>
metadata = struct();
metadata.method = Step02Meta.OPRCenterMethod;
