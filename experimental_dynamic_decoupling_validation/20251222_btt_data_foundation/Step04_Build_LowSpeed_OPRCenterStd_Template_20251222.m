...
case_data = struct();
point_cloud = [];
speed_diagnostic = struct();

if cfg.step04_use_cached_source && isfile(source_cache_path)
    try
        loaded = load(source_cache_path, 'LowSpeedTemplateSourceData', 'point_cloud', 'speed_diagnostic');
        if isfield(loaded, 'LowSpeedTemplateSourceData') && isfield(loaded, 'point_cloud')
            case_data = loaded.LowSpeedTemplateSourceData;
            point_cloud = ensure_step04_point_cloud_schema_local(loaded.point_cloud);
            if isfield(loaded, 'speed_diagnostic')
                speed_diagnostic = loaded.speed_diagnostic;
            end
            used_cache = true;
            fprintf('>>> [Step04] Loaded low-speed source cache:\n  %s\n', source_cache_path);
        end
    catch ME
...
if ~used_cache
    ...
    if ~isfield(speed_diagnostic, 'opr_semantics_table') || ...
            ~isfield(speed_diagnostic, 'opr_events_per_revolution') || ...
            speed_diagnostic.opr_events_per_revolution ~= cfg.step04_opr_events_per_revolution
        [~, speed_diagnostic] = build_low_speed_speed_interpolant_local(case_data.opr_times(:), cfg);
    end
    if cfg.step04_stable_window_enable && needs_stable_window_selection_step04_local(point_cloud)
...
function tf = needs_stable_window_selection_step04_local(point_cloud)
tf = false;
for i = 1:numel(point_cloud)
    if point_cloud(i).pulse_count > 0 && ~point_cloud(i).stable_window_enabled
        tf = true;
        return;
    end
end
...
function point_cloud = ensure_step04_point_cloud_schema_local(point_cloud)
...
names = fieldnames(default_fields);
for i = 1:numel(point_cloud)
    for j = 1:numel(names)
        name = names{j};
        if ~isfield(point_cloud, name) || isempty(point_cloud(i).(name))
            point_cloud(i).(name) = default_fields.(name);
        end
    end
    if ~isfield(point_cloud, 'point_count') || isempty(point_cloud(i).point_count)
        point_cloud(i).point_count = numel(point_cloud(i).x_mm);
    end
    if ~isfield(point_cloud, 'pulse_count') || isempty(point_cloud(i).pulse_count)
...
empty = struct( ...
    ...
    'stable_window_score', NaN, 'stable_window_candidate_count', 0, ...
    'stable_window_reason', '', ...
    'source_point_policy', '', 'final_point_count', 0, ...
